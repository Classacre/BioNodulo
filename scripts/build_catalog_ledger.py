        ...
        for statement in body:
            ...
            if node_id is not None:
                if node_id:
                    ...
                    normalized_ast = ast.dump(statement, include_attributes=False)
                    found.append(
                        SourceNode(
                            node_id=node_id,
                            module=module,
                            ...
                            ast_sha256=_sha256(normalized_ast.encode("utf-8")),
...
def _module_analysis(module: str, source: str, known_modules: set[str]) -> _ModuleAnalysis:
    ...
    for statement in tree.body:
        if isinstance(statement, ast.ClassDef):
            ...
            classes[statement.name] = _ClassSymbol(
                module=module,
                class_name=statement.name,
                node_id=node_id,
                base_symbols=tuple(ast.unparse(base) for base in statement.bases),
...
def resolve_aliases(module_sources: Mapping[str, str]) -> dict[str, AliasResolution]:
    ...
    for module, analysis in analyses.items():
        for class_name, symbol in analysis.classes.items():
            symbols[(module, class_name)] = symbol
...
def _edge_ports(edges: Sequence[Any], instance_id: str) -> tuple[tuple[str, ...], tuple[str, ...]]:
    inputs: set[str] = set()
    outputs: set[str] = set()
    for edge in edges:
        if not isinstance(edge, dict):
            raise ReconciliationError("workflow edge must be an object")
        source = edge.get("from", {})
        target = edge.get("to", {})
        if isinstance(source, dict) and source.get("node") == instance_id:
            port = source.get("output")
            if not isinstance(port, str):
                raise ReconciliationError(f"edge from {instance_id!r} has no string output port")
            outputs.add(port)
        if isinstance(target, dict) and target.get("node") == instance_id:
            port = target.get("input")
            if not isinstance(port, str):
                raise ReconciliationError(f"edge to {instance_id!r} has no string input port")
            inputs.add(port)
...
def _template_references(
...
) -> tuple[dict[str, tuple[TemplateReference, ...]], int, int, int]:
    ...
    for kind, paths in (("template", template_paths), ("example", example_paths)):
        for source_path in paths:
            ...
            for node in nodes:
                ...
                input_ports, output_ports = _edge_ports(edges, instance_id)
    ...
    return frozen, len(template_paths), len(example_paths), instance_count
...
def reconcile_repository(
    ...
    *,
    ...
    template_references, template_count, example_count, instance_count = _template_references(
        repo_path,
        behavior_ref,
        behavior_ids,
    )
    expected_counts = (EXPECTED_TEMPLATE_COUNT, EXPECTED_EXAMPLE_COUNT, EXPECTED_TEMPLATE_INSTANCES)
    if (template_count, example_count, instance_count) != expected_counts:
        raise ReconciliationError(
            "workflow inventory count differs: "
            f"expected={expected_counts}, actual={(template_count, example_count, instance_count)}"
        )
...
def ledger_document(result: Reconciliation) -> dict[str, Any]:
    ...
    document: dict[str, Any] = {
        ...
        "canonicalizer": {
            "ast_normalization": "ast.dump(include_attributes=False)",
            "json_encoding": "sort_keys,compact,ascii,newline",
            "name": "bionodulo.catalog-ledger",
            "python_ast_runtime": platform.python_version(),
            "version": 1,
        },
        "digests": {
