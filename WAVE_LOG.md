...
- Workflow execution now honors `_branch_meta.skip_downstream` for flow-routing nodes, including cached router results, so explicit downstream skips and their descendants do not execute.
...
- Manifest package requirements now flow into custom-node `object_info` provenance and missing-node resolution reports, so extension recovery can surface package-level Python requirements from `bionodulo.toml`.
- The `/api/manager/reload` contract is now covered end-to-end: deleted manifest custom nodes disappear from `/api/object_info` after reload while built-in nodes remain available.
...
- Help Wiki page navigation/search titles now come only from `helpWiki.pages` i18n keys, so localized search no longer matches hidden English fallback page names.
- Help Wiki node lifecycle status pills now localize known display labels like deprecated while preserving canonical lifecycle metadata for search and compatibility.
...
- Collaboration version diff/history actions now avoid English `Diff` and metadata labels in Spanish render coverage.
- Getting Started News release controls and bundled changelog items now avoid English `release notes`, `toasts`, `Rework`, `backend`, and `Overlay` wording in Spanish render coverage.
- AI workflow assistant local fallback responses now avoid English `Sample Sheet`, `Variant Calling`, `single-cell`, plotting-node, and chart-mode wording in Spanish render coverage.
- Keyboard shortcuts workflow category and workflow action labels now avoid English `Workflow` / `workflow` wording in Spanish render coverage.
- Top bar run/export controls and resume-checkpoint menu copy now avoid English `workflow` and `checkpoint` wording in Spanish render coverage.
- Workflow tab add-button title and accessible label now avoid English `workflow` wording in Spanish render coverage.
...
- Import modal upload hint now avoids English `workflow` wording in Spanish render coverage.
- Node library saved-subgraph delete action now avoids English `blueprint` wording in Spanish render coverage.
- Workflow stats overlay expand/collapse labels now avoid English `workflow` wording in Spanish render coverage.
- Workflow Doctor title, healthy state, and empty-workflow finding now avoid English `workflow` wording in Spanish render coverage.
- Comments panel missing-workflow fallback labels now avoid English `Workflow` wording in Spanish render coverage.
- Collaboration user list missing-workflow fallback labels now avoid English `Workflow` wording in Spanish render coverage.
- Collaboration badge follow-user missing-workflow fallback labels now avoid English `Workflow` wording in Spanish render coverage.
- Share dialog title and offline notice now avoid English `workflow` wording in Spanish render coverage.
- Template gallery save-template prompt now avoids English `workflow` wording in Spanish prompt coverage.
- Version history save-version prompt now avoids English `workflow` wording in Spanish prompt coverage.
- Version history restore confirmation now avoids English `workflow` wording in Spanish prompt coverage.
- Comments panel empty-state copy now avoids English `workflows` wording in Spanish render coverage.
- Comments panel join-required error now avoids English `workflow` wording in Spanish render coverage.
- App collaboration link-ready and template-share unavailable messages now avoid English `workflow` wording in Spanish helper coverage.
- Templates panel save-template, preview-step, and uncategorized-template labels now avoid English `workflow` wording in Spanish render coverage.
- Templates panel preview-step fallback summaries now avoid English `Workflow` wording in Spanish render coverage.
- Command palette run, current-workflow, selection, and doctor workflow commands now avoid English `workflow` wording in Spanish command-copy coverage.
- Workflow auto-name command labels and rename feedback now avoid English `workflow` wording in Spanish command-copy coverage.
- Workflow tab, duplicate, URL-import, recent, and untitled fallbacks now avoid English `workflow` wording in Spanish command/fallback coverage.
- Share-URL command description now avoids English `workflow` wording in Spanish command-copy coverage.
- View fit-all command description now avoids English `workflow` wording in Spanish command-copy coverage.
- Workflow export/import command labels now avoid English `workflow` wording in Spanish command-copy coverage.
...
- Environment panel empty-state hint now avoids English `workflow` wording in Spanish render coverage.
- Workflow thumbnail empty-state label now avoids English `workflow` wording in Spanish canvas-thumbnail coverage.
- Collaboration auth join description now avoids English `workflows` wording in Spanish dialog coverage.
- Getting Started intro, quick-start, AI tip, and recents heading now avoid English `workflow` / `workflows` wording in Spanish modal coverage.
- Console load-run error messages now avoid English `workflow` wording in Spanish helper coverage.
- Settings canvas auto-save description now avoids English `workflows` wording in Spanish settings-panel coverage.
- Collaboration audit workflow target labels now avoid English `Workflow` wording in Spanish audit-log coverage.
- Workspace workflow-file preview actions now avoid English `workflow` wording in Spanish workspace-panel coverage.
- AI assistant drawer shell, prompts, proposal, success, and fallback copy now avoid English `workflow` / `workflows` wording in Spanish modal coverage.
- Getting Started AI assistant changelog item now avoids English `workflows` wording in Spanish release-note coverage.
- Getting Started resources wiki description now avoids English `workflows` wording in Spanish resources-card coverage.
- Help Wiki workflow-converters title, body, and search result now avoid English `workflow` / `workflows` wording in Spanish wiki coverage.
- Help Wiki getting-started article body now avoids English `workflow` / `workflows` wording in Spanish wiki coverage.
- Help Wiki canvas-features article body now avoids English `workflow` / `workflows` wording in Spanish wiki coverage.
- Help Wiki templates-guide article body now avoids English `workflow` / `workflows` wording in Spanish wiki coverage.
- Help Wiki hpc-integration article body now avoids English `workflow` / `workflows` wording in Spanish wiki coverage.
- HPC job-script preview placeholder now avoids English `workflow` wording in Spanish panel coverage.
- Help Wiki keyboard-shortcuts article labels now avoid English `workflow` wording in Spanish wiki coverage.
...
- Runtime artifact action failures now render localized inline wrappers for trigger evaluation, due-trigger submission, pause resolution, and checkpoint resolution while preserving raw details inside the translated message.
- Templates panel remote index failures now show localized load-failed copy when no local fallback templates are available, while preserving structured diagnostics for the raw fetch error.
- Comments panel post, reply, resolve, and delete failures now show localized inline fallback copy instead of raw action exception messages, while preserving structured diagnostics for each action.
...
- PDB Download now accepts `mmcif` as an alias for the canonical RCSB CIF download path, exposing the familiar mmCIF label while keeping `.cif` artifacts and metadata stable.
...
- GEO Query now preserves the total ESearch result count in output metadata separately from returned summary count.
...
- Ensembl VEP now accepts inline Ensembl region-format `variants` via `variant_format=ensembl` and posts them to the region endpoint.
...
- NCBI ESearch now exposes the planned `mesh` database option while leaving EFetch database options unchanged.
...
- Startup queue/history hydration now uses the shared run-list API validator, covering the backend `{ pending, running }` and `{ history }` shapes plus timestamp aliases before mapping records into the console.
...
- BottomConsole report fetch failures now show localized inline fallback copy instead of raw report API exception messages, while preserving structured diagnostics and skipping cancelled fetches.
...
- ExportModal PNG download embedding failures now show localized inline fallback copy instead of raw metadata exception messages, while preserving structured diagnostics for the failed embed action.
...
- SettingsPanel palette-import failures now report a structured `logError` scope while preserving the existing localized import-failed toast.
...
- AIWorkflowModal unexpected chat fallback failures now report a structured `logError` scope while preserving local fallback replies and leaving user-aborted requests unlogged.
...
- Dynamic Add Node command-palette descriptions now localize category fallback labels for display while preserving canonical category keywords for search.
