...
- registry_snapshot.jsonl (not committed): the historical raw crawl snapshot.
  The resumable crawler was retired because it could retain stale metadata.
  Use the verified synchronization commands in [SYNC.md](SYNC.md) for a fresh
  snapshot; the historical reports above are not regenerated by that command.
