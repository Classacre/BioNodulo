...
- Research: `search_literature` queries PubMed and returns abstracts and free full-text links, so method choices are grounded in current papers. `literature_search` searches OpenAlex across all fields (citation counts, open-access PDFs, optional year_from filter); `pubmed_search` is the lighter biomedical search; `arxiv_search` finds the newest preprints. `get_paper` resolves a DOI, arXiv id, or PMID to a metadata card with abstract; `citation_lookup` gives a DOI's cited-by count and top citing works — use it to vet influence and to enrich DOIs from a run's CITATION_DOIS export. All are free and keyless.
