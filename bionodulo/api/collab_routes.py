...
@collab_api_router.post("/workflows/{workflow_id}/comments", status_code=201)
async def create_comment(
    ...
    comment = Comment(
        ...
        user_id=user_id,
        user_name=user_name,
        user_color=token_payload.get("color", "#3b82f6"),
    ...
    await _get_audit_logger(request).log(
        ...
        payload={"node_id": body.node_id, "parent_id": body.parent_id, "content_length": len(body.content)},
    )

    return comment.to_dict()


@collab_api_router.get("/workflows/{workflow_id}/comments")
...
@collab_api_router.get("/comments")
async def list_accessible_comments(
    request: Request,
    token_payload: dict[str, Any] = Depends(get_token_from_header_or_query),
) -> dict[str, Any]:
    """List comments across workflows readable by the current user."""
    store = _get_collab_store(request)
    user_id = token_payload.get("sub", "")
    workflow_ids = store.list_workflow_ids_for_user(user_id)
    ...
    for workflow_id in workflow_ids:
        ...
        if doc is not None:
            ...
            if name:
                workflow_names[workflow_id] = str(name)
...
@collab_api_router.patch("/comments/{comment_id}")
async def update_comment(
    ...
    updated = store.update_comment(comment_id, body.content)
    if updated is None:
        raise HTTPException(status_code=404, detail="Comment not found")
...
@collab_api_router.post("/comments/{comment_id}/resolve")
...
@collab_api_router.delete("/comments/{comment_id}", status_code=status.HTTP_204_NO_CONTENT, response_model=None)
async def delete_comment(
    comment_id: str,
    request: Request,
    token_payload: dict[str, Any] = Depends(get_token_from_header_or_query),
) -> None:
    """Delete a comment (only by the original author or workflow owner)."""
    store = _get_collab_store(request)
