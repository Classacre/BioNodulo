...
read_methylation <- function(path, sample_id) {
...
methylation <- Map(read_methylation, methylation_files, as.character(samples[[sample_column]]))
bsseq <- DSS::makeBSseqData(methylation, sampleNames = as.character(samples[[sample_column]]))
test <- DSS::DMLtest(bsseq, group1 = which(groups == unique(groups)[1]), group2 = which(groups == unique(groups)[2]), smoothing = smoothing)
...
if (nrow(dmrs) == 0) {
  ...
  readr::write_tsv(data.frame(chr = character(), start = integer(), end = integer(), length = integer(), nCG = integer(), pval = numeric(), areaStat = numeric()), output_stats)
} else {
  ...
  bed <- data.frame(
    ...
    score = pmin(1000L, pmax(0L, as.integer(-log10(stats$pval) * 100))),
    stringsAsFactors = FALSE
  )
