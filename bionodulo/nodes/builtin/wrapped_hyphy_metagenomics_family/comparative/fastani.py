...
class FastANINode(ToolsIUCCommandContract):
    ...
    def VALIDATE_INPUTS(cls, inputs: dict[str, Any]) -> bool | str:
        if not _as_list(inputs.get("query")):
            return "FastANI requires at least one query genome"
        if not _as_list(inputs.get("reference")):
            return "FastANI requires at least one reference genome"
        try:
            threads = int(inputs.get("threads", 1))
            fragment_length = int(inputs.get("frag_len", 3000))
            minimum_fraction = float(inputs.get("min_fraction", 0.2))
            kmer = int(inputs.get("kmer", 16))
        except (TypeError, ValueError):
            return "FastANI numeric options are invalid"
        if threads < 1:
            return "FastANI threads must be a positive integer"
        if fragment_length < 100:
            return "FastANI fragment length must be at least 100"
        if not 0 <= minimum_fraction <= 1:
            return "FastANI minimum fraction must be between 0 and 1"
        if not 4 <= kmer <= 32:
            return "FastANI k-mer size must be between 4 and 32"
        return True
