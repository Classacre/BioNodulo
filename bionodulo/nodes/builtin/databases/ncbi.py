"""ncbi — databases node(s). One tool per file (extracted from ncbi.py)."""
...
class NCBIESearchNode(BaseNode):
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'query': ('STRING', {'default': '', 'description': 'NCBI Entrez search query'}), 'database': (list(NCBI_ESEARCH_DATABASES), {'default': 'pubmed'})}, 'optional': {'max_results': ('INT', {'default': 20, 'min': 1, 'max': 10000}), 'retstart': ('INT', {'default': 0, 'min': 0, 'advanced': True}), 'sort': ('STRING', {'default': 'relevance'}), 'api_key': ('STRING', {'default': '', 'advanced': True}), 'return_uids': ('BOOLEAN', {'default': True, 'advanced': True})}, 'hidden': {}}
    ...
    async def run(self, **kwargs: Any) -> dict[str, Any]:
        ...
        params: dict[str, Any] = {'db': database, 'term': query, 'retmode': 'json', 'retmax': max_results, 'tool': 'bionodulo', 'email': _default_ncbi_email()}
        ...
        if not return_uids and ids:
            fetch_params: dict[str, Any] = {'db': database, 'id': ','.join(ids), 'rettype': 'acc', 'retmode': 'text', 'tool': 'bionodulo', 'email': _default_ncbi_email()}
            if api_key:
        ...
        return {'outputs': {'id_list': result_ids, 'total_count': count, 'query_translation': str(result.get('querytranslation', ''))}}
...
class NCBIEFetchNode(BaseNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'accessions': ('STRING', {'default': '', 'description': 'Record IDs or accessions as a list, JSON list, or comma-separated string'}), 'database': (list(NCBI_EFETCH_DATABASES), {'default': 'nuccore'})}, 'optional': {'rettype': ('STRING', {'default': 'fasta', 'options': list(NCBI_EFETCH_RETTYPES)}), 'retmode': ('STRING', {'default': 'text', 'options': list(NCBI_EFETCH_RETMODES)}), 'api_key': ('STRING', {'default': '', 'advanced': True}), 'batch_size': ('INT', {'default': 100, 'min': 1, 'max': 500}), 'email': ('STRING', {'default': '', 'advanced': True}), 'id_list': ('ANY', {'default': '', 'advanced': True, 'description': 'Backward-compatible record ID input'}), 'output_name': ('STRING', {'default': ''})}, 'hidden': {}}
    ...
    async def run(self, **kwargs: Any) -> dict[str, Any]:
        ...
        for batch in batches:
            params: dict[str, Any] = {'db': database, 'id': ','.join(batch), 'rettype': rettype, 'retmode': retmode, 'tool': 'bionodulo', 'email': email}
            if api_key:
                params['api_key'] = api_key
        ...
        return {'outputs': {'records': str(out_path), 'metadata': {'database': database, 'ids': ids, 'rettype': rettype, 'retmode': retmode, 'record_count': len(ids), 'batch_size': batch_size, 'batch_count': len(batches), 'records_path': str(out_path)}}}
...
class NCBIBLASTNode(BaseNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'query_sequence': ('STRING', {'default': '', 'multiline': True, 'description': 'FASTA or raw query sequence'}), 'program': (list(BLAST_PROGRAMS), {'default': 'blastn'}), 'database': (list(BLAST_DATABASES), {'default': 'nt'})}, 'optional': {'evalue': ('FLOAT', {'default': 10.0, 'min': 0.0001}), 'max_hits': ('INT', {'default': 50, 'min': 1, 'max': 5000}), 'output_format': (list(BLAST_OUTPUT_FORMATS), {'default': 'JSON2'}), 'timeout_minutes': ('INT', {'default': 30, 'min': 1, 'max': 120}), 'poll_interval_seconds': ('FLOAT', {'default': 60.0, 'min': 1, 'advanced': True}), 'email': ('STRING', {'default': '', 'advanced': True})}, 'hidden': {}}

    async def _submit_blast_job(self, *, query: str, program: str, database: str, evalue: float, max_hits: int, output_format: str, email: str) -> tuple[str, int | None]:
        api_program = 'blastn' if program == 'megablast' else program
        params: dict[str, Any] = {'CMD': 'Put', 'QUERY': query, 'PROGRAM': api_program, 'DATABASE': database, 'EXPECT': str(evalue), 'HITLIST_SIZE': str(max_hits), 'FORMAT_TYPE': output_format, 'tool': 'bionodulo', 'email': email}
        if program == 'megablast':
            params['MEGABLAST'] = 'on'
        response = await _blast_request_text('POST', params)
        return _parse_blast_submission(response)

    async def _poll_blast_results(self, *, rid: str, output_format: str, timeout_minutes: int, poll_interval_seconds: float) -> str:
        ...
        while True:
            ...
            status_text = await _blast_request_text('GET', {'CMD': 'Get', 'RID': rid, 'FORMAT_OBJECT': 'SearchInfo'})
            status = _blast_status(status_text)
            if status == 'WAITING':
                continue
            if status == 'READY':
                return await _blast_request_text('GET', {'CMD': 'Get', 'RID': rid, 'FORMAT_TYPE': output_format})
            if status == 'FAILED':
    ...
    async def run(self, **kwargs: Any) -> dict[str, Any]:
        ...
        rid, rtoe_seconds = await self._submit_blast_job(query=query, program=program, database=database, evalue=evalue, max_hits=max_hits, output_format=output_format, email=email)
        logger.info('Submitted NCBI BLAST job: RID=%s', rid)
        raw_results = await self._poll_blast_results(rid=rid, output_format=output_format, timeout_minutes=timeout_minutes, poll_interval_seconds=poll_interval_seconds)
        ...
        summary: dict[str, Any] = {'rid': rid, 'rtoe_seconds': rtoe_seconds, 'program': program, 'database': database, 'format': output_format, 'results_path': str(result_path)}
        summary.update(_blast_result_summary(raw_results, output_format))
        summary_path = out_dir / 'blast_summary.json'
        summary_path.write_text(json.dumps(summary, indent=2), encoding='utf-8')
        return {'outputs': {'blast_results': str(result_path), 'blast_summary': str(summary_path)}}
...
class NCBIBLASTParseNode(BaseNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'blast_results': ('FILE', {'description': 'Saved BLAST JSON2 or XML result file'})}, 'optional': {'input_format': ('STRING', {'default': 'auto', 'options': list(BLAST_PARSE_INPUT_FORMATS)}), 'output_format': ('STRING', {'default': 'TSV', 'options': list(BLAST_PARSE_OUTPUT_FORMATS)}), 'max_hits': ('INT', {'default': 50, 'min': 1, 'max': 100000})}, 'hidden': {}}
    ...
    async def run(self, **kwargs: Any) -> dict[str, Any]:
        ...
        summary = {'input_format': input_format, 'output_format': output_format, 'source_path': str(blast_results), 'parsed_hits_path': str(hits_path), 'parsed_hit_count': len(parsed_hits), 'available_hit_count': len(all_hits), 'queries': sorted({str(hit.get('query', '')) for hit in all_hits if hit.get('query')})}
        summary_path = out_dir / 'blast_parse_summary.json'
        summary_path.write_text(json.dumps(summary, indent=2), encoding='utf-8')
        return {'outputs': {'parsed_hits': str(hits_path), 'parse_summary': str(summary_path)}}
