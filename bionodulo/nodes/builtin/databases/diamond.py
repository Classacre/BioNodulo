"""diamond — databases node(s). One tool per file (extracted from wrapped_protein_taxonomy.py)."""
...
class DiamondMakeDBNode(CommandNode):
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        cmd = ['diamond', 'makedb', '--threads', str(inputs.get('threads', 12)), '--in', str(inputs.get('infile', '')), '--db', f'{_out(inputs)}/database']
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'infile': ('FASTA', {'description': 'Protein FASTA reference'}), 'threads': ('INT', {'default': 12, 'min': 1, 'max': 128, 'display': 'slider'})}, 'optional': {'taxonmap': ('TSV', {'description': 'Protein accession to taxid mapping', 'advanced': True}), 'taxonnodes': ('TSV', {'description': 'NCBI nodes.dmp', 'advanced': True}), 'taxonnames': ('TSV', {'description': 'NCBI names.dmp', 'advanced': True})}, 'hidden': {'output': ('STRING', {})}}
