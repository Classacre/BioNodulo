"""uniprot — databases node(s). One tool per file (extracted from uniprot.py)."""
...
UNIPROT_SUMMARY_COLUMNS = ('accession', 'entry_name', 'protein_name', 'organism', 'gene_names', 'sequence_length')
...
async def _request_json(resource: str, *, params: dict[str, Any] | None=None, retries: int=MAX_RETRIES, timeout: float=REQUEST_TIMEOUT_S) -> dict[str, Any]:
    response = await _request(resource, params=params, retries=retries, timeout=timeout)
    return response.json()
async def _request_text(resource: str, *, params: dict[str, Any] | None=None, retries: int=MAX_RETRIES, timeout: float=REQUEST_TIMEOUT_S) -> str:
    response = await _request(resource, params=params, retries=retries, timeout=timeout)
    return response.text
async def _request(resource: str, *, params: dict[str, Any] | None, retries: int, timeout: float) -> httpx.Response:
    resource = resource.lstrip('/')
    url = f'{UNIPROT_BASE_URL}/{resource}'
    client = APIHttpClient(cache=_UNIPROT_CACHE, rate_limiter=_UNIPROT_RATE_LIMITER)
    try:
        return await client.request('GET', url, params=params, headers={'User-Agent': UNIPROT_USER_AGENT}, timeout=timeout, retries=retries, retry_delay=RETRY_DELAY_S, cache_ttl=UNIPROT_CACHE_TTL_S)
...
def _with_summary(payload: dict[str, Any]) -> dict[str, Any]:
    if isinstance(payload.get('representativeMember'), dict):
        ...
        enriched['summary'] = {'accession': str(accessions[0] if accessions else representative.get('memberId', '')), 'entry_name': str(payload.get('id', '')), 'protein_name': str(representative.get('proteinName') or payload.get('name', '')).removeprefix('Cluster: '), 'organism': str(common_taxon.get('scientificName', '')), 'gene_names': [], 'sequence_length': representative.get('sequenceLength') or sequence.get('length'), 'member_count': payload.get('memberCount')}
        return enriched
    if payload.get('uniParcId'):
        ...
        enriched = dict(payload)
        enriched['summary'] = {'accession': str(accessions[0] if accessions else ''), 'entry_name': str(payload.get('uniParcId', '')), 'protein_name': '', 'organism': organism, 'gene_names': [], 'sequence_length': sequence.get('length'), 'cross_reference_count': payload.get('crossReferenceCount')}
        return enriched
    sequence = payload.get('sequence') if isinstance(payload.get('sequence'), dict) else {}
    organism = payload.get('organism') if isinstance(payload.get('organism'), dict) else {}
    enriched = dict(payload)
    enriched['summary'] = {'accession': str(payload.get('primaryAccession', '')), 'entry_name': str(payload.get('uniProtkbId', '')), 'protein_name': _first_protein_name(payload), 'organism': str(organism.get('scientificName', '')), 'gene_names': _gene_names(payload), 'sequence_length': sequence.get('length')}
    return enriched
def _summary_row(payload: dict[str, Any]) -> dict[str, str]:
    ...
    return {'accession': str(summary.get('accession') or ''), 'entry_name': str(summary.get('entry_name') or ''), 'protein_name': str(summary.get('protein_name') or ''), 'organism': str(summary.get('organism') or ''), 'gene_names': gene_text, 'sequence_length': '' if sequence_length is None else str(sequence_length)}
def _write_summary_tsv(path: Path, entries: list[dict[str, Any]]) -> None:
    with path.open('w', encoding='utf-8', newline='') as handle:
        writer = csv.DictWriter(handle, fieldnames=list(UNIPROT_SUMMARY_COLUMNS), delimiter='\t', lineterminator='\n')
        writer.writeheader()
...
class UniProtSearchNode(BaseNode):
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'query': ('STRING', {'default': '', 'description': 'UniProt query, e.g. gene:TP53 AND organism_id:9606'})}, 'optional': {'database': ('STRING', {'default': 'uniprotkb', 'options': list(UNIPROT_SEARCH_DATABASES)}), 'max_results': ('INT', {'default': 25, 'min': 1, 'max': 500}), 'size': ('INT', {'default': 25, 'min': 1, 'max': 500, 'advanced': True}), 'format': ('STRING', {'default': 'json', 'options': list(UNIPROT_SEARCH_FORMATS), 'advanced': True}), 'reviewed_only': ('BOOLEAN', {'default': False}), 'include_isoform': ('BOOLEAN', {'default': False, 'advanced': True}), 'fields': ('STRING', {'default': UNIPROT_SEARCH_FIELDS, 'advanced': True}), 'output_name': ('STRING', {'default': '', 'description': 'Optional TSV filename stem'})}, 'hidden': {}}
    ...
    async def run(self, **kwargs: Any) -> dict[str, Any]:
        ...
        params: dict[str, Any] = {'query': effective_query, 'format': output_format, 'fields': fields, 'size': max_results}
        ...
        if output_format == 'tsv':
            ...
            return {'outputs': {'results_table': str(table_path), 'results_data': {'query': query, 'effective_query': effective_query, 'database': database, 'format': output_format, 'record_count': _count_tsv_records(tsv_text)}}}
        if output_format not in UNIPROT_STRUCTURED_SEARCH_FORMATS:
            ...
            return {'outputs': {'results_table': str(raw_path), 'results_data': {'query': query, 'effective_query': effective_query, 'database': database, 'format': output_format, 'record_count': None, 'raw_path': str(raw_path)}}}
        ...
        return {'outputs': {'results_table': str(table_path), 'results_data': {'query': query, 'effective_query': effective_query, 'record_count': len(entries), 'entries': entries}}}
...
class UniProtRetrieveNode(BaseNode):
    ...
    SEARCH_ALIASES = ['uniprot', 'protein', 'retrieve', 'fetch', 'sequence', 'fasta', 'annotation', 'swissprot', 'trembl', 'database']
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'uniprot_ids': ('STRING', {'default': '', 'description': 'UniProt accession(s), comma-separated'})}, 'optional': {'format': ('STRING', {'default': 'json', 'options': ['json', 'fasta']}), 'accession': ('STRING', {'default': '', 'advanced': True, 'description': 'Backward-compatible UniProt accession(s), comma-separated'}), 'include_fasta': ('BOOLEAN', {'default': True}), 'include_isoform': ('BOOLEAN', {'default': False, 'advanced': True}), 'output_name': ('STRING', {'default': '', 'description': 'Optional FASTA filename stem'})}, 'hidden': {}}
    ...
    async def run(self, **kwargs: Any) -> dict[str, Any]:
        ...
        if len(entries) == 1:
            ...
            protein_data = {'accessions': accessions, 'record_count': len(entries), 'entries': entries}
        sequence_path = ''
        if include_fasta:
            output_name = str(kwargs.get('output_name', '') or '').strip()
            if output_name:
        ...
        return {'outputs': {'protein_data': protein_data, 'sequence': sequence_path}}
