"""sra — databases node(s). One tool per file (extracted from ncbi.py)."""
...
class SRADownloadNode(BaseNode):
    ...
    SEARCH_ALIASES = ['sra', 'sequence read archive', 'download', 'fastq', 'fasta', 'ngs', 'reads', 'prefetch']
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'accessions': ('STRING', {'default': '', 'description': 'SRR/ERR/DRR accessions as a list, JSON list, or comma-separated string'})}, 'optional': {'accession': ('STRING', {'default': '', 'advanced': True, 'description': 'Backward-compatible singular SRR/ERR/DRR accession'}), 'split_files': ('BOOLEAN', {'default': True, 'description': 'Split paired-end reads into separate files'}), 'threads': ('INT', {'default': 4, 'min': 1, 'max': 32}), 'skip_technical': ('BOOLEAN', {'default': True}), 'output_format': (list(SRA_OUTPUT_FORMATS), {'default': 'fastq'}), 'format': ('STRING', {'default': 'fastq', 'options': ['fastq', 'fasta'], 'advanced': True})}, 'hidden': {}}
    ...
    async def run(self, **kwargs: Any) -> dict[str, Any]:
        ...
        for accession in accessions:
            ...
            try:
                ...
                if prefetch_result['returncode'] != 0:
                    ...
                    continue
                dump_cmd = ['fasterq-dump', '--outdir', str(out_dir), '--threads', str(threads)]
        ...
        return {'outputs': {'fastq_files': downloaded_files, 'download_report': str(report_path)}}
