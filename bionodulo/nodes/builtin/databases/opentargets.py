"""opentargets — databases node(s). One tool per file (extracted from opentargets.py)."""
...
OPENTARGETS_API_CACHE = APICache.from_environment(default_ttl_seconds=OPENTARGETS_CACHE_TTL_S)
...
def _node_output_dir(node: BaseNode, context: Any) -> Path:
    ...
    return out_dir
async def _graphql_request(query: str, variables: dict[str, Any], *, retries: int=MAX_RETRIES, timeout: float=REQUEST_TIMEOUT_S) -> dict[str, Any]:
    client = APIHttpClient(cache=OPENTARGETS_API_CACHE, rate_limiter=OPENTARGETS_RATE_LIMITER)
    try:
        response = await client.request('POST', OPENTARGETS_GRAPHQL_URL, headers={'User-Agent': OPENTARGETS_USER_AGENT}, json={'query': query, 'variables': variables}, timeout=timeout, retries=retries, retry_delay=RETRY_DELAY_S, cache_ttl=None)
...
def _target_summary(payload: dict[str, Any] | None) -> dict[str, str]:
    payload = payload or {}
    return {'id': str(payload.get('id', '') or ''), 'symbol': str(payload.get('approvedSymbol', '') or ''), 'name': str(payload.get('approvedName', '') or '')}
...
def _target_to_disease_payload(payload: dict[str, Any], *, min_score: float) -> dict[str, Any]:
    ...
    associations = [_association_from_target_row(target_info, row) for row in rows if isinstance(row, dict) and float(row.get('score', 0.0) or 0.0) >= min_score]
    return {'target': target_info, 'disease': {}, 'total_available': int(associated.get('count', len(rows)) or 0) if isinstance(associated, dict) else 0, 'associations': associations}
def _disease_to_target_payload(payload: dict[str, Any], *, min_score: float) -> dict[str, Any]:
    ...
    rows = associated.get('rows', []) if isinstance(associated, dict) else []
    associations = [_association_from_disease_row(disease_info, row) for row in rows if isinstance(row, dict) and float(row.get('score', 0.0) or 0.0) >= min_score]
    return {'target': {}, 'disease': disease_info, 'total_available': int(associated.get('count', len(rows)) or 0) if isinstance(associated, dict) else 0, 'associations': associations}
def _association_from_target_row(target: dict[str, str], row: dict[str, Any]) -> dict[str, Any]:
    ...
    return {'target_id': target['id'], 'target_symbol': target['symbol'], 'target_name': target['name'], 'disease_id': str(disease.get('id', '') or ''), 'disease_name': str(disease.get('name', '') or ''), 'score': float(row.get('score', 0.0) or 0.0), 'datatype_scores': _score_map(row.get('datatypeScores'))}
def _association_from_disease_row(disease: dict[str, str], row: dict[str, Any]) -> dict[str, Any]:
    target = row.get('target') if isinstance(row.get('target'), dict) else {}
    return {'target_id': str(target.get('id', '') or ''), 'target_symbol': str(target.get('approvedSymbol', '') or ''), 'target_name': str(target.get('approvedName', '') or ''), 'disease_id': disease['id'], 'disease_name': disease['name'], 'score': float(row.get('score', 0.0) or 0.0), 'datatype_scores': _score_map(row.get('datatypeScores'))}
def _evidence_from_pair_payload(payload: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for item in evidence_items:
        ...
        evidence_rows.append({'target_id': str(item_target.get('id', '') or target_info['id']), 'target_symbol': str(item_target.get('approvedSymbol', '') or target_info['symbol']), 'disease_id': str(disease.get('id', '') or ''), 'disease_name': str(disease.get('name', '') or ''), 'score': float(item.get('score', 0.0) or 0.0), 'datatype_scores': f"{item.get('datatypeId', '')}/{item.get('datasourceId', '')}", 'evidence_count': 1})
    return evidence_rows
...
def _write_evidence_tsv(path: Path, associations: list[dict[str, Any]], evidence_rows: list[dict[str, Any]]) -> None:
    with path.open('w', encoding='utf-8', newline='') as handle:
        ...
        writer.writerow(['target_id', 'target_symbol', 'disease_id', 'disease_name', 'score', 'datatype_scores', 'evidence_count'])
        for association in associations:
            writer.writerow([association.get('target_id', ''), association.get('target_symbol', ''), association.get('disease_id', ''), association.get('disease_name', ''), _format_score(association.get('score', 0.0)), _format_datatype_scores(association.get('datatype_scores', {})), int(association.get('evidence_count', 0) or 0)])
        for evidence in evidence_rows:
            writer.writerow([evidence.get('target_id', ''), evidence.get('target_symbol', ''), evidence.get('disease_id', ''), evidence.get('disease_name', ''), _format_score(evidence.get('score', 0.0)), _format_datatype_scores(evidence.get('datatype_scores', '')), int(evidence.get('evidence_count', 0) or 0)])
...
class OpenTargetsNode(BaseNode):
    ...
    SEARCH_ALIASES = ['open targets', 'opentargets', 'target-disease', 'association', 'evidence', 'drug discovery', 'genetics']
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'target': ('STRING', {'default': '', 'description': 'Ensembl target ID, e.g. ENSG00000141510'}), 'disease': ('STRING', {'default': '', 'description': 'EFO disease ID, e.g. EFO_0000616'})}, 'optional': {'query_mode': ('STRING', {'default': 'association', 'options': ['association', 'target_to_diseases', 'disease_to_targets']}), 'max_results': ('INT', {'default': 25, 'min': 1, 'max': 500}), 'min_score': ('FLOAT', {'default': 0.0, 'min': 0.0, 'max': 1.0, 'step': 0.05}), 'include_evidence': ('BOOLEAN', {'default': False})}, 'hidden': {'context': ('CONTEXT', {})}}
    ...
    async def run(self, **kwargs: Any) -> dict[str, Any]:
        ...
        if query_mode == 'target_to_diseases':
            ...
            payload = await _graphql_request(TARGET_ASSOCIATED_DISEASES_QUERY, {'ensemblId': target, 'size': max_results})
            ...
            payload = await _graphql_request(DISEASE_ASSOCIATED_TARGETS_QUERY, {'efoId': disease, 'size': max_results})
            ...
            payload = await _graphql_request(TARGET_ASSOCIATED_DISEASES_QUERY, {'ensemblId': target, 'size': max_results}) if target else await _graphql_request(DISEASE_ASSOCIATED_TARGETS_QUERY, {'efoId': disease, 'size': max_results})
            normalized = _target_to_disease_payload(payload, min_score=min_score) if target else _disease_to_target_payload(payload, min_score=min_score)
        evidence_rows: list[dict[str, Any]] = []
        if include_evidence and target and disease:
            pair_payload = await _graphql_request(PAIR_EVIDENCE_QUERY, {'ensemblId': target, 'efoIds': [disease], 'size': max_results})
        ...
        result = {'query_mode': query_mode, 'target': normalized['target'], 'disease': normalized['disease'], 'record_count': len(normalized['associations']), 'total_available': normalized['total_available'], 'min_score': min_score, 'include_evidence': include_evidence, 'associations': normalized['associations']}
