"""ensembl — databases node(s). One tool per file (extracted from ensembl.py)."""
...
ENSEMBL_JSON_HEADERS = {'Accept': 'application/json', 'Content-Type': 'application/json', 'User-Agent': ENSEMBL_USER_AGENT}
VEP_TABLE_COLUMNS = ('input', 'gene_symbol', 'gene_id', 'transcript_id', 'consequence_terms', 'impact')
COMMON_ENSEMBL_SPECIES_OPTIONS = ('homo_sapiens', 'mus_musculus', 'rattus_norvegicus', 'danio_rerio', 'drosophila_melanogaster', 'caenorhabditis_elegans', 'saccharomyces_cerevisiae')
...
def _node_output_dir(node: BaseNode, context: Any) -> Path:
    ...
    return out_dir
async def _request_json(resource: str, params: dict[str, Any] | None=None, *, base_url: str=ENSEMBL_BASE_URL, retries: int=MAX_RETRIES, timeout: float=REQUEST_TIMEOUT_S) -> dict[str, Any]:
    response = await _request(resource, params or {}, base_url=base_url, retries=retries, timeout=timeout)
    return response.json()
async def _post_json(resource: str, json_body: dict[str, Any], params: dict[str, Any] | None=None, *, base_url: str=ENSEMBL_BASE_URL, retries: int=MAX_RETRIES, timeout: float=REQUEST_TIMEOUT_S) -> Any:
    response = await _post(resource, json_body, params or {}, base_url=base_url, retries=retries, timeout=timeout)
    return response.json()
async def _request(resource: str, params: dict[str, Any], *, base_url: str, retries: int, timeout: float) -> httpx.Response:
    resource = resource.lstrip('/')
    url = f"{base_url.rstrip('/')}/{resource}"
    client = APIHttpClient(cache=ENSEMBL_API_CACHE, rate_limiter=ENSEMBL_RATE_LIMITER)
    try:
        return await client.request('GET', url, params=params, headers=ENSEMBL_JSON_HEADERS, timeout=timeout, retries=retries, retry_delay=RETRY_DELAY_S, cache_ttl=ENSEMBL_CACHE_TTL_S)
...
async def _post(resource: str, json_body: dict[str, Any], params: dict[str, Any], *, base_url: str, retries: int, timeout: float) -> httpx.Response:
    resource = resource.lstrip('/')
    url = f"{base_url.rstrip('/')}/{resource}"
    client = APIHttpClient(cache=ENSEMBL_API_CACHE, rate_limiter=ENSEMBL_RATE_LIMITER)
    try:
        return await client.request('POST', url, params=params, headers=ENSEMBL_JSON_HEADERS, json=json_body, timeout=timeout, retries=retries, retry_delay=RETRY_DELAY_S, cache_ttl=None)
    except httpx.HTTPStatusError as exc:
        status = exc.response.status_code
        body = exc.response.text[:500]
        raise RuntimeError(f'Ensembl {resource} failed with HTTP {status}: {body}') from exc
...
def _is_stable_id(query: str) -> bool:
    return bool(re.match('^ENS[A-Z]*[GTPE]\\d+', query.strip(), re.IGNORECASE))
def _summary(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {'id': str(payload.get('id', '')), 'display_name': str(payload.get('display_name', '')), 'description': str(payload.get('description', '')), 'species': str(payload.get('species', '')), 'assembly_name': str(payload.get('assembly_name', '')), 'object_type': str(payload.get('object_type', '')), 'biotype': str(payload.get('biotype', '')), 'location': location}
...
def _vep_summary_rows(payload: Any) -> list[dict[str, str]]:
    ...
    for record in records:
        ...
        if not isinstance(consequences, list) or not consequences:
            rows.append({'input': input_value, 'gene_symbol': '', 'gene_id': '', 'transcript_id': '', 'consequence_terms': '', 'impact': ''})
            continue
        for consequence in consequences:
            ...
            rows.append({'input': input_value, 'gene_symbol': str(consequence.get('gene_symbol', '')), 'gene_id': str(consequence.get('gene_id', '')), 'transcript_id': str(consequence.get('transcript_id', '')), 'consequence_terms': terms_text, 'impact': str(consequence.get('impact', ''))})
    return rows
...
class EnsemblGeneLookupNode(BaseNode):
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'gene_symbol': ('STRING', {'default': '', 'description': 'Gene symbol or Ensembl stable ID'}), 'species': ('STRING', {'default': 'homo_sapiens', 'options': list(COMMON_ENSEMBL_SPECIES_OPTIONS)})}, 'optional': {'query': ('STRING', {'default': '', 'advanced': True, 'description': 'Backward-compatible gene symbol or stable ID input'}), 'expand': ('BOOLEAN', {'default': True, 'description': 'Include transcripts when available'}), 'assembly': ('STRING', {'default': 'current', 'options': ['current', 'GRCh37']}), 'fetch_homologs': ('BOOLEAN', {'default': False, 'advanced': True}), 'homolog_species': ('STRING', {'default': '', 'advanced': True, 'description': 'Optional target species for homology lookup'})}, 'hidden': {}}
        ...
        if fetch_homologs and gene_id:
            if len(homolog_species_list) > 1:
                ...
                for target_species in homolog_species_list:
                    ...
                    gene_info['homologs_by_species'][target_species] = await _request_json(f"homology/id/{quote(gene_id, safe='')}", params=homolog_params, base_url=base_url)
            else:
                homolog_params: dict[str, Any] = {'type': 'orthologues'}
                if homolog_species_list:
                    homolog_params['target_species'] = homolog_species_list[0]
                gene_info['homologs'] = await _request_json(f"homology/id/{quote(gene_id, safe='')}", params=homolog_params, base_url=base_url)
...
class EnsemblVEPNode(BaseNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'variants': ('STRING', {'default': '', 'multiline': True, 'description': 'HGVS variants, one per line'}), 'species': ('STRING', {'default': 'homo_sapiens', 'options': list(COMMON_ENSEMBL_SPECIES_OPTIONS)})}, 'optional': {'variant_format': ('STRING', {'default': 'hgvs', 'options': ['hgvs', 'vcf', 'ensembl']}), 'vcf_file': ('VCF', {'default': '', 'advanced': True, 'description': 'Backward-compatible VCF file containing variants to annotate'}), 'assembly': ('STRING', {'default': 'current', 'options': ['current', 'GRCh38', 'GRCh37']}), 'canonical': ('BOOLEAN', {'default': True}), 'domains': ('BOOLEAN', {'default': False}), 'gene_phenotype': ('BOOLEAN', {'default': False}), 'variant_class': ('BOOLEAN', {'default': True}), 'sift': ('BOOLEAN', {'default': True, 'advanced': True}), 'polyphen': ('BOOLEAN', {'default': True, 'advanced': True}), 'maf': ('BOOLEAN', {'default': False, 'advanced': True})}, 'hidden': {}}
    ...
    async def run(self, **kwargs: Any) -> dict[str, Any]:
        ...
        species = str(kwargs.get('species', 'homo_sapiens')).strip() or 'homo_sapiens'
        base_url = _base_url_for_assembly(str(kwargs.get('assembly', 'current')))
        params = {'canonical': 1 if bool(kwargs.get('canonical', True)) else 0, 'domains': 1 if bool(kwargs.get('domains', False)) else 0, 'gene_phenotype': 1 if bool(kwargs.get('gene_phenotype', False)) else 0, 'variant_class': 1 if bool(kwargs.get('variant_class', True)) else 0, 'SiftPrediction': 'yes' if bool(kwargs.get('sift', True)) else 'no', 'PolyPhen': 'yes' if bool(kwargs.get('polyphen', True)) else 'no', 'MAF': 'yes' if bool(kwargs.get('maf', False)) else 'no'}
        if variant_format == 'hgvs':
            ...
            resource = f"vep/{quote(species, safe='')}/region"
        payload = await _post_json(resource, {'variants': variants}, params=params, base_url=base_url)
