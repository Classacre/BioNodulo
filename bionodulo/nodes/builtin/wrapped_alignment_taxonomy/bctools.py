"""bctools — wrapped_alignment_taxonomy node(s). One tool per file (extracted from wrapped_alignment_taxonomy.py)."""
...
class BctoolsExtractCrosslinkedNucleotidesNode(_BctoolsBaseNode):
    ...
    SEARCH_ALIASES = _BctoolsBaseNode._base_aliases('Get crosslinked nucleotides', 'bctools_extract_crosslinked_nucleotides', 'coords2clnt.py', 'crosslinking coordinates', 'threeprime')
    RETURN_TYPES = ('BED',)
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'alignment_coordinates': ('BED', {'description': 'BED alignments'})}, 'optional': {'threeprime': ('BOOLEAN', {'default': False, 'description': 'Set crosslink site one nucleotide downstream of the 3-prime end'}), 'script_path': cls._script_input()}, 'hidden': {'output': ('STRING', {})}}
...
class BctoolsExtractAlignmentEndsNode(_BctoolsBaseNode):
    ...
    SEARCH_ALIASES = _BctoolsBaseNode._base_aliases('Extract alignment ends', 'bctools_extract_alignment_ends', 'extract_aln_ends.py', 'SAM', 'BAM', 'outer coordinates')
    RETURN_TYPES = ('BED',)
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'alignments': ('FILE', {'description': 'SAM or BAM alignments'})}, 'optional': {'script_path': cls._script_input()}, 'hidden': {'output': ('STRING', {})}}
...
class BctoolsExtractBarcodesNode(_BctoolsBaseNode):
    ...
    SEARCH_ALIASES = _BctoolsBaseNode._base_aliases('Extract barcodes', 'bctools_extract_barcodes', 'extract_bcs.py', 'barcode pattern', 'cleaned reads')
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'reads': ('FASTQ', {'description': 'Barcoded FASTQ reads'})}, 'optional': {'barcode_pattern': ('STRING', {'default': '', 'pattern': '^[XN]*$', 'description': '5-prime pattern where X bases are extracted and N bases are retained'}), 'script_path': cls._script_input()}, 'hidden': {'output': ('STRING', {})}}
...
class BctoolsMergePcrDuplicatesNode(_BctoolsBaseNode):
    ...
    SEARCH_ALIASES = _BctoolsBaseNode._base_aliases('Merge PCR duplicates', 'bctools_merge_pcr_duplicates', 'merge_pcr_duplicates.py', 'PCR duplicates', 'unique molecular identifiers')
    RETURN_TYPES = ('BED',)
    RETURN_NAMES = ('events',)
    ...
    @classmethod
    def render_command(cls, inputs: dict[str, Any]) -> str:
        return _shell_join([cls._script(inputs), str(inputs.get('alignments_bed', '')), str(inputs.get('barcode_library', '')), '--outfile', cls._output_path(inputs)])
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'alignments_bed': ('BED', {'description': 'BED6 alignments with read IDs'}), 'barcode_library': ('FASTQ', {'description': 'FASTQ UMI barcode library'})}, 'optional': {'script_path': cls._script_input()}, 'hidden': {'output': ('STRING', {})}}
...
class BctoolsRemoveTailNode(_BctoolsBaseNode):
    ...
    SEARCH_ALIASES = _BctoolsBaseNode._base_aliases("Remove 3'-end nts", 'bctools_remove_tail', 'remove_tail.py', '3-prime tail', 'FASTQ trimming')
    RETURN_TYPES = ('FASTQ',)
    ...
    @classmethod
    def render_command(cls, inputs: dict[str, Any]) -> str:
        return _shell_join([cls._script(inputs), str(inputs.get('reads_fastq', '')), str(cls._length(inputs)), '>', cls._output_path(inputs)])
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'reads_fastq': ('FASTQ', {'description': 'FASTQ reads'})}, 'optional': {'length': ('INT', {'default': 0, 'min': 0, 'description': 'Number of 3-prime bases to remove'}), 'script_path': cls._script_input()}, 'hidden': {'output': ('STRING', {})}}
...
class BctoolsRemoveSpuriousEventsNode(_BctoolsBaseNode):
    ...
    SEARCH_ALIASES = _BctoolsBaseNode._base_aliases('Remove spurious', 'bctools_remove_spurious_events', 'rm_spurious_events.py', 'spurious events', 'crosslinking events', 'threshold')
    RETURN_TYPES = ('BED',)
    ...
    @classmethod
    def render_command(cls, inputs: dict[str, Any]) -> str:
        return _shell_join([cls._script(inputs), str(inputs.get('events', '')), '--threshold', str(cls._threshold(inputs)), '--outfile', cls._output_path(inputs)])
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'events': ('BED', {'description': 'BED6 crosslinking events'})}, 'optional': {'threshold': ('FLOAT', {'default': 0.1, 'min': 0, 'description': 'Fraction of the maximum count used to remove low-support events'}), 'script_path': cls._script_input()}, 'hidden': {'output': ('STRING', {})}}
