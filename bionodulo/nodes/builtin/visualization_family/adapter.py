"""Shared parsers and native renderers for focused visualization nodes."""
...
PLOTLY_JS_GIT_COMMIT = "22efc2fb76f4c890a2c33448e6f1485ecab77f26"
PYSAM_AUDIT_VERSION = "0.23.3"
PYSAM_GIT_COMMIT = "cefbaa9079b0b2ad65dd8d60a532bc3eb31389ee"
PYBIGWIG_AUDIT_VERSION = "0.3.24"
PYBIGWIG_GIT_COMMIT = "7300b0a4599e7f72085c3c27c19b375e3a2c2cc0"
HTS_SPECS_GIT_COMMIT = "da617203a9527537746e200abda2885bec3a822c"
PLOTLY_JS_SOURCE_URL = f"https://github.com/plotly/plotly.js/tree/{PLOTLY_JS_GIT_COMMIT}"
PYSAM_SOURCE_URL = f"https://github.com/pysam-developers/pysam/tree/{PYSAM_GIT_COMMIT}"
PYBIGWIG_SOURCE_URL = f"https://github.com/deeptools/pyBigWig/tree/{PYBIGWIG_GIT_COMMIT}"
VCF_SPEC_SOURCE_URL = f"https://github.com/samtools/hts-specs/blob/{HTS_SPECS_GIT_COMMIT}/VCFv4.5.tex"
SAM_SPEC_SOURCE_URL = f"https://github.com/samtools/hts-specs/blob/{HTS_SPECS_GIT_COMMIT}/SAMv1.tex"
PNG_SPECIFICATION_URL = "https://www.w3.org/TR/png-3/"
SVG_SPECIFICATION_URL = "https://www.w3.org/TR/SVG2/"
BEDGRAPH_SPECIFICATION_URL = "https://genome.ucsc.edu/goldenPath/help/bedgraph.html"


class VisualizationNode(BaseNode):
    """Pinned product-native rendering contract shared by this family."""
    ...
    DOCUMENTATION_URL = (
        ...
        f"{BIO_NODULO_BASELINE_COMMIT}/bionodulo/nodes/builtin/visualization.py"
    )
    SOURCE_URL = DOCUMENTATION_URL
    UPSTREAM_SOURCE = "bionodulo/nodes/builtin/visualization.py"
    STATIC_RENDERER = "BioNodulo native SVG and PNG renderer using the Python standard library"
    STATIC_FORMAT_AUTHORITIES = (SVG_SPECIFICATION_URL, PNG_SPECIFICATION_URL)
    INTERACTIVE_RENDERER = f"Plotly.js {PLOTLY_JS_VERSION} loaded from cdn.plot.ly"
    INTERACTIVE_GIT_COMMIT = PLOTLY_JS_GIT_COMMIT
    INTERACTIVE_SOURCE_URL = PLOTLY_JS_SOURCE_URL
    INTERACTIVE_RUNTIME_SEMANTICS = (
        "HTML outputs embed deterministic data and layout JSON but load the pinned Plotly.js CDN script "
        "when opened; PNG and SVG outputs are self-contained."
    )
    PREVIEW_SEMANTICS = (
        "Successful runs register the produced image or HTML path with the execution context when "
        "register_preview is available."
    )
    EXIT_SEMANTICS = (
        "Missing inputs, malformed rows, unsupported modes, and invalid numeric bounds raise before "
        "a successful output mapping is returned."
    )
        ...
        for section in ("required", "optional"):
            for name, spec in cls.INPUT_TYPES().get(section, {}).items():
                ...
                if isinstance(type_spec, list):
                    if name != "format":
                        allowed = tuple(str(item).lower() for item in type_spec)
                        if str(value).strip().lower() not in allowed:
                            return f"Input '{name}' must be one of: {', '.join(type_spec)}"
                else:
                    if type_spec == "BOOLEAN" and not isinstance(value, bool):
                    ...
                    elif type_spec in {"FILE", "BAI"} and not str(value).strip():
                ...
                choices = None if name == "layout" else config.get("options")
                if choices and str(value).strip().lower() not in {str(choice).lower() for choice in choices}:
    ...
    def PLAN_OUTPUTS(cls, inputs: dict[str, Any], output_dir: str | Path) -> list[Path]:
        output_format = str(inputs.get("format", "png") or "png").strip().lower()
        format_spec = cls.INPUT_TYPES().get("optional", {}).get("format")
        if format_spec and output_format not in tuple(format_spec[0]):
            raise ValueError(f"Unsupported {cls.NODE_ID} format: {output_format}")
        node_dir = Path(output_dir) / cls.NODE_ID
        return [node_dir / pattern.format(format=output_format) for pattern in cls.OUTPUT_PATTERNS]
...
def _read_ma_rows(
    ...
    *,
    ...
    if not rows:
        raise ValueError(
            f"No numeric positive rows found for columns '{mean_column}', '{logfc_column}', and '{pvalue_column}'"
        )
    return rows
...
def _read_coverage_table(path: Path, *, region: tuple[str, int, int]) -> list[CoverageBin]:
    ...
    with path.open("r", encoding="utf-8-sig", newline="") as handle:
        ...
        if has_header:
            ...
            cov_field = (
                field_map.get("coverage") or field_map.get("depth") or field_map.get("value") or field_map.get("score")
            )
...
def _assign_tree_coordinates(
    ...
    *,
    ...
    leaf_positions = {
        id(leaf): layout.top + (index + 1) * layout.plot_height / (len(leaves) + 1) for index, leaf in enumerate(leaves)
    }
...
def _polar_tree_positions(
    ...
    *,
    ...
    leaf_angles = {
        id(leaf): (-math.pi / 2) + (2 * math.pi * index / max(1, len(leaves))) for index, leaf in enumerate(leaves)
    }
    positions: dict[int, tuple[float, float]] = {}
...
def _histogram(
    values: tuple[float, ...], bins: int, low: float | None, high: float | None
...
def _render_svg(
    ...
    *,
    ...
    parts = [
        ...
        f'<line x1="{zero_x:.2f}" y1="{y0}" x2="{zero_x:.2f}" y2="{y1}" stroke="#CBD5E1" stroke-width="1"/>',
        ...
        traces.append(
            {
                ...
                "text": [
                    (f"{html.escape(point.gene)}<br>log2 fold change: {point.logfc:.6g}<br>p-value: {point.pvalue:.6g}")
                    for point in group
                ],
...
def _render_ma_svg(
    ...
    *,
    ...
    parts = [
        ...
        f'<rect x="{x0}" y="{y0}" width="{layout.plot_width}" height="{layout.plot_height}" '
        'fill="#F8FAFC" stroke="#CBD5E1" stroke-width="1"/>',
        f'<line x1="{x0}" y1="{zero_y:.2f}" x2="{x1}" y2="{zero_y:.2f}" stroke="#111827" stroke-width="1.2"/>',
...
def _scatter_marker_payload(
    ...
    *,
    ...
    marker: dict[str, Any] = {
        "size": [
            round(_scatter_radius(point, point_size=point_size, size_range=size_range) * 2.0, 2) for point in points
        ],
        "opacity": _clamp(alpha, 0.1, 1.0),
        "line": {"color": "#ffffff", "width": 0.8},
    }
        ...
        traces.append(
            {
                ...
                "text": [
                    _scatter_hover_text(point, color_column=color_column, size_column=size_column) for point in points
                ],
                "hovertemplate": "%{text}<extra></extra>",
...
def _render_line_svg(
    ...
    *,
    ...
    for series_index, item in enumerate(series):
        ...
        point_text = " ".join(
            f"{_project_xy_x(x, bounds, layout):.2f},{_project_xy_y(y, bounds, layout):.2f}" for x, y in ordered_points
        )
    ...
    *,
    ...
    hover_text = [
        [
            ...
            else (f"{html.escape(matrix.row_labels[row_index])}<br>{html.escape(matrix.column_labels[col_index])}: NA")
    ...
    traces = [
        {
            ...
            "customdata": [[item.chromosome, item.start, item.end, item.coverage] for item in bins],
            "hovertemplate": (
                "%{customdata[0]}:%{customdata[1]}-%{customdata[2]}<br>coverage: %{customdata[3]}<extra></extra>"
            ),
            "marker": {
                "color": colour,
                "opacity": 0.78,
...
def _render_tree_html(
    ...
    *,
        ...
        branch_traces.append(
            {
                ...
                "hovertemplate": ("branch length: %{customdata[1]}<br>node: %{customdata[0]}<extra></extra>"),
                "showlegend": False,
            }
        )
    ...
    tip_trace = {
        ...
        "hovertemplate": ("<b>%{customdata[0]}</b><br>branch length: %{customdata[1]}<extra></extra>"),
        "marker": {
            "color": "#2563EB",
            "size": 7,
    ...
    bootstrap_nodes = [
        node
        for node in _tree_internal_nodes(root)
        if (support := _bootstrap_value(node.name)) is not None and show_bootstrap and support >= bootstrap_threshold
    ]
    if bootstrap_nodes:
        traces.append(
            {
...
def _draw_panel_frame(
    pixels: bytearray, layout: PlotLayout, panel: tuple[float, float, float, float]
        ...
        traces.append(
            {
                ...
                "hovertemplate": ("%{customdata[0]}<br>%{customdata[1]}-%{customdata[2]}<extra>Chromosomes</extra>"),
                "showlegend": index == 0,
            }
        )
        ...
        traces.append(
            {
                ...
                "customdata": [
                    _circos_interval_payload(gene.chromosome, gene.start, gene.end, gene.label) for _ in angle_values
                ],
                "line": {"color": "#16A34A", "width": 10},
                "hovertemplate": (
                    "%{customdata[3]}<br>%{customdata[0]}:%{customdata[1]}-%{customdata[2]}<extra>Genes</extra>"
                ),
        ...
        traces.append(
            {
                ...
                "hovertemplate": ("%{customdata[2]}<br>%{customdata[0]}:%{customdata[1]}<extra>Variants</extra>"),
        ...
        traces.append(
            {
                ...
                "hovertemplate": (
                    "%{customdata[0]}:%{customdata[1]}-%{customdata[2]}<br>CNV: %{customdata[4]}<extra>CNV</extra>"
                ),
            ...
            traces.append(
                {
                    ...
                    "customdata": [[item.chromosome, item.start, item.end, item.coverage] for item in coverage],
                    "hovertemplate": (
...
def _draw_igv_variants_png(
    ...
    for variant in variants:
        ...
        _draw_line(
            pixels, layout.width, layout.height, x, int(round(y + 8)), x, int(round(y + height - 8)), (220, 38, 38)
        )
    ...
    *,
    ...
    for point, plot_x in plot_points:
        ...
        colour = (
            sig_rgb
            if point.pvalue < significance_threshold
            else _hex_to_rgb(_manhattan_colour(chr_colors, chromosome_index[point.chromosome]))
        )
        _draw_circle(pixels, layout.width, layout.height, x, y, radius, colour)
...
def _render_bar_svg(
    ...
    *,
    ...
    if orientation == "horizontal":
        ...
        parts.append(
            f'<line x1="{zero_x:.2f}" y1="{y0}" x2="{zero_x:.2f}" y2="{y1}" stroke="#111827" stroke-width="1.4"/>'
        )
        ...
        parts.append(
            f'<line x1="{x0}" y1="{zero_y:.2f}" x2="{x1}" y2="{zero_y:.2f}" stroke="#111827" stroke-width="1.4"/>'
        )
...
class _ForestPlotContract(VisualizationNode):
    """Create a forest plot from meta-analysis effect estimates."""

    LEGACY_NODE_ID = "forest_plot"
    OUTPUT_PATTERNS = ("forest_plot.{format}",)
...
class _LineChartContract(VisualizationNode):
    """Create a multi-series line chart from a CSV/TSV table."""

    LEGACY_NODE_ID = "line_chart"
    OUTPUT_PATTERNS = ("line_chart.{format}",)
...
class _HeatmapContract(VisualizationNode):
    """Create a heatmap from a row-labelled numeric matrix."""

    LEGACY_NODE_ID = "heatmap"
    OUTPUT_PATTERNS = ("heatmap.{format}",)
...
class _ManhattanPlotContract(VisualizationNode):
    """Create a GWAS Manhattan plot from association results."""

    LEGACY_NODE_ID = "manhattan_plot"
    OUTPUT_PATTERNS = ("manhattan_plot.{format}",)
...
class _CoveragePlotContract(VisualizationNode):
    ...
    OPTIONAL_PYBIGWIG_AUDIT_VERSION = PYBIGWIG_AUDIT_VERSION
    OPTIONAL_PYBIGWIG_GIT_COMMIT = PYBIGWIG_GIT_COMMIT
    OPTIONAL_PYBIGWIG_SOURCE_URL = PYBIGWIG_SOURCE_URL
...
class _PhylogeneticTreeViewerContract(VisualizationNode):
    """Render phylogenetic trees from Newick files."""

    LEGACY_NODE_ID = "phylo_tree_viewer"
    OUTPUT_PATTERNS = ("phylo_tree.{format}",)
    FORMAT_SEMANTICS = "BioNodulo's pinned source implements its own strict Newick parser."
...
class _VCFStatsChartContract(VisualizationNode):
    ...
    LEGACY_NODE_ID = "vcf_stats_chart"
    OUTPUT_PATTERNS = ("vcf_stats.{format}", "vcf_stats.json")
    FORMAT_SPEC_VERSION = "VCF 4.5"
    FORMAT_SPEC_GIT_COMMIT = HTS_SPECS_GIT_COMMIT
    FORMAT_SPEC_SOURCE_URL = VCF_SPEC_SOURCE_URL
    DISPLAY_NAME = "VCF Stats Chart"
    CATEGORY = "visualization"
    DESCRIPTION = (
        "Generate comprehensive VCF statistics charts: variant types, quality, Ti/Tv, and chromosome distribution."
    )
...
class _CircosPlotContract(VisualizationNode):
    """Create a static Circos-style circular genome plot."""

    LEGACY_NODE_ID = "circos_plot"
    OUTPUT_PATTERNS = ("circos_plot.{format}",)
    BACKEND_SEMANTICS = "Product-native Circos-style renderer; the Circos executable is not invoked."
...
class _IGVSnapshotContract(VisualizationNode):
    """Create an IGV-style static genome browser snapshot."""

    LEGACY_NODE_ID = "igv_snapshot"
    OUTPUT_PATTERNS = ("igv_snapshot.{format}",)
    OPTIONAL_PYSAM_AUDIT_VERSION = PYSAM_AUDIT_VERSION
    OPTIONAL_PYSAM_GIT_COMMIT = PYSAM_GIT_COMMIT
    OPTIONAL_PYSAM_SOURCE_URL = PYSAM_SOURCE_URL
    OPTIONAL_PYBIGWIG_AUDIT_VERSION = PYBIGWIG_AUDIT_VERSION
    OPTIONAL_PYBIGWIG_GIT_COMMIT = PYBIGWIG_GIT_COMMIT
    OPTIONAL_PYBIGWIG_SOURCE_URL = PYBIGWIG_SOURCE_URL
    BACKEND_SEMANTICS = "Product-native IGV-style renderer; IGV or igv.js is not launched."
...
class _BarChartContract(VisualizationNode):
    """Create a bar chart from a CSV/TSV table."""

    LEGACY_NODE_ID = "bar_chart"
    OUTPUT_PATTERNS = ("bar_chart.{format}",)
...
class _VolcanoPlotContract(VisualizationNode):
    """Create a volcano plot from differential-expression results."""

    LEGACY_NODE_ID = "volcano_plot"
    OUTPUT_PATTERNS = ("volcano_plot.{format}",)
...
class _MAPlotContract(VisualizationNode):
    """Create an MA plot from differential-expression results."""

    LEGACY_NODE_ID = "ma_plot"
    OUTPUT_PATTERNS = ("ma_plot.{format}",)
...
class _ScatterPlotContract(VisualizationNode):
    """Create an XY scatter plot from a CSV/TSV table."""

    LEGACY_NODE_ID = "scatter_plot"
    OUTPUT_PATTERNS = ("scatter_plot.{format}",)
