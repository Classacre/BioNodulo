"""kaiju2krona — taxonomy node(s). One tool per file (extracted from wrapped_protein_taxonomy.py)."""
...
class Kaiju2KronaNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'kaiju', 'kaiju2krona', 'Krona import', 'selected ranks', 'taxonomy sunburst']
    RETURN_TYPES = ('TSV',)
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        cmd = ['kaiju2krona', '-t', f'{reference}/nodes.dmp', '-n', f'{reference}/names.dmp', '-i', str(inputs.get('kaiju_table', '')), '-o', f'{out}/kaiju_krona.tsv']
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        ...
        return {'required': {'kaiju_table': ('TSV', {'description': 'Kaiju output table'}), 'reference_database': ('DIRECTORY', {'description': 'Kaiju database directory containing nodes.dmp and names.dmp'})}, 'optional': {'include_unclassified': ('BOOLEAN', {'default': False, 'description': 'Include count for unclassified reads'}), 'selected_ranks': ('STRING', {'default': [], 'options': ranks, 'multiple': True, 'description': 'Taxonomic ranks to print as dot-delimited Krona paths'})}, 'hidden': {'output': ('STRING', {})}}
