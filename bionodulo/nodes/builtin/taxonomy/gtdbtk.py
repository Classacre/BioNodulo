"""gtdbtk — taxonomy node(s). One tool per file (extracted from wrapped_variant_assembly.py)."""
...
class GTDBTkClassifyWFNode(CommandNode):
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        cmd.extend(['&&', 'export', f"GTDBTK_DATA_PATH={inputs.get('gtdbtk_data_path', '')}", '&&', 'gtdbtk', 'classify_wf', '--genome_dir', input_dir, '--extension', extension, '--out_dir', output_dir, '--cpus', str(inputs.get('threads', 4)), '--min_perc_aa', str(inputs.get('min_perc_aa', 10))])
        if inputs.get('force'):
            cmd.append('--force')
        ...
        if inputs.get('output_process_log'):
            cmd.extend(['&&', 'cat', f'{output_dir}/gtdbtk.warnings.log', f'{output_dir}/gtdbtk.log', '>', f'{out}/process.log'])
        return cmd
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('FASTA_LIST', {'description': 'Genome FASTA or FASTA.GZ files to classify'}), 'gtdbtk_data_path': ('DIRECTORY', {'description': 'Local GTDB-Tk reference database path'})}, 'optional': {'extension': ('STRING', {'default': 'fna.gz', 'description': 'Input genome extension visible to GTDB-Tk'}), 'threads': ('INT', {'default': 4, 'min': 1, 'max': 256, 'display': 'slider'}), 'min_perc_aa': ('INT', {'default': 10, 'min': 0, 'max': 100}), 'force': ('BOOLEAN', {'default': False, 'advanced': True}), 'min_af': ('FLOAT', {'default': 0.65, 'min': 0, 'max': 1}), 'full_tree': ('BOOLEAN', {'default': False, 'advanced': True}), 'skip_ani_screen': ('BOOLEAN', {'default': True, 'description': 'Skip ANI screen when a Mash DB is unavailable', 'advanced': True}), 'output_process_log': ('BOOLEAN', {'default': False, 'description': 'Emit combined GTDB-Tk warnings and process log'})}, 'hidden': {'output': ('STRING', {})}}
