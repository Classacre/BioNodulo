...
class XCMSRetentionCorrectionNode(MetabolomicsCommandNode):
    ...
    EXIT_SEMANTICS = (
        "XCMS stops when the RDS is not an XCMS experiment, obiwarp cannot align the samples, "
        "or correspondence/gap filling fails; BioNodulo requires all three artifacts."
    )

    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {
            "required": {"xcms_object": ("FILE", {"description": "XcmsExperiment RDS from peak detection"})},
            "optional": {
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        sample_groups = split_paths(inputs.get("sample_groups"))
        if sample_groups:
            sample_group_block = textwrap.dedent(
                f"""\
                sample_groups <- {r_string_vector(sample_groups)}
        ...
        script = textwrap.dedent(
            ...
            if (!is(xdata, "XcmsExperiment") && !is(xdata, "XCMSnExp")) {{
                stop("XCMS Retention Time Correction requires an XcmsExperiment or XCMSnExp RDS object.")
            }}
