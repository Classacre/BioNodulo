"""Shared contracts for focused developer utility nodes."""
...
CPYTHON_DATETIME_SOURCE_URL = f"https://github.com/python/cpython/blob/{CPYTHON_GIT_COMMIT}/Lib/_pydatetime.py"
CPYTHON_BUILTINS_SOURCE_URL = f"https://github.com/python/cpython/blob/{CPYTHON_GIT_COMMIT}/Python/bltinmodule.c"


class UtilityDevNode(BaseNode):
    """Pinned Python semantics shared by native developer utilities."""
    ...
    DOCUMENTATION_URL = (
        ...
        f"{BIO_NODULO_BASELINE_COMMIT}/bionodulo/nodes/builtin/utility_dev.py"
    )
    SOURCE_URL = DOCUMENTATION_URL
    UPSTREAM_SOURCE = "bionodulo/nodes/builtin/utility_dev.py"
    RUNTIME_VERSION = CPYTHON_VERSION
    RUNTIME_GIT_COMMIT = CPYTHON_GIT_COMMIT
    RUNTIME_SOURCE_URLS = (
        CPYTHON_ASYNCIO_SOURCE_URL,
        CPYTHON_CODECS_SOURCE_URL,
        CPYTHON_DATETIME_SOURCE_URL,
        CPYTHON_BUILTINS_SOURCE_URL,
    )
    EXIT_SEMANTICS = "Invalid modes, bounds, dates, encodings, and file paths raise before success."
...
def _safe_output_name(value: Any) -> str:
    name = Path(str(value or "type_cast.txt")).name
    safe = "".join(char if char.isalnum() or char in {"-", "_", "."} else "_" for char in name)
    safe = safe.strip(".")
    return safe or "type_cast.txt"
...
class _DebugContract(UtilityDevNode):
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {
            "required": {
                "value": ("ANY", {"default": "", "description": "Value to debug"}),
            },
            "optional": {
...
class _BreakpointContract(UtilityDevNode):
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {
            "required": {
                "value": ("ANY", {"default": "", "description": "Value to inspect"}),
            },
            "optional": {
...
class _DateTimeContract(UtilityDevNode):
    """Get current date/time, format timestamps, and add/subtract days."""

    LEGACY_NODE_ID = "datetime"
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        ...
        return {
            ...
            "optional": {
                ...
                "timezone": (["local", "UTC"], {"default": "local", "description": "Timezone mode"}),
            },
            "hidden": {},
        }

    async def run(self, **kwargs: Any) -> tuple[str, int, str]:
        self.require_valid_inputs(kwargs, skip_choices=frozenset({"operation", "timezone"}))
        ...
        timezone_mode = str(kwargs.get("timezone", "local") or "local").strip().lower()
        if timezone_mode not in {"local", "utc"}:
            raise ValueError(f"Unsupported timezone: {timezone_mode}")
        tz = timezone.utc if timezone_mode == "utc" else None
        now = datetime.now(tz)
...
class _TypeCastContract(UtilityDevNode):
    """Convert between primitive types and simple file content."""

    LEGACY_NODE_ID = "type_cast"
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {
            "required": {
                ...
                "value": ("ANY", {"default": "", "description": "Value to convert"}),
            },
            "optional": {
    ...
    def PLAN_OUTPUTS(cls, inputs: dict[str, Any], output_dir: str | Path) -> list[Path]:
        target_type = str(inputs.get("target_type", "STRING") or "STRING")
        if target_type != "FILE_FROM_STRING":
            return []
        return [Path(output_dir) / cls.NODE_ID / _safe_output_name(inputs.get("output_name", "type_cast.txt"))]

    async def run(self, **kwargs: Any) -> tuple[str, int, float, bool, str]:
        self.require_valid_inputs(kwargs, skip_choices=frozenset({"target_type"}))
