"""crossmap — alignment node(s). One tool per file (extracted from wrapped_alignment_taxonomy.py)."""
...
class CrossMapBamNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'CrossMap', 'crossmap_bam', 'liftover BAM', 'coordinate conversion', 'BAM assembly conversion', 'chain file', 'optional BAM tags']
    RETURN_TYPES = ('BAM',)
    RETURN_NAMES = ('output',)
    ...
    def render_command(cls, inputs: dict[str, Any]) -> str:
        ...
        if inputs.get('optional_tags'):
            cmd.append('-a')
        cmd.extend(['-m', str(inputs.get('insert_size', 200.0)), '-s', str(inputs.get('insert_size_stdev', 30.0)), '-t', str(inputs.get('insert_size_fold', 3.0)), staged_bam, f'{out}/output'])
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('BAM', {'description': 'BAM alignments to lift over'}), 'input_chain': ('TXT', {'description': 'LiftOver chain file'})}, 'optional': {'index_source': ('STRING', {'default': 'history', 'options': cls.INDEX_SOURCE_OPTIONS, 'description': 'Galaxy source selector for cached or history chain files'}), 'optional_tags': ('BOOLEAN', {'default': False, 'description': 'Add CrossMap optional BAM mapping tags'}), 'insert_size': ('FLOAT', {'default': 200.0, 'min': 0, 'description': 'Average paired-end insert size in bp'}), 'insert_size_stdev': ('FLOAT', {'default': 30.0, 'min': 0, 'description': 'Standard deviation of paired-end insert size'}), 'insert_size_fold': ('FLOAT', {'default': 3.0, 'min': 0, 'description': 'Proper-pair distance threshold as a multiple of insert-size stdev'})}, 'hidden': {'output': ('STRING', {})}}
