"""bowtie2 — alignment node(s). One tool per file (extracted from alignment.py)."""
...
class Bowtie2BuildNode(CommandNode):
    ...
    VERSION = '2.5.5'
    COMMAND = ['bowtie2-build', '--threads', '{inputs.threads}', '{inputs.reference}', '{output}/index']
    ...
    def reference_cache_id(cls, inputs: dict[str, Any]) -> Optional[str]:
        ...
        return _rc.compute_ref_id('bowtie2', [_rc.file_identity(inputs.get('reference', '')), f'bowtie2-{cls.VERSION}'])
...
class Bowtie2AlignNode(CommandNode):
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        cmd = ['bowtie2', '-p', str(inputs.get('threads', 8)), '-x', str(inputs.get('index', ''))]
        r1, r2 = _split_reads(inputs)
        if r1 and r2:
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'index': ('INDEX_DIR', {'description': 'Bowtie2 index prefix'}), 'threads': ('INT', {'default': 8, 'min': 1, 'max': 64, 'display': 'slider'})}, 'optional': {'reads': ('FASTQ_LIST', {'description': 'Paired-end FASTQ reads [R1, R2]'}), 'r1': ('FASTQ', {'description': 'Forward reads (R1)'}), 'r2': ('FASTQ', {'description': 'Reverse reads (R2)'}), 'rg_id': ('STRING', {'default': '1', 'label': 'Read Group ID'}), 'rg_sample': ('STRING', {'default': 'sample', 'label': 'Sample Name'}), 'very_sensitive': ('BOOLEAN', {'default': False, 'label': 'Very Sensitive', 'advanced': True}), 'no_mixed': ('BOOLEAN', {'default': False, 'label': 'No Mixed', 'advanced': True})}, 'hidden': {'output': ('STRING', {})}}
...
class Bowtie2IndexNode(CommandNode):
    ...
    VERSION = '2.5.3'
    SHELL = True
    COMMAND = ['bowtie2-inspect', '{inputs.index}', '>', '{output}/reference.fasta']

    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'index': ('INDEX_DIR', {'description': 'Bowtie2 index prefix'})}, 'optional': {}, 'hidden': {'output': ('STRING', {})}}
