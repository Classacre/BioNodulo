"""tracy — alignment node(s). One tool per file (extracted from wrapped_taxonomy_humann.py)."""
...
class TracyAlignNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'Tracy', 'tracy Align', 'tracy trace alignment', 'Sanger chromatogram alignment', 'AB1 trace alignment', 'SCF trace alignment']
    ...
    OPTION_DEFAULTS = {'kmer': 15, 'support': 3, 'maxindel': 1000, 'trim': 0, 'trimLeft': 50, 'trimRight': 50, 'linelimit': 60, 'gapopen': -10, 'gapext': -4, 'match': 3, 'mismatch': -5}
    INT_MIN_OPTIONS = {'kmer': 1, 'support': 1, 'maxindel': 1, 'linelimit': 1, 'trimLeft': 0, 'trimRight': 0, 'match': 0}
    ...
    def render_command(cls, inputs: dict[str, Any]) -> str:
        ...
        if inputs.get('index_genome'):
            ...
            setup = [f'bgzip -c {shlex.quote(reference)} > {shlex.quote(indexed_reference)}', _shell_join(['tracy', 'index', '-o', f'{out}/genome.fasta.fm9', indexed_reference])]
