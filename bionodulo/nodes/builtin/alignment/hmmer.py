"""hmmer — alignment node(s). One tool per file (extracted from wrapped_protein_taxonomy.py)."""
...
class HMMERHmmalignNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'hmmer', 'hmmalign', 'profile HMM alignment', 'Stockholm alignment']
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        cmd.extend(['--outformat', 'stockholm', str(inputs.get('hmmfile', '')), str(inputs.get('seq', ''))])
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'seq': ('FASTA', {'description': 'FASTA sequences to align against the profile HMM'}), 'hmmfile': ('FILE', {'description': 'Single-profile HMM model'}), 'input_format_select': ('STRING', {'default': '--amino', 'options': ['--amino', '--dna', '--rna'], 'description': 'Alphabet for the sequences and model'})}, 'optional': {'trim': ('BOOLEAN', {'default': False, 'description': 'Trim terminal nonaligned residues from the Stockholm alignment'})}, 'hidden': {'output': ('STRING', {})}}
