"""bwameth — alignment node(s). One tool per file (extracted from wrapped_alignment_taxonomy.py)."""
...
class BwaMethNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'bwameth', 'bwa-meth', 'BWA methylation', 'BS-Seq alignment', 'bisulfite sequencing', 'WGBS', 'RRBS']
    RETURN_TYPES = ('BAM',)
    RETURN_NAMES = ('output',)
    ...
    def render_command(cls, inputs: dict[str, Any]) -> str:
        ...
        if cls._reference_source(inputs) == 'history':
            ...
            commands.extend([_shell_join(['mkdir', '-p', index_dir]), _shell_join(['ln', '-sf', str(inputs.get('reference', '')), reference_path]), _shell_join(['bwameth.py', 'index', reference_path])])
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input_singles': ('FASTQ', {'description': 'Single-end FASTQ reads'})}, 'optional': {'reference_source': ('STRING', {'default': 'history', 'options': cls.REFERENCE_SOURCE_OPTIONS, 'description': 'Use a history FASTA or an indexed bwa-meth reference'}), 'reference': ('FASTA', {'default': '', 'description': 'History reference FASTA to index with bwa-meth'}), 'reference_path': ('FASTA', {'default': '', 'description': 'Built-in/indexed bwa-meth reference FASTA path'}), 'single_or_paired_opts': ('STRING', {'default': 'single', 'options': cls.READ_MODE_OPTIONS, 'description': 'Single-end, paired-end, or paired collection input mode'}), 'input_mate1': ('FASTQ', {'default': '', 'description': 'First paired-end FASTQ read'}), 'input_mate2': ('FASTQ', {'default': '', 'description': 'Second paired-end FASTQ read'}), 'input_mate1_forward': ('FASTQ', {'default': '', 'description': 'Forward FASTQ read from a paired collection'}), 'input_mate1_reverse': ('FASTQ', {'default': '', 'description': 'Reverse FASTQ read from a paired collection'}), 'readGroup': ('STRING', {'default': '', 'description': 'Optional complete SAM read group string, such as @RG\\tID:foo\\tSM:bar'})}, 'hidden': {'output': ('STRING', {})}}
