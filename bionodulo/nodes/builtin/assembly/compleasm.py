"""compleasm — assembly node(s). One tool per file (extracted from wrapped_variant_assembly.py)."""
...
class CompleasmNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'compleasm', 'compleasm genome completeness', 'BUSCO lineage', 'assembly completeness', 'miniprot', 'single-copy orthologs']
    ...
    VERSION = '0.2.6'
    SHELL = True
    OUTPUT_FILES = {'full_table_busco': ('full_table_busco_format.tsv', 'full_table_busco.tsv'), 'full_table': ('full_table.tsv', 'full_table.tsv'), 'miniprot': ('miniprot_output.gff', 'miniprot.gff'), 'translated_protein': ('translated_protein.fasta', 'translated_protein.fasta'), 'summary': ('summary.txt', 'summary.txt')}
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        cmd = ['mkdir', '-p', galaxy_db, '&&', 'ln', '-s', f'{cls._database_path(inputs)}/lineages/{lineage}', f'{galaxy_db}/{lineage}', '&&', 'touch', f'{galaxy_db}/{lineage}.done', '&&', 'compleasm', 'run', '-a', str(inputs.get('input', '')), '-o', galaxy_output, '--mode', str(inputs.get('mode', 'busco')), '-L', galaxy_db, '-l', lineage, '-t', f"${{GALAXY_SLOTS:-{inputs.get('threads', 1)}}}"]
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('FASTA', {'description': 'Input genome assembly FASTA'}), 'busco_database_path': ('DIRECTORY', {'description': 'Cached BUSCO database root containing lineage directories'}), 'lineage_dataset': ('STRING', {'description': 'BUSCO lineage dataset name'})}, 'optional': {'mode': ('STRING', {'default': 'busco', 'options': ['busco', 'lite'], 'description': 'Use BUSCO/hmmsearch mode or lite mode'}), 'specified_contigs': ('STRING', {'default': '', 'description': 'Optional space-separated contig names to evaluate'}), 'outputs': ('STRING_LIST', {'default': ['full_table_busco'], 'options': cls.OUTPUT_ORDER, 'description': 'Compleasm outputs to copy from Galaxy work directory'}), 'threads': ('INT', {'default': 1, 'min': 1, 'max': 128, 'display': 'slider'})}, 'hidden': {'output': ('STRING', {})}}
    ...
    def VALIDATE_INPUTS(cls, inputs: dict[str, Any]) -> bool | str:
        ...
        if specified_contigs and (not re.fullmatch('[0-9A-Za-z_ ]+', specified_contigs)):
