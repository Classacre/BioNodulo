"""assembly — assembly node(s). One tool per file (extracted from wrapped_phylogeny_assembly.py)."""
...
class AssemblyStatsNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'assembly-stats', 'Assembly stats', 'asm2stats.minmaxgc.pl', 'genome assembly metrics', 'assembly visualisation', 'snail plot', 'N50', 'GC content']
    ...
    def render_command(cls, inputs: dict[str, Any]) -> str:
        ...
        parts = ['SRC="$(dirname $(which asm2stats.pl))/../opt/assembly-stats"', f'mkdir -p {shlex.quote(json_dir)}', f'cp -r "$SRC/css/" {shlex.quote(output_files)}', f'cp -r "$SRC/js/" {shlex.quote(output_files)}', f"cp {tool_directory}/d3-tip.js {shlex.quote(f'{output_files}/js/d3-tip.js')}", f"cp {tool_directory}/assembly-stats.html {shlex.quote(f'{out}/output.html')}", f'cp {tool_directory}/assembly-stats.html {shlex.quote(output_files)}', f"asm2stats.minmaxgc.pl {input_fasta} > {shlex.quote(f'{json_dir}/output.assembly-stats.json')}"]
        return ' && '.join(parts)
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input_fasta': ('FASTA', {'description': 'Genome assembly FASTA'})}, 'optional': {'output_format': ('STRING', {'default': 'html', 'options': ['html', 'json'], 'description': 'Galaxy output format'})}, 'hidden': {'output': ('STRING', {}), 'tool_directory': ('STRING', {})}}
