"""angsd — population_genetics node(s). One tool per file (extracted from wrapped_amplicon_trimming.py)."""
...
class ANGSDNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'ANGSD', 'angsd', 'ANGSD internal counts', 'X-contamination', 'low coverage sequencing', 'population genetics', 'BAM internal counts']
    RETURN_TYPES = ('FILE',)
    ...
    def render_command(cls, inputs: dict[str, Any]) -> str:
        ...
        cmd = ['angsd', '-bam', bam_filelist, '-out', f'{out}/output', '-nThreads', slots, '-doCounts', '1', '-iCounts', '1', '-minMapQ', str(inputs.get('min_mapq', 20)), '-minQ', str(inputs.get('min_q', 20)), '-r', str(inputs.get('region', ''))]
    ...
    def VALIDATE_INPUTS(cls, inputs: dict[str, Any]) -> bool | str:
        ...
        if not re.fullmatch('[\\w\\d\\._:-]+', region):
        ...
        for name, default, minimum in (('min_mapq', 20, 0), ('min_q', 20, 0), ('threads', 1, 1)):
            try:
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input_bams': ('BAM', {'multiple': True, 'description': 'Coordinate-sorted BAM files'}), 'region': ('STRING', {'description': 'Target region in ANGSD format, such as chr or chr:start-end', 'regex': '^[\\w\\d\\._:-]+$'})}, 'optional': {'bam_indices': ('FILE', {'default': [], 'multiple': True, 'advanced': True, 'description': 'Optional BAM index files aligned with input_bams'}), 'min_mapq': ('INT', {'default': 20, 'min': 0, 'description': 'Discard reads below this mapping quality'}), 'min_q': ('INT', {'default': 20, 'min': 0, 'description': 'Discard bases below this quality'}), 'threads': ('INT', {'default': 1, 'min': 1, 'max': 128, 'description': 'ANGSD thread count'})}, 'hidden': {'output': ('STRING', {})}}
...
class ANGSDContaminationNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'ANGSD X-Contamination', 'angsd_contamination', 'X chromosome contamination', 'nuclear contamination', 'ancient DNA contamination', 'HapMap ChrX', 'EAGER contamination']
    ...
    def render_command(cls, inputs: dict[str, Any]) -> str:
        out = _out(inputs)
        commands = [_shell_join(['mkdir', '-p', out]), _shell_join(['ln', '-s', str(inputs.get('icnts_file', '')), f'{out}/counts.icnts.gz']), _shell_join(['ln', '-s', str(inputs.get('hapmap_file', '')), f'{out}/hapmap.gz']), f'cd {shlex.quote(out)}', 'contamination -a counts.icnts.gz -h hapmap.gz 2> contamination_report.out', 'python3 print_x_contamination.py contamination_report.out']
        return ' && '.join(commands)
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'icnts_file': ('FILE', {'description': 'ANGSD internal counts output (.icnts.gz)'}), 'hapmap_file': ('FILE', {'description': 'HapMap ChrX reference file (.gz)'})}, 'optional': {'generate_json': ('BOOLEAN', {'default': False, 'description': 'Also expose the MultiQC JSON report generated by the parser'})}, 'hidden': {'output': ('STRING', {})}}
