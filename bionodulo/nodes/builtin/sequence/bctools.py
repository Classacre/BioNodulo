"""bctools — sequence node(s). One tool per file (extracted from wrapped_alignment_taxonomy.py)."""
...
class BctoolsConvertToBinaryBarcodeNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'bctools', 'Create binary barcodes', 'bctools_convert_to_binary_barcode', 'convert_bc_to_binary_RY.py', 'binary barcodes', 'RY-space barcodes', 'UMI', 'uvCLAP', 'FLASH']
    RETURN_TYPES = ('FASTQ',)
    ...
    @classmethod
    def render_command(cls, inputs: dict[str, Any]) -> str:
        cmd = [str(inputs.get('script_path', 'convert_bc_to_binary_RY.py') or 'convert_bc_to_binary_RY.py'), str(inputs.get('barcodes', '')), '>', cls._output_path(inputs)]
        return _shell_join(cmd)
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'barcodes': ('FASTQ', {'description': 'FASTQ file containing barcode reads to convert'})}, 'optional': {'script_path': ('FILE', {'default': 'convert_bc_to_binary_RY.py', 'advanced': True, 'description': 'Path to the bctools convert_bc_to_binary_RY.py executable'})}, 'hidden': {'output': ('STRING', {})}}
