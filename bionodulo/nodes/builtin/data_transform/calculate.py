"""calculate — data_transform node(s). One tool per file (extracted from wrapped_alignment_taxonomy.py)."""
...
class CalculateNumericParamNode(BaseNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'calculate_numeric_param', 'Calculate numeric parameter value', 'numeric parameter', 'arithmetic parameter', 'integer parameter', 'float parameter', 'workflow expression']
    ...
    _AST_OPERATORS = (ast.Expression, ast.BinOp, ast.Constant, ast.UnaryOp, ast.Add, ast.Sub, ast.Mult, ast.Div, ast.Pow, ast.Mod, ast.USub, ast.UAdd)
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'components': ('JSON', {'is_list': True, 'description': 'Galaxy repeat components with component_value and arithmetic operator'})}, 'optional': {'component_value': ('FLOAT', {'default': 1.0, 'description': 'Single numeric component value'}), 'arith': ('STRING', {'default': '+', 'options': cls.ARITHMETIC_OPERATORS, 'description': 'Arithmetic operator for the single component fallback'}), 'output_type': ('STRING', {'default': 'integer', 'options': cls.OUTPUT_TYPES, 'description': 'Galaxy output type selector'})}, 'hidden': {}}
