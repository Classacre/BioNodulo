"""compose — data_transform node(s). One tool per file (extracted from wrapped_alignment_taxonomy.py)."""
...
class ComposeTextParamNode(BaseNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'compose_text_param', 'Compose text parameter value', 'workflow text parameter', 'text parameter', 'integer parameter', 'float parameter', 'concatenate parameter values']
    RETURN_TYPES = ('STRING',)
    RETURN_NAMES = ('out1',)
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'components': ('JSON', {'is_list': True, 'description': 'Galaxy repeat components with select_param_type and component_value'})}, 'optional': {'select_param_type': ('STRING', {'default': 'text', 'options': cls.PARAM_TYPES, 'description': 'Parameter type for a single component fallback'}), 'component_value': ('STRING', {'default': '', 'description': 'Single component value'})}, 'hidden': {}}
