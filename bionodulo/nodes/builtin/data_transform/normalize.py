"""normalize — data_transform node(s). One tool per file (extracted from normalize_data.py)."""
...
class NormalizeDataNode(BaseNode):
    """Apply statistical normalization to numeric table columns."""
    NODE_ID = 'normalize_data'
    DISPLAY_NAME = 'Normalize Data'
    CATEGORY = 'data_transform'
    DESCRIPTION = 'Apply statistical normalization to numeric table data: min-max, z-score, MAD z-score, quantile, log transforms, CPM, TPM-like scaling, and CLR.'
    SEARCH_ALIASES = ['normalize', 'standardize', 'scale', 'z-score', 'min-max', 'quantile normalize', 'log transform', 'cpm', 'tpm', 'clr', 'centered log ratio']
    RETURN_TYPES = ('CSV',)
    RETURN_NAMES = ('normalized_table',)
    REQUIRES_EXTERNAL_TOOLS = False
    _METHODS = ['min_max', 'z_score', 'z_score_mad', 'quantile', 'log2', 'log10', 'log1p', 'cpm', 'tpm_from_counts', 'clr']

    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'table': ('FILE', {'description': 'CSV or TSV table with numeric data'}), 'method': ('STRING', {'default': 'z_score', 'options': cls._METHODS})}, 'optional': {'id_columns': ('STRING', {'default': ''}), 'axis': ('STRING', {'default': 'columns', 'options': ['columns', 'rows']}), 'pseudocount': ('FLOAT', {'default': 1.0, 'min': 0.0, 'max': 1000.0}), 'min_max_range': ('STRING', {'default': '0,1'}), 'output_type': ('STRING', {'default': 'AUTO', 'options': ['AUTO', 'CSV', 'TSV']})}, 'hidden': {}}
    ...
    def _quantile(matrix: list[list[float]]) -> list[list[float]]:
        ...
        rank_means = [sum((sorted_column[rank] for sorted_column in sorted_columns)) / len(sorted_columns) for rank in range(len(matrix))]
    ...
    def _cpm(matrix: list[list[float]]) -> list[list[float]]:
        ...
        return [[value / column_sums[index] * 1000000 for index, value in enumerate(row)] for row in matrix]
    ...
    def _tpm_like(cls, matrix: list[list[float]]) -> list[list[float]]:
        ...
        return [[value / column_sums[index] * 1000000 for index, value in enumerate(row)] for row in cpm]
    ...
    @staticmethod
    def _apply_axis(matrix: list[list[float]], axis: str, transform: Any) -> list[list[float]]:
        if not matrix:
            return []
        if axis == 'columns':
    ...
    def _scale_min_max(values: list[float], target_min: float, target_max: float) -> list[float]:
        ...
        return [(value - source_min) / (source_max - source_min) * (target_max - target_min) + target_min for value in values]
