"""filter — data_transform node(s). One tool per file (extracted from filter_vcf.py)."""
...
class FilterVCFNode(CommandNode):
    ...
    DESCRIPTION = 'Filter VCF/BCF variant files using bcftools. Supports QUAL, DP, AF thresholds, region filtering, biallelic/SNP/indel selection, PASS-only filtering, and custom bcftools filter expressions.'
    SEARCH_ALIASES = ['vcf filter', 'bcf filter', 'variant filter', 'bcftools filter', 'quality filter', 'depth filter', 'region filter', 'snp filter', 'indel filter', 'pass filter', 'allele frequency']
    RETURN_TYPES = ('VCF_GZ',)
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'vcf': ('VCF,VCF_GZ,BCF', {'description': 'Input VCF, bgzipped VCF, or BCF file'})}, 'optional': {'regions': ('STRING', {'default': '', 'tooltip': 'Comma-separated regions: chr1:1000-2000,chr2:500-1000'}), 'targets': ('STRING', {'default': '', 'tooltip': 'Comma-separated target regions (streamed, indexed input required)'}), 'min_qual': ('FLOAT', {'default': 0.0, 'min': 0.0, 'max': 999999.0, 'tooltip': 'Minimum QUAL score'}), 'min_dp': ('INT', {'default': 0, 'min': 0, 'max': 999999, 'tooltip': 'Minimum INFO/DP depth'}), 'max_dp': ('INT', {'default': 0, 'min': 0, 'max': 999999, 'tooltip': 'Maximum INFO/DP depth (0 = no limit)'}), 'min_af': ('FLOAT', {'default': 0.0, 'min': 0.0, 'max': 1.0, 'tooltip': 'Minimum allele frequency'}), 'max_af': ('FLOAT', {'default': 0.0, 'min': 0.0, 'max': 1.0, 'tooltip': 'Maximum allele frequency (0 = no limit)'}), 'pass_only': ('BOOLEAN', {'default': False, 'tooltip': 'Keep only PASS variants'}), 'biallelic_only': ('BOOLEAN', {'default': False, 'tooltip': 'Keep only biallelic sites'}), 'snp_only': ('BOOLEAN', {'default': False, 'tooltip': 'Keep only SNPs'}), 'indel_only': ('BOOLEAN', {'default': False, 'tooltip': 'Keep only indels'}), 'custom_filter': ('STRING', {'default': '', 'tooltip': "Custom bcftools filter expression, e.g. INFO/ANN ~ 'missense_variant'"}), 'samples': ('STRING', {'default': '', 'tooltip': 'Comma-separated sample names to retain (empty = all)'}), 'output_type': (['VCF', 'VCF_GZ', 'BCF'], {'default': 'VCF_GZ'}), 'threads': ('INT', {'default': 1, 'min': 1, 'max': 64})}, 'hidden': {'output_dir': ('STRING', {})}}
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        if filters:
            return [*view_cmd, '|', 'bcftools', 'filter', '--include', ' && '.join(filters), '|', *output_cmd]
