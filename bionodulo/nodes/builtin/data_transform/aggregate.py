"""aggregate — data_transform node(s). One tool per file (extracted from aggregate.py)."""
...
class AggregateNode(BaseNode):
    ...
    DESCRIPTION = 'Group a table by one or more columns and compute aggregate statistics such as sum, count, mean, median, min, max, std, var, first, last, and nunique.'
    SEARCH_ALIASES = ['aggregate', 'group by', 'summarize', 'sum', 'count', 'average', 'mean', 'median', 'min', 'max', 'stddev', 'groupby', 'rollup']
    RETURN_TYPES = ('CSV',)
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'table': ('FILE', {'description': 'CSV or TSV table with a header row'}), 'group_columns': ('STRING', {'default': '', 'description': 'Comma-separated group columns'}), 'agg_column': ('STRING', {'default': '', 'description': 'Column to aggregate'}), 'agg_function': ('STRING', {'default': 'sum', 'options': cls._FUNCTIONS})}, 'optional': {'agg_column_2': ('STRING', {'default': ''}), 'agg_function_2': ('STRING', {'default': '', 'options': [''] + cls._FUNCTIONS}), 'output_type': ('STRING', {'default': 'AUTO', 'options': ['AUTO', 'CSV', 'TSV']})}, 'hidden': {}}
    ...
    async def run(self, **kwargs: Any) -> tuple[str]:
        ...
        aggregations = self._aggregation_specs(fieldnames, str(kwargs.get('agg_column', '')), str(kwargs.get('agg_function', 'sum')), str(kwargs.get('agg_column_2', '')), str(kwargs.get('agg_function_2', '')))
    ...
    @classmethod
    def _aggregation_specs(cls, fieldnames: list[str], agg_column: str, agg_function: str, agg_column_2: str, agg_function_2: str) -> list[tuple[str, str]]:
