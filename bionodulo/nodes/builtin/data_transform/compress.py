"""compress — data_transform node(s). One tool per file (extracted from wrapped_alignment_taxonomy.py)."""
...
class CompressFileNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'compress_file', 'Compress file(s)', 'gzip compression', 'gzip -cf', 'gzipped output', 'compress dataset']
    RETURN_TYPES = ('FILE',)
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('FILE', {'description': 'Dataset to compress with gzip'})}, 'optional': {}, 'hidden': {'output': ('STRING', {})}}
