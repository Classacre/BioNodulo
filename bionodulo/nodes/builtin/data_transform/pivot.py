"""pivot — data_transform node(s). One tool per file (extracted from pivot_table.py)."""
...
class PivotTableNode(BaseNode):
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'table': ('FILE', {'description': 'CSV or TSV table with a header row'}), 'operation': ('STRING', {'default': 'pivot_wide', 'options': ['pivot_wide', 'melt_long', 'pivot_table_agg']})}, 'optional': {'index_column': ('STRING', {'default': '', 'description': 'Row ID column for pivot operations'}), 'index_columns': ('STRING', {'default': '', 'description': 'Comma-separated row ID columns for pivot operations'}), 'names_from': ('STRING', {'default': '', 'description': 'Column whose values become wide headers'}), 'columns_column': ('STRING', {'default': '', 'description': 'Column whose values become wide headers'}), 'values_from': ('STRING', {'default': '', 'description': 'Column whose values fill pivot cells'}), 'values_column': ('STRING', {'default': '', 'description': 'Column whose values fill pivot cells'}), 'fill_value': ('STRING', {'default': ''}), 'id_columns': ('STRING', {'default': '', 'description': 'Comma-separated columns to preserve when melting'}), 'id_vars': ('STRING', {'default': '', 'description': 'Comma-separated columns to preserve when melting'}), 'value_columns': ('STRING', {'default': '', 'description': 'Comma-separated columns to melt'}), 'value_vars': ('STRING', {'default': '', 'description': 'Comma-separated columns to melt'}), 'variable_name': ('STRING', {'default': 'variable'}), 'var_name': ('STRING', {'default': '', 'description': 'Name of the long-format variable column'}), 'value_name': ('STRING', {'default': 'value'}), 'agg_func': ('STRING', {'default': 'sum', 'options': ['sum', 'mean', 'count', 'min', 'max', 'median', 'std']}), 'delimiter': ('STRING', {'default': 'auto', 'options': ['auto', 'tsv', 'csv']}), 'output_type': ('STRING', {'default': 'AUTO', 'options': ['AUTO', 'CSV', 'TSV']})}, 'hidden': {}}
    ...
    async def run(self, **kwargs: Any) -> tuple[str]:
        ...
        if operation == 'pivot_wide':
            suffix = 'wide'
            out_fields, out_rows = self._pivot_wide(fieldnames, rows, self._first_value(kwargs, 'index_column', 'index_columns'), self._first_value(kwargs, 'names_from', 'columns_column'), self._first_value(kwargs, 'values_from', 'values_column'), str(kwargs.get('fill_value', '') or ''), aggregate=False)
        elif operation == 'melt_long':
            suffix = 'long'
            out_fields, out_rows = self._melt_long(fieldnames, rows, self._first_value(kwargs, 'id_columns', 'id_vars'), self._first_value(kwargs, 'value_columns', 'value_vars'), self._first_value(kwargs, 'variable_name', 'var_name', default='variable'), str(kwargs.get('value_name', 'value') or 'value'))
        elif operation == 'pivot_table_agg':
            suffix = 'pivot'
            out_fields, out_rows = self._pivot_wide(fieldnames, rows, self._first_value(kwargs, 'index_column', 'index_columns'), self._first_value(kwargs, 'names_from', 'columns_column'), self._first_value(kwargs, 'values_from', 'values_column'), str(kwargs.get('fill_value', '') or ''), aggregate=True, agg_func=str(kwargs.get('agg_func', 'sum') or 'sum'))
        else:
    ...
    def _pivot_wide(self, fieldnames: list[str], rows: list[dict[str, str]], index_column: str, names_from: str, values_from: str, fill_value: str, aggregate: bool, agg_func: str='sum') -> tuple[list[str], list[dict[str, Any]]]:
        ...
        for row in rows:
            ...
            if index_value not in grouped:
                grouped[index_value] = {column: row.get(column, '') for column in index_columns}
            if aggregate:
    ...
    def _melt_long(self, fieldnames: list[str], rows: list[dict[str, str]], id_columns_value: str, value_columns_value: str, variable_name: str, value_name: str) -> tuple[list[str], list[dict[str, Any]]]:
