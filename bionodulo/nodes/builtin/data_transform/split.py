"""split — data_transform node(s). One tool per file (extracted from split_file.py)."""
...
class SplitFileNode(BaseNode):
    """Split a file into multiple chunks."""
    NODE_ID = 'split_file'
    DISPLAY_NAME = 'Split File'
    CATEGORY = 'data_transform'
    DESCRIPTION = 'Split a file into chunks by line count, column value, approximate file size, or record count.'
    SEARCH_ALIASES = ['split', 'chunk', 'partition', 'divide', 'shard', 'split csv', 'split fastq', 'split by chromosome', 'batch split', 'file chunks']
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'file': ('FILE', {'description': 'Input file to split'}), 'split_mode': ('STRING', {'default': 'by_line_count', 'options': ['by_line_count', 'by_column_value', 'by_file_size', 'by_record_count']})}, 'optional': {'lines_per_chunk': ('INT', {'default': 1000, 'min': 1, 'max': 10000000}), 'split_column': ('STRING', {'default': ''}), 'max_size_mb': ('FLOAT', {'default': 100.0, 'min': 1.0, 'max': 10000.0}), 'records_per_chunk': ('INT', {'default': 1000, 'min': 1, 'max': 10000000}), 'has_header': ('BOOLEAN', {'default': True}), 'output_type': ('STRING', {'default': 'AUTO', 'options': ['AUTO', 'CSV', 'TSV', 'FASTQ', 'FASTA']})}, 'hidden': {}}
    ...
    async def run(self, **kwargs: Any) -> tuple[str]:
        ...
        if split_mode == 'by_line_count':
            self._split_by_line_count(input_path, output_dir, separator, extension, int(kwargs.get('lines_per_chunk', 1000) or 1000), bool(kwargs.get('has_header', True)))
        elif split_mode == 'by_column_value':
            self._split_by_column_value(input_path, output_dir, separator, extension, str(kwargs.get('split_column', '') or ''), bool(kwargs.get('has_header', True)))
        elif split_mode == 'by_file_size':
            self._split_by_file_size(input_path, output_dir, extension, float(kwargs.get('max_size_mb', 100.0) or 100.0))
        elif split_mode == 'by_record_count':
            self._split_by_record_count(input_path, output_dir, separator, extension, int(kwargs.get('records_per_chunk', 1000) or 1000), bool(kwargs.get('has_header', True)))
        else:
    ...
    def _split_by_line_count(self, input_path: Path, output_dir: Path, output_separator: str, extension: str, lines_per_chunk: int, has_header: bool) -> None:
    ...
    def _split_by_column_value(self, input_path: Path, output_dir: Path, output_separator: str, extension: str, split_column: str, has_header: bool) -> None:
        if not has_header:
    ...
    @staticmethod
    def _split_by_record_count(input_path: Path, output_dir: Path, output_separator: str, extension: str, records_per_chunk: int, has_header: bool) -> None:
        ...
        if output_separator in {',', '\t'} and suffix not in {'.fa', '.fasta', '.fna', '.fq', '.fastq'}:
            SplitFileNode()._split_by_line_count(input_path, output_dir, output_separator, extension, records_per_chunk, has_header)
            return
    ...
    def _safe_name(value: str) -> str:
        return re.sub('[^A-Za-z0-9._-]+', '_', str(value).strip()).strip('._')
