"""extract — data_transform node(s). One tool per file (extracted from data_transform.py)."""
...
class ExtractColumnsNode(BaseNode):
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'table': ('FILE', {'description': 'CSV or TSV table with a header row'}), 'columns': ('STRING', {'description': 'Comma-separated columns in output order'})}, 'optional': {'rename_map': ('STRING', {'default': '', 'description': 'Comma-separated old:new renames'}), 'column_indices': ('STRING', {'default': '', 'description': 'Comma-separated 0-based column indices'}), 'rename_to': ('STRING', {'default': '', 'description': 'Comma-separated output column names'}), 'drop_mode': ('BOOLEAN', {'default': False, 'description': 'Drop selected columns instead of keeping them'}), 'delimiter': ('STRING', {'default': 'auto', 'options': ['auto', 'tsv', 'csv']}), 'output_type': ('STRING', {'default': 'AUTO', 'options': ['AUTO', 'CSV', 'TSV']})}, 'hidden': {}}
    ...
    async def run(self, **kwargs: Any) -> tuple[str]:
        ...
        selected = self._selected_columns(fieldnames, str(kwargs.get('columns', '') or ''), str(kwargs.get('column_indices', '') or ''))
        ...
        output_rows = [{output_name: row.get(source_name, '') for source_name, output_name in zip(selected, output_fields)} for row in rows]
    ...
    def _output_fields(selected: list[str], rename_map: dict[str, str], rename_to: str) -> list[str]:
        ...
        if positional_names:
            if len(positional_names) != len(selected):
                raise ValueError(f'rename_to length ({len(positional_names)}) must match selected columns ({len(selected)})')
