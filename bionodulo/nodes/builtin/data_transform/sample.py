"""sample — data_transform node(s). One tool per file (extracted from sample_subset.py)."""
...
class SampleSubsetNode(BaseNode):
    """Subset records from sequence and table files."""
    NODE_ID = 'sample_subset'
    DISPLAY_NAME = 'Sample Subset'
    CATEGORY = 'data_transform'
    DESCRIPTION = 'Randomly subset records from sequence or table files. Supports reproducible random sampling, first-N selection, and every-Nth selection.'
    SEARCH_ALIASES = ['sample', 'subset', 'downsample', 'random sample', 'fastq subset', 'fasta subset', 'reduce size']
    RETURN_TYPES = ('FILE',)
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'file': ('FASTQ,FASTA,CSV,TSV,FASTQ_LIST', {}), 'n': ('INT', {'default': 1000, 'min': 1, 'max': 100000000})}, 'optional': {'mode': (['random', 'first_n', 'every_nth', 'stratified'], {'default': 'random'}), 'seed': ('INT', {'default': 42, 'min': 0, 'max': 999999}), 'stratify_column': ('STRING', {'default': ''}), 'every_n': ('INT', {'default': 10, 'min': 2, 'max': 1000000}), 'output_type': (['AUTO', 'CSV', 'TSV', 'FASTQ', 'FASTA'], {'default': 'AUTO'})}, 'hidden': {}}
    ...
    @staticmethod
    def _subset_records(records: list[list[str]], mode: str, n: int, seed: int, every_n: int) -> list[list[str]]:
        if mode == 'first_n':
    ...
    @staticmethod
    def _subset_table(input_path: Path, output_path: Path, mode: str, n: int, seed: int, every_n: int, stratify_column: str) -> None:
