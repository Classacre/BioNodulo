"""write — utils node(s). One tool per file (extracted from utility_file_format.py)."""
...
class WriteFileNode(BaseNode):
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'content': ('STRING', {'default': '', 'multiline': True, 'description': 'Content to write'}), 'file_path': ('STRING', {'default': '', 'description': 'Destination file path'})}, 'optional': {'format': ('STRING', {'default': 'text', 'options': ['text', 'json']}), 'encoding': ('STRING', {'default': 'utf-8', 'description': 'Text encoding'})}, 'hidden': {}}
