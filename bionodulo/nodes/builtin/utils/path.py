"""path — utils node(s). One tool per file (extracted from utility_file_format.py)."""
...
class PathOperationsNode(BaseNode):
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'operation': (['basename', 'dirname', 'extension', 'stem', 'join', 'exists', 'absolute', 'relative', 'is_file', 'is_dir', 'with_suffix', 'with_name'], {'default': 'basename', 'description': 'Path operation'}), 'path': ('STRING', {'default': '', 'description': 'File or directory path'})}, 'optional': {'path_b': ('STRING', {'default': '', 'description': 'Second path component for join or base path for relative'}), 'suffix': ('STRING', {'default': '', 'description': 'Replacement suffix for with_suffix'}), 'name': ('STRING', {'default': '', 'description': 'Replacement filename for with_name'})}, 'hidden': {}}
