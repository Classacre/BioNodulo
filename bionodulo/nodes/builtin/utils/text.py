"""text — utils node(s). One tool per file (extracted from utility_collections.py)."""
...
class TextTemplateNode(BaseNode):
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'template': ('STRING', {'default': '', 'multiline': True, 'description': 'Template using $name or ${name} placeholders'}), 'variables': ('STRING', {'default': '{}', 'multiline': True, 'description': 'JSON object with template variables'})}, 'optional': {}, 'hidden': {}}
