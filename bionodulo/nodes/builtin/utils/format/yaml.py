"""yaml — utils/format node(s). One tool per file (extracted from utility_file_format.py)."""
...
class YMLOperationsNode(BaseNode):
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'operation': (['get', 'set', 'keys', 'to_json', 'validate', 'parse', 'pretty', 'pretty_print', 'stringify'], {'default': 'parse', 'description': 'YAML operation'}), 'yaml_input': ('STRING', {'default': '', 'multiline': True, 'description': 'YAML string or path to YAML file'})}, 'optional': {'key': ('STRING', {'default': '', 'description': 'Dot-separated key path'}), 'value': ('STRING', {'default': '', 'description': 'Value for set operation'})}, 'hidden': {}}
