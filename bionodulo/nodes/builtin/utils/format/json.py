"""json — utils/format node(s). One tool per file (extracted from utility_file_format.py)."""
...
class JSONOperationsNode(BaseNode):
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'operation': (['get', 'set', 'delete', 'keys', 'pretty', 'pretty_print', 'minify', 'validate', 'parse', 'stringify'], {'default': 'pretty', 'description': 'JSON operation'}), 'json_input': ('STRING', {'default': '{}', 'multiline': True, 'description': 'JSON string or path to JSON file'})}, 'optional': {'key': ('STRING', {'default': '', 'description': 'Dot-separated key path'}), 'value': ('STRING', {'default': '', 'description': 'Value for set operation'}), 'indent': ('INT', {'default': 2, 'min': 0, 'max': 8, 'description': 'Pretty-print indentation'})}, 'hidden': {}}
