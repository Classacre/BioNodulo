"""list — utils node(s). One tool per file (extracted from utility_collections.py)."""
...
class ListOperationsNode(BaseNode):
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'operation': (['join', 'append', 'prepend', 'get', 'slice', 'reverse', 'unique', 'sort', 'length', 'contains'], {'default': 'length', 'description': 'List operation'}), 'items': ('STRING', {'default': '', 'multiline': True, 'description': 'JSON list, comma text, or newline text'})}, 'optional': {'item': ('STRING', {'default': '', 'description': 'Item for append/prepend/contains'}), 'index': ('INT', {'default': 0, 'description': 'Index for get'}), 'start': ('INT', {'default': 0, 'description': 'Start index for slice'}), 'end': ('INT', {'default': -1, 'description': 'End index for slice; negative means through the end'}), 'delimiter': ('STRING', {'default': ',', 'description': 'Delimiter for join'}), 'reverse': ('BOOLEAN', {'default': False, 'description': 'Reverse sort order'})}, 'hidden': {}}
