"""bp — annotation node(s). One tool per file (extracted from wrapped_core_data.py)."""
...
class BpGenbank2Gff3Node(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'bp_genbank2gff3', 'Genbank to GFF3', 'GenBank', 'GFF3', 'BioPerl', 'Unflattener', 'Sequence Ontology', 'Bio::Tools::GFF']
    RETURN_TYPES = ('GFF3',)
    RETURN_NAMES = ('gff3',)
    ...
    def render_command(cls, inputs: dict[str, Any]) -> str:
        ...
        cmd.extend(['--outdir', '-', '--ethresh', cls._ethresh(inputs), cls._model(inputs), '--typesource', cls._typesource(inputs), str(inputs.get('genbank', '')), '>', cls._output_path(inputs)])
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'genbank': ('FILE', {'description': 'GenBank flat file to convert to GFF3'})}, 'optional': {'infer_subfeatures': ('BOOLEAN', {'default': True, 'description': 'Infer exon and mRNA subfeatures'}), 'sofile': ('STRING', {'default': '__none__', 'options': cls.SOFILE_OPTIONS, 'description': 'Sequence Ontology source'}), 'so_url': ('STRING', {'default': '', 'description': 'Sequence Ontology OBO URL when sofile is url'}), 'ethresh': ('STRING', {'default': '1', 'options': cls.ERROR_THRESHOLDS, 'description': 'Error threshold for the BioPerl unflattener'}), 'model': ('STRING', {'default': '--CDS', 'options': cls.MODELS, 'description': 'GFF3 gene model'}), 'typesource': ('STRING', {'default': 'contig', 'description': 'Sequence Ontology type for the landmark feature'})}, 'hidden': {'output': ('STRING', {})}}
