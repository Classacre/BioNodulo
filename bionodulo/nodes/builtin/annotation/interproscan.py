"""interproscan — annotation node(s). One tool per file (extracted from annotation.py)."""
...
class InterProScanNode(CommandNode):
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        cmd = ['interproscan.sh', '-i', str(inputs.get('fasta', '')), '-b', f'{out_dir}/ipr', '-f', 'TSV,JSON,GFF3', '-cpu', str(inputs.get('threads', 4))]
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'fasta': ('FASTA', {'description': 'Protein FASTA'}), 'threads': ('INT', {'default': 4, 'min': 1, 'max': 64})}, 'optional': {'applications': ('STRING', {'default': '', 'description': 'e.g., Pfam,Gene3D,PANTHER'}), 'goterms': ('BOOLEAN', {'default': True}), 'iprlookup': ('BOOLEAN', {'default': True}), 'pathways': ('BOOLEAN', {'default': True})}, 'hidden': {'output': ('STRING', {})}}
