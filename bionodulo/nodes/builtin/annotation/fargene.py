"""fargene — annotation node(s). One tool per file (extracted from wrapped_variant_assembly.py)."""
...
class FargeneNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'fARGene', 'fragmented antibiotic resistance genes', 'antibiotic resistance gene identifier', 'ARG prediction', 'metagenomic resistance genes']
    ...
    def _add_optional_flags(cls, cmd: list[str], inputs: dict[str, Any]) -> None:
        ...
        for key, flag in [('retrieve_whole', '--retrieve-whole'), ('no_orf_predict', '--no-orf-predict'), ('no_quality_filtering', '--no-quality-filtering'), ('no_assembly', '--no-assembly'), ('orf_finder', '--orf-finder'), ('store_peptides', '--store-peptides')]:
            if inputs.get(key):
                cmd.append(flag)
    ...
    def render_command(cls, inputs: dict[str, Any]) -> str:
        ...
        if input_type in {'paired', 'collection'}:
            ...
            cmd.append('*.fasta')
        cmd.extend(['--hmm-model', str(inputs.get('models', 'class_a')), '--output', 'fargene_output', '--tmp-dir', 'tmp', '-p', f"${{GALAXY_SLOTS:-{inputs.get('threads', 4)}}}"])
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input_type': ('STRING', {'options': cls.INPUT_TYPES_ALLOWED, 'description': 'Paired reads, paired collection, or contigs/genomes'}), 'models': ('STRING', {'default': 'class_a', 'options': cls.MODELS, 'description': 'Resistance gene HMM model'})}, 'optional': {'R1': ('FASTQ', {'default': '', 'description': 'Forward reads for paired input'}), 'R2': ('FASTQ', {'default': '', 'description': 'Reverse reads for paired input'}), 'R1_identifier': ('STRING', {'default': '', 'advanced': True, 'description': 'Galaxy element identifier for R1'}), 'R2_identifier': ('STRING', {'default': '', 'advanced': True, 'description': 'Galaxy element identifier for R2'}), 'input_collection': ('FASTQ_LIST', {'default': [], 'multiple': True, 'description': 'Paired read collection'}), 'input_sequence': ('FASTA', {'default': [], 'multiple': True, 'description': 'Input contigs or genomes'}), 'sequence_identifiers': ('STRING', {'default': [], 'multiple': True, 'advanced': True, 'description': 'Galaxy element identifiers for sequences'}), 'score': ('FLOAT', {'default': 0.0, 'min': 0, 'description': 'Threshold for classifying nearly complete genes'}), 'meta_score': ('FLOAT', {'default': 0.0, 'min': 0, 'description': 'Fragment score per amino acid'}), 'protein': ('BOOLEAN', {'default': False, 'description': 'Use protein mode'}), 'min_orf_length': ('INT', {'default': 90, 'min': 1, 'description': 'Minimum predicted ORF length'}), 'retrieve_whole': ('BOOLEAN', {'default': False, 'description': 'Retrieve whole sequence where a hit is detected'}), 'no_orf_predict': ('BOOLEAN', {'default': False, 'description': 'Disable ORF prediction'}), 'no_quality_filtering': ('BOOLEAN', {'default': False, 'description': 'Disable metagenomic quality filtering'}), 'no_assembly': ('BOOLEAN', {'default': False, 'description': 'Skip assembly and contig retrieval'}), 'orf_finder': ('BOOLEAN', {'default': False, 'description': 'Use NCBI ORFfinder instead of prodigal'}), 'store_peptides': ('BOOLEAN', {'default': False, 'description': 'Store translated sequences'}), 'threads': ('INT', {'default': 4, 'min': 1, 'max': 128, 'display': 'slider'})}, 'hidden': {'output': ('STRING', {})}}
