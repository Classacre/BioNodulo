"""annotatemyids — annotation node(s). One tool per file (extracted from wrapped_core_data.py)."""
...
class AnnotateMyIDsNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'annotateMyIDs', 'annotatemyids', 'AnnotationDbi', 'Bioconductor', 'org.Hs.eg.db', 'gene identifier annotation', 'Ensembl to Entrez', 'gene symbols', 'GO annotation', 'KEGG annotation']
    ...
    ID_TYPES = ['ENSEMBL', 'ENSEMBLPROT', 'ENSEMBLTRANS', 'ENTREZID', 'FLYBASE', 'GO', 'PATH', 'MGI', 'REFSEQ', 'SYMBOL', 'ZFIN']
    OUTPUT_COLUMNS = ['ALIAS', 'ENSEMBL', 'ENTREZID', 'EVIDENCE', 'SYMBOL', 'GENENAME', 'REFSEQ', 'GO', 'ONTOLOGY', 'PATH']
    ...
    def _script_body(cls, inputs: dict[str, Any]) -> str:
        ...
        return '\n'.join(['options( show.error.messages=F, error = function () { cat( geterrmessage(), file=stderr() ); q( "no", 1, F ) } )', '', 'loc <- Sys.setlocale("LC_MESSAGES", "en_US.UTF-8")', '', f'id_type <- "{id_type}"', f'organism <- "{organism}"', f'output_cols <- "{output_cols}"', f"file_has_header <- {cls._bool_r(inputs, 'file_has_header')}", f"remove_dups <- {cls._bool_r(inputs, 'remove_dups')}", '', f"""input <- read.table({str(inputs.get('id_file', ''))!r}, header=file_has_header, sep="\\t", quote="")""", 'ids <- as.character(input[, 1])', '', 'if(organism == "Hs"){', '    suppressPackageStartupMessages(library(org.Hs.eg.db))', '    db <- org.Hs.eg.db', '} else if (organism == "Mm"){', '    suppressPackageStartupMessages(library(org.Mm.eg.db))', '    db <- org.Mm.eg.db', '} else if (organism == "Dm"){', '    suppressPackageStartupMessages(library(org.Dm.eg.db))', '    db <- org.Dm.eg.db', '} else if (organism == "Dr"){', '    suppressPackageStartupMessages(library(org.Dr.eg.db))', '    db <- org.Dr.eg.db', '} else if (organism == "Rn"){', '    suppressPackageStartupMessages(library(org.Rn.eg.db))', '    db <- org.Rn.eg.db', '} else if (organism == "At"){', '    suppressPackageStartupMessages(library(org.At.tair.db))', '    db <- org.At.tair.db', '} else if (organism == "Gg"){', '    suppressPackageStartupMessages(library(org.Gg.eg.db))', '    db <- org.Gg.eg.db', '} else if (organism == "Bt"){', '    suppressPackageStartupMessages(library(org.Bt.eg.db))', '    db <- org.Bt.eg.db', '} else {', '    cat(paste("Organism type not supported", organism))', '}', '', 'cols <- unlist(strsplit(output_cols, ","))', 'result <- select(db, keys=ids, keytype=id_type, columns=cols)', '', 'if(remove_dups) {', f'    result <- result[!duplicated(result${id_type}),]', '}', '', f'write.table(result, file={cls._out_tab_path(inputs)!r}, sep="\\t", row.names=FALSE, quote=FALSE)'])
    ...
    def render_command(cls, inputs: dict[str, Any]) -> str:
        ...
        commands = [f'mkdir -p {shlex.quote(out)}', f"cat > {shlex.quote(script_path)} <<'RSCRIPT'\n{cls._script_body(inputs)}\nRSCRIPT\n{run_script}"]
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'id_file': ('TSV', {'description': 'Tabular file whose first column contains identifiers to annotate'})}, 'optional': {'file_has_header': ('BOOLEAN', {'default': False}), 'organism': ('STRING', {'default': 'Hs', 'options': cls.ORGANISMS}), 'id_type': ('STRING', {'default': 'ENSEMBL', 'options': cls.ID_TYPES}), 'output_cols': ('STRING', {'default': list(cls.DEFAULT_OUTPUT_COLUMNS), 'options': cls.OUTPUT_COLUMNS, 'multiple': True, 'display': 'checkboxes'}), 'remove_dups': ('BOOLEAN', {'default': False}), 'rscriptOpt': ('BOOLEAN', {'default': False})}, 'hidden': {'output': ('STRING', {})}}
