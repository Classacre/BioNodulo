"""snpeff — annotation node(s). One tool per file (extracted from annotation.py)."""
...
class SnpEffNode(CommandNode):
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        cmd = ['java', '-jar', f"-Xmx{inputs.get('memory', 4)}g", 'snpEff.jar', '-v', '-stats', f'{out_dir}/summary_report.html']
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'vcf': ('VCF_GZ', {'description': 'VCF to annotate'}), 'genome': ('STRING', {'default': 'GRCh38', 'description': 'SnpEff genome (GRCh38, GRCh37, GRCm39)'}), 'memory': ('INT', {'default': 4, 'min': 1, 'max': 128, 'label': 'Max Memory (GB)'})}, 'optional': {'canonical': ('BOOLEAN', {'default': False, 'description': 'Canonical transcripts only'}), 'no_upstream': ('BOOLEAN', {'default': False}), 'no_downstream': ('BOOLEAN', {'default': False}), 'no_intergenic': ('BOOLEAN', {'default': False})}, 'hidden': {'output': ('STRING', {})}}
