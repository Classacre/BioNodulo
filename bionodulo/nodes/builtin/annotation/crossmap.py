"""crossmap — annotation node(s). One tool per file (extracted from wrapped_alignment_taxonomy.py)."""
...
class CrossMapBedNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'CrossMap', 'crossmap_bed', 'liftover BED', 'coordinate conversion', 'genome assembly conversion', 'chain file']
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('BED', {'description': 'BED file with 3 to 12 columns'}), 'input_chain': ('TXT', {'description': 'LiftOver chain file'})}, 'optional': {'index_source': ('STRING', {'default': 'history', 'options': cls.INDEX_SOURCE_OPTIONS, 'description': 'Galaxy source selector for cached or history chain files'}), 'chromid': ('STRING', {'default': 'a', 'options': cls.CHROMID_OPTIONS, 'description': 'Chromosome ID style: as-is, long chrN, or short N'}), 'merge_unmapped_entries': ('BOOLEAN', {'default': False, 'description': 'Merge failed and converted BED entries into one output'})}, 'hidden': {'output': ('STRING', {})}}
...
class CrossMapGffNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'CrossMap', 'crossmap_gff', 'liftover GFF', 'liftover GTF', 'coordinate conversion', 'GFF assembly conversion', 'GTF assembly conversion', 'chain file']
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('GFF_GTF', {'description': 'GFF3, GFF, or GTF feature annotation to lift over'}), 'input_chain': ('TXT', {'description': 'LiftOver chain file'})}, 'optional': {'index_source': ('STRING', {'default': 'history', 'options': cls.INDEX_SOURCE_OPTIONS, 'description': 'Galaxy source selector for cached or history chain files'}), 'include_fails': ('BOOLEAN', {'default': False, 'description': 'Include failed liftovers in the output with CrossMap fail markers'})}, 'hidden': {'output': ('STRING', {})}}
...
class CrossMapRegionNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'CrossMap', 'crossmap_region', 'liftover BED regions', 'whole region liftover', 'coordinate conversion', 'BED assembly conversion', 'chain file']
    RETURN_TYPES = ('BED',)
    RETURN_NAMES = ('output',)
    REQUIRED_EXECUTABLES = ['CrossMap']
    DOCUMENTATION_URL = f'{DOI_URL}{CROSSMAP_CITATION_DOI}'
    ...
    def render_command(cls, inputs: dict[str, Any]) -> str:
        out = _out(inputs)
        cmd = ['CrossMap', 'region', str(inputs.get('input_chain', '')), str(inputs.get('input', '')), f'{out}/output', '--chromid', cls._chromid(inputs), '--ratio', str(cls._ratio(inputs))]
        return _shell_join(cmd)
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('BED', {'description': 'BED regions to lift over as whole intervals'}), 'input_chain': ('TXT', {'description': 'LiftOver chain file'})}, 'optional': {'index_source': ('STRING', {'default': 'history', 'options': cls.INDEX_SOURCE_OPTIONS, 'description': 'Galaxy source selector for cached or history chain files'}), 'ratio': ('FLOAT', {'default': 0.85, 'min': 0, 'max': 1, 'description': 'Minimum ratio of bases that must remap'}), 'chromid': ('STRING', {'default': 'a', 'options': cls.CHROMID_OPTIONS, 'description': 'Chromosome ID style: as-is, short N, or long chrN'})}, 'hidden': {'output': ('STRING', {})}}
