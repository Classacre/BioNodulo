"""blastxml — annotation node(s). One tool per file (extracted from wrapped_alignment_taxonomy.py)."""
...
class BlastxmlToGappedGff3Node(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'blastxml_to_gapped_gff3', 'BlastXML', 'gapped GFF3', 'BLAST XML', 'match_part', 'Gap', 'GFF3']
    RETURN_TYPES = ('GFF3',)
    RETURN_NAMES = ('output',)
    ...
    @classmethod
    def render_command(cls, inputs: dict[str, Any]) -> str:
        cmd = ['python', str(inputs.get('script_path', 'blastxml_to_gapped_gff3.py') or 'blastxml_to_gapped_gff3.py'), str(inputs.get('blastxml', '')), '--min_gap', str(cls._min_gap(inputs))]
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'blastxml': ('FILE', {'description': 'BLAST XML alignment results'})}, 'optional': {'min_gap': ('INT', {'default': 3, 'min': 0, 'description': 'Maximum gap size before generating a new match_part'}), 'trim': ('STRING', {'default': '--trim_end', 'options': cls.TRIM_OPTIONS, 'description': 'Trim neither end, both ends, or only the alignment end'}), 'script_path': ('FILE', {'default': 'blastxml_to_gapped_gff3.py', 'advanced': True, 'description': 'Path to the Galaxy blastxml_to_gapped_gff3.py helper script'})}, 'hidden': {'output': ('STRING', {})}}
