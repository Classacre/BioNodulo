"""annovar — annotation node(s). One tool per file (extracted from annotation.py)."""
...
class ANNOVARNode(CommandNode):
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        convert = ['convert2annovar.pl', '-format', 'vcf4', '-withzyg', '-includeinfo', vcf, '>', avinput]
        annotate = ['&&', 'table_annovar.pl', avinput, humandb_dir, '-buildver', buildver, '-out', f'{out_dir}/annovar', '-remove', '-protocol', protocol, '-operation', operation, '-nastring', '.', '-vcfinput', '-polish']
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'vcf': ('VCF_GZ', {'description': 'Input VCF'}), 'humandb_dir': ('DIRECTORY', {'description': 'ANNOVAR humandb'}), 'buildver': ('STRING', {'default': 'hg38', 'options': ['hg38', 'hg19']}), 'protocol': ('STRING', {'default': 'refGene,cytoBand,gnomad40_genome,clinvar_20220320'}), 'operation': ('STRING', {'default': 'g,r,f,f', 'description': 'g=gene,r=region,f=filter'})}, 'optional': {}, 'hidden': {'output': ('STRING', {})}}
