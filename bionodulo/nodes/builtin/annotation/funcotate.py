"""funcotate — annotation node(s). One tool per file (extracted from annotation.py)."""
...
class FuncotateTableNode(CommandNode):
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        cmd = ['gatk', 'Funcotator', '-R', str(inputs.get('reference', '')), '-V', vcf, '-O', annotated, '--output-file-format', output_format, '--data-sources-path', str(inputs.get('data_sources', '')), '--ref-version', ref_version]
        ...
        summary_payload = f"'tool\\tgatk Funcotator\\ninput\\t{vcf}\\noutput\\t{annotated}\\nformat\\t{output_format}\\nref_version\\t{ref_version}\\n'"
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'vcf': ('VCF_GZ', {'description': 'Input VCF to annotate'}), 'reference': ('FASTA', {'description': 'Reference FASTA used for the VCF'}), 'data_sources': ('DIRECTORY', {'description': 'Funcotator data sources directory'}), 'ref_version': ('STRING', {'default': 'hg38', 'options': ['hg38', 'hg19']})}, 'optional': {'output_format': ('STRING', {'default': 'MAF', 'options': ['MAF', 'VCF']}), 'transcript_selection_mode': ('STRING', {'default': '', 'options': ['', 'CANONICAL', 'BEST_EFFECT', 'ALL'], 'advanced': True}), 'annotation_defaults': ('STRING', {'default': '', 'description': 'Comma-separated KEY:VALUE defaults', 'advanced': True}), 'annotation_overrides': ('STRING', {'default': '', 'description': 'Comma-separated KEY:VALUE overrides', 'advanced': True}), 'intervals': ('FILE', {'default': '', 'description': 'Optional intervals to annotate', 'advanced': True})}, 'hidden': {'output': ('STRING', {})}}
