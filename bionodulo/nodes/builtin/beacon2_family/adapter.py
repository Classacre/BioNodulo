"""Shared Beacon2 query contracts for focused endpoint nodes."""
# ruff: noqa: F403,F405
...
class BeaconContractNode(ValidatedCommandContract):
    """Beacon2 Galaxy query wrappers pinned to their exact Tools-IUC source."""

    GIT_URL = "https://github.com/galaxyproject/tools-iuc.git"
    GIT_COMMIT = TOOLS_IUC_GIT_COMMIT
    SOURCE_URL = (
        f"https://github.com/galaxyproject/tools-iuc/tree/{TOOLS_IUC_GIT_COMMIT}/tools/beacon2-import"
    )
    PACKAGE_CONSTRAINT = "beacon2-import==2.2.4"
    GALAXY_WRAPPER_VERSION_SOURCE = "Each endpoint retains its exact wrapper VERSION value."
    GALAXY_WRAPPER_GIT_COMMIT = TOOLS_IUC_GIT_COMMIT
    GALAXY_WRAPPER_SOURCE_URL = SOURCE_URL
    UPSTREAM_PACKAGE_GIT_COMMIT = ""
    UPSTREAM_PACKAGE_PIN_STATUS = (
        "The Galaxy wrapper and package version are pinned; no separate upstream package commit is recorded centrally."
    )
    EXIT_SEMANTICS = "Missing database credentials and invalid endpoint filters fail validation before submission."


class _Beacon2SearchBaseContract(BeaconContractNode):
...
class _Beacon2AnalysesContract(_Beacon2SearchBaseContract):
    """Query the analyses collection in a Beacon database."""

    LEGACY_NODE_ID = "beacon2_analyses"
...
class _Beacon2BiosamplesContract(_Beacon2SearchBaseContract):
    """Query the biosamples collection in a Beacon database."""

    LEGACY_NODE_ID = "beacon2_biosamples"
...
class _Beacon2BracketContract(_Beacon2SearchBaseContract):
    """Query Beacon genomic variations by bracketed start and end ranges."""

    LEGACY_NODE_ID = "beacon2_bracket"
...
class _Beacon2CNVContract(_Beacon2SearchBaseContract):
    """Query Beacon copy number variants from genomicVariations."""

    LEGACY_NODE_ID = "beacon2_cnv"
...
class _Beacon2CohortsContract(_Beacon2SearchBaseContract):
    """Query the cohorts collection in a Beacon database."""

    LEGACY_NODE_ID = "beacon2_cohorts"
...
class _Beacon2DatasetsContract(_Beacon2SearchBaseContract):
    """Query the datasets collection in a Beacon database."""

    LEGACY_NODE_ID = "beacon2_datasets"
...
class _Beacon2GeneContract(_Beacon2SearchBaseContract):
    """Query Beacon genomic variants by gene symbol."""

    LEGACY_NODE_ID = "beacon2_gene"
...
class _Beacon2IndividualsContract(_Beacon2SearchBaseContract):
    """Query the individuals collection in a Beacon database."""

    LEGACY_NODE_ID = "beacon2_individuals"
...
class _Beacon2RangeContract(_Beacon2SearchBaseContract):
    """Query Beacon genomic variants by sequence range."""

    LEGACY_NODE_ID = "beacon2_range"
...
class _Beacon2RunsContract(_Beacon2SearchBaseContract):
    """Query the runs collection in a Beacon database."""

    LEGACY_NODE_ID = "beacon2_runs"
...
class _Beacon2SequenceContract(_Beacon2SearchBaseContract):
    """Query Beacon for a precise alternate/reference sequence."""

    LEGACY_NODE_ID = "beacon2_sequence"
