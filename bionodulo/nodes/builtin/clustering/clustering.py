"""clustering — clustering node(s). One tool per file (extracted from wrapped_core_data.py)."""
...
class ClusteringFromDistmatNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'clustering_from_distmat', 'Distance matrix-based hierarchical clustering', 'distance matrix', 'hierarchical clustering', 'SciPy linkage', 'UPGMA', 'WPGMA', 'dendrogram', 'newick', 'cut_tree', 'cluster assignments']
    ...
    def render_command(cls, inputs: dict[str, Any]) -> str:
        ...
        cmd = ['python', str(inputs.get('script_path', 'clustering_from_distmat.py') or 'clustering_from_distmat.py'), str(inputs.get('distmat', '')), 'result', '--method', str(inputs.get('method', 'average') or 'average')]
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'distmat': ('TSV', {'description': 'Symmetric tabular distance matrix with sample names'})}, 'optional': {'method': ('STRING', {'default': 'average', 'options': cls.METHODS, 'description': 'SciPy linkage clustering method'}), 'missing_names': ('STRING', {'default': '', 'options': cls.MISSING_NAMES_OPTIONS, 'description': 'Input omits row names, column names, or neither'}), 'cluster_assignment': ('STRING', {'default': 'dendrogram-only', 'options': cls.CLUSTER_ASSIGNMENT_OPTIONS, 'description': 'Generate only a dendrogram or also cut the tree into clusters'}), 'n_cluster': ('INT', {'default': 5, 'min': 1}), 'height': ('FLOAT', {'default': 5.0}), 'min_cluster_size': ('INT', {'default': 2, 'min': 1}), 'generate_dendrogram': ('BOOLEAN', {'default': False, 'description': 'Also keep the Newick dendrogram when generating assignments'}), 'script_path': ('FILE', {'default': 'clustering_from_distmat.py', 'advanced': True, 'description': 'Path to the Galaxy clustering_from_distmat.py helper script'})}, 'hidden': {'output': ('STRING', {})}}
