"""cd — clustering node(s). One tool per file (extracted from wrapped_core_data.py)."""
...
class CDHitNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'cd-hit', 'cd_hit', 'CD-HIT', 'cd-hit-est', 'cd-hit-2d', 'cd-hit-est-2d', 'sequence clustering', 'non-redundant sequences', 'representative sequences']
    ...
    def render_command(cls, inputs: dict[str, Any]) -> str:
        ...
        cmd = [cls._binary(inputs), '-i', str(inputs.get('fasta_in', '')), '-o', cls._output_path(inputs), '-c', str(inputs.get('similarity', 0.9)), '-n', str(inputs.get('nucleotide_wordsize', 10) if sequence_type == 'nucleotide' else inputs.get('protein_wordsize', 5))]
        ...
        return _shell_join(cmd).replace("'${GALAXY_MEMORY_MB:-0}'", '${GALAXY_MEMORY_MB:-0}').replace("'${GALAXY_SLOTS:-1}'", '${GALAXY_SLOTS:-1}')
    ...
    def VALIDATE_INPUTS(cls, inputs: dict[str, Any]) -> bool | str:
        ...
        if style == 'local':
            for key in ('align_coverage_long', 'align_coverage_short', 'cutoff_diff_len', 'cutoff_diff_len2'):
                ...
                if validation is not True:
                    return validation
            for key, default in (('align_coverage_long_control', 99999999), ('align_coverage_short_control', 99999999), ('align_coverage_min', 0), ('aa_cutoff_diff_len', 999999), ('aa_cutoff_diff_len2', 0)):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'fasta_in': ('FASTA', {'description': 'Primary protein or nucleotide FASTA sequences'})}, 'optional': {'operation': ('STRING', {'default': 'cluster', 'options': cls.OPERATIONS, 'description': 'Cluster one dataset or compare against a second dataset'}), 'fasta_in2': ('FASTA', {'default': '', 'description': 'Second FASTA dataset for cd-hit-2d comparisons'}), 'sequence_type': ('STRING', {'default': 'protein', 'options': cls.SEQUENCE_TYPES, 'description': 'Protein uses cd-hit; nucleotide uses cd-hit-est'}), 'similarity': ('FLOAT', {'default': 0.9, 'min': 0.4, 'max': 1.0, 'description': 'Sequence identity threshold'}), 'protein_wordsize': ('INT', {'default': 5, 'min': 2, 'max': 5, 'description': 'Protein word size'}), 'nucleotide_wordsize': ('INT', {'default': 10, 'min': 4, 'max': 11, 'description': 'Nucleotide word size'}), 'redtol': ('INT', {'default': 2, 'description': 'Tolerance for redundancy in protein mode'}), 'compare_both_strands': ('BOOLEAN', {'default': False, 'description': 'Compare both strands in nucleotide mode'}), 'mask': ('STRING', {'default': '', 'description': 'Masking letters for nucleotide mode, for example NX'}), 'match': ('INT', {'default': 2, 'description': 'Nucleotide match score'}), 'mismatch': ('INT', {'default': -2, 'description': 'Nucleotide mismatch score'}), 'gap': ('INT', {'default': -6, 'description': 'Nucleotide gap opening score'}), 'gap_ext': ('INT', {'default': -1, 'description': 'Nucleotide gap extension score'}), 'band_width': ('INT', {'default': 20, 'min': 1, 'description': 'Alignment band width'}), 'throw_away_len': ('INT', {'default': 10, 'min': 1, 'description': 'Length threshold for throwing away short sequences'}), 'identity_style': ('STRING', {'default': 'global', 'options': cls.IDENTITY_STYLES, 'description': 'Use global or local sequence identity'}), 'align_coverage_long': ('FLOAT', {'default': 0.0, 'min': 0, 'max': 1, 'description': 'Local-mode coverage for longer sequence'}), 'align_coverage_long_control': ('INT', {'default': 99999999, 'min': 0, 'description': 'Maximum uncovered residues for longer sequence'}), 'align_coverage_short': ('FLOAT', {'default': 0.0, 'min': 0, 'max': 1, 'description': 'Local-mode coverage for shorter sequence'}), 'align_coverage_short_control': ('INT', {'default': 99999999, 'min': 0, 'description': 'Maximum uncovered residues for shorter sequence'}), 'align_coverage_min': ('INT', {'default': 0, 'min': 0, 'description': 'Minimum alignment coverage in residues'}), 'cutoff_diff_len': ('FLOAT', {'default': 0.0, 'min': 0, 'max': 1, 'description': 'Length difference cutoff'}), 'aa_cutoff_diff_len': ('INT', {'default': 999999, 'min': 0, 'description': 'Length difference cutoff in residues'}), 'cutoff_diff_len2': ('FLOAT', {'default': 1.0, 'min': 0, 'max': 1, 'description': '2D local-mode length difference cutoff'}), 'aa_cutoff_diff_len2': ('INT', {'default': 0, 'min': 0, 'description': '2D local-mode length difference cutoff in residues'}), 'max_unmatched_per_l': ('FLOAT', {'default': 1.0, 'min': 0, 'max': 1, 'description': 'Maximum unmatched fraction for longer sequence'}), 'max_unmatched_per_s': ('FLOAT', {'default': 1.0, 'min': 0, 'max': 1, 'description': 'Maximum unmatched fraction for shorter sequence'}), 'max_unmatched_len': ('INT', {'default': 99999999, 'min': 0, 'description': 'Maximum unmatched length'}), 'accurate': ('BOOLEAN', {'default': False, 'description': 'Use accurate but slower clustering'}), 'inram': ('BOOLEAN', {'default': True, 'description': 'Store sequences in RAM'}), 'sort_cluster': ('BOOLEAN', {'default': False, 'description': 'Sort clusters by size'}), 'sort_fasta': ('BOOLEAN', {'default': False, 'description': 'Sort output FASTA by cluster size'}), 'print_alignment_overlap': ('BOOLEAN', {'default': False, 'description': 'Print alignment overlap in the .clstr output'}), 'desclen': ('INT', {'default': 20, 'min': 0, 'description': 'Description length for .clstr output'})}, 'hidden': {'output': ('STRING', {})}}
