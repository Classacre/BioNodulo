"""Domain gold-standard visualization nodes.

These wrap the tools the bioinformatics community treats as the standard for
their domain, so our templates produce the figures reviewers expect:

- ``krona``: interactive Krona taxonomy sunburst (metagenomics standard).
- ``scanpy_umap``: Scanpy QC violins + UMAP embedding (single-cell standard).

Both follow the script/command-runner pattern used by ``r_plot``: build the
...
class _LegacyKronaTaxonomyNode(BaseNode):
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {
            "required": {
                "classification": ("FILE", {"label": "Kraken2 classifications", "description": "Per-read Kraken2 output"}),
            },
            "optional": {
                "query_column": ("INT", {"default": 2, "min": 1, "max": 50, "label": "Read-ID column", "advanced": True}),
                "taxid_column": ("INT", {"default": 3, "min": 1, "max": 50, "label": "TaxID column", "advanced": True}),
                "output_name": ("STRING", {"default": "krona.html", "label": "Output filename", "advanced": True}),
            },
        }
    ...
    async def run(self, **kwargs: Any) -> tuple[str, ...]:
        ...
        else:  # pragma: no cover - exercised only outside the executor
            import asyncio
    ...
    @staticmethod
    def render_command(
        *, classification: str, query_column: int, taxid_column: int, output: str
    ) -> list[str]:
        return [
            "ktImportTaxonomy",
            "-q", str(query_column),
...
class _LegacyScanpyUmapNode(BaseNode):
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {
            "required": {
                "matrix_h5": ("FILE", {"label": "10x feature-barcode matrix (.h5)"}),
            },
            "optional": {
                "min_genes": ("INT", {"default": 200, "min": 0, "max": 5000, "label": "Min genes / cell", "advanced": True}),
                "min_cells": ("INT", {"default": 3, "min": 0, "max": 1000, "label": "Min cells / gene", "advanced": True}),
                "n_pcs": ("INT", {"default": 30, "min": 2, "max": 100, "label": "PCs", "advanced": True}),
                "n_neighbors": ("INT", {"default": 15, "min": 2, "max": 100, "label": "Neighbors", "advanced": True}),
                "resolution": ("FLOAT", {"default": 1.0, "min": 0.1, "max": 3.0, "step": 0.1, "label": "Leiden resolution", "advanced": True}),
            },
        }
    ...
    async def run(self, **kwargs: Any) -> tuple[str, ...]:
        ...
        script_path.write_text(
            self._build_script(
                h5=str(kwargs["matrix_h5"]),
                min_genes=int(kwargs.get("min_genes", 200) or 200),
                min_cells=int(kwargs.get("min_cells", 3) or 3),
                n_pcs=int(kwargs.get("n_pcs", 30) or 30),
                n_neighbors=int(kwargs.get("n_neighbors", 15) or 15),
                resolution=float(kwargs.get("resolution", 1.0) or 1.0),
