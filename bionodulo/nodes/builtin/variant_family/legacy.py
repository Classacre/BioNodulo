...
from bionodulo.nodes.builtin.variant_family.evidence import VariantEvidenceContract
from bionodulo.nodes.command_node import CommandNode


class _Sniffles2Contract(VariantEvidenceContract):
    """Call structural variants from long-read alignments with Sniffles2."""
    LEGACY_NODE_ID = "sniffles2"
...
class _Sniffles2CallContract(_Sniffles2Contract):
    """Workflow-compatible Sniffles2 structural variant caller alias."""

    LEGACY_NODE_ID = "sniffles2_call"
...
class _PBSVContract(VariantEvidenceContract):
...
class _VCFComparisonContract(VariantEvidenceContract):
    """Compare two VCF callsets with RTG vcfeval."""
    LEGACY_NODE_ID = "vcf_comparison"
...
class _Strelka2Contract(VariantEvidenceContract):
    """Call germline or somatic small variants with Strelka2."""
    LEGACY_NODE_ID = "strelka2"
...
class _MELTMobileElementsContract(VariantEvidenceContract):
    """Call mobile element insertions with MELT Single mode."""

    LEGACY_NODE_ID = "melt_mobile_elements"
...
class _SURVIVORMergeContract(VariantEvidenceContract):
    """Merge structural variant callsets into a consensus VCF with SURVIVOR."""
    LEGACY_NODE_ID = "survivor_merge"
...
class _CuteSVContract(VariantEvidenceContract):
    """Call long-read structural variants with cuteSV."""
    LEGACY_NODE_ID = "cutesv"
...
class _SVIMContract(VariantEvidenceContract):
...
class _SmooveContract(VariantEvidenceContract):
    """Call and genotype structural variants with smoove."""
    LEGACY_NODE_ID = "smoove"
...
class _CNVkitBatchContract(VariantEvidenceContract):
    """Run the CNVkit batch pipeline for copy-number analysis."""
    LEGACY_NODE_ID = "cnvkit_batch"
...
class _CNVkitCallContract(VariantEvidenceContract):
    """Convert CNVkit segment ratios to copy-number calls."""
    LEGACY_NODE_ID = "cnvkit_call"
...
class _CNVkitPlotContract(VariantEvidenceContract):
    """Generate CNVkit scatter and heatmap PDF plots."""
    LEGACY_NODE_ID = "cnvkit_plot"
...
class _CNVnatorContract(VariantEvidenceContract):
    """Call copy-number variants with CNVnator read-depth analysis."""
    LEGACY_NODE_ID = "cnvnator"
...
class _ControlFREECContract(VariantEvidenceContract):
    """Call copy-number variants with Control-FREEC."""
    LEGACY_NODE_ID = "control_freec"
...
class _BcftoolsIndexContract(VariantEvidenceContract):
    """Index a VCF/BCF file."""
    LEGACY_NODE_ID = "bcftools_index"
...
class _PlatypusContract(VariantEvidenceContract):
    """Call haplotype-based variants with Platypus."""
    LEGACY_NODE_ID = "platypus"
...
class _DeepVariantContract(VariantEvidenceContract):
    """Call small variants with DeepVariant."""
    LEGACY_NODE_ID = "deepvariant"
...
class _Clair3Contract(VariantEvidenceContract):
    """Call small variants from long-read alignments with Clair3."""

    LEGACY_NODE_ID = "clair3"
    DISPLAY_NAME = "Clair3"
    CATEGORY = "variant"
...
class _VcfToolsFilterContract(VariantEvidenceContract):
    """Filter VCF with VCFtools."""
    LEGACY_NODE_ID = "vcftools_filter"
