"""bigscape — secondary_metabolism node(s). One tool per file (extracted from wrapped_variant_assembly.py)."""
...
class BiGSCAPENode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'BiG-SCAPE', 'BiG-SCAPE gene cluster families', 'biosynthetic gene clusters', 'BGC networks', 'GCF clustering', 'MIBiG', 'Pfam-A']
    ...
    def _identifiers(cls, inputs: dict[str, Any], files: list[str]) -> list[str]:
        ...
        return [_safe_identifier(identifiers[index]) if index < len(identifiers) and identifiers[index] else _safe_name(path) for index, path in enumerate(files)]
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        cmd.extend(['&&', 'bigscape', '--inputdir', input_dir])
        ...
        cmd.extend(['--domain_overlap_cutoff', str(inputs.get('domain_overlap_cutoff', 0.1)), '--min_bgc_size', str(inputs.get('min_big_size', inputs.get('min_bgc_size', 0)))])
        if inputs.get('mix'):
            cmd.append('--mix')
        ...
        cmd.extend(['&&', 'cp', f'{result_dir}/index.html', f'{out}/index.html', '&&', 'cp', '-r', f'{result_dir}/html_content', html_files])
        if inputs.get('log'):
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        ...
        return {'required': {'inputdir': ('FILE_LIST', {'multiple': True, 'description': 'GenBank BGC files to include in clustering'}), 'pfam_dir': ('FILE', {'description': 'Pfam-A.hmm HMM database file'})}, 'optional': {'element_identifiers': ('STRING', {'default': [], 'multiple': True, 'advanced': True, 'description': 'Galaxy collection element identifiers'}), 'mibig': ('STRING', {'default': '', 'options': cls.MIBIG_OPTIONS, 'description': 'Optional MIBiG database flag'}), 'label': ('STRING', {'default': '', 'description': 'Extra label added to BiG-SCAPE run names'}), 'verbose': ('BOOLEAN', {'default': False, 'description': 'Print detailed progress information'}), 'log': ('BOOLEAN', {'default': False, 'description': 'Capture BiG-SCAPE stdout to a log output'}), 'include_singletons': ('BOOLEAN', {'default': False, 'description': 'Include BGCs below the cutoff distance'}), 'domain_overlap_cutoff': ('FLOAT', {'default': 0.1, 'min': 0.0, 'max': 1.0}), 'min_big_size': ('INT', {'default': 0, 'min': 0, 'description': 'Minimum BGC size in bp'}), 'mix': ('BOOLEAN', {'default': False, 'description': 'Mix all BGC classes in the analysis'}), 'no_classify': ('BOOLEAN', {'default': False, 'description': 'Disable product-class based output'}), 'banned_classes': ('STRING_LIST', {'default': [], 'options': class_options, 'description': 'Classes excluded from classification'}), 'cutoffs': ('FLOAT_LIST', {'default': [0.3], 'description': 'Raw distance cutoff values'}), 'clans_off': ('BOOLEAN', {'default': False, 'description': 'Turn off second-layer GCF-to-GCC clustering'}), 'clan_cutoff': ('FLOAT_LIST', {'default': [], 'description': 'Optional GCF and GCC clan cutoff values'}), 'hybrids_off': ('BOOLEAN', {'default': False, 'description': 'Exclude hybrid predicted products'}), 'mode': ('STRING', {'default': 'glocal', 'options': ['glocal', 'global', 'auto'], 'description': 'Alignment mode'}), 'anchorfile': ('FILE', {'default': '', 'description': 'Optional custom anchor domain file'}), 'anchor_identifier': ('STRING', {'default': '', 'advanced': True, 'description': 'Safe filename for the staged anchor file'}), 'force_hmmscan': ('BOOLEAN', {'default': False, 'description': 'Force hmmscan domain prediction'}), 'domain_includelist': ('FILE', {'default': '', 'description': 'Optional domain include list'}), 'threads': ('INT', {'default': 8, 'min': 1, 'max': 128, 'display': 'slider'})}, 'hidden': {'output': ('STRING', {})}}
