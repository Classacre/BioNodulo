...
def _cramino_metrics_name(inputs: dict[str, Any]) -> str:
    return {
        ...
        "tsv": "metrics.tsv",
        "text": "metrics.txt",
    }.get(str(inputs.get("outfmt", "text")), "metrics.txt")
...
class CraminoNode(CommandNode):
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {
            "required": {
                "input_file": ("BAM", {"description": "BAM or CRAM file to inspect"}),
            },
            "optional": {
                "reference": ("FASTA", {"description": "Reference FASTA for CRAM decompression"}),
                "ubam": ("BOOLEAN", {"default": True, "description": "Report metrics for all reads rather than aligned reads only"}),
                "spliced": ("BOOLEAN", {"default": False, "description": "Report splice-site metrics"}),
                "phased": ("BOOLEAN", {"default": False, "description": "Calculate phased-read and phase-block metrics"}),
                "karyotype": ("BOOLEAN", {"default": False, "description": "Report normalized reads per chromosome"}),
                "min_read_len": ("INT", {"default": "", "min": 0, "description": "Minimum read length to include"}),
                "outfmt": ("STRING", {"default": "text", "options": ["text", "json", "tsv"], "description": "Metrics output format"}),
                "arrow": ("BOOLEAN", {"default": False, "description": "Write Arrow output for NanoPlot or NanoComp"}),
                "histtype": ("STRING", {"default": "no", "options": ["no", "hist", "hist_count"], "description": "Generate a histogram plot data file or bin-count table"}),
                "scaled": ("BOOLEAN", {"default": False, "description": "Scale histogram bins by total basepairs"}),
            },
...
class BamUtilClipOverlapNode(CommandNode):
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {
            "required": {
                "input": ("BAM", {"description": "Coordinate-sorted SAM/BAM alignment file"}),
            },
            "optional": {
                "storeOrig": ("STRING", {"default": "", "description": "SAM tag used to store original CIGAR values"}),
                "stats": ("BOOLEAN", {"default": False, "description": "Output overlap clipping statistics"}),
                "readName": ("BOOLEAN", {"default": False, "description": "Treat input as read-name sorted"}),
                "overlapsOnly": ("BOOLEAN", {"default": False, "description": "Only output overlapping read pairs"}),
                "excludeFlags": ("INT", {"default": "", "description": "Skip records with any of these SAM flag bits set"}),
                "unmapped": ("BOOLEAN", {"default": False, "description": "Mark fully clipped reads as unmapped"}),
            },
            "hidden": {"output": ("STRING", {})},
        }
...
class BamUtilDiffNode(CommandNode):
    ...
    def PLAN_OUTPUTS(cls, inputs: dict[str, Any], output_dir: str | Path) -> list[Path]:
        ...
        return [
            _common_output(cls.NODE_ID, f"{output_stem}.{ext}", output_dir),
            _common_output(cls.NODE_ID, f"{output_stem}_only1_{in1_stem}.{ext}", output_dir),
            _common_output(cls.NODE_ID, f"{output_stem}_only2_{in2_stem}.{ext}", output_dir),
        ]
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {
            "required": {
                "in1": ("BAM", {"description": "First coordinate-sorted SAM/BAM alignment file"}),
                "in2": ("BAM", {"description": "Second coordinate-sorted SAM/BAM alignment file"}),
            },
            "optional": {
                "posDiff": ("INT", {"default": 100000, "min": 0, "description": "Maximum position difference for potentially matching records"}),
                "onlyDiffs": ("BOOLEAN", {"default": False, "description": "Only print compared fields that differ"}),
                "fields_choice": ("STRING", {"default": "default", "options": ["default", "all", "select"], "description": "SAM/BAM fields to compare"}),
                "flag": ("BOOLEAN", {"default": False, "description": "Compare SAM flags"}),
                "mapQual": ("BOOLEAN", {"default": False, "description": "Compare mapping qualities"}),
                "mate": ("BOOLEAN", {"default": False, "description": "Compare mate chromosome and position"}),
                "isize": ("BOOLEAN", {"default": False, "description": "Compare insert sizes"}),
                "seq": ("BOOLEAN", {"default": False, "description": "Compare sequence bases"}),
                "baseQual": ("BOOLEAN", {"default": False, "description": "Compare base qualities"}),
                "noCigar": ("BOOLEAN", {"default": False, "description": "Do not compare CIGAR strings"}),
                "noPos": ("BOOLEAN", {"default": False, "description": "Do not compare positions"}),
                "tagchoice": ("STRING", {"default": "none", "options": ["none", "everyTag", "specify"], "description": "SAM tag comparison mode"}),
                "tags": ("STRING", {"default": "", "description": "Comma-separated tags to compare, such as AS:i,MD:Z"}),
                "output_as": ("STRING", {"default": "diff.txt", "options": ["diff.txt", "diff.bam", "diff.sam"], "description": "Output format"}),
            },
            "hidden": {"output": ("STRING", {})},
        }
