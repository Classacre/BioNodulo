"""Mock-friendly EMBL-EBI Clustal Omega REST owner."""
...
from .legacy import _count_fasta_records, _html_to_text, _safe_filename
...
EBI_CLUSTALO_RATE_LIMITER = TokenBucketRateLimiter(
    rate_per_second=EBI_CLUSTALO_RATE_LIMIT_PER_SECOND,
    burst=1,
)
EBI_CLUSTALO_SEQUENCE_TYPES = ("protein", "dna", "rna")
EBI_CLUSTALO_OUTPUT_FORMATS = (
    "fa",
    "clustal",
    "clustal_num",
    "msf",
...
async def _ebi_clustalo_post_text(
    ...
    *,
    ...
    response = await _ebi_clustalo_request(
        "POST",
        endpoint,
        data=data,
        retries=retries,
...
async def _ebi_clustalo_request(
    ...
    *,
    ...
    try:
        ...
        raise RuntimeError(
            f"EBI Clustal Omega {endpoint} failed with HTTP {status}: {body}"
        ) from exc
...
@source_pinned("ebi_clustal_omega")
class EBIClustalOmegaNode(BaseNode):
    ...
    EXPERIMENTAL = True
    DOCUMENTATION_URL = EBI_CLUSTALO_BASE_URL
    VERSION = "1.0.0"
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {
            "required": {
                ...
                "email": (
                    "STRING",
                    {"default": "", "description": "Email address required by EMBL-EBI Job Dispatcher"},
                ),
            },
            "optional": {
                "sequence_type": (
                    "STRING",
                    {"default": "protein", "options": list(EBI_CLUSTALO_SEQUENCE_TYPES)},
                ),
                "output_format": (
                    "STRING",
                    {"default": "fa", "options": list(EBI_CLUSTALO_OUTPUT_FORMATS)},
                ),
                ...
                "iterations": (
                    "INT",
                    {"default": 0, "min": 0, "max": EBI_CLUSTALO_MAX_ITERATIONS, "advanced": True},
                ),
                "timeout_minutes": ("INT", {"default": 30, "min": 1, "max": 240}),
                "poll_interval_seconds": (
                    "FLOAT",
                    {"default": 10.0, "min": 0.1, "advanced": True},
                ),
                "output_name": (
                    "STRING",
    ...
    async def run(self, **kwargs: Any) -> dict[str, Any]:
        ...
        result_types = _ebi_clustalo_result_types(
            await _ebi_clustalo_get_text(f"resulttypes/{job_id}")
        )
        ...
        alignment_text = await _ebi_clustalo_get_text(
            f"result/{job_id}/{alignment_result_type}"
        )
        ...
        alignment_path.write_text(
            alignment_text if alignment_text.endswith("\n") else alignment_text + "\n",
            encoding="utf-8",
        )
    ...
    def _submit_params(
        ...
        *,
        ...
        if not 0 <= iterations <= EBI_CLUSTALO_MAX_ITERATIONS:
            raise ValueError(
                f"EBI Clustal Omega iterations must be between 0 and {EBI_CLUSTALO_MAX_ITERATIONS}"
            )
        if iterations:
        ...
        *,
        ...
        while True:
            ...
            if elapsed_minutes > timeout_minutes:
                raise RuntimeError(
                    f"EBI Clustal Omega job {job_id} timed out after {timeout_minutes} minutes"
                )
            ...
            if status in EBI_CLUSTALO_RUNNING_STATUSES:
                ...
                continue
            raise RuntimeError(
                f"EBI Clustal Omega job {job_id} returned unrecognised status: {status}"
            )
