"""biapy — ai node(s). One tool per file (extracted from wrapped_variant_assembly.py)."""
...
class BiaPyNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'BiaPy', 'biapy', 'Build a workflow with BiaPy', 'accessible deep learning on bioimages', 'bioimage analysis', 'image segmentation', 'object detection', 'image denoising', 'BioImage Model Zoo']
    ...
    def _yaml_command(cls, inputs: dict[str, Any], out: str) -> str:
        ...
        if mode == 'custom_cfg':
            ...
            cmd.extend(['--new_config', '--num_cpus', threads, '--out_config_path', config, '--biapy_version', cls.VERSION, '--workflow', str(inputs.get('workflow', 'semantic')), '--dims', str(inputs.get('is_3d', inputs.get('dims', '2d'))), '--obj_slices', str(inputs.get('obj_slices', '')), '--obj_size', str(inputs.get('obj_size', '0-25')), '--img_channel', str(inputs.get('img_channel', 1))])
    ...
    def _raw_output_command(cls, out: str) -> str:
        ...
        raw_dir = f'{out}/raw'
        candidates = ['per_image_instances', 'full_image_instances', 'per_image_binarized', 'full_image_binarized', 'full_image', 'per_image_local_max_check', 'as_3d_stack_binarized', 'per_image']
    ...
    def render_command(cls, inputs: dict[str, Any]) -> str:
        ...
        command_parts = ['set -xeu', 'export OPENCV_IO_ENABLE_OPENEXR=0', 'WORKTMP=$(mktemp -d galaxy-torchinductor.XXXXXX)', 'export TORCHINDUCTOR_CACHE_DIR=$WORKTMP/torchinductor', 'mkdir -p $TORCHINDUCTOR_CACHE_DIR', _shell_join(['mkdir', '-p', f'{out}/output', f'{out}/output/my_experiment/checkpoints'])]
        ...
        command_parts.append(_shell_join(['biapy', '--config', f'{out}/config.yaml', '--result_dir', f'{out}/output', '--name', 'my_experiment', '--run_id', '1', '--gpu', '${GALAXY_BIAPY_GPU_STRING:-""}']).replace('\'${GALAXY_BIAPY_GPU_STRING:-""}\'', '${GALAXY_BIAPY_GPU_STRING:-""}'))
        if phase in {'train_test', 'test'}:
            if 'raw' in outputs:
            ...
            if 'metrics' in outputs and _as_list(inputs.get('gt_test')):
                command_parts.append(f"{_shell_join(['mkdir', '-p', f'{out}/metrics'])} && mv {shlex.quote(f'{out}/output/my_experiment/results/my_experiment_1/test_results_metrics.csv')} {shlex.quote(f'{out}/metrics/')} 2>/dev/null || true")
        if phase in {'train_test', 'train'}:
    ...
    def PLAN_OUTPUTS(cls, inputs: dict[str, Any], output_dir: str | Path) -> list[Path]:
        ...
        mapping = {'raw': out / 'raw', 'post_proc': out / 'post_proc', 'metrics': out / 'metrics', 'tcharts': out / 'train_charts', 'tlogs': out / 'train_logs'}
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {}, 'optional': {'selected_mode': ('STRING', {'default': 'custom_cfg', 'options': cls.MODES, 'description': 'Reuse an existing YAML config or create one'}), 'config_path': ('YAML', {'default': '', 'description': 'Existing BiaPy YAML configuration'}), 'biapy_model_path': ('FILE', {'default': '', 'description': 'Optional BiaPy safetensors checkpoint'}), 'workflow': ('STRING', {'default': 'semantic', 'options': cls.WORKFLOWS, 'description': 'BiaPy workflow type'}), 'phase': ('STRING', {'default': 'train_test', 'options': cls.PHASES}), 'is_3d': ('STRING', {'default': '2d', 'options': ['2d', '3d', '2d_stack']}), 'obj_slices': ('STRING', {'default': '', 'options': ['', '1-5', '5-10', '10-20', '20-60', '60+']}), 'obj_size': ('STRING', {'default': '0-25', 'options': ['0-25', '25-100', '100-200', '200-500', '500+']}), 'img_channel': ('INT', {'default': 1, 'min': 1, 'max': 10}), 'model_source': ('STRING', {'default': 'biapy', 'options': cls.MODEL_SOURCES}), 'bmz_model_name': ('STRING', {'default': ''}), 'raw_train': ('FILE', {'default': [], 'is_list': True, 'description': 'Training raw images'}), 'gt_train': ('FILE', {'default': [], 'is_list': True, 'description': 'Training target images'}), 'raw_test': ('FILE', {'default': [], 'is_list': True, 'description': 'Test raw images'}), 'gt_test': ('FILE', {'default': [], 'is_list': True, 'description': 'Optional test target images'}), 'selected_outputs': ('STRING', {'default': ['raw'], 'options': cls.OUTPUT_OPTIONS, 'multiple': True, 'description': 'BiaPy output collections to expose'}), 'create_yaml_script': ('FILE', {'default': 'create_yaml.py', 'advanced': True, 'description': 'Path to the Galaxy create_yaml.py helper'}), 'threads': ('INT', {'default': 1, 'min': 1, 'max': 128, 'display': 'slider'})}, 'hidden': {'output': ('STRING', {})}}
