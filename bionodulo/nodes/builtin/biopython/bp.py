"""bp — biopython node(s). One tool per file (extracted from biopython_nodes.py)."""
...
def _write_summary_preview(context: Any, out_dir: Path, *, title: str, columns: list[str], rows: list[list[Any]], note: str='', label: str='Summary') -> Path:
...
class SeqIOReadNode(BaseNode):
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input_file': ('FILE', {'label': 'Sequence File'}), 'format': ('STRING', {'default': 'fasta', 'options': ['fasta', 'fastq', 'genbank', 'embl', 'swiss', 'stockholm', 'clustal', 'phylip', 'nexus'], 'label': 'Format'})}, 'optional': {'alphabet': ('STRING', {'default': '', 'options': ['', 'dna', 'rna', 'protein'], 'label': 'Alphabet Hint', 'advanced': True})}}
    ...
    async def run(self, **kwargs: Any) -> tuple[str, ...]:
        ...
        for rec in records:
            seq_str = str(rec.seq)
            sequences.append({'id': rec.id, 'description': rec.description, 'length': len(seq_str), 'seq_preview': seq_str[:100], 'seq_full': seq_str})
            total_len += len(seq_str)
            gc = seq_str.count('G') + seq_str.count('C')
            gc_sum += gc / len(seq_str) * 100 if seq_str else 0
        stats = {'count': len(records), 'total_length': total_len, 'average_length': total_len / len(records) if records else 0, 'average_gc': gc_sum / len(records) if records else 0}
        seq_path = out_dir / 'sequences.json'
        stats_path = out_dir / 'stats.json'
        seq_path.write_text(json.dumps(sequences, indent=2), encoding='utf-8')
        stats_path.write_text(json.dumps(stats, indent=2), encoding='utf-8')
        _write_summary_preview(context, out_dir, title=f"SeqIO Read — {stats['count']} record(s) parsed", note=f"total {stats['total_length']:,} bp · avg length {stats['average_length']:.0f} bp · avg GC {stats['average_gc']:.1f}%", columns=['ID', 'Description', 'Length (bp)', 'Sequence preview'], rows=[[s['id'], s['description'], s['length'], s['seq_preview']] for s in sequences[:50]], label='Parsed Sequences')
...
class SeqIOWriteNode(BaseNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'sequences_json': ('FILE', {'label': 'Sequences JSON'}), 'output_format': ('STRING', {'default': 'fasta', 'options': ['fasta', 'fastq', 'genbank', 'embl', 'swiss', 'stockholm'], 'label': 'Output Format'}), 'output_name': ('STRING', {'default': 'output.fasta', 'label': 'Output Filename'})}}
    ...
    async def run(self, **kwargs: Any) -> tuple[str, ...]:
        ...
        for item in data:
            ...
            rec = SeqRecord(Seq(seq), id=item.get('id', 'unknown'), description=item.get('description', ''), annotations={'molecule_type': 'DNA'})
            records.append(rec)
        out_path = out_dir / name
        SeqIO.write(records, str(out_path), fmt)
        _write_summary_preview(context, out_dir, title=f'SeqIO Write — {len(records)} record(s) → {fmt}', note=f'Wrote {out_path.name} ({out_path.stat().st_size:,} bytes)', columns=['ID', 'Description', 'Length (bp)'], rows=[[r.id, r.description, len(r.seq)] for r in records[:50]], label=f'{fmt} output')
        return (str(out_path),)
...
class SequenceTranslateNode(BaseNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input_file': ('FILE', {'label': 'DNA FASTA'})}, 'optional': {'table': ('STRING', {'default': 'Standard', 'options': ['Standard', 'Vertebrate Mitochondrial', 'Bacterial', 'Alternative Yeast Nuclear', 'Ciliate Nuclear'], 'label': 'Translation Table', 'advanced': True}), 'to_stop': ('BOOLEAN', {'default': True, 'label': 'Stop at first STOP codon', 'advanced': True})}}
    ...
    async def run(self, **kwargs: Any) -> tuple[str, ...]:
        ...
        table_map = {'Standard': 1, 'Vertebrate Mitochondrial': 2, 'Bacterial': 11, 'Alternative Yeast Nuclear': 12, 'Ciliate Nuclear': 6}
        ...
        _write_summary_preview(context, out_dir, title=f'Translate DNA — {len(records)} protein(s)', note=f'Translation table: {table} · to_stop={bool(to_stop)}', columns=['ID', 'Length (aa)', 'Protein sequence'], rows=[[pid, len(seq), seq[:80] + '…' if len(seq) > 80 else seq] for pid, seq in proteins[:50]], label='Translated Proteins')
        return (str(out_path),)
...
class SequenceStatsNode(BaseNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input_file': ('FILE', {'label': 'Sequence File'})}, 'optional': {'format': ('STRING', {'default': 'fasta', 'options': ['fasta', 'fastq', 'genbank'], 'label': 'Format', 'advanced': True})}}
    ...
    async def run(self, **kwargs: Any) -> tuple[str, ...]:
        ...
        for rec in SeqIO.parse(str(input_file), fmt):
            ...
            try:
                ...
                mw = None
            results.append({'id': rec.id, 'length': len(seq_str), 'gc_content': round(gc, 2), 'molecular_weight': round(mw, 2) if mw else None})
...
class BLASTSearchNode(BaseNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'query': ('FILE', {'label': 'Query FASTA'}), 'subject': ('FILE', {'label': 'Subject/DB FASTA'}), 'program': ('STRING', {'default': 'blastn', 'options': ['blastn', 'blastp', 'blastx', 'tblastn', 'tblastx'], 'label': 'BLAST Program'})}, 'optional': {'evalue': ('FLOAT', {'default': 0.001, 'min': 0.0, 'max': 100.0, 'step': 0.001, 'label': 'E-value threshold', 'advanced': True}), 'max_hits': ('INT', {'default': 10, 'min': 1, 'max': 500, 'label': 'Max Hits', 'advanced': True}), 'outfmt': ('STRING', {'default': '5', 'options': ['5', '6', '7'], 'label': 'Output Format', 'advanced': True})}}
    ...
    async def run(self, **kwargs: Any) -> tuple[str, ...]:
        ...
        cmd = [program, '-query', str(query), '-subject', str(subject), '-evalue', str(evalue), '-max_target_seqs', str(max_hits), '-outfmt', str(outfmt), '-out', str(out_path)]
    ...
    @staticmethod
    def _register_hits_preview(context: Any, out_dir: Path, out_path: Path, outfmt: str, program: str) -> None:
        ...
        try:
                    ...
                    for record in NCBIXML.parse(handle):
                        ...
                        for alignment in record.alignments:
                            for hsp in alignment.hsps:
                                ident = 100.0 * hsp.identities / hsp.align_length if hsp.align_length else 0.0
                                rows.append([query, alignment.hit_id or alignment.hit_def.split()[0], f'{ident:.1f}%', f'{hsp.expect:.1e}', f'{hsp.score:g}'])
            else:
            ...
            rows = []
        _write_summary_preview(context, out_dir, title=f'BLAST ({program}) — {len(rows)} hit(s)', note=f'Output format {outfmt} · {out_path.name}', columns=['Query', 'Subject', '% identity', 'E-value', 'Score'], rows=rows[:50], label='BLAST Hits')
...
class MSAViewNode(BaseNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'alignment_file': ('FILE', {'label': 'Alignment File'}), 'format': ('STRING', {'default': 'clustal', 'options': ['clustal', 'stockholm', 'phylip', 'fasta', 'nexus'], 'label': 'Format'})}}
    ...
    async def run(self, **kwargs: Any) -> tuple[str, ...]:
        ...
        summary = {'num_sequences': len(alignment), 'alignment_length': alignment.get_alignment_length(), 'ids': [rec.id for rec in alignment]}
        consensus = []
        ...
        try:
            ...
            colour_map = {'A': '#ef4444', 'T': '#10b981', 'G': '#f59e0b', 'C': '#3b82f6', 'U': '#10b981', '-': '#e2e8f0', 'N': '#94a3b8'}
            nrows = len(alignment)
            ...
            for r_idx, rec in enumerate(alignment):
                for c_idx in range(col_limit):
                    ...
                    ax.add_patch(Rectangle((c_idx, nrows - r_idx - 1), 1, 1, facecolor=colour_map.get(ch, '#ffffff'), edgecolor='none'))
            ...
            ax.set_xticks([])
            ax.set_title(f"Alignment — {summary['num_sequences']} sequences × {summary['alignment_length']} cols" + (f' (first {col_limit} shown)' if col_limit < alignment.get_alignment_length() else ''), fontsize=10)
