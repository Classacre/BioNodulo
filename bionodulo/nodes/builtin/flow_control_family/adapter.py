"""Shared BioNodulo-native contracts for focused flow-control nodes."""
...
CPYTHON_ASYNCIO_DOCUMENTATION_URL = "https://docs.python.org/3.12/library/asyncio-task.html#sleeping"
CPYTHON_TIME_DOCUMENTATION_URL = "https://docs.python.org/3.12/library/time.html#time.monotonic"
CPYTHON_ASYNCIO_SOURCE_URL = f"https://github.com/python/cpython/blob/{CPYTHON_GIT_COMMIT}/Lib/asyncio/tasks.py"
CPYTHON_TIME_SOURCE_URL = f"https://github.com/python/cpython/blob/{CPYTHON_GIT_COMMIT}/Modules/timemodule.c"


class FlowControlNode(BaseNode):
    """Pinned source identity shared by BioNodulo's native control nodes."""
    ...
    DOCUMENTATION_URL = (
        ...
        f"{BIO_NODULO_BASELINE_COMMIT}/bionodulo/nodes/builtin/flow_control.py"
    )
    SOURCE_URL = DOCUMENTATION_URL
    UPSTREAM_SOURCE = "bionodulo/nodes/builtin/flow_control.py"
    RUNTIME_VERSION = CPYTHON_VERSION
    RUNTIME_GIT_COMMIT = CPYTHON_GIT_COMMIT
    RUNTIME_DOCUMENTATION_URLS = (
        CPYTHON_ASYNCIO_DOCUMENTATION_URL,
        CPYTHON_TIME_DOCUMENTATION_URL,
    )
    RUNTIME_SOURCE_URLS = (
        CPYTHON_ASYNCIO_SOURCE_URL,
        CPYTHON_TIME_SOURCE_URL,
    )
    ASYNC_SEMANTICS = (
        "Await asyncio.sleep so the event loop can schedule other tasks; measure elapsed durations "
        "with the monotonic clock so wall-clock changes cannot move deadlines backwards."
    )
    EXIT_SEMANTICS = "Invalid control modes and bounds raise; successful runs return named output mappings."

    @staticmethod
    def _choice(value: Any, name: str, choices: list[str] | tuple[str, ...]) -> str:
    ...
    def _bounded_float(value: Any, name: str, minimum: float, maximum: float) -> float:
        ...
        try:
            ...
            raise ValueError(f"{name} must be a number between {minimum:g} and {maximum:g}") from exc
...
class _IfConditionContract(FlowControlNode):
    """Route a value to a true or false output based on a condition."""

    LEGACY_NODE_ID = "if_condition"
    ...
    async def run(self, **kwargs: Any) -> dict[str, Any]:
        ...
        mode = self._choice(
            kwargs.get("condition_mode", "boolean"),
            "if_condition condition_mode",
            input_types["required"]["condition_mode"][0],
        )
        compare_to = kwargs.get("compare_to", "")
        case_sensitive = bool(kwargs.get("case_sensitive", True))
        invert = bool(kwargs.get("invert", False))
        combinator = self._choice(
            kwargs.get("combinator", "and") or "and",
            "if_condition combinator",
            ("and", "or"),
        )
        mode_2 = self._choice(
            kwargs.get("condition_mode_2", "none") or "none",
            "if_condition condition_mode_2",
            input_types["optional"]["condition_mode_2"][0],
        )
        compare_to_2 = kwargs.get("compare_to_2", "")
        output_mode = self._choice(
            kwargs.get("output_mode", "route") or "route",
            "if_condition output_mode",
            ("route", "signal"),
        )
...
class _SwitchContract(FlowControlNode):
    ...
    def _return_names_for(cls, num_branches: Any) -> tuple[str, ...]:
        branch_count = cls._bounded_int(
            num_branches,
            "Switch num_branches",
            cls.MIN_NUM_BRANCHES,
            cls.MAX_NUM_BRANCHES,
        )
    ...
    def _selected_rule_outputs(
        ...
        *,
        ...
        branch_names = return_names[:-1]
        match_mode = cls._choice(match_mode, "switch match_mode", ("first", "all"))
        fallback = cls._choice(fallback, "switch fallback", ("drop", "last", "error"))
...
class _ForEachContract(FlowControlNode):
    ...
    async def run(self, **kwargs: Any) -> dict[str, Any]:
        ...
        iteration_mode = self._choice(
            kwargs.get("iteration_mode", "single") or "single",
            "foreach iteration_mode",
            ("single", "batch"),
        )
        self._choice(
            kwargs.get("collect_mode", "list") or "list",
            "foreach collect_mode",
            ("list", "concat", "merge"),
        )
        batch_size = self._bounded_int(kwargs.get("batch_size", 1), "foreach batch_size", 1, 1000)
        max_iterations = self._bounded_int(
            kwargs.get("max_iterations", 1000),
            "foreach max_iterations",
            1,
            100000,
        )
...
class _TryCatchContract(FlowControlNode):
    ...
    async def run(self, **kwargs: Any) -> dict[str, Any]:
        ...
        max_retries = self._bounded_int(
            kwargs.get("max_retries", 0) or 0,
            "try_catch max_retries",
            0,
            10,
        )
        retry_delay = self._bounded_float(
            kwargs.get("retry_delay", 1.0) or 0.0,
            "try_catch retry_delay",
            0.0,
            300.0,
        )
        ...
        retry_count = self._bounded_int(
            kwargs.get("_retry_count", 0) or 0,
            "try_catch retry_count",
            0,
            10,
        )
...
class _GateContract(FlowControlNode):
    ...
    async def run(self, **kwargs: Any) -> dict[str, Any]:
        ...
        value = kwargs.get("value")
        mode = self._choice(
            kwargs.get("condition_mode", "file_exists") or "file_exists",
            "gate condition_mode",
            tuple(self._CONDITION_MODES),
        )
        compare_to = kwargs.get("compare_to", "")
        on_fail = self._choice(
            kwargs.get("on_fail", "skip") or "skip",
            "gate on_fail",
            ("skip", "halt", "default"),
        )
    ...
    def _evaluate(value: Any, mode: str, compare_to: Any) -> bool:
        ...
        if mode == "is_not_empty":
            return not _GateContract._evaluate(value, "is_empty", compare_to)
...
class _MergeContract(FlowControlNode):
    """Fan in multiple inputs and combine them with a selected strategy."""

    LEGACY_NODE_ID = "merge"
    DISPLAY_NAME = "Merge"
    CATEGORY = "flow_control"
    DESCRIPTION = (
        "Combine multiple input branches using append, zip, dict merge, first/last valid, or interleave strategies."
    )
    ...
    async def run(self, **kwargs: Any) -> dict[str, Any]:
        kwargs.pop("context", None)
        num_inputs = self._bounded_int(kwargs.get("num_inputs", 2), "merge num_inputs", 1, 10)
        strategy = self._choice(
            kwargs.get("strategy", "append") or "append",
            "merge strategy",
            ("append", "zip", "dict_merge", "first_valid", "last_valid", "interleave"),
        )
        wait_mode = self._choice(
            kwargs.get("wait_mode", "all") or "all",
            "merge wait_mode",
            ("all", "any", "first_n"),
        )
        wait_n = self._bounded_int(kwargs.get("wait_n", 1), "merge wait_n", 1, 10)
        timeout = self._bounded_float(kwargs.get("timeout", 0.0) or 0.0, "merge timeout", 0.0, 86400.0)
        ...
        if timeout > 0 and wait_mode == "all" and len(values) < num_inputs:
            raise RuntimeError(
                f"Merge timed out after {timeout:g}s waiting for all inputs ({len(values)}/{num_inputs} received)"
            )
    ...
    def _merge_values(cls, values: list[Any], strategy: str) -> Any:
        ...
        if strategy == "zip":
            ...
            if not sequences:
                return []
            return [
                tuple(sequence[index] for sequence in sequences) for index in range(min(len(seq) for seq in sequences))
            ]
...
class _DelayWaitContract(FlowControlNode):
    ...
    async def run(self, **kwargs: Any) -> dict[str, Any]:
        kwargs.pop("context", None)
        mode = self._choice(
            kwargs.get("mode", "delay") or "delay",
            "delay_wait mode",
            ("delay", "until_time", "file_exists", "file_not_exists", "process_complete", "poll_url"),
        )
        value = kwargs.get("value")
        max_wait = self._bounded_float(
            kwargs.get("max_wait", 0.0) or 0.0,
            "delay_wait max_wait",
            0.0,
            86400.0,
        )
        on_timeout = self._choice(
            kwargs.get("on_timeout", "error") or "error",
            "delay_wait on_timeout",
            ("error", "pass_through"),
        )
        self._bounded_float(
            kwargs.get("poll_interval", 5.0) or 0.0,
            "delay_wait poll_interval",
            0.0,
            3600.0,
        )
        if mode == "until_time" and not str(kwargs.get("target_time", "") or "").strip():
            raise ValueError("delay_wait target_time is required for until_time mode")
        if (
            mode in {"file_exists", "file_not_exists", "process_complete"}
            and not str(kwargs.get("watch_path", "") or "").strip()
        ):
            raise ValueError(f"delay_wait watch_path is required for {mode} mode")
        if mode == "poll_url" and not str(kwargs.get("poll_url", "") or "").strip():
            raise ValueError("delay_wait poll_url is required for poll_url mode")
    ...
    async def _wait_for_mode(self, mode: str, kwargs: dict[str, Any], max_wait: float) -> bool:
        if mode == "delay":
            delay_seconds = self._bounded_float(
                kwargs.get("delay_seconds", 5.0) or 0.0,
                "delay_wait delay_seconds",
                0.0,
                86400.0,
            )
        ...
        poll_interval = self._bounded_float(
            kwargs.get("poll_interval", 5.0) or 0.0,
            "delay_wait poll_interval",
            0.0,
            3600.0,
...
class _SleepContract(FlowControlNode):
    """Pause execution for a fixed number of seconds."""

    LEGACY_NODE_ID = "sleep"
    DISPLAY_NAME = "Sleep"
    CATEGORY = "flow_control"
    ...
    async def run(self, **kwargs: Any) -> dict[str, Any]:
        kwargs.pop("context", None)
        seconds = self._bounded_float(kwargs.get("seconds", 1.0), "sleep seconds", 0.0, 86400.0)
        value = kwargs.get("value")
        started_at = time.monotonic()
...
class _WaitForContract(FlowControlNode):
    """Wait until a simple condition is met."""

    LEGACY_NODE_ID = "wait_for"
    ...
    async def run(self, **kwargs: Any) -> dict[str, Any]:
        kwargs.pop("context", None)
        condition = self._choice(
            kwargs.get("condition", "file_exists") or "file_exists",
            "wait_for condition",
            ("file_exists", "file_not_exists", "elapsed_time"),
        )
        timeout = self._bounded_float(
            kwargs.get("timeout", 0.0) or 0.0,
            "wait_for timeout",
            0.0,
            86400.0,
        )
        ...
        self._bounded_float(
            kwargs.get("poll_interval", 1.0) or 0.0,
            "wait_for poll_interval",
            0.0,
            3600.0,
        )
        if condition in {"file_exists", "file_not_exists"} and not str(kwargs.get("path", "") or "").strip():
            raise ValueError(f"wait_for path is required for {condition} condition")
        value = kwargs.get("value")
        started_at = time.monotonic()
...
class _BreakContinueContract(FlowControlNode):
    """Emit an explicit loop-control signal for ForEach body subgraphs."""

    LEGACY_NODE_ID = "break_continue"
...
class _CounterAccumulatorContract(FlowControlNode):
    """Maintain counters and accumulated values across loop iterations."""

    LEGACY_NODE_ID = "counter_accumulator"
    ...
    async def run(self, **kwargs: Any) -> dict[str, Any]:
        kwargs.pop("context", None)
        operation = self._choice(
            kwargs.get("operation", "increment") or "increment",
            "counter_accumulator operation",
            (
                "increment",
                "decrement",
                "add",
        ...
        accumulator_key = str(kwargs.get("accumulator_key", "default")).strip()
        if not accumulator_key:
            raise ValueError("counter_accumulator accumulator_key must be non-empty")
        access_mode = self._choice(
            kwargs.get("access_mode", "read_write") or "read_write",
            "counter_accumulator access_mode",
            ("read_write", "read_only", "write_only"),
        )
        iteration = self._bounded_int(
            kwargs.get("_iteration", 0) or 0,
            "counter_accumulator iteration",
            0,
            2147483647,
        )
...
class _ParallelForContract(FlowControlNode):
    """Scatter items into chunks and gather externally produced parallel results."""

    LEGACY_NODE_ID = "parallel_for"
    DISPLAY_NAME = "Parallel For"
    CATEGORY = "flow_control"
    DESCRIPTION = (
        "Scatter items across parallel branches, then gather results with all, any, first, or sorted strategies."
    )
    ...
    async def run(self, **kwargs: Any) -> dict[str, Any]:
        ...
        max_concurrency = self._bounded_int(
            kwargs.get("max_concurrency", 4),
            "parallel_for max_concurrency",
            1,
            100,
        )
        gather = self._choice(
            kwargs.get("gather", "all") or "all",
            "parallel_for gather",
            ("all", "any", "first", "sorted"),
        )
        first_n = self._bounded_int(
            kwargs.get("first_n", 1),
            "parallel_for first_n",
            1,
            100,
        )
        sort_key = str(kwargs.get("sort_key", "") or "")
        chunk_size = self._bounded_int(
            kwargs.get("chunk_size", 1),
            "parallel_for chunk_size",
            1,
            100,
        )
...
class _WhileLoopContract(FlowControlNode):
    """Track conditional loop state for iterative workflow sections."""

    LEGACY_NODE_ID = "while_loop"
    ...
    async def run(self, **kwargs: Any) -> dict[str, Any]:
        kwargs.pop("context", None)
        condition_mode = self._choice(
            kwargs.get("condition_mode", "file_not_exists") or "file_not_exists",
            "while_loop condition_mode",
            tuple(self._CONDITION_MODES),
        )
        value = kwargs.get("value")
        compare_to = kwargs.get("compare_to", "")
        max_iterations = self._bounded_int(
            ...
            "while_loop max_iterations",
            1,
            10000,
        )
        check_frequency = self._bounded_int(
            kwargs.get("check_frequency", 1),
            "while_loop check_frequency",
            1,
            100,
        )
        ...
        state_max_iterations = self._bounded_int(
            loop_state.get("max_iterations", max_iterations),
            "while_loop state max_iterations",
            1,
            10000,
        )
        if iteration >= state_max_iterations:
            loop_state["is_complete"] = True
            return self._result(processed, iteration, False, "max_iterations", loop_state, inactive=[])

        check_frequency = self._bounded_int(
            loop_state.get("check_frequency", check_frequency),
            "while_loop state check_frequency",
            1,
            100,
        )
        ...
        mode = self._choice(
            loop_state.get("condition_mode", condition_mode),
            "while_loop state condition_mode",
            tuple(self._CONDITION_MODES),
        )
