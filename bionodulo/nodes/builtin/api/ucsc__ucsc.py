"""ucsc — api node(s). One tool per file (extracted from ucsc.py)."""
...
UCSC_GENOMES = ('hg38', 'hg19', 'mm39', 'mm10', 'rn7', 'rn6', 'danRer11', 'dm6', 'ce11', 'sacCer3')
...
async def _request(endpoint: str, params: dict[str, Any], *, retries: int, timeout: float) -> httpx.Response:
    ...
    try:
        return await client.request('GET', url, params=params, headers={'User-Agent': UCSC_USER_AGENT}, timeout=timeout, retries=retries, retry_delay=RETRY_DELAY_S, cache_ttl=UCSC_CACHE_TTL_S, follow_redirects=True)
...
class UCSCGenomeBrowserNode(BaseNode):
    ...
    SEARCH_ALIASES = ['ucsc', 'genome browser', 'sequence', 'coordinates', 'genome', 'hg19', 'hg38', 'annotation']
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'coordinates': ('STRING', {'default': '', 'description': 'Genomic coordinates, e.g. chr17:43044295-43125364'}), 'genome': ('STRING', {'default': 'hg38', 'options': list(UCSC_GENOMES)})}, 'optional': {'query_type': ('STRING', {'default': 'sequence', 'options': list(UCSC_QUERY_TYPES)}), 'track': ('STRING', {'default': '', 'options': list(UCSC_TRACKS)}), 'max_items': ('INT', {'default': 1000, 'min': 1, 'max': 100000, 'advanced': True})}, 'hidden': {}}
    ...
    async def run(self, **kwargs: Any) -> dict[str, Any]:
        ...
        if query_type == 'tracks':
            ...
            metadata = {'query_type': query_type, 'genome': genome, 'coordinates': coordinates, 'ucsc_response': payload}
        ...
        if query_type in {'sequence', 'dna_sequence'}:
            payload = await _request_json('getData/sequence', {'genome': genome, 'chrom': chrom, 'start': start, 'end': end})
            sequence = _sequence_from_response(payload)
            metadata = {'query_type': query_type, 'genome': genome, 'coordinates': coordinates, 'chrom': chrom, 'start': start, 'end': end, 'sequence_length': len(sequence), 'ucsc_response': payload}
            ...
            payload = await _request_json('getData/track', {'genome': genome, 'track': track, 'chrom': chrom, 'start': start, 'end': end, 'maxItemsOutput': max_items})
            metadata = {'query_type': query_type, 'genome': genome, 'coordinates': coordinates, 'track': track, 'chrom': chrom, 'start': start, 'end': end, 'ucsc_response': payload}
            json_path = out_dir / 'annotations.json'
            fasta_path.write_text(f'>{genome}:{coordinates}\n\n', encoding='utf-8')
            json_path.write_text(json.dumps(metadata, indent=2), encoding='utf-8')
        return {'outputs': {'sequence_fasta': str(fasta_path), 'annotations_json': str(json_path)}}
