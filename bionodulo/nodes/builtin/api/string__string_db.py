"""string — api node(s). One tool per file (extracted from string_db.py)."""
...
async def _request(endpoint: str, params: dict[str, Any], *, retries: int, timeout: float) -> httpx.Response:
    ...
    try:
        return await client.request('GET', url, params=params, headers={'User-Agent': STRING_USER_AGENT}, timeout=timeout, retries=retries, retry_delay=RETRY_DELAY_S, cache_ttl=STRING_CACHE_TTL_S)
...
def _params_for(*, identifiers: list[str], species: int, query_type: str, required_score: int, network_flavor: str, add_nodes: int) -> tuple[str, dict[str, Any]]:
    base: dict[str, Any] = {'identifiers': _string_identifier_param(identifiers), 'species': species, 'caller_identity': 'BioNodulo'}
    if query_type == 'network':
        base.update({'required_score': required_score, 'add_nodes': add_nodes, 'network_flavor': network_flavor})
    ...
    base.update({'required_score': required_score, 'add_white_nodes': add_nodes, 'network_flavor': network_flavor})
...
class STRINGDBNode(BaseNode):
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'protein_ids': ('STRING', {'default': '', 'description': 'Protein IDs or gene symbols as a list, JSON list, or comma-separated string'})}, 'optional': {'species': ('INT', {'default': 9606, 'min': 1, 'description': 'NCBI taxonomy ID'}), 'query_type': ('STRING', {'default': 'network', 'options': list(STRING_QUERY_TYPES)}), 'required_score': ('INT', {'default': 400, 'min': 0, 'max': 1000}), 'network_flavor': ('STRING', {'default': 'evidence', 'options': list(NETWORK_FLAVORS)}), 'add_nodes': ('INT', {'default': 0, 'min': 0, 'max': 50}), 'protein_table': ('FILE', {'default': '', 'description': 'Optional CSV/TSV table containing protein IDs or gene symbols'}), 'id_column': ('STRING', {'default': '', 'description': 'Column to read from protein_table'})}, 'hidden': {}}
    ...
    async def run(self, **kwargs: Any) -> dict[str, Any]:
        ...
        endpoint, params = _params_for(identifiers=identifiers, species=species, query_type=query_type, required_score=required_score, network_flavor=network_flavor, add_nodes=add_nodes)
        ...
        if query_type == 'image':
            ...
            metadata_extra: dict[str, Any] = {'image_url': image_url, 'image_path': str(image_path), 'image_params': params}
        else:
        ...
        metadata = {'query_type': query_type, 'endpoint': endpoint, 'identifiers': identifiers, 'species': species, 'required_score': required_score, 'network_flavor': network_flavor, 'add_nodes': add_nodes, 'record_count': len(rows), 'rows': rows, **metadata_extra}
