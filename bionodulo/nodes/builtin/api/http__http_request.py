"""http — api node(s). One tool per file (extracted from http_request.py)."""
...
async def _request(*, method: str, url: str, params: dict[str, Any], headers: dict[str, str], json_body: Any | None, data: Any | None, timeout: float, follow_redirects: bool, retries: int=MAX_RETRIES, retry_delay: float=RETRY_DELAY_S, cache_ttl: float | None=None, rate_limit_per_second: float | None=None) -> httpx.Response:
    cache = APICache(ttl_seconds=cache_ttl) if cache_ttl and cache_ttl > 0 else None
    rate_limiter = TokenBucketRateLimiter(rate_per_second=rate_limit_per_second, burst=1) if rate_limit_per_second and rate_limit_per_second > 0 else None
    client = APIHttpClient(cache=cache, rate_limiter=rate_limiter)
    try:
        return await client.request(method, url, params=params, headers=headers, json=json_body, data=data, timeout=timeout, follow_redirects=follow_redirects, retries=retries, retry_delay=retry_delay, cache_ttl=cache_ttl)
...
class HTTPRequestNode(BaseNode):
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'url': ('STRING', {'default': '', 'description': 'HTTP or HTTPS URL'}), 'method': (list(HTTP_METHODS), {'default': 'GET'})}, 'optional': {'query_params': ('STRING', {'default': '{}', 'multiline': True, 'description': 'JSON object of query parameters'}), 'headers': ('STRING', {'default': '{}', 'multiline': True, 'description': 'JSON object of request headers'}), 'body_format': ('STRING', {'default': 'none', 'options': ['none', 'json', 'text', 'form']}), 'body': ('STRING', {'default': '', 'multiline': True, 'displayOptions': {'show': {'body_format': ['json', 'text', 'form']}}}), 'auth_mode': ('STRING', {'default': 'none', 'options': ['none', 'bearer', 'basic']}), 'bearer_token': ('STRING', {'default': '', 'advanced': True, 'displayOptions': {'show': {'auth_mode': ['bearer']}}}), 'username': ('STRING', {'default': '', 'advanced': True, 'displayOptions': {'show': {'auth_mode': ['basic']}}}), 'password': ('STRING', {'default': '', 'advanced': True, 'displayOptions': {'show': {'auth_mode': ['basic']}}}), 'timeout': ('FLOAT', {'default': REQUEST_TIMEOUT_S, 'min': 1}), 'follow_redirects': ('BOOLEAN', {'default': True}), 'cache_ttl': ('FLOAT', {'default': 0, 'min': 0, 'description': 'Cache GET/HEAD responses for N seconds; 0 disables cache'}), 'rate_limit_per_second': ('FLOAT', {'default': 0, 'min': 0, 'description': 'Maximum requests per second; 0 disables rate limiting'}), 'output_name': ('STRING', {'default': '', 'description': 'Optional response filename stem'})}, 'hidden': {}}
    ...
    async def run(self, **kwargs: Any) -> dict[str, Any]:
        ...
        headers = {'User-Agent': HTTP_USER_AGENT, **_string_headers(_parse_mapping(kwargs.get('headers', '{}'), 'headers'))}
        ...
        response = await _request(method=method, url=url, params=params, headers=headers, json_body=json_body, data=data, timeout=timeout, follow_redirects=follow_redirects, **request_options)
        ...
        metadata: dict[str, Any] = {'status_code': response.status_code, 'url': str(response.url), 'content_type': content_type, 'headers': dict(response.headers), 'response_body': str(body_path)}
        if 'json' in content_type:
            try:
                ...
                pass
        return {'outputs': {'response_body': str(body_path), 'metadata': metadata}}
