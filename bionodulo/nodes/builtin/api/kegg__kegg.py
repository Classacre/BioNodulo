"""kegg — api node(s). One tool per file (extracted from kegg.py)."""
...
QUERY_TYPES = ('pathway_info', 'pathway_genes', 'pathway_image', 'list_pathways', 'gene_info', 'find_genes', 'compound_info', 'find_compounds', 'link_kegg')
...
def _safe_filename(value: str) -> str:
    ...
    return name or 'kegg'
async def _request_text(resource: str, *, retries: int=MAX_RETRIES, timeout: float=REQUEST_TIMEOUT_S) -> str:
...
async def _request(resource: str, *, retries: int, timeout: float) -> httpx.Response:
    ...
    try:
        return await client.request('GET', url, headers={'User-Agent': KEGG_USER_AGENT}, timeout=timeout, retries=retries, retry_delay=RETRY_DELAY_S, cache_ttl=KEGG_CACHE_TTL_S)
...
async def _download_file(url: str, path: Path, *, retries: int=MAX_RETRIES, timeout: float=REQUEST_TIMEOUT_S) -> None:
    client = APIHttpClient(cache=KEGG_API_CACHE, rate_limiter=KEGG_RATE_LIMITER)
    try:
        response = await client.request('GET', url, headers={'User-Agent': KEGG_USER_AGENT}, timeout=timeout, retries=retries, retry_delay=RETRY_DELAY_S, cache_ttl=KEGG_CACHE_TTL_S, follow_redirects=True)
...
def _normalise_pathway_id(query: str, organism: str) -> str:
    ...
    if text.startswith('path:'):
        text = text.split(':', 1)[1]
    if re.fullmatch('[a-z]{3}\\d{5}', text):
        return text
    if re.fullmatch('\\d{5}', text):
...
class KEGGPathwayNode(BaseNode):
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'query': ('STRING', {'default': '', 'description': 'Pathway ID, gene ID, keyword, or KEGG ID list'}), 'query_type': ('STRING', {'default': 'pathway_info', 'options': list(QUERY_TYPES)})}, 'optional': {'organism': ('STRING', {'default': 'hsa', 'options': list(KEGG_ORGANISMS)}), 'download_image': ('BOOLEAN', {'default': False}), 'output_name': ('STRING', {'default': '', 'description': 'Optional output filename stem'})}, 'hidden': {}}
    ...
    async def run(self, **kwargs: Any) -> dict[str, Any]:
        ...
        data = {'query': query, 'effective_query': effective_query, 'query_type': query_type, 'organism': organism, 'resource': resource, **parsed}
