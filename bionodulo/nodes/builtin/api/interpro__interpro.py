"""interpro — api node(s). One tool per file (extracted from interpro.py)."""
...
class InterProScanNode(BaseNode):
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'sequence': ('STRING', {'default': '', 'multiline': True, 'description': 'Protein sequence in FASTA or raw amino-acid form'})}, 'optional': {'applications': ('STRING', {'default': 'pfam,gene3d,superfamily,smart'}), 'goterms': ('BOOLEAN', {'default': True}), 'pathways': ('BOOLEAN', {'default': False}), 'email': ('STRING', {'default': '', 'advanced': True}), 'timeout_minutes': ('INT', {'default': 30, 'min': 1, 'max': 60}), 'poll_interval_seconds': ('FLOAT', {'default': 10.0, 'min': 0.1, 'advanced': True})}, 'hidden': {}}

    async def _submit_job(self, *, sequence: str, applications: str, goterms: bool, pathways: bool, email: str) -> str:
        job_id = await _post_text('run', {'email': email, 'title': 'bionodulo_interpro', 'sequence': sequence, 'appl': applications, 'goterms': 'true' if goterms else 'false', 'pathways': 'true' if pathways else 'false'})
        job_id = job_id.strip()
        if not job_id:
            raise RuntimeError('InterProScan did not return a job ID')
        return job_id

    async def _poll_job(self, *, job_id: str, timeout_minutes: int, poll_interval_seconds: float) -> dict[str, Any]:
    ...
    async def run(self, **kwargs: Any) -> dict[str, Any]:
        ...
        job_id = await self._submit_job(sequence=sequence, applications=applications, goterms=goterms, pathways=pathways, email=email)
        result = await self._poll_job(job_id=job_id, timeout_minutes=timeout_minutes, poll_interval_seconds=poll_interval_seconds)
        payload = {'job_id': job_id, 'applications': applications, 'goterms': goterms, 'pathways': pathways, 'interproscan_result': result}
