"""pdb — api node(s). One tool per file (extracted from rcsb_pdb.py)."""
...
class PDBDownloadNode(BaseNode):
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'pdb_ids': ('STRING', {'default': '', 'description': 'PDB ID(s), comma-separated'}), 'format': ('STRING', {'default': 'cif', 'options': list(PDB_FORMATS)})}, 'optional': {'fetch_metadata': ('BOOLEAN', {'default': True}), 'download_density': ('BOOLEAN', {'default': False})}, 'hidden': {}}
    ...
    async def run(self, **kwargs: Any) -> dict[str, Any]:
        ...
        for pdb_id in pdb_ids:
            ...
            structures.append({'pdb_id': pdb_id, 'format': fmt, 'structure_file': str(structure_path), 'density_file': density_file, 'metadata': metadata})
        metadata_path = out_dir / 'pdb_metadata.json'
        metadata_path.write_text(json.dumps({'record_count': len(structures), 'structures': structures}, indent=2), encoding='utf-8')
        return {'outputs': {'structure_file': first_structure, 'pdb_metadata': str(metadata_path)}}
