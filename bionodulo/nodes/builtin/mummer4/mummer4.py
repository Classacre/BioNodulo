"""mummer4 — mummer4 node(s). One tool per file (extracted from mummer4.py)."""
...
def _link_inputs(cmd: list[str], reference: Any, query: Any) -> None:
    cmd.extend(['ln', '-s', str(reference), 'reference.fa', '&&', 'ln', '-s', str(query), 'query.fa', '&&'])
def _add_plot_args(cmd: list[str], inputs: dict[str, Any], *, prefix: str='', include_sequences: bool=False) -> None:
    ...
    if inputs.get(f'{prefix}custom_range', inputs.get('custom_range', False)):
        cmd.extend(['-x', f"[{inputs.get(f'{prefix}min_x', inputs.get('min_x', 0))}:{inputs.get(f'{prefix}max_x', inputs.get('max_x', 100))}]", '-y', f"[{inputs.get(f'{prefix}min_y', inputs.get('min_y', 0))}:{inputs.get(f'{prefix}max_y', inputs.get('max_y', 100))}]"])
...
def _plot_input_types(prefix: str) -> dict[str, tuple[str, dict[str, Any]]]:
    return {f'{prefix}breaklen': ('INT', {'default': 20, 'min': 0, 'description': 'Plot breakpoint highlight length', 'advanced': True}), f'{prefix}color': ('STRING', {'default': '', 'options': ['', '-color'], 'description': 'Disable percent-similarity color gradient', 'advanced': True}), f'{prefix}coverage': ('STRING', {'default': '', 'options': ['', '-c'], 'description': 'Generate coverage plot instead of dotplot', 'advanced': True}), f'{prefix}filter': ('BOOLEAN', {'default': False, 'description': 'Plot only best delta alignments', 'advanced': True}), f'{prefix}fat': ('BOOLEAN', {'default': False, 'description': 'Layout using fattest alignment only', 'advanced': True}), f'{prefix}plot_ids': ('BOOLEAN', {'default': False, 'description': 'Limit plot to a specific reference/query ID', 'advanced': True}), f'{prefix}ref_id': ('STRING', {'default': '', 'description': 'Reference sequence ID for plotting', 'advanced': True}), f'{prefix}query_id': ('STRING', {'default': '', 'description': 'Query sequence ID for plotting', 'advanced': True}), f'{prefix}size': ('STRING', {'default': 'small', 'options': ['small', 'medium', 'large'], 'description': 'Plot size'}), f'{prefix}title': ('STRING', {'default': 'Title', 'description': 'Plot title'}), f'{prefix}snp': ('BOOLEAN', {'default': False, 'description': 'Highlight SNP locations', 'advanced': True}), f'{prefix}custom_range': ('BOOLEAN', {'default': False, 'description': 'Use custom x/y axis ranges', 'advanced': True}), f'{prefix}min_x': ('INT', {'default': 0, 'advanced': True}), f'{prefix}max_x': ('INT', {'default': 100, 'advanced': True}), f'{prefix}min_y': ('INT', {'default': 0, 'advanced': True}), f'{prefix}max_y': ('INT', {'default': 100, 'advanced': True})}
...
class Mummer4NucmerNode(_Mummer4BaseNode):
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        cmd.extend(['-b', str(inputs.get('breaklen', 200)), '-c', str(inputs.get('mincluster', 65)), '-D', str(inputs.get('diagdiff', 5)), '-d', str(inputs.get('diagfactor', 0.12))])
        _add_bool(cmd, inputs.get('noextend'), '--noextend')
        _add_select(cmd, inputs.get('direction', ''))
        cmd.extend(['-g', str(inputs.get('maxgap', 90)), '-l', str(inputs.get('minmatch', 20)), '-L', str(inputs.get('minalign', 0))])
        ...
        if out_format == 'delta':
            ...
            return cmd
        cmd.extend(['&&', 'samtools', 'dict', 'reference.fa', '>', 'outsamhead', '&&', 'tail', '-n', '+3', 'outsam.sam', '>>', 'outsamhead', '&&', 'samtools', 'sort', '-@', threads, '-T', '${TMPDIR:-.}', 'outsamhead', '|'])
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'reference_sequence': ('FASTA', {'description': 'Reference FASTA'}), 'query_sequence': ('FASTA', {'description': 'Query FASTA'}), 'out_format': ('STRING', {'default': 'delta', 'options': ['delta', 'bam-long', 'cram-long']})}, 'optional': {'plot': ('BOOLEAN', {'default': False, 'description': 'Generate a MUMmer dotplot for delta output'}), 'anchoring': ('STRING', {'default': '', 'options': ['', '--mum', '--maxmatch']}), 'breaklen': ('INT', {'default': 200, 'min': 0}), 'mincluster': ('INT', {'default': 65, 'min': 1}), 'diagdiff': ('INT', {'default': 5, 'min': 0}), 'diagfactor': ('FLOAT', {'default': 0.12, 'min': 0}), 'noextend': ('BOOLEAN', {'default': False}), 'direction': ('STRING', {'default': '', 'options': ['', '-f', '-r']}), 'maxgap': ('INT', {'default': 90, 'min': 0}), 'minmatch': ('INT', {'default': 20, 'min': 1}), 'minalign': ('INT', {'default': 0, 'min': 0}), 'nooptimize': ('BOOLEAN', {'default': False, 'advanced': True}), 'nosimplify': ('BOOLEAN', {'default': False, 'advanced': True}), 'threads': ('INT', {'default': 1, 'min': 1, 'max': 128}), 'banded': ('BOOLEAN', {'default': False, 'advanced': True}), 'large': ('BOOLEAN', {'default': False, 'advanced': True}), 'genome': ('BOOLEAN', {'default': False, 'advanced': True}), 'max_chunk': ('INT', {'default': '', 'advanced': True}), **_plot_input_types('plot_')}, 'hidden': {'output': ('STRING', {})}}
...
class Mummer4DnadiffNode(_Mummer4BaseNode):
    ...
    def PLAN_OUTPUTS(cls, inputs: dict[str, Any], output_dir: str | Path) -> list[Path]:
        ...
        if str(inputs.get('report_only', 'yes')) == 'no':
            paths.extend([out / 'out.delta', out / 'out.1delta', out / 'out.mdelta', out / 'out.1coords', out / 'out.mcoords', out / 'out.snps', out / 'out.rdiff', out / 'out.qdiff'])
        return paths

    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'reference_sequence': ('FASTA', {'description': 'Reference FASTA'}), 'query_sequence': ('FASTA', {'description': 'Query FASTA'})}, 'optional': {'report_only': ('STRING', {'default': 'yes', 'options': ['yes', 'no'], 'description': 'Return only the general report or all dnadiff files'})}, 'hidden': {'output': ('STRING', {})}}
...
class Mummer4DeltaFilterNode(_Mummer4BaseNode):
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        cmd.extend(['-i', str(inputs.get('min_identity', 0)), '-l', str(inputs.get('min_length', 0))])
        _add_select(cmd, inputs.get('overlap', '-q'))
        cmd.extend(['-u', str(inputs.get('min_uniqueness', 0)), '-o', str(inputs.get('max_overlap', 100)), str(inputs.get('delta', '')), '>', f'{out}/delta-filter.txt'])
        return cmd
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'delta': ('FILE', {'description': 'Nucmer delta file'})}, 'optional': {'alignment': ('STRING', {'default': '-m', 'options': ['-m', '-1', '-g']}), 'min_identity': ('FLOAT', {'default': 0, 'min': 0, 'max': 100}), 'min_length': ('INT', {'default': 0, 'min': 0}), 'overlap': ('STRING', {'default': '-q', 'options': ['-q', '-r']}), 'min_uniqueness': ('FLOAT', {'default': 0, 'min': 0, 'max': 100}), 'max_overlap': ('FLOAT', {'default': 100, 'min': 0, 'max': 100})}, 'hidden': {'output': ('STRING', {})}}
...
class Mummer4ShowCoordsNode(_Mummer4BaseNode):
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        cmd.extend(['-c', '-H', '-I', str(inputs.get('identity', 75.0)), '-l', '-L', str(inputs.get('min_alignment_length', 100))])
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'delta': ('FILE', {'description': 'Nucmer delta file'})}, 'optional': {'merge': ('BOOLEAN', {'default': False}), 'identity': ('FLOAT', {'default': 75.0, 'min': 0, 'max': 100}), 'direction': ('BOOLEAN', {'default': False}), 'min_alignment_length': ('INT', {'default': 100, 'min': 0}), 'annotate': ('BOOLEAN', {'default': False}), 'sort': ('STRING', {'default': '-q', 'options': ['-q', '-r']})}, 'hidden': {'output': ('STRING', {})}}
...
class Mummer4MummerNode(_Mummer4BaseNode):
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        if inputs.get('advanced'):
            cmd.extend(['-k', str(inputs.get('suffix', 1)), '-suflink', str(inputs.get('suflink', 0)), '-child', str(inputs.get('child', 0)), '-skip', str(inputs.get('skip', 10)), '-kmer', str(inputs.get('kmer', 1))])
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'reference_sequence': ('FASTA', {'description': 'Reference FASTA'}), 'query_sequence': ('FASTA', {'description': 'Query FASTA'})}, 'optional': {'anchoring': ('STRING', {'default': '', 'options': ['', '-mum', '-maxmatch']}), 'min': ('INT', {'default': 20, 'min': 1}), 'direction': ('STRING', {'default': '-b', 'options': ['-b', '-r']}), 'force': ('BOOLEAN', {'default': False}), 'chars': ('BOOLEAN', {'default': False}), 'print_length': ('BOOLEAN', {'default': False}), 'substring': ('BOOLEAN', {'default': False}), 'position': ('BOOLEAN', {'default': False}), 'threads': ('INT', {'default': 1, 'min': 1, 'max': 128}), 'advanced': ('BOOLEAN', {'default': False, 'advanced': True}), 'suffix': ('INT', {'default': 1, 'advanced': True}), 'suflink': ('INT', {'default': 0, 'advanced': True}), 'child': ('INT', {'default': 0, 'advanced': True}), 'skip': ('INT', {'default': 10, 'advanced': True}), 'kmer': ('INT', {'default': 1, 'advanced': True}), 'plot': ('BOOLEAN', {'default': False}), **_plot_input_types('plot_')}, 'hidden': {'output': ('STRING', {})}}
...
class Mummer4MummerplotNode(_Mummer4BaseNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'delta': ('FILE', {'description': 'MUMmer or nucmer match/delta file'}), 'reference_sequence': ('FASTA', {'description': 'Reference FASTA'}), 'query_sequence': ('FASTA', {'description': 'Query FASTA'})}, 'optional': {**_plot_input_types(''), 'seq_input': ('BOOLEAN', {'default': False, 'description': 'Pass ordered reference/query FASTA files to mummerplot'}), 'layout': ('BOOLEAN', {'default': False, 'description': 'Layout a delta multiplot intelligibly'}), 'extra_outs': ('STRING', {'default': 'plot', 'options': ['plot', 'all'], 'description': 'Return just plot or all mummerplot intermediate files'})}, 'hidden': {'output': ('STRING', {})}}
