"""drep — metagenomics node(s). One tool per file (extracted from wrapped_variant_assembly.py)."""
...
class DrepCompareNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'dRep', 'dRep compare', 'dRep genome comparison', 'genome dereplication', 'average nucleotide identity', 'Mash ANI clustering']
    RETURN_TYPES = ('TXT', 'TXT', 'PDF', 'PDF', 'PDF', 'PDF', 'CSV', 'CSV', 'CSV', 'CSV')
    RETURN_NAMES = ('log', 'warnings', 'primary_clustering_dendrogram', 'secondary_clustering_dendrograms', 'secondary_clustering_mds', 'clustering_scatterplots', 'bdb', 'cdb', 'mdb', 'ndb')
    ...
    OUTPUTS = {'log': ('outdir/log/logger.log', 'log.txt'), 'warnings': ('outdir/log/warnings.txt', 'warnings.txt'), 'Primary_clustering_dendrogram': ('outdir/figures/Primary_clustering_dendrogram.pdf', 'Primary_clustering_dendrogram.pdf'), 'Secondary_clustering_dendrograms': ('outdir/figures/Secondary_clustering_dendrograms.pdf', 'Secondary_clustering_dendrograms.pdf'), 'Secondary_clustering_MDS': ('outdir/figures/Secondary_clustering_MDS.pdf', 'Secondary_clustering_MDS.pdf'), 'Clustering_scatterplots': ('outdir/figures/Clustering_scatterplots.pdf', 'Clustering_scatterplots.pdf'), 'Bdb': ('outdir/data_tables/Bdb.csv', 'Bdb.csv'), 'Cdb': ('outdir/data_tables/Cdb.csv', 'Cdb.csv'), 'Mdb': ('outdir/data_tables/Mdb.csv', 'Mdb.csv'), 'Ndb': ('outdir/data_tables/Ndb.csv', 'Ndb.csv')}
    ...
    def _add_mash_options(cls, cmd: list[str], inputs: dict[str, Any]) -> None:
        cmd.extend(['--MASH_sketch', str(inputs.get('MASH_sketch', 1000)), '--P_ani', str(inputs.get('P_ani', 0.9))])
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'genomes': ('FASTA_LIST', {'multiple': True, 'min': 2, 'description': 'Genome FASTA files to compare'})}, 'optional': {'genome_identifiers': ('STRING', {'default': [], 'multiple': True, 'advanced': True, 'description': 'Galaxy collection element identifiers'}), 'comparison_steps': ('STRING', {'default': 'default', 'options': cls.COMPARISON_STEPS, 'description': 'Genome comparison stages to run'}), 'MASH_sketch': ('INT', {'default': 1000, 'min': 0, 'description': 'Mash sketch size'}), 'P_ani': ('FLOAT', {'default': 0.9, 'min': 0, 'max': 1, 'description': 'ANI threshold for primary clusters'}), 'multiround_primary_clustering': ('BOOLEAN', {'default': False, 'description': 'Cluster primary chunks separately before merging'}), 'primary_chunksize': ('INT', {'default': 5000, 'min': 1, 'description': 'Genome chunk size for primary clustering'}), 'S_algorithm': ('STRING', {'default': 'ANImf', 'options': cls.SECONDARY_ALGORITHMS, 'description': 'Secondary clustering algorithm'}), 'greedy_secondary_clustering': ('BOOLEAN', {'default': False, 'description': 'Use greedy secondary clustering with fastANI'}), 'n_PRESET': ('STRING', {'default': 'normal', 'options': cls.NUCMER_PRESETS, 'description': 'Nucmer preset'}), 'coverage_method': ('STRING', {'default': 'larger', 'options': cls.COVERAGE_METHODS, 'description': 'Alignment coverage calculation'}), 'S_ani': ('FLOAT', {'default': 0.99, 'min': 0, 'max': 1, 'description': 'ANI threshold for secondary clusters'}), 'cov_thresh': ('FLOAT', {'default': 0.1, 'min': 0, 'max': 1, 'description': 'Minimum overlap for secondary comparisons'}), 'clusterAlg': ('STRING', {'default': 'average', 'options': cls.CLUSTER_ALGORITHMS, 'description': 'SciPy linkage algorithm'}), 'run_tertiary_clustering': ('BOOLEAN', {'default': False, 'description': 'Run an additional clustering pass'}), 'warn_dist': ('FLOAT', {'default': 0.25, 'min': 0, 'max': 1, 'description': 'Distance from threshold for cluster warnings'}), 'warn_sim': ('FLOAT', {'default': 0.98, 'min': 0, 'max': 1, 'description': 'Similarity threshold for warnings'}), 'warn_aln': ('FLOAT', {'default': 0.25, 'min': 0, 'max': 1, 'description': 'Minimum aligned fraction for warnings'}), 'select_outputs': ('STRING_LIST', {'default': cls.DEFAULT_OUTPUTS.copy(), 'options': list(cls.OUTPUTS), 'description': 'Galaxy dRep outputs to collect'}), 'threads': ('INT', {'default': 1, 'min': 1, 'max': 128, 'display': 'slider'})}, 'hidden': {'output': ('STRING', {})}}
...
class DrepDereplicateNode(DrepCompareNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'dRep', 'dRep dereplicate', 'dRep genome dereplication', 'bin dereplication', 'metagenome genome recovery', 'representative genomes']
    RETURN_TYPES = ('DIRECTORY', 'TXT', 'TXT', 'PDF', 'PDF', 'PDF', 'PDF', 'CSV', 'CSV', 'CSV', 'CSV', 'PDF', 'PDF', 'CSV', 'TSV')
    RETURN_NAMES = ('dereplicated_genomes', 'log', 'warnings', 'primary_clustering_dendrogram', 'secondary_clustering_dendrograms', 'secondary_clustering_mds', 'clustering_scatterplots', 'bdb', 'cdb', 'mdb', 'ndb', 'cluster_scoring', 'winning_genomes', 'widb', 'chdb')
    REQUIRED_EXECUTABLES = ['dRep']
    ...
    DEFAULT_OUTPUTS = ['log', 'warnings', 'Primary_clustering_dendrogram', 'Clustering_scatterplots', 'Cluster_scoring', 'Winning_genomes', 'Widb']
    OUTPUTS = {**DrepCompareNode.OUTPUTS, 'Cluster_scoring': ('outdir/figures/Cluster_scoring.pdf', 'Cluster_scoring.pdf'), 'Winning_genomes': ('outdir/figures/Winning_genomes.pdf', 'Winning_genomes.pdf'), 'Widb': ('outdir/data_tables/Widb.csv', 'Widb.csv'), 'Chdb': ('outdir/data/checkM/checkM_outdir/Chdb.tsv', 'Chdb.tsv')}
    ...
    def render_command(cls, inputs: dict[str, Any]) -> str:
        ...
        cmd.extend(['--warn_aln', str(inputs.get('warn_aln', 0.25))])
        cmd.extend(['--processors', f"${{GALAXY_SLOTS:-{inputs.get('threads', 1)}}}"])
        commands.append(_shell_join(cmd).replace("'${GALAXY_SLOTS:-", '${GALAXY_SLOTS:-').replace("}'", '}') + ' || (rc=$?; ls -ltr `find outdir -type f`; cat outdir/data/checkM/checkM_outdir/checkm.log; cat outdir/log/logger.log; exit $rc)')
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        ...
        optional.update({'length': ('INT', {'default': 50000, 'min': 1, 'description': 'Minimum genome length'}), 'completeness': ('INT', {'default': 75, 'min': 0, 'max': 100, 'description': 'Minimum genome completeness percent'}), 'contamination': ('INT', {'default': 25, 'min': 0, 'max': 100, 'description': 'Maximum genome contamination percent'}), 'quality_source': ('STRING', {'default': 'checkm', 'options': cls.QUALITY_SOURCES, 'description': 'Genome quality filtering source'}), 'checkM_method': ('STRING', {'default': 'lineage_wf', 'options': cls.CHECKM_METHODS, 'description': 'CheckM workflow'}), 'set_recursion': ('INT', {'default': '', 'min': 1, 'advanced': True, 'description': 'Optional Python recursion limit'}), 'checkm_group_size': ('INT', {'default': 2000, 'min': 1, 'description': 'Number of genomes passed to CheckM at a time'}), 'genomeInfo': ('CSV', {'default': '', 'description': 'CSV quality information for genomes'}), 'completeness_weight': ('FLOAT', {'default': 1, 'description': 'Scoring weight for completeness'}), 'contamination_weight': ('FLOAT', {'default': 5, 'description': 'Scoring weight for contamination'}), 'strain_heterogeneity_weight': ('FLOAT', {'default': 1, 'min': 0, 'max': 1, 'description': 'Scoring weight for strain heterogeneity'}), 'N50_weight': ('FLOAT', {'default': 0.5, 'description': 'Scoring weight for log genome N50'}), 'size_weight': ('FLOAT', {'default': 0, 'description': 'Scoring weight for log genome size'}), 'centrality_weight': ('FLOAT', {'default': 1, 'description': 'Scoring weight for cluster centrality'}), 'extra_weight_table': ('TSV', {'default': '', 'description': 'Genome-specific extra scoring weights'}), 'select_outputs': ('STRING_LIST', {'default': cls.DEFAULT_OUTPUTS.copy(), 'options': list(cls.OUTPUTS), 'description': 'Galaxy dRep outputs to collect'})})
        return {'required': parent['required'], 'optional': optional, 'hidden': parent['hidden']}
    ...
    def VALIDATE_INPUTS(cls, inputs: dict[str, Any]) -> bool | str:
        ...
        for name in ['completeness_weight', 'contamination_weight', 'strain_heterogeneity_weight', 'N50_weight', 'size_weight', 'centrality_weight']:
            raw = inputs.get(name)
            if raw is None or str(raw) == '':
                continue
            try:
