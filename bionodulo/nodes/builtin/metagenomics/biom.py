"""biom — metagenomics node(s). One tool per file (extracted from wrapped_taxonomy_humann.py)."""
...
class BiomSummarizeTableNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'BIOM', 'biom-format', 'biom_summarize_table', 'biom summarize-table', 'summarize sample data', 'summarize observation data', 'microbiome table summary']
    RETURN_TYPES = ('TXT',)
    ...
    def render_command(cls, inputs: dict[str, Any]) -> str:
        cmd = ['biom', 'summarize-table', '--input-fp', str(inputs.get('input_fp', '')), '--output-fp', cls._output_path(inputs)]
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input_fp': ('FILE', {'description': 'Input BIOM table'})}, 'optional': {'qualitative': ('BOOLEAN', {'default': True, 'description': 'Present counts as unique observation ids rather than observation counts'}), 'observations': ('BOOLEAN', {'default': True, 'description': 'Summarize over observations instead of samples'})}, 'hidden': {'output': ('STRING', {})}}
...
class BiomNormalizeTableNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'BIOM', 'biom-format', 'biom_normalize_table', 'biom normalize-table', 'relative abundance', 'presence absence', 'normalize microbiome table']
    RETURN_TYPES = ('FILE',)
    RETURN_NAMES = ('output_fp',)
    REQUIRED_EXECUTABLES = ['biom']
    ...
    @classmethod
    def render_command(cls, inputs: dict[str, Any]) -> str:
        cmd = ['biom', 'normalize-table', '--input-fp', str(inputs.get('input_fp', '')), '--output-fp', cls._output_path(inputs)]
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input_fp': ('FILE', {'description': 'Input BIOM table to normalize'})}, 'optional': {'relative_abund': ('BOOLEAN', {'default': True, 'description': 'Convert table values to relative abundance'}), 'presence_absence': ('BOOLEAN', {'default': True, 'description': 'Convert table values to presence or absence'}), 'axis': ('STRING', {'default': 'sample', 'options': cls.AXES, 'description': 'Normalize over samples or observations'})}, 'hidden': {'output': ('STRING', {})}}
...
class BiomSubsetTableNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'BIOM', 'biom-format', 'biom_subset_table', 'biom subset-table', 'sample IDs', 'observation IDs', 'subset microbiome table']
    RETURN_TYPES = ('FILE',)
    RETURN_NAMES = ('output_fp',)
    REQUIRED_EXECUTABLES = ['biom']
    ...
    @classmethod
    def render_command(cls, inputs: dict[str, Any]) -> str:
        cmd = ['biom', 'subset-table', '--input-json-fp', str(inputs.get('input_json_fp', '')), '--output-fp', cls._output_path(inputs), '--axis', cls._axis(inputs), '--ids', str(inputs.get('ids', ''))]
        return _shell_join(cmd)
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input_json_fp': ('FILE', {'description': 'Input BIOM table to subset'}), 'ids': ('FILE', {'description': 'Single-column text or tabular file of IDs to retain'})}, 'optional': {'axis': ('STRING', {'default': 'sample', 'options': cls.AXES, 'description': 'Subset sample IDs or observation IDs'})}, 'hidden': {'output': ('STRING', {})}}
...
class BiomFromUcNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'BIOM', 'biom-format', 'biom_from_uc', 'biom from-uc', 'UC file', 'vsearch', 'uclust', 'usearch']
    RETURN_TYPES = ('FILE',)
    RETURN_NAMES = ('output_fp',)
    REQUIRED_EXECUTABLES = ['biom']
    ...
    @classmethod
    def render_command(cls, inputs: dict[str, Any]) -> str:
        cmd = ['biom', 'from-uc', '--input-fp', str(inputs.get('input_fp', '')), '--output-fp', cls._output_path(inputs)]
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input_fp': ('FILE', {'description': 'Input vsearch, uclust, or usearch UC file'})}, 'optional': {'rep_set_fp': ('FASTA', {'default': '', 'description': 'Optional representative sequences FASTA labeled with OTU identifiers'})}, 'hidden': {'output': ('STRING', {})}}
...
class BiomAddMetadataNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'BIOM', 'biom-format', 'biom_add_metadata', 'biom add-metadata', 'sample metadata', 'observation metadata', 'taxonomy metadata']
    RETURN_TYPES = ('FILE',)
    RETURN_NAMES = ('output_fp',)
    REQUIRED_EXECUTABLES = ['biom']
    ...
    @classmethod
    def render_command(cls, inputs: dict[str, Any]) -> str:
        cmd = ['biom', 'add-metadata', '--input-fp', str(inputs.get('input_fp', '')), '--output-fp', cls._output_path(inputs)]
        for input_name, flag in (('sample_metadata_fp', '--sample-metadata-fp'), ('observation_metadata_fp', '--observation-metadata-fp'), ('sc_separated', '--sc-separated'), ('sc_pipe_separated', '--sc-pipe-separated'), ('int_fields', '--int-fields'), ('float_fields', '--float-fields'), ('sample_header', '--sample-header'), ('observation_header', '--observation-header')):
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        ...
        return {'required': {'input_fp': ('FILE', {'description': 'Input BIOM table'})}, 'optional': {'sample_metadata_fp': ('TSV', {'default': '', 'description': 'Optional sample metadata table'}), 'observation_metadata_fp': ('TSV', {'default': '', 'description': 'Optional observation metadata table'}), 'sc_separated': ('STRING', {'default': '', 'description': f'{text_field_description} to split on semicolons'}), 'sc_pipe_separated': ('STRING', {'default': '', 'description': f'{text_field_description} to split on semicolons and pipes'}), 'int_fields': ('STRING', {'default': '', 'description': f'{text_field_description} to cast as integers'}), 'float_fields': ('STRING', {'default': '', 'description': f'{text_field_description} to cast as floating point numbers'}), 'sample_header': ('STRING', {'default': '', 'description': 'Comma-separated sample metadata field names'}), 'observation_header': ('STRING', {'default': '', 'description': 'Comma-separated observation metadata field names'}), 'output_as_json': ('BOOLEAN', {'default': True, 'description': 'Write output as JSON-formatted BIOM1 instead of HDF5'})}, 'hidden': {'output': ('STRING', {})}}
...
class BiomConvertNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'BIOM', 'biom-format', 'biom_convert', 'biom convert', 'BIOM1', 'BIOM2', 'HDF5', 'TSV-formatted table']
    RETURN_TYPES = ('FILE',)
    RETURN_NAMES = ('output_fp',)
    REQUIRED_EXECUTABLES = ['biom']
    ...
    TABLE_TYPES = ['OTU table', 'Pathway table', 'Function table', 'Ortholog table', 'Gene table', 'Metabolite table', 'Taxon table', 'Table']
    ...
    @classmethod
    def render_command(cls, inputs: dict[str, Any]) -> str:
        cmd = ['biom', 'convert', '--input-fp', 'input', '--output-fp', cls._output_path(inputs)]
        if cls._input_type(inputs) == 'tsv':
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input_fp': ('FILE', {'description': 'Input tabular table or BIOM table'})}, 'optional': {'input_type': ('STRING', {'default': 'tsv', 'options': cls.INPUT_TYPES_OPTIONS, 'description': 'Source format: tabular text or BIOM'}), 'process_obs_metadata': ('STRING', {'default': '', 'options': cls.PROCESS_OBS_METADATA_OPTIONS, 'description': 'Process observation metadata when converting from tabular text'}), 'output_type': ('STRING', {'default': 'biom', 'options': cls.OUTPUT_TYPES, 'description': 'Target format: BIOM or TSV-formatted classic table'}), 'header_key': ('STRING', {'default': '', 'description': 'Observation metadata key to include when writing TSV'}), 'output_metadata_id': ('STRING', {'default': '', 'description': 'TSV output metadata column name'}), 'tsv_metadata_formatter': ('STRING', {'default': 'naive', 'options': cls.TSV_METADATA_FORMATTERS, 'description': 'Formatter for observation metadata when writing TSV'}), 'table_type': ('STRING', {'default': 'Table', 'options': cls.TABLE_TYPES, 'description': 'BIOM table semantic type'}), 'biom_type': ('STRING', {'default': 'json', 'options': cls.BIOM_TYPES, 'description': 'BIOM output representation: JSON BIOM1 or HDF5 BIOM2'}), 'collapsed_samples': ('BOOLEAN', {'default': False, 'description': 'Use collapsed samples when writing HDF5 BIOM'}), 'collapsed_observations': ('BOOLEAN', {'default': False, 'description': 'Use collapsed observations when writing HDF5 BIOM'}), 'sample_metadata_fp': ('TSV', {'default': '', 'description': 'Optional sample metadata mapping file for BIOM output'}), 'observation_metadata_fp': ('TSV', {'default': '', 'description': 'Optional observation metadata mapping file for BIOM output'})}, 'hidden': {'output': ('STRING', {})}}
