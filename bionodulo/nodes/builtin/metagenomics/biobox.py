"""biobox — metagenomics node(s). One tool per file (extracted from wrapped_variant_assembly.py)."""
...
class BioboxAddTaxidNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'Biobox add taxid', 'biobox_add_taxid.py', 'CAMI AMBER biobox taxid', 'ContigID2TaxID', 'BinID2TaxID']
    RETURN_TYPES = ('TSV',)
    RETURN_NAMES = ('output',)
    ...
    def render_command(cls, inputs: dict[str, Any]) -> str:
        ...
        commands = [_shell_join(['mkdir', '-p', out]), _shell_join(['ln', '-s', str(inputs.get('biobox_file', '')), 'biobox.tsv']), _shell_join(['ln', '-s', str(inputs.get(taxid_input, '')), staged_taxid])]
        commands.append(_shell_join(['biobox_add_taxid.py', 'biobox.tsv', taxid_flag, staged_taxid, '-k_c', str(inputs.get('key_col', '')), '-t_c', str(inputs.get('taxid_col', ''))]))
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'biobox_file': ('TSV', {'description': 'Input CAMI AMBER biobox file'}), 'input_mode': ('STRING', {'default': 'contig', 'options': cls.INPUT_MODES, 'description': 'Taxonomy mapping input type'}), 'key_col': ('INT', {'min': 1, 'description': 'Column containing contig or bin identifiers'}), 'taxid_col': ('INT', {'min': 1, 'description': 'Column containing NCBI TaxIDs'})}, 'optional': {'contig2taxid': ('TSV', {'default': '', 'description': 'ContigID2TaxID table, for contig mode'}), 'binid2taxid': ('TSV', {'default': '', 'description': 'BinID2TaxID table, for bin mode'})}, 'hidden': {'output': ('STRING', {})}}
