"""binette — metagenomics node(s). One tool per file (extracted from wrapped_variant_assembly.py)."""
...
class BinetteNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'Binette', 'Binette binning refinement', 'binning refinement', 'metagenomic binning', 'MAG refinement', 'CheckM2 database', 'contig-to-bin tables']
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        cmd.extend(['&&', 'binette', '-b', f'{input_dir}/*.tsv', '-c', f'{out}/input_contigs.fasta'])
        if inputs.get('proteins'):
            cmd.extend(['-p', f'{out}/input_proteins.fasta'])
        cmd.extend(['--min_completeness', str(inputs.get('min_completeness', 40)), '-t', f"${{GALAXY_SLOTS:-{inputs.get('threads', 1)}}}", '-o', f'{output_dir}/', '-w', str(inputs.get('contamination_weight', 2)), '--checkm2_db', cls._checkm2_db(inputs, out)])
        return cmd
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'contig2bin_tables': ('TSV', {'multiple': True, 'min_items': 2, 'description': 'At least two contig-to-bin tables from independent binning tools'}), 'contigs': ('FASTA', {'description': 'Assembly contigs used to generate the binning tables'})}, 'optional': {'proteins': ('FASTA', {'default': '', 'description': 'Optional Prodigal-format predicted protein FASTA'}), 'min_completeness': ('INT', {'default': 40, 'min': 0, 'max': 100, 'description': 'Minimum completeness threshold for final bins'}), 'contamination_weight': ('INT', {'default': 2, 'description': 'Weight applied to contamination in the bin selection score'}), 'database_type': ('STRING', {'default': 'cached', 'options': ['cached', 'his'], 'description': 'Use a cached CheckM2 DIAMOND database or a database from workflow history'}), 'checkm2_db': ('FILE', {'default': '', 'description': 'History CheckM2 DIAMOND database for database_type=his'}), 'checkm2_db_path': ('FILE', {'default': '', 'description': 'Cached CheckM2 DIAMOND database path for database_type=cached'}), 'threads': ('INT', {'default': 1, 'min': 1, 'max': 128, 'display': 'slider'})}, 'hidden': {'output': ('STRING', {})}}
