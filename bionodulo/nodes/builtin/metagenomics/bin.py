"""bin — metagenomics node(s). One tool per file (extracted from wrapped_variant_assembly.py)."""
...
class BinningRefinerNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'Binning refiner', 'Binning_refiner', 'Binning refiner metagenome bins', 'bin_refiner', 'genome bins', 'metagenome bin refinement', 'contamination reduction', 'refined bins']
    ...
    def _element_identifiers(cls, inputs: dict[str, Any], input_bins: list[str]) -> list[str]:
        ...
        return [_safe_identifier(identifiers[index]) if index < len(identifiers) and identifiers[index] else _safe_name(input_bin) for index, input_bin in enumerate(input_bins)]
    ...
    def render_command(cls, inputs: dict[str, Any]) -> str:
        ...
        commands.extend([_shell_join(['Binning_refiner', '-i', input_dir, '-p', 'refined', '-m', str(inputs.get('m', 512))]), _shell_join(['mv', f'{output_root}/refined_contigs.txt', f'{out}/refined_contigs.tsv']), _shell_join(['mv', f'{output_root}/refined_sources_and_length.txt', f'{out}/sources_and_length.tsv'])])
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input_bins': ('FASTA_LIST', {'multiple': True, 'description': 'Binned FASTA or FASTA.GZ files produced by metagenome binning tools'})}, 'optional': {'m': ('INT', {'default': 512, 'min': 1, 'description': 'Minimum size in Kbp for a refined bin to be retained'}), 'element_identifiers': ('STRING', {'default': [], 'multiple': True, 'advanced': True, 'description': 'Optional Galaxy collection element names'}), 'input_exts': ('STRING', {'default': [], 'multiple': True, 'advanced': True, 'description': 'Optional datatype extensions for staged bins'})}, 'hidden': {'output': ('STRING', {})}}
