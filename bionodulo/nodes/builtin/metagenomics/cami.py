"""cami — metagenomics node(s). One tool per file (extracted from wrapped_variant_assembly.py)."""
...
class CamiAmberNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'CAMI AMBER', 'AMBER metagenome binning evaluation', 'Assessment of Metagenome BinnERs', 'binning benchmark', 'genome reconstruction metrics']
    ...
    def render_command(cls, inputs: dict[str, Any]) -> str:
        ...
        commands.extend([_shell_join(['cp', 'output/index.html', f'{out}/index.html']), _shell_join(['cp', 'output/results.tsv', f'{out}/results.tsv']), _shell_join(['cp', 'output/genome_metrics_cami1.tsv', f'{out}/genome_metrics_cami1.tsv']), _shell_join(['cp', 'output/bin_metrics.tsv', f'{out}/bin_metrics.tsv'])])
    ...
    def PLAN_OUTPUTS(cls, inputs: dict[str, Any], output_dir: str | Path) -> list[Path]:
        ...
        return [out / 'index.html', out / 'results.tsv', out / 'genome_metrics_cami1.tsv', out / 'bin_metrics.tsv']

    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'gold_standard_file': ('TSV', {'description': 'Gold standard CAMI biobox file with sequence lengths'}), 'binning_files': ('TSV', {'multiple': True, 'description': 'CAMI biobox binning files to evaluate'})}, 'optional': {'labels': ('STRING', {'default': [], 'multiple': True, 'description': 'Optional labels for binning files'}), 'filter': ('INT', {'default': 0, 'min': 0, 'description': 'Filter out the n smallest genome bins'}), 'min_length': ('INT', {'default': '', 'min': 1, 'description': 'Minimum sequence length'}), 'desc': ('STRING', {'default': '', 'description': 'HTML report description'}), 'min_completeness': ('INT', {'default': [], 'multiple': True, 'min': 0, 'max': 100, 'description': 'Minimum completeness thresholds'}), 'max_contamination': ('INT', {'default': [], 'multiple': True, 'min': 0, 'max': 100, 'description': 'Maximum contamination thresholds'}), 'remove_genomes': ('TSV', {'default': '', 'description': 'Genome removal list'}), 'remove_keyword': ('STRING', {'default': '', 'description': 'Keyword for genome removal list'}), 'genome_coverage': ('TSV', {'default': '', 'description': 'Genome coverage table'}), 'ncbi_mode': ('STRING', {'default': 'none', 'options': cls.NCBI_MODES, 'description': 'NCBI taxonomy source'}), 'ncbi_files': ('TSV', {'default': [], 'multiple': True, 'description': 'Manual NCBI nodes, merged, and names DMP files'}), 'ncbi_identifiers': ('STRING', {'default': [], 'multiple': True, 'advanced': True, 'description': 'Galaxy element identifiers for manual NCBI files'}), 'ncbi_dir': ('DIRECTORY', {'default': '', 'description': 'NCBI taxonomy directory from a data manager'})}, 'hidden': {'output': ('STRING', {})}}
...
class CamiAmberAddNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'CAMI AMBER add length column', 'AMBER gold standard length', 'add_length_column.py', 'biobox length column', 'metagenome benchmark gold standard']
    RETURN_TYPES = ('TSV',)
    RETURN_NAMES = ('file',)
    ...
    def render_command(cls, inputs: dict[str, Any]) -> str:
        ...
        commands = [_shell_join(['mkdir', '-p', out]), _shell_join(['ln', '-s', str(inputs.get('gold_standard_file', '')), gold_name]), _shell_join(['ln', '-s', str(inputs.get('fasta_file', '')), fasta_name])]
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'gold_standard_file': ('TSV', {'description': 'Input CAMI biobox gold standard file'}), 'fasta_file': ('FILE', {'description': 'Matching FASTA/FASTQ file, optionally compressed'})}, 'optional': {'gold_standard_identifier': ('STRING', {'default': '', 'advanced': True, 'description': 'Galaxy element identifier'}), 'fasta_identifier': ('STRING', {'default': '', 'advanced': True, 'description': 'Galaxy element identifier'})}, 'hidden': {'output': ('STRING', {})}}
...
class CamiAmberConvertNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'CAMI AMBER convert to biobox', 'AMBER biobox conversion', 'convert_fasta_bins_to_biobox_format.py', 'FASTA bins to biobox', 'binning TSV']
    ...
    def render_command(cls, inputs: dict[str, Any]) -> str:
        ...
        if work == 'single':
            for identifier in identifiers:
                commands.append(_shell_join(['convert_fasta_bins_to_biobox_format.py', '-o', f'output/{cls._single_output_name(identifier)}', identifier]))
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'work': ('STRING', {'default': 'single', 'options': cls.WORK_OPTIONS, 'description': 'Convert each bin separately or merge all bins'}), 'files': ('FASTA', {'multiple': True, 'description': 'FASTA bin files'})}, 'optional': {'file_identifiers': ('STRING', {'default': [], 'multiple': True, 'advanced': True, 'description': 'Galaxy element identifiers'})}, 'hidden': {'output': ('STRING', {})}}
