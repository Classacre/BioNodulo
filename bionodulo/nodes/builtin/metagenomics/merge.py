"""merge — metagenomics node(s). One tool per file (extracted from wrapped_hyphy_metagenomics.py)."""
...
class MergeMetaPhlAnTablesNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'MetaPhlAn', 'merge_metaphlan_tables.py', 'relative abundance', 'abundance tables', 'GTDB profiles']
    RETURN_TYPES = ('TSV',)
    ...
    def render_command(cls, inputs: dict[str, Any]) -> str:
        ...
        commands = [f'ln -s {shlex.quote(abundance_table)} {shlex.quote(input_name)}' for abundance_table, input_name in zip(abundance_tables, input_names, strict=False)]
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'abundance_tables': ('TSV', {'multiple': True, 'description': 'One or more MetaPhlAn predicted taxon relative abundance tables'})}, 'optional': {'element_identifiers': ('STRING', {'default': [], 'multiple': True, 'description': 'Optional sample labels used to name symlinked inputs before merging'}), 'gtdb_profiles': ('BOOLEAN', {'default': False, 'description': 'Merge GTDB-based MetaPhlAn profiles using semicolon-separated clades'})}, 'hidden': {'output': ('STRING', {})}}
