"""humann — metagenomics node(s). One tool per file (extracted from wrapped_taxonomy_humann.py)."""
...
class HUMAnNJoinTablesNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'HUMAnN', 'humann_join_tables', 'Join merge', 'gene table', 'pathway table', 'taxonomy table', 'MetaPhlAn table', 'multi-sample table']
    RETURN_TYPES = ('TSV',)
    RETURN_NAMES = ('output',)
    ...
    def render_command(cls, inputs: dict[str, Any]) -> str:
        ...
        commands.extend((_shell_join(['ln', '-s', table, f'tmp_dir/{input_name}']) for table, input_name in zip(tables, input_names, strict=False)))
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'inputs': ('TSV', {'multiple': True, 'description': 'Gene, pathway, or taxonomy tables to join'})}, 'optional': {'element_identifiers': ('STRING', {'default': [], 'multiple': True, 'description': 'Optional Galaxy element identifiers used to name joined samples'})}, 'hidden': {'output': ('STRING', {})}}
...
class HUMAnNRenormTableNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'HUMAnN', 'humann_renorm_table', 'Renormalize', 'copies per million', 'relative abundance', 'community total', 'levelwise total', 'UNMAPPED']
    RETURN_TYPES = ('TSV',)
    RETURN_NAMES = ('output',)
    ...
    @classmethod
    def render_command(cls, inputs: dict[str, Any]) -> str:
        cmd = ['humann_renorm_table', '--input', str(inputs.get('input', '')), '-o', cls._output_path(inputs), '--units', str(inputs.get('units', 'cpm')), '--mode', str(inputs.get('mode', 'community')), '--special', 'y' if inputs.get('special', True) else 'n']
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('TSV', {'description': 'HUMAnN gene or pathway table'})}, 'optional': {'units': ('STRING', {'default': 'cpm', 'options': cls.UNITS, 'description': 'Normalize to copies per million or relative abundance units'}), 'mode': ('STRING', {'default': 'community', 'options': cls.MODES, 'description': 'Normalize using community totals or per-level totals'}), 'special': ('BOOLEAN', {'default': True, 'description': 'Include special features such as UNMAPPED and UNINTEGRATED'}), 'update_snames': ('BOOLEAN', {'default': True, 'description': 'Update sample-name RPK suffixes to the selected units'})}, 'hidden': {'output': ('STRING', {})}}
...
class HUMAnNSplitTableNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'HUMAnN', 'humann_split_table', 'Split', 'merged table', 'one file per sample', 'taxonomy index', 'PICRUSt']
    ...
    @classmethod
    def render_command(cls, inputs: dict[str, Any]) -> str:
        cmd = ['humann_split_table', '--input', str(inputs.get('input', '')), '-o', cls._output_path(inputs)]
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('TSV', {'description': 'Merged HUMAnN gene or pathway table'})}, 'optional': {'taxonomy_index': ('INT', {'default': '', 'min': 0, 'description': 'Index of the gene in taxonomy data when splitting PICRUSt-style tables'}), 'taxonomy_level': ('STRING', {'default': '', 'options': cls.TAXONOMY_LEVELS, 'description': 'Taxonomy level to use for PICRUSt metagenome contribution output'})}, 'hidden': {'output': ('STRING', {})}}
...
class HUMAnNSplitStratifiedTableNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'HUMAnN', 'humann_split_stratified_table', 'Split a HUMAnN table', 'stratified table', 'unstratified table', 'gene families']
    ...
    @classmethod
    def render_command(cls, inputs: dict[str, Any]) -> str:
        return _shell_join(['humann_split_stratified_table', '--input', str(inputs.get('input', '')), '--output', cls._output_dir(inputs)])
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('TSV', {'description': 'Stratified HUMAnN table'})}, 'hidden': {'output': ('STRING', {})}}
...
class HUMAnNReduceTableNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'HUMAnN', 'humann_reduce_table', 'Reduce', 'joined HUMAnN table', 'row-wise summary', 'max sum mean min', 'sort by value']
    RETURN_TYPES = ('TSV',)
    RETURN_NAMES = ('output',)
    ...
    @classmethod
    def render_command(cls, inputs: dict[str, Any]) -> str:
        return _shell_join(['humann_reduce_table', '--input', str(inputs.get('input', '')), '-o', cls._output_path(inputs), '--function', str(inputs.get('function', 'max')), '--sort-by', str(inputs.get('sort_by', 'name'))])
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('TSV', {'description': 'Joined HUMAnN gene, pathway, or taxonomic table'})}, 'optional': {'function': ('STRING', {'default': 'max', 'options': cls.FUNCTIONS, 'description': 'Summary function to apply across each row'}), 'sort_by': ('STRING', {'default': 'name', 'options': cls.SORT_OPTIONS, 'description': 'Sort reduced rows by feature name, reduced value, or pathway level'})}, 'hidden': {'output': ('STRING', {})}}
...
class HUMAnNRegroupTableNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'HUMAnN', 'humann_regroup_table', 'Regroup', 'gene families', 'MetaCyc reactions', 'UniRef90', 'custom mapping', 'UNGROUPED']
    RETURN_TYPES = ('TSV',)
    RETURN_NAMES = ('output',)
    ...
    @classmethod
    def render_command(cls, inputs: dict[str, Any]) -> str:
        cmd = ['humann_regroup_table', '--input', str(inputs.get('input', '')), '--output', cls._output_path(inputs), '--function', str(inputs.get('function', 'sum'))]
        ...
        if grouping_type == 'standard':
            ...
            if inputs.get('reversed', False):
                cmd.append('--reversed')
        cmd.extend(['--precision', str(inputs.get('precision', 3)), '--ungrouped', cls._yn(inputs.get('ungrouped'), default=True), '--protected', cls._yn(inputs.get('protected'), default=True)])
        return _shell_join(cmd)
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('TSV', {'description': 'HUMAnN gene families table'})}, 'optional': {'function': ('STRING', {'default': 'sum', 'options': cls.FUNCTIONS, 'description': 'Combine grouped features by sum or mean'}), 'grouping_type': ('STRING', {'default': 'standard', 'options': cls.GROUPING_TYPES, 'description': 'Use built-in, installed utility mapping, or custom grouping'}), 'groups': ('STRING', {'default': 'uniref90_rxn', 'options': cls.STANDARD_GROUPS, 'description': 'Built-in regrouping from UniRef families to MetaCyc reactions', 'displayOptions': {'show': {'grouping_type': ['standard']}}}), 'grouping': ('FILE', {'default': '', 'description': 'Installed HUMAnN utility mapping file for large regrouping', 'displayOptions': {'show': {'grouping_type': ['large']}}}), 'custom': ('TSV', {'default': '', 'description': 'Custom groups mapping file', 'displayOptions': {'show': {'grouping_type': ['custom']}}}), 'reversed': ('BOOLEAN', {'default': False, 'description': 'Treat the mapping as feature-to-groups instead of groups-to-features', 'displayOptions': {'show': {'grouping_type': ['large', 'custom']}}}), 'precision': ('INT', {'default': 3, 'min': 0, 'description': 'Decimal places to round grouped abundances'}), 'ungrouped': ('BOOLEAN', {'default': True, 'description': 'Include UNGROUPED for features that did not map to a group'}), 'protected': ('BOOLEAN', {'default': True, 'description': 'Carry through protected features such as UNMAPPED'})}, 'hidden': {'output': ('STRING', {})}}
...
class HUMAnNRenameTableNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'HUMAnN', 'humann_rename_table', 'Rename features', 'feature names', 'MetaCyc reactions', 'UniRef90 name', 'custom mapping', 'NO_NAME']
    RETURN_TYPES = ('TSV',)
    RETURN_NAMES = ('output',)
    ...
    STANDARD_NAMES = ['metacyc-rxn', 'metacyc-pwy', 'infogo1000', 'kegg-module', 'ec', 'go', 'pfam', 'eggnog', 'kegg-pathway', 'kegg-orthology']
    ...
    @classmethod
    def render_command(cls, inputs: dict[str, Any]) -> str:
        cmd = ['humann_rename_table', '--input', str(inputs.get('input', '')), '-o', cls._output_path(inputs)]
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('TSV', {'description': 'HUMAnN gene, pathway, or regrouped feature table'})}, 'optional': {'renaming_type': ('STRING', {'default': 'standard', 'options': cls.RENAMING_TYPES, 'description': 'Use built-in, installed utility mapping, or custom name mapping'}), 'names': ('STRING', {'default': 'metacyc-rxn', 'options': cls.STANDARD_NAMES, 'description': 'Built-in feature namespace to rename', 'displayOptions': {'show': {'renaming_type': ['standard']}}}), 'advanced_names': ('FILE', {'default': '', 'description': 'Installed HUMAnN utility name mapping file', 'displayOptions': {'show': {'renaming_type': ['advanced']}}}), 'custom': ('TSV', {'default': '', 'description': 'Custom two-column feature-to-name mapping file', 'displayOptions': {'show': {'renaming_type': ['custom']}}}), 'simplify': ('BOOLEAN', {'default': False, 'description': 'Remove non-alphanumeric characters from names'})}, 'hidden': {'output': ('STRING', {})}}
...
class HUMAnNUnpackPathwaysNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'HUMAnN', 'humann_unpack_pathways', 'Unpack pathway abundances', 'pathway abundance', 'gene family abundance', 'EC abundance', 'reaction mapping', 'remove taxonomy']
    RETURN_TYPES = ('TSV',)
    RETURN_NAMES = ('output',)
    ...
    @classmethod
    def render_command(cls, inputs: dict[str, Any]) -> str:
        cmd = ['humann_unpack_pathways', '--input-genes', str(inputs.get('input_genes', '')), '--input-pathways', str(inputs.get('input_pathways', ''))]
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input_genes': ('TSV', {'description': 'HUMAnN gene family or EC abundance table'}), 'input_pathways': ('TSV', {'description': 'HUMAnN pathway abundance table'})}, 'optional': {'gene_mapping': ('TSV', {'default': '', 'description': 'Optional gene-family-to-reaction mapping table'}), 'pathway_mapping': ('TSV', {'default': '', 'description': 'Optional reaction-to-pathway mapping table'}), 'remove_taxonomy': ('BOOLEAN', {'default': False, 'description': 'Remove taxonomy stratification from unpacked pathway rows'})}, 'hidden': {'output': ('STRING', {})}}
...
class HUMAnNBarplotNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'HUMAnN', 'humann_barplot', 'Barplot', 'stratified HUMAnN features', 'focal feature', 'top taxa', 'Bray-Curtis', 'metadata sorting']
    RETURN_TYPES = ('IMAGE',)
    ...
    INT_DEFAULTS = {'top_taxa': 18, 'max_metalevels': 7, 'legend_cols': 3, 'legend_rows': 10}
    FLOAT_DEFAULTS = {'height': 11.0, 'width': 6.0, 'legend_height': 1.0}
    ...
    @classmethod
    def render_command(cls, inputs: dict[str, Any]) -> str:
        cmd = ['humann_barplot', '--input', str(inputs.get('input', ''))]
        last_metadata = str(inputs.get('last_metadata', '')).strip()
        if last_metadata:
            cmd.extend(['--last-metadata', last_metadata])
        cmd.extend(['--focal-feature', str(inputs.get('focal_feature', '')), '--top-taxa', str(inputs.get('top_taxa', 18))])
        ...
        cmd.extend(['--max-metalevels', str(inputs.get('max_metalevels', 7)), '--scaling', str(inputs.get('scaling', 'original'))])
        ...
        if inputs.get('no_grid', True):
            cmd.append('--no-grid')
        cmd.extend(['--dimensions', str(inputs.get('height', 11.0)), str(inputs.get('width', 6.0))])
        units = str(inputs.get('units', '')).strip()
        if units:
            cmd.extend(['--units', units])
        cmd.extend(['--legend-cols', str(inputs.get('legend_cols', 3)), '--legend-rows', str(inputs.get('legend_rows', 10)), '--legend-height', str(inputs.get('legend_height', 1.0)), '--output', cls._output_path(inputs)])
        return _shell_join(cmd)
    ...
    def VALIDATE_INPUTS(cls, inputs: dict[str, Any]) -> bool | str:
        ...
        if any((sort_value in {'braycurtis', 'braycurtis_w'} for sort_value in sort_values)) and (not inputs.get('remove_zeros', False)):
        ...
        for key, message in (('top_taxa', 'Top taxa must be zero or greater'), ('max_metalevels', 'Maximum metadata levels must be zero or greater'), ('legend_cols', 'Legend columns must be zero or greater'), ('legend_rows', 'Legend rows must be zero or greater')):
            error = cls._validate_nonnegative_int(inputs.get(key, cls.INT_DEFAULTS[key]), message)
            if error:
                return error
        for key, message in (('height', 'Plot height must be greater than zero'), ('width', 'Plot width must be greater than zero')):
            error = cls._validate_positive_float(inputs.get(key, cls.FLOAT_DEFAULTS[key]), message)
            if error:
                return error
        error = cls._validate_nonnegative_float(inputs.get('legend_height', cls.FLOAT_DEFAULTS['legend_height']), 'Legend height must be zero or greater')
        if error:
            return error
        ymin = inputs.get('ymin', '')
        ymax = inputs.get('ymax', '')
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('TSV', {'description': 'HUMAnN table with optional metadata'}), 'focal_feature': ('STRING', {'description': 'Feature ID of interest'})}, 'optional': {'last_metadata': ('STRING', {'default': '', 'description': 'Name of the last metadata row before feature rows'}), 'top_taxa': ('INT', {'default': 18, 'min': 0, 'description': 'Maximum taxa to highlight'}), 'as_genera': ('BOOLEAN', {'default': False, 'description': 'Collapse species to genera'}), 'exclude_unclassified': ('BOOLEAN', {'default': False, 'description': 'Exclude the unclassified stratum'}), 'remove_zeros': ('BOOLEAN', {'default': False, 'description': 'Remove samples with zero sum for the focal feature'}), 'sort': ('STRING', {'default': ['none'], 'multiple': True, 'options': cls.SORT_OPTIONS, 'description': 'Sample sorting methods evaluated in order'}), 'taxa_colormap': ('STRING', {'default': '', 'description': 'Named matplotlib colormap or taxa color mapping file'}), 'focal_metadata': ('STRING', {'default': '', 'description': 'Metadata row to highlight or group by'}), 'meta_colormap': ('STRING', {'default': '', 'description': 'Named matplotlib colormap or metadata color mapping file'}), 'max_metalevels': ('INT', {'default': 7, 'min': 0, 'description': 'Metadata levels to keep before collapsing rare levels'}), 'scaling': ('STRING', {'default': 'original', 'options': cls.SCALING_OPTIONS, 'description': 'Scale total bar heights while preserving taxon proportions'}), 'ymin': ('FLOAT', {'default': '', 'description': 'Minimum y-axis limit'}), 'ymax': ('FLOAT', {'default': '', 'description': 'Maximum y-axis limit'}), 'no_grid': ('BOOLEAN', {'default': True, 'description': 'Hide y-axis grid lines'}), 'height': ('FLOAT', {'default': 11.0, 'min': 0, 'description': 'Image height in inches'}), 'width': ('FLOAT', {'default': 6.0, 'min': 0, 'description': 'Image width in inches'}), 'units': ('STRING', {'default': '', 'description': 'Y-axis abundance units'}), 'legend_cols': ('INT', {'default': 3, 'min': 0, 'description': 'Legend columns'}), 'legend_rows': ('INT', {'default': 10, 'min': 0, 'description': 'Legend rows'}), 'legend_height': ('FLOAT', {'default': 1.0, 'min': 0, 'description': 'Legend-to-data-axis height ratio'}), 'format': ('STRING', {'default': 'pdf', 'options': cls.OUTPUT_FORMATS, 'description': 'Output plot format'})}, 'hidden': {'output': ('STRING', {})}}
