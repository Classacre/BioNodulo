"""bedtools — genomics node(s). One tool per file (extracted from wrapped_bedtools.py)."""
...
class BEDToolsCoverageNode(CommandNode):
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'inputA': ('BED', {'description': 'File A intervals on which coverage is calculated'}), 'inputB': ('BED_LIST', {'description': 'One or more file B interval or BAM inputs'})}, 'optional': {'split': ('BOOLEAN', {'default': False, 'description': 'Treat split BED12/BAM alignments as distinct intervals'}), 'strandedness': ('BOOLEAN', {'default': False, 'description': 'Require same-strand overlaps'}), 'd': ('BOOLEAN', {'default': False, 'description': 'Report depth at each position'}), 'hist': ('BOOLEAN', {'default': False, 'description': 'Report coverage histogram'}), 'mean': ('BOOLEAN', {'default': False, 'description': 'Report mean depth'}), 'overlap_a': ('FLOAT', {'default': '', 'min': 0, 'max': 1, 'description': 'Minimum overlap fraction of A'}), 'overlap_b': ('FLOAT', {'default': '', 'min': 0, 'max': 1, 'description': 'Minimum overlap fraction of B'}), 'reciprocal_overlap': ('BOOLEAN', {'default': False, 'advanced': True}), 'a_or_b': ('BOOLEAN', {'default': False, 'advanced': True}), 'sorted': ('BOOLEAN', {'default': False, 'description': 'Use sorted input mode'})}, 'hidden': {'output': ('STRING', {})}}
...
class BEDToolsGenomeCoverageNode(CommandNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input_type': ('STRING', {'default': 'bed', 'options': ['bed', 'bam']}), 'input': ('FILE', {'description': 'Sorted BED/GFF/VCF/BAM input'}), 'report': ('STRING', {'default': 'bg', 'options': ['bg', 'hist'], 'description': 'BedGraph or histogram output'})}, 'optional': {'genome': ('TSV', {'description': 'Genome chromosome sizes file required for BED-like input'}), 'zero_regions': ('BOOLEAN', {'default': False, 'description': 'Report zero-coverage regions with -bga'}), 'scale': ('FLOAT', {'default': 1.0, 'min': 0}), 'max': ('INT', {'default': '', 'min': 0, 'description': 'Histogram max depth bin'}), 'split': ('BOOLEAN', {'default': False, 'description': 'Treat split BED12/BAM alignments as distinct intervals'}), 'strand': ('STRING', {'default': '', 'options': ['', '+', '-'], 'description': 'Restrict coverage to one strand'}), 'd': ('BOOLEAN', {'default': False, 'description': 'Report 1-based per-position depth'}), 'dz': ('BOOLEAN', {'default': False, 'description': 'Report 0-based non-zero per-position depth'}), 'five': ('BOOLEAN', {'default': False, 'description': "Calculate coverage of 5' positions"}), 'three': ('BOOLEAN', {'default': False, 'description': "Calculate coverage of 3' positions"})}, 'hidden': {'output': ('STRING', {})}}
...
class BEDToolsSubtractNode(CommandNode):
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        remove_flags = {'remove_feature': '-A', 'remove_feature_sum': '-N', '-A': '-A', '-N': '-N'}
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'inputA': ('BED', {'description': 'Intervals to subtract from'}), 'inputB': ('BED', {'description': 'Intervals used to mask or remove A bases'})}, 'optional': {'strand': ('STRING', {'default': '', 'options': ['', 'same', 'opposite']}), 'overlap': ('FLOAT', {'default': '', 'min': 0, 'max': 1, 'description': 'Minimum overlap required as a fraction of A'}), 'remove_if_overlap': ('STRING', {'default': '', 'options': ['', 'remove_feature', 'remove_feature_sum'], 'description': 'Remove entire A feature on any overlap, or on cumulative overlap with -f'})}, 'hidden': {'output': ('STRING', {})}}
...
class BEDToolsMergeNode(CommandNode):
    ...
    @classmethod
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        strand_flags = {'same': ['-s'], 'forward': ['-S', '+'], 'reverse': ['-S', '-'], '-s': ['-s'], '-S +': ['-S', '+'], '-S -': ['-S', '-']}
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('FILE', {'description': 'Presorted BED/GFF/VCF/BAM intervals to merge'}), 'distance': ('INT', {'default': 0, 'description': 'Maximum distance between intervals to merge'})}, 'optional': {'strand': ('STRING', {'default': '', 'options': ['', 'same', 'forward', 'reverse']}), 'header': ('BOOLEAN', {'default': False, 'description': 'Print input header before results'}), 'columns': ('STRING', {'default': '', 'description': 'Comma-separated columns to summarize'}), 'operations': ('STRING', {'default': '', 'description': 'Comma-separated operations such as sum,mean,count,collapse,distinct'})}, 'hidden': {'output': ('STRING', {})}}
...
class BEDToolsSortNode(CommandNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('FILE', {'description': 'BED, GFF, VCF, bedGraph, or EncodePeak intervals to sort'})}, 'optional': {'sort_by': ('STRING', {'default': '', 'options': ['', '-sizeA', '-sizeD', '-chrThenSizeA', '-chrThenSizeD', '-chrThenScoreA', '-chrThenScoreD']}), 'genome': ('TSV', {'description': 'Optional genome chromosome sizes file for sort order'})}, 'hidden': {'output': ('STRING', {})}}
...
class BEDToolsGetFastaNode(CommandNode):
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        if inputs.get('split'):
            cmd.append('-split')
        cmd.extend(['-fi', 'input.fasta', '-bed', str(inputs.get('input', '')), '-fo', f'{_out(inputs)}/{output_name}'])
        return cmd
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('BED', {'description': 'Intervals used to extract sequence'}), 'fasta': ('FASTA', {'description': 'Reference FASTA file'})}, 'optional': {'name': ('BOOLEAN', {'default': False, 'description': 'Use BED name and coordinates in FASTA headers'}), 'name_only': ('BOOLEAN', {'default': False, 'description': 'Use only the BED name in FASTA headers'}), 'tab': ('BOOLEAN', {'default': False, 'description': 'Emit tab-delimited name and sequence output'}), 'strand': ('BOOLEAN', {'default': False, 'description': 'Reverse complement antisense features'}), 'split': ('BOOLEAN', {'default': False, 'description': 'Use BED12 blocks rather than full interval spans'})}, 'hidden': {'output': ('STRING', {})}}
...
class BEDToolsComplementNode(CommandNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('BED', {'description': 'Sorted interval file whose uncovered genome intervals are reported'}), 'genome': ('TSV', {'description': 'Two-column chromosome sizes genome file'})}, 'optional': {}, 'hidden': {'output': ('STRING', {})}}
...
class BEDToolsFlankNode(CommandNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('BED', {'description': 'Intervals to flank'}), 'genome': ('TSV', {'description': 'Two-column chromosome sizes genome file'})}, 'optional': {'addition_mode': ('STRING', {'default': 'b', 'options': ['b', 'lr']}), 'both': ('FLOAT', {'default': 1, 'min': 0, 'description': 'Symmetric flank size'}), 'left': ('FLOAT', {'default': 0, 'min': 0, 'description': 'Left/upstream flank size'}), 'right': ('FLOAT', {'default': 0, 'min': 0, 'description': 'Right/downstream flank size'}), 'pct': ('BOOLEAN', {'default': False, 'description': 'Interpret sizes as fractions of feature length'}), 'strand': ('BOOLEAN', {'default': False, 'description': 'Interpret left/right relative to feature strand'})}, 'hidden': {'output': ('STRING', {})}}
...
class BEDToolsSlopNode(CommandNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'inputA': ('BED', {'description': 'Intervals to resize'}), 'genome': ('TSV', {'description': 'Two-column chromosome sizes genome file'})}, 'optional': {'addition_mode': ('STRING', {'default': 'b', 'options': ['b', 'lr']}), 'both': ('FLOAT', {'default': 1, 'min': 0, 'description': 'Symmetric extension size'}), 'left': ('FLOAT', {'default': 0, 'min': 0, 'description': 'Left/upstream extension size'}), 'right': ('FLOAT', {'default': 0, 'min': 0, 'description': 'Right/downstream extension size'}), 'pct': ('BOOLEAN', {'default': False, 'description': 'Interpret sizes as fractions of feature length'}), 'strand': ('BOOLEAN', {'default': False, 'description': 'Interpret left/right relative to feature strand'}), 'header': ('BOOLEAN', {'default': False, 'description': 'Print the input header before results'})}, 'hidden': {'output': ('STRING', {})}}
...
class BEDToolsWindowNode(CommandNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'inputA': ('FILE', {'description': 'A intervals or BAM alignments'}), 'inputB': ('BED', {'description': 'B intervals to search near A'})}, 'optional': {'addition_mode': ('STRING', {'default': 'window', 'options': ['window', 'lr']}), 'window': ('INT', {'default': 1000, 'min': 0, 'description': 'Symmetric window size'}), 'left': ('INT', {'default': 1000, 'min': 0, 'description': 'Left/upstream window size'}), 'right': ('INT', {'default': 1000, 'min': 0, 'description': 'Right/downstream window size'}), 'strand': ('STRING', {'default': '', 'options': ['', 'same', 'opposite']}), 'bed': ('BOOLEAN', {'default': False, 'description': 'Write BED output for BAM input'}), 'original': ('BOOLEAN', {'default': False, 'description': 'Report each A feature once if any B hit is found'}), 'number': ('BOOLEAN', {'default': False, 'description': 'Report number of B hits for each A feature'}), 'nooverlaps': ('BOOLEAN', {'default': False, 'description': 'Report only A features with no B hits'}), 'header': ('BOOLEAN', {'default': False, 'description': 'Print the input header before results'})}, 'hidden': {'output': ('STRING', {})}}
...
class BEDToolsMapNode(CommandNode):
    ...
    @classmethod
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        cmd = ['bedtools', 'map', '-a', str(inputs.get('inputA', '')), '-b', str(inputs.get('inputB', ''))]
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'inputA': ('BED', {'description': 'Sorted A intervals'}), 'inputB': ('BED', {'description': 'Sorted B intervals with columns to summarize'}), 'columns': ('STRING', {'default': '5', 'description': 'Comma-separated B columns to summarize'}), 'operations': ('STRING', {'default': 'mean', 'description': 'Comma-separated summary operations'})}, 'optional': {'strand': ('STRING', {'default': '', 'options': ['', 'same', 'opposite']}), 'overlap': ('FLOAT', {'default': '', 'min': 0, 'max': 1, 'description': 'Minimum overlap fraction of A'}), 'overlap_b': ('FLOAT', {'default': '', 'min': 0, 'max': 1, 'description': 'Minimum overlap fraction of B'}), 'reciprocal': ('BOOLEAN', {'default': False, 'description': 'Require reciprocal overlap'}), 'split': ('BOOLEAN', {'default': False, 'description': 'Treat split BED12/BAM entries as distinct intervals'}), 'header': ('BOOLEAN', {'default': False, 'description': 'Print the input header before results'}), 'genome': ('TSV', {'description': 'Optional genome chromosome sizes file'})}, 'hidden': {'output': ('STRING', {})}}
...
class BEDToolsMultiIntersectNode(CommandNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'inputs': ('BED_LIST', {'description': 'Two or more sorted interval files'})}, 'optional': {'names': ('STRING_LIST', {'description': 'Optional custom labels matching the inputs order'}), 'header': ('BOOLEAN', {'default': False, 'description': 'Add output header'}), 'cluster': ('BOOLEAN', {'default': False, 'description': 'Invoke clustering algorithm'}), 'filler': ('STRING', {'default': 'N/A', 'description': 'Text for no-coverage values'}), 'empty': ('BOOLEAN', {'default': False, 'description': 'Report regions with zero coverage across all files'}), 'genome': ('TSV', {'description': 'Genome chromosome sizes file required when empty is enabled'})}, 'hidden': {'output': ('STRING', {})}}
...
class BEDToolsClusterNode(CommandNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'inputA': ('BED', {'description': 'Sorted interval file to cluster'})}, 'optional': {'strand': ('BOOLEAN', {'default': False, 'description': 'Only cluster features on the same strand'}), 'distance': ('INT', {'default': 0, 'description': 'Maximum distance between features in the same cluster'})}, 'hidden': {'output': ('STRING', {})}}
...
class BEDToolsJaccardNode(CommandNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'inputA': ('BED', {'description': 'Sorted interval file A'}), 'inputB': ('BED', {'description': 'Sorted interval file B'})}, 'optional': {'overlap': ('FLOAT', {'default': '', 'min': 0, 'max': 1, 'description': 'Minimum overlap fraction of A'}), 'overlap_b': ('FLOAT', {'default': '', 'min': 0, 'max': 1, 'description': 'Minimum overlap fraction of B'}), 'reciprocal': ('BOOLEAN', {'default': False, 'description': 'Require reciprocal overlap'}), 'strand': ('BOOLEAN', {'default': False, 'description': 'Require same-strand overlaps'}), 'split': ('BOOLEAN', {'default': False, 'description': 'Treat split BED12/BAM entries as distinct intervals'})}, 'hidden': {'output': ('STRING', {})}}
...
class BEDToolsFisherNode(CommandNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'inputA': ('BED', {'description': 'Interval file A'}), 'inputB': ('BED', {'description': 'Interval file B'}), 'genome': ('TSV', {'description': 'Two-column chromosome sizes genome file'})}, 'optional': {'strand': ('STRING', {'default': '', 'options': ['', 'same', 'opposite']}), 'split': ('BOOLEAN', {'default': False, 'description': 'Treat split BED12/BAM entries as distinct intervals'}), 'overlap': ('FLOAT', {'default': '', 'min': 0, 'max': 1, 'description': 'Minimum overlap fraction of A'}), 'reciprocal': ('BOOLEAN', {'default': False, 'description': 'Require reciprocal overlap'}), 'merge': ('BOOLEAN', {'default': False, 'description': 'Merge overlapping intervals before testing'})}, 'hidden': {'output': ('STRING', {})}}
...
class BEDToolsRelativeDistanceNode(CommandNode):
    ...
    CITATION_TEXT = f'{BEDTOOLS_CITATION_TEXT}; Exploring Massive, Genome Scale Datasets with the GenometriCorr Package.'
    VERSION = '2.31.1'
    SHELL = True

    @classmethod
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        cmd = ['bedtools', 'reldist', '-a', str(inputs.get('inputA', '')), '-b', str(inputs.get('inputB', ''))]
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'inputA': ('BED', {'description': 'Interval file A'}), 'inputB': ('BED', {'description': 'Interval file B'})}, 'optional': {'detail': ('BOOLEAN', {'default': False, 'description': 'Report relative distance for each A interval'})}, 'hidden': {'output': ('STRING', {})}}
...
class BEDToolsSpacingNode(CommandNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('BED', {'description': 'Sorted interval file'})}, 'optional': {}, 'hidden': {'output': ('STRING', {})}}
...
class BEDToolsGroupByNode(CommandNode):
    ...
    @classmethod
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        cmd = ['bedtools', 'groupby', '-i', str(inputs.get('inputA', '')), '-g', str(inputs.get('group', '1,2,3')), '-c', str(inputs.get('columns', inputs.get('cols', ''))), '-o', str(inputs.get('operation', 'sum'))]
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'inputA': ('BED', {'description': 'Interval or tabular file to group'}), 'columns': ('STRING', {'default': '4', 'description': 'Comma-separated columns to summarize'}), 'group': ('STRING', {'default': '1,2,3', 'description': 'Columns or ranges to group by'}), 'operation': ('STRING', {'default': 'sum', 'options': ['sum', 'min', 'max', 'absmin', 'absmax', 'mean', 'median', 'mode', 'antimode', 'stdev', 'sstdev', 'collapse', 'count', 'distinct', 'first', 'last', 'freqasc', 'freqdesc']})}, 'optional': {}, 'hidden': {'output': ('STRING', {})}}
...
class BEDToolsBamToBedNode(CommandNode):
    ...
    @classmethod
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        option_aliases = {'': '', 'bed': '', 'bed6': '', 'bed12': '-bed12', '-bed12': '-bed12', 'bedpe': '-bedpe', '-bedpe': '-bedpe'}
        ...
        if option == '-bedpe':
            ...
            cmd.extend(['samtools', 'sort', '-n', '-@', str(inputs.get('threads', 4)), '-T', f'{out}/tmp', str(inputs.get('input', '')), '>', bedtools_input, '&&'])
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('BAM', {'description': 'BAM alignment file to convert'}), 'option': ('STRING', {'default': '', 'options': ['', 'bed12', 'bedpe'], 'description': 'Output BED flavor: BED6, blocked BED12, or paired BEDPE'})}, 'optional': {'split': ('BOOLEAN', {'default': False, 'description': 'Split spliced alignments into distinct BED records'}), 'ed_score': ('BOOLEAN', {'default': False, 'description': 'Use BAM edit distance as the BED score'}), 'tag': ('STRING', {'default': '', 'description': 'Numeric BAM tag to use as the BED score'}), 'threads': ('INT', {'default': 4, 'min': 1, 'description': 'Threads for BEDPE name sorting'})}, 'hidden': {'output': ('STRING', {})}}
...
class BEDToolsBed12ToBed6Node(CommandNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('BED', {'description': 'BED12 file to expand into BED6 blocks'})}, 'optional': {}, 'hidden': {'output': ('STRING', {})}}
...
class BEDToolsBedToBamNode(CommandNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('BED', {'description': 'BED feature file to convert'}), 'genome': ('TSV', {'description': 'Two-column chromosome sizes genome file'})}, 'optional': {'bed12': ('BOOLEAN', {'default': False, 'description': 'Convert blocked BED12 records into spliced BAM alignments'}), 'mapq': ('INT', {'default': 255, 'min': 0, 'max': 255, 'description': 'Mapping quality value for output alignments'})}, 'hidden': {'output': ('STRING', {})}}
...
class BEDToolsBedpeToBamNode(CommandNode):
    ...
    @classmethod
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        cmd = ['bedtools', 'bedpetobam', '-mapq', str(inputs.get('mapq', 255)), '-i', str(inputs.get('input', ''))]
        _bedtools_add_genome(cmd, inputs)
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('BED', {'description': 'BEDPE or BED-like paired feature file'}), 'genome': ('TSV', {'description': 'Two-column chromosome sizes genome file'})}, 'optional': {'mapq': ('INT', {'default': 255, 'min': 0, 'max': 255, 'description': 'Mapping quality value for output alignments'})}, 'hidden': {'output': ('STRING', {})}}
...
class BEDToolsMakeWindowsNode(CommandNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'type': ('STRING', {'default': 'bed', 'options': ['bed', 'genome'], 'description': 'Create windows over BED intervals or a genome file'}), 'action': ('STRING', {'default': 'windowsize', 'options': ['windowsize', 'number'], 'description': 'Window by fixed size or fixed count'})}, 'optional': {'input': ('BED', {'description': 'BED intervals used when type is bed'}), 'genome': ('TSV', {'description': 'Genome chromosome sizes file used when type is genome'}), 'windowsize': ('INT', {'default': 1, 'min': 1, 'description': 'Window size in bases'}), 'step_size': ('INT', {'default': '', 'min': 1, 'description': 'Step size for sliding windows'}), 'number': ('INT', {'default': 1, 'min': 1, 'description': 'Number of windows per input interval'}), 'sourcename': ('STRING', {'default': '', 'options': ['', 'src', 'winnum', 'srcwinnum'], 'description': 'ID naming style for generated windows'})}, 'hidden': {'output': ('STRING', {})}}
...
class BEDToolsAnnotateNode(CommandNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'inputA': ('BED', {'description': 'Intervals to annotate'}), 'beds': ('BED_LIST', {'description': 'One or more annotation interval files'})}, 'optional': {'names': ('STRING_LIST', {'description': 'Optional labels matching the annotation files'}), 'strand': ('STRING', {'default': '', 'options': ['', 'same', 'opposite']}), 'counts': ('BOOLEAN', {'default': False, 'description': 'Report counts instead of only coverage fractions'}), 'both': ('BOOLEAN', {'default': False, 'description': 'Report counts followed by coverage fractions'})}, 'hidden': {'output': ('STRING', {})}}
...
class BEDToolsExpandNode(CommandNode):
    ...
    @classmethod
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        cmd = ['bedtools', 'expand', '-c', str(inputs.get('columns', inputs.get('cols', ''))), '-i', str(inputs.get('input', ''))]
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('BED', {'description': 'BED-like file containing comma-separated values'}), 'columns': ('STRING', {'default': '4', 'description': 'Comma-separated columns to expand'})}, 'optional': {}, 'hidden': {'output': ('STRING', {})}}
...
class BEDToolsMaskFastaNode(CommandNode):
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        if inputs.get('soft'):
            cmd.append('-soft')
        cmd.extend(['-mc', str(inputs.get('mask_character', inputs.get('mc', 'N'))), '-fi', str(inputs.get('fasta', '')), '-bed', str(inputs.get('input', '')), '-fo', f'{_out(inputs)}/masked.fasta'])
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('BED', {'description': 'Intervals used to mask the FASTA'}), 'fasta': ('FASTA', {'description': 'FASTA sequences to mask'})}, 'optional': {'soft': ('BOOLEAN', {'default': False, 'description': 'Soft-mask by converting bases to lowercase'}), 'mask_character': ('STRING', {'default': 'N', 'description': 'Hard-mask replacement character'}), 'full_header': ('BOOLEAN', {'default': False, 'description': 'Match and emit the full FASTA header'})}, 'hidden': {'output': ('STRING', {})}}
...
class BEDToolsMultiCovNode(CommandNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('BED', {'description': 'Sorted intervals to count over'}), 'bams': ('BAM_LIST', {'description': 'Sorted and indexed BAM files'})}, 'optional': {'strand': ('STRING', {'default': '', 'options': ['', 'same', 'opposite']}), 'overlap': ('FLOAT', {'default': '', 'min': 0, 'max': 1, 'description': 'Minimum overlap fraction'}), 'reciprocal': ('BOOLEAN', {'default': False, 'description': 'Require reciprocal overlap'}), 'split': ('BOOLEAN', {'default': False, 'description': 'Treat split or spliced alignments as separate intervals'}), 'q': ('INT', {'default': 0, 'min': 0, 'max': 255, 'description': 'Minimum mapping quality'}), 'duplicate': ('BOOLEAN', {'default': False, 'description': 'Include duplicate reads'}), 'failed': ('BOOLEAN', {'default': False, 'description': 'Include failed-QC reads'}), 'proper': ('BOOLEAN', {'default': False, 'description': 'Only count proper pairs'})}, 'hidden': {'output': ('STRING', {})}}
...
class BEDToolsNucNode(CommandNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('BED', {'description': 'Intervals whose nucleotide content is profiled'}), 'fasta': ('FASTA', {'description': 'Reference FASTA file'})}, 'optional': {'strand': ('BOOLEAN', {'default': False, 'description': 'Profile sequence according to strand'}), 'seq': ('BOOLEAN', {'default': False, 'description': 'Print the extracted sequence'}), 'pattern': ('STRING', {'default': '', 'description': 'Sequence pattern to count'}), 'ignore_case': ('BOOLEAN', {'default': False, 'description': 'Ignore case when matching pattern'})}, 'hidden': {'output': ('STRING', {})}}
...
class BEDToolsRandomNode(CommandNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'genome': ('TSV', {'description': 'Genome chromosome sizes file'})}, 'optional': {'length': ('INT', {'default': 100, 'min': 1, 'description': 'Length of each random interval'}), 'intervals': ('INT', {'default': 1000000, 'min': 1, 'description': 'Number of intervals to generate'}), 'seed': ('INT', {'default': '', 'description': 'Optional random seed'})}, 'hidden': {'output': ('STRING', {})}}
...
class BEDToolsShuffleNode(CommandNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'inputA': ('BED', {'description': 'Intervals to randomly redistribute'}), 'genome': ('TSV', {'description': 'Genome chromosome sizes file'})}, 'optional': {'bedpe': ('BOOLEAN', {'default': False, 'description': 'Input is BEDPE format'}), 'seed': ('INT', {'default': '', 'description': 'Optional random seed'}), 'exclude': ('BED', {'description': 'Regions where shuffled intervals must not be placed'}), 'include': ('BED', {'description': 'Regions where shuffled intervals must be placed'}), 'overlap': ('FLOAT', {'default': '', 'min': 0, 'max': 1, 'description': 'Maximum tolerated overlap with excluded regions'}), 'chrom': ('BOOLEAN', {'default': False, 'description': 'Keep intervals on their original chromosome'}), 'chromfirst': ('BOOLEAN', {'default': False, 'description': 'Choose chromosome uniformly before choosing position'}), 'no_overlap': ('BOOLEAN', {'default': False, 'description': 'Do not allow shuffled intervals to overlap each other'}), 'allow_beyond': ('BOOLEAN', {'default': False, 'description': 'Allow intervals to extend beyond chromosome end'}), 'maxtries': ('INT', {'default': 1000, 'min': 1, 'description': 'Maximum placement attempts per interval'})}, 'hidden': {'output': ('STRING', {})}}
...
class BEDToolsUnionBedGraphNode(CommandNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'inputs': ('BEDGRAPH_LIST', {'description': 'Sorted non-overlapping BedGraph files'})}, 'optional': {'names': ('STRING_LIST', {'description': 'Optional column labels matching the input files'}), 'header': ('BOOLEAN', {'default': False, 'description': 'Print a header row'}), 'filler': ('STRING', {'default': 'N/A', 'description': 'Value for no coverage in a file'}), 'empty': ('BOOLEAN', {'default': False, 'description': 'Report regions with zero coverage across all files'}), 'genome': ('TSV', {'description': 'Genome chromosome sizes file required when empty is enabled'})}, 'hidden': {'output': ('STRING', {})}}
...
class BEDToolsClosestBedNode(CommandNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'inputA': ('BED', {'description': 'Query intervals'}), 'inputB': ('BED_LIST', {'description': 'One or more databases of intervals to search'})}, 'optional': {'ties': ('STRING', {'default': 'all', 'options': ['all', 'first', 'last'], 'description': 'How equally close B records are handled'}), 'strand': ('STRING', {'default': '', 'options': ['', 'same', 'opposite']}), 'distance': ('BOOLEAN', {'default': False, 'description': 'Report distance as an extra column'}), 'distance_mode': ('STRING', {'default': '', 'options': ['', 'ref', 'a', 'b'], 'description': 'Report signed upstream/downstream distances'}), 'ignore_upstream': ('BOOLEAN', {'default': False, 'description': 'Ignore upstream features when using -D'}), 'ignore_downstream': ('BOOLEAN', {'default': False, 'description': 'Ignore downstream features when using -D'}), 'first_upstream': ('BOOLEAN', {'default': False, 'description': 'Choose first upstream feature when using -D'}), 'first_downstream': ('BOOLEAN', {'default': False, 'description': 'Choose first downstream feature when using -D'}), 'ignore_overlaps': ('BOOLEAN', {'default': False, 'description': 'Ignore B features that overlap A'}), 'mdb': ('STRING', {'default': 'each', 'options': ['each', 'all'], 'description': 'Resolve closest hits per B file or across all B files'}), 'k': ('INT', {'default': '', 'min': 1, 'description': 'Report the k closest hits'})}, 'hidden': {'output': ('STRING', {})}}
...
class BEDToolsIntersectBedNode(CommandNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'inputA': ('FILE', {'description': 'A file: BED-like, BAM, VCF, or GFF'}), 'inputB': ('FILE_LIST', {'description': 'One or more B files to intersect with A'})}, 'optional': {'names': ('STRING_LIST', {'description': 'Optional labels for B files'}), 'overlap_mode': ('STRING_LIST', {'description': 'Reporting flags such as -wa, -wb, -wo, -wao, or -loj'}), 'split': ('BOOLEAN', {'default': False, 'description': 'Treat split BED12/BAM alignments as distinct intervals'}), 'strand': ('STRING', {'default': '', 'options': ['', 'same', 'opposite']}), 'overlap': ('FLOAT', {'default': '', 'min': 0, 'max': 1, 'description': 'Minimum overlap fraction of A'}), 'overlap_b': ('FLOAT', {'default': '', 'min': 0, 'max': 1, 'description': 'Minimum overlap fraction of B'}), 'reciprocal': ('BOOLEAN', {'default': False, 'description': 'Require reciprocal overlap'}), 'either_fraction': ('BOOLEAN', {'default': False, 'description': 'Allow either A or B overlap fraction to be satisfied'}), 'invert': ('BOOLEAN', {'default': False, 'description': 'Report A records with no overlaps'}), 'once': ('BOOLEAN', {'default': False, 'description': 'Report each A record once if any overlap exists'}), 'count': ('BOOLEAN', {'default': False, 'description': 'Report overlap count for each A record'}), 'bed': ('BOOLEAN', {'default': False, 'description': 'When A is BAM, write BED output'}), 'sorted': ('BOOLEAN', {'default': False, 'description': 'Use sorted input algorithm'}), 'genome': ('TSV', {'description': 'Genome chromosome sizes file for sorted mode'}), 'header': ('BOOLEAN', {'default': False, 'description': 'Print the A file header before results'})}, 'hidden': {'output': ('STRING', {})}}
...
class BEDToolsBedToIgvNode(CommandNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('FILE', {'description': 'BED-like interval file to convert into IGV snapshot commands'})}, 'optional': {'sort': ('STRING', {'default': '', 'options': ['', 'base', 'position', 'strand', 'quality', 'sample', 'readGroup'], 'description': 'BAM sorting mode to apply before snapshots'}), 'clps': ('BOOLEAN', {'default': False, 'description': 'Collapse aligned reads before each snapshot'}), 'name': ('BOOLEAN', {'default': False, 'description': 'Use column 4 interval names as image filenames'}), 'slop': ('INT', {'default': 0, 'min': 0, 'description': 'Flanking base pairs on each side of each interval'}), 'img': ('STRING', {'default': 'png', 'options': ['png', 'eps', 'svg'], 'description': 'Snapshot image format'})}, 'hidden': {'output': ('STRING', {})}}
...
class BEDToolsLinksNode(CommandNode):
    ...
    @classmethod
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        cmd = ['bedtools', 'links', '-base', str(inputs.get('basename', 'http://genome.ucsc.edu')), '-org', str(inputs.get('org', 'human')), '-db', str(inputs.get('db', 'hg19')), '-i', str(inputs.get('input', ''))]
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('FILE', {'description': 'BED-like interval file to link into a genome browser'})}, 'optional': {'basename': ('STRING', {'default': 'http://genome.ucsc.edu', 'description': 'Base URL for the UCSC Genome Browser instance'}), 'org': ('STRING', {'default': 'human', 'description': 'UCSC organism name'}), 'db': ('STRING', {'default': 'hg19', 'description': 'UCSC genome build'})}, 'hidden': {'output': ('STRING', {})}}
...
class BEDToolsOverlapBedNode(CommandNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('FILE', {'description': 'Input rows containing two coordinate ranges'}), 'cols': ('STRING', {'default': '', 'description': 'Comma-separated 1-based columns: start1,end1,start2,end2'})}, 'hidden': {'output': ('STRING', {})}}
...
class BEDToolsTagBedNode(CommandNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'inputA': ('BAM', {'description': 'BAM alignments to annotate'}), 'inputB': ('FILE_LIST', {'description': 'BED-like annotation files used to populate tags'})}, 'optional': {'strand': ('STRING', {'default': '', 'options': ['', 'same', 'opposite']}), 'overlap': ('FLOAT', {'default': '', 'min': 0, 'max': 1, 'description': 'Minimum overlap fraction of each alignment'}), 'tag': ('STRING', {'default': 'YB', 'description': 'BAM tag name to populate'}), 'field': ('STRING', {'default': '-labels', 'options': ['-labels', '-scores', '-names', '-labels -intervals'], 'description': 'Annotation field used as tag value'})}, 'hidden': {'output': ('STRING', {})}}
