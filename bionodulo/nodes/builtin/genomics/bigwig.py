"""bigwig — genomics node(s). One tool per file (extracted from wrapped_variant_assembly.py)."""
...
class BigWigOutlierBedNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'BigWig outliers', 'bigwig_outlier_bed', 'pybigtools', 'coverage outliers', 'BED features', 'quantile cutoff', 'contig statistics']
    ...
    VERSION = '0.2.5'
    SHELL = True
    BED_OUTPUTS = {'bedouthilo': 'high_low_regions.bed', 'bedouthi': 'high_regions.bed', 'bedoutlo': 'low_regions.bed', 'bedoutzero': 'zero_regions.bed'}
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        ...
        return {'required': {'bigwig': ('BIGWIG', {'multiple': True, 'description': 'One or more BigWig files sharing the same reference sequence'})}, 'optional': {'bigwiglabels': ('STRING', {'default': [], 'multiple': True, 'description': 'Optional labels aligned to the BigWig inputs'}), 'minwin': ('INT', {'default': 10, 'min': 1, 'description': 'Minimum continuous bases required for a BED feature'}), 'qhi': ('FLOAT', {'default': 0.99999, 'min': 0, 'max': 1, 'description': 'Upper quantile cutoff for high regions'}), 'qlo': ('FLOAT', {'default': 1e-05, 'min': 0, 'max': 1, 'description': 'Lower quantile cutoff for low regions'}), 'outbeds': ('STRING', {'default': 'outhilo', 'options': outbed_options, 'description': 'Select high/low/zero BED outputs or table-only mode'}), 'tableout': ('STRING', {'default': 'create', 'options': ['create', 'donotmake'], 'description': 'Whether to write the contig statistics table'}), 'script': ('FILE', {'default': 'bigwig_outlier_bed.py', 'advanced': True, 'description': 'Path to the Galaxy bigwig_outlier_bed.py helper script'})}, 'hidden': {'output': ('STRING', {})}}
