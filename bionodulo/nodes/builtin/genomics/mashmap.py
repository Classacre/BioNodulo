"""mashmap — genomics node(s). One tool per file (extracted from wrapped_hyphy_metagenomics.py)."""
from __future__ import annotations
from bionodulo.nodes.builtin._wrapped_tool_utils import *


class MashMapNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'mashmap', 'MashMap3', 'local alignment boundaries', 'PAF', 'genome mapping', 'long read mapping', 'minmers', 'minhash']
    RETURN_TYPES = ('PAF',)
    ...
    def render_command(cls, inputs: dict[str, Any]) -> str:
        ...
        cmd = ['mashmap', '--threads', '${GALAXY_SLOTS:-1}', '--perc_identity', str(inputs.get('perc_identity', 85.0)), '--segLength', str(inputs.get('seqLength', 5000)), '--filter_mode', str(inputs.get('filter_mode', 'map'))]
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'query': ('FASTA_LIST', {'multiple': True, 'description': 'One or more query FASTA or FASTQ sequences'}), 'reflist': ('FASTA_LIST', {'multiple': True, 'description': 'One or more reference FASTA or FASTQ sequences'})}, 'optional': {'perc_identity': ('FLOAT', {'default': 85.0, 'min': 0, 'max': 100, 'description': 'Minimum identity threshold to report'}), 'seqLength': ('INT', {'default': 5000, 'min': 1, 'description': 'Minimum segment length in base pairs'}), 'sketchSize': ('INT', {'default': 0, 'min': 0, 'description': 'Sketch size; 0 lets MashMap choose automatically'}), 'dense': ('BOOLEAN', {'default': True, 'description': 'Increase seed density for more accurate ANI estimates'}), 'kmerThreshold': ('FLOAT', {'default': '', 'min': 0.0, 'description': 'Ignore the top fraction of most frequent k-mer windows'}), 'kmerComplexity': ('FLOAT', {'default': '', 'min': 0.0, 'max': 1.0, 'description': 'Threshold for k-mer complexity filtering'}), 'filter_mode': ('STRING', {'default': 'map', 'options': list(cls.FILTER_MODES), 'description': 'Alignment filtering strategy'}), 'reportPercentage': ('BOOLEAN', {'default': False, 'description': 'Report predicted ANI values in [0, 100]'}), 'noMerge': ('BOOLEAN', {'default': False, 'description': 'Do not merge consecutive segment-level mappings'}), 'noHgFilter': ('BOOLEAN', {'default': False, 'description': 'Use MashMap2-style first-pass filtering'})}, 'hidden': {'output': ('STRING', {})}}
