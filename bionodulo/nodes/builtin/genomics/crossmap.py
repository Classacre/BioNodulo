"""crossmap — genomics node(s). One tool per file (extracted from wrapped_alignment_taxonomy.py)."""
...
class CrossMapBigWigNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'CrossMap', 'crossmap_bw', 'liftover BigWig', 'coordinate conversion', 'BigWig assembly conversion', 'chain file']
    RETURN_TYPES = ('BIGWIG',)
    RETURN_NAMES = ('output',)
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('BIGWIG', {'description': 'BigWig signal track to lift over'}), 'input_chain': ('TXT', {'description': 'LiftOver chain file'})}, 'optional': {'index_source': ('STRING', {'default': 'history', 'options': cls.INDEX_SOURCE_OPTIONS, 'description': 'Galaxy source selector for cached or history chain files'})}, 'hidden': {'output': ('STRING', {})}}
...
class CrossMapWigNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'CrossMap', 'crossmap_wig', 'liftover Wiggle', 'liftover WIG', 'coordinate conversion', 'Wiggle assembly conversion', 'bedGraph output', 'chain file']
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('FILE', {'description': 'Wiggle signal track to lift over'}), 'input_chain': ('TXT', {'description': 'LiftOver chain file'})}, 'optional': {'index_source': ('STRING', {'default': 'history', 'options': cls.INDEX_SOURCE_OPTIONS, 'description': 'Galaxy source selector for cached or history chain files'})}, 'hidden': {'output': ('STRING', {})}}
