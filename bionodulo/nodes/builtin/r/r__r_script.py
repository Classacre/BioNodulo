"""r — r node(s). One tool per file (extracted from r_script.py)."""
...
class DataFrameBuilderNode(BaseNode):
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'x_column': ('STRING', {'default': 'x', 'label': 'X Column Name'}), 'x_values': ('STRING', {'default': '1,2,3,4,5', 'multiline': True, 'label': 'X Values (comma-separated)'}), 'y_column': ('STRING', {'default': 'y', 'label': 'Y Column Name'}), 'y_values': ('STRING', {'default': '2,4,6,8,10', 'multiline': True, 'label': 'Y Values (comma-separated)'})}, 'optional': {'group_column': ('STRING', {'default': '', 'label': 'Group Column Name (optional)', 'advanced': True}), 'group_values': ('STRING', {'default': '', 'multiline': True, 'label': 'Group Values (optional)', 'advanced': True})}}
...
class RPlotNode(BaseNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'data_csv': ('FILE', {'label': 'Data CSV'}), 'plot_type': ('STRING', {'default': 'scatter', 'options': ['scatter', 'line', 'bar', 'histogram', 'boxplot', 'density', 'heatmap', 'custom'], 'label': 'Plot Type'}), 'x_axis': ('STRING', {'default': 'x', 'label': 'X Axis Column'}), 'y_axis': ('STRING', {'default': 'y', 'label': 'Y Axis Column'})}, 'optional': {'color_column': ('STRING', {'default': '', 'label': 'Color/Fill Column', 'advanced': True}), 'title': ('STRING', {'default': '', 'label': 'Plot Title', 'advanced': True}), 'x_label': ('STRING', {'default': '', 'label': 'X Axis Label', 'advanced': True}), 'y_label': ('STRING', {'default': '', 'label': 'Y Axis Label', 'advanced': True}), 'width': ('INT', {'default': 800, 'min': 200, 'max': 4000, 'step': 50, 'display': 'slider', 'label': 'Width (px)', 'advanced': True}), 'height': ('INT', {'default': 600, 'min': 200, 'max': 4000, 'step': 50, 'display': 'slider', 'label': 'Height (px)', 'advanced': True}), 'custom_script': ('STRING', {'default': '', 'multiline': True, 'label': 'Custom R Script (overrides template)', 'advanced': True})}}
    ...
    async def run(self, **kwargs: Any) -> tuple[str, ...]:
        ...
        if not custom_script.strip() and data_csv:
            try:
                ...
                if missing:
                    ...
                    raise ValueError(f"Column(s) not found in data: {', '.join(missing)}. Available columns: {available}")
            except ValueError:
                raise
            except Exception:
                pass
        if custom_script.strip():
            ...
            script = self._build_ggplot_script(data_csv, plot_type, x_axis, y_axis, color_col, title, x_label, y_label, width, height, png_path)
    ...
    @staticmethod
    def _build_ggplot_script(data_csv: str, plot_type: str, x_axis: str, y_axis: str, color_col: str, title: str, x_label: str, y_label: str, width: int, height: int, png_path: Path) -> str:
...
class RScriptNode(BaseNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'script': ('FILE', {'label': 'R Script File'})}, 'optional': {'args': ('STRING', {'default': '', 'label': 'Arguments', 'advanced': True})}}
