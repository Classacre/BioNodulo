"""adapter — trimming node(s). One tool per file (extracted from wrapped_amplicon_trimming.py)."""
...
class AdapterRemovalNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'AdapterRemoval', 'adapter_removal', 'adapterremoval', 'adapter trimming', 'FASTQ trimming', 'read merging', 'ancient DNA']
    RETURN_TYPES = ('TEXT', 'FASTQ', 'FASTQ', 'FASTQ', 'FASTQ', 'FASTQ', 'FASTQ', 'FASTQ', 'FASTQ')
    RETURN_NAMES = ('output_settings', 'output_truncated', 'output_forward_truncated', 'output_reverse_truncated', 'output_interleaved_truncated', 'output_singleton_truncated', 'output_collapsed', 'output_collapsed_truncated', 'output_discarded')
    ...
    def _add_fastq_options(cls, parts: list[str], inputs: dict[str, Any]) -> None:
        parts.extend(['--qualitybase', '33', '--qualitybase-output', '33', '--qualitymax', str(cls._default_int(inputs, 'qualitymax', 41))])
    ...
    def _add_trimming_options(cls, parts: list[str], inputs: dict[str, Any]) -> None:
        parts.extend(['--adapter1', str(inputs.get('adapter1') or ADAPTER_REMOVAL_ADAPTER1), '--adapter2', str(inputs.get('adapter2') or ADAPTER_REMOVAL_ADAPTER2)])
        cls._append_flag_value(parts, '--adapter-list', inputs.get('adapter_list'))
        parts.extend(['--minadapteroverlap', str(cls._default_int(inputs, 'minadapteroverlap', 0)), '--mm', str(cls._default_float(inputs, 'mm', 3.0)), '--shift', str(cls._default_int(inputs, 'shift', 2))])
        if inputs.get('trim5p'):
            parts.extend(['--trim5p', str(cls._default_int(inputs, 'trim5p_mate1', 0)), str(cls._default_int(inputs, 'trim5p_mate2', 0))])
        if inputs.get('trim3p'):
            parts.extend(['--trim3p', str(cls._default_int(inputs, 'trim3p_mate1', 0)), str(cls._default_int(inputs, 'trim3p_mate2', 0))])
        ...
        parts.extend(['--minlength', str(cls._default_int(inputs, 'minlength', 15)), '--maxlength', str(cls._default_int(inputs, 'maxlength', 4294967295))])
    ...
    def _planned_output_names(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        optional_map = {'output_singleton': 'output_singleton_truncated', 'output_collapsed': 'output_collapsed', 'output_collapsed_truncated': 'output_collapsed_truncated', 'output_discarded': 'output_discarded'}
    ...
    def render_command(cls, inputs: dict[str, Any]) -> str:
        ...
        if input_type == 'single':
            ...
            if cls._interleaved_output_enabled(inputs):
                ...
                parts.extend(['--output1', cls._output_path(inputs, 'output_forward_truncated'), '--output2', cls._output_path(inputs, 'output_reverse_truncated')])
        selected = cls._output_select(inputs)
        optional_flags = [('output_singleton', '--singleton', 'output_singleton_truncated'), ('output_collapsed', '--outputcollapsed', 'output_collapsed'), ('output_collapsed_truncated', '--outputcollapsedtruncated', 'output_collapsed_truncated'), ('output_discarded', '--discarded', 'output_discarded')]
    ...
    def PLAN_OUTPUTS(cls, inputs: dict[str, Any], output_dir: str | Path) -> list[Path]:
        ...
        return [out / (f'{name}.txt' if name == 'output_settings' else f'{name}.fastq') for name in cls._planned_output_names(inputs)]

    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input_type': ('STRING', {'default': 'single', 'options': ['single', 'pair', 'paired', 'interleaved'], 'description': 'Galaxy input mode: single, separate pair, paired collection, or interleaved reads'}), 'read1': ('FASTQ', {'description': 'Single, forward, or interleaved FASTQ reads'})}, 'optional': {'read2': ('FASTQ', {'default': '', 'description': 'Reverse FASTQ reads for pair mode'}), 'reads_collection': ('FASTQ_LIST', {'default': '', 'description': 'Paired collection as [forward, reverse] or a mapping with forward/reverse keys'}), 'read1_ext': ('STRING', {'default': '', 'description': 'Galaxy datatype extension for read1 symlink naming', 'advanced': True}), 'read2_ext': ('STRING', {'default': '', 'description': 'Galaxy datatype extension for read2 symlink naming', 'advanced': True}), 'interleaved_output': ('STRING', {'default': 'no', 'options': ['no', 'yes'], 'description': 'Write paired-end reads to one interleaved output'}), 'identify_adapters': ('BOOLEAN', {'default': False, 'description': 'Attempt consensus adapter identification from overlapping pairs'}), 'combined_output': ('BOOLEAN', {'default': False, 'description': 'Replace discarded or merged reads with a single N in paired output'}), 'qualitymax': ('INT', {'default': 41, 'min': 0, 'max': 93, 'description': 'Maximum Phred score expected in input and output files'}), 'convert_uracils': ('BOOLEAN', {'default': False, 'description': 'Convert uracils to thymine'}), 'mask_degenerate_bases': ('BOOLEAN', {'default': False, 'description': 'Mask ambiguous IUPAC bases'}), 'adapter1': ('STRING', {'default': ADAPTER_REMOVAL_ADAPTER1, 'description': 'Adapter sequence expected in mate 1 reads'}), 'adapter2': ('STRING', {'default': ADAPTER_REMOVAL_ADAPTER2, 'description': 'Adapter sequence expected in mate 2 reads'}), 'adapter_list': ('TSV', {'default': '', 'description': 'Optional tabular adapter sequence list'}), 'minadapteroverlap': ('INT', {'default': 0, 'min': 0, 'description': 'Minimum adapter overlap for single-end trimming'}), 'mm': ('FLOAT', {'default': 3.0, 'description': 'Mismatch fraction or reciprocal mismatch rate'}), 'shift': ('INT', {'default': 2, 'min': 0, 'max': 93, 'description': "Allow alignment slip at the 5' end"}), 'trim5p': ('BOOLEAN', {'default': False, 'description': "Trim fixed bases from 5' ends"}), 'trim5p_mate1': ('INT', {'default': 0, 'min': 0, 'description': "5' trim length for mate 1"}), 'trim5p_mate2': ('INT', {'default': 0, 'min': 0, 'description': "5' trim length for mate 2"}), 'trim3p': ('BOOLEAN', {'default': False, 'description': "Trim fixed bases from 3' ends"}), 'trim3p_mate1': ('INT', {'default': 0, 'min': 0, 'description': "3' trim length for mate 1"}), 'trim3p_mate2': ('INT', {'default': 0, 'min': 0, 'description': "3' trim length for mate 2"}), 'trimns': ('BOOLEAN', {'default': False, 'description': 'Trim consecutive terminal Ns'}), 'maxns': ('INT', {'default': 1000, 'min': 0, 'description': 'Discard reads with more Ns than this after trimming'}), 'trimqualities': ('BOOLEAN', {'default': False, 'description': 'Trim terminal low-quality stretches'}), 'sliding_window': ('BOOLEAN', {'default': False, 'description': 'Use sliding-window quality trimming'}), 'window_size': ('INT', {'default': 0, 'min': 0, 'description': 'Sliding-window size'}), 'minquality': ('INT', {'default': 2, 'min': 0, 'description': 'Low-quality trimming threshold'}), 'preserve5p': ('BOOLEAN', {'default': False, 'description': "Preserve 5' bases during quality trimming"}), 'minlength': ('INT', {'default': 15, 'min': 0, 'description': 'Discard reads shorter than this'}), 'maxlength': ('INT', {'default': 4294967295, 'min': 0, 'description': 'Discard reads longer than this'}), 'collapse': ('BOOLEAN', {'default': False, 'description': 'Merge overlapping reads into consensus sequences'}), 'minalignmentlength': ('INT', {'default': 11, 'min': 0, 'description': 'Minimum mate overlap for collapsing'}), 'collapse_deterministic': ('BOOLEAN', {'default': False, 'description': 'Use deterministic consensus bases when collapsing'}), 'collapse_conservatively': ('BOOLEAN', {'default': False, 'description': 'Use conservative FASTQ-join inspired collapsing'}), 'output_select': ('STRING', {'default': 'none', 'list': True, 'options': ['none', 'output_singleton', 'output_collapsed', 'output_collapsed_truncated', 'output_discarded'], 'description': 'Optional Galaxy outputs to request'})}, 'hidden': {'output': ('STRING', {})}}
