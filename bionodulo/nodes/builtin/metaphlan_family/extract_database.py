"""Focused owner for ``extract_metaphlan_database``."""
# ruff: noqa: F403,F405
...
class ExtractMetaPhlAnDatabaseNode(ToolsIUCCommandContract):
    ...
    def render_command(cls, inputs: dict[str, Any]) -> str:
        ...
        metadata_cmd = [
            "python",
            str(inputs.get("customizemetadata_script", "customizemetadata.py")),
            "transform_pkl_to_json",
            "--pkl",
            f"{database_prefix}.pkl",
            "--json",
            f"{out}/marker_metadata.json",
        ]
    ...
    def VALIDATE_INPUTS(cls, inputs: dict[str, Any]) -> bool | str:
        if not str(inputs.get("database_path", "")).strip():
            return "MetaPhlAn database directory is required"
        if not str(inputs.get("database_key", "")).strip():
            return "MetaPhlAn database key is required"
        if not str(inputs.get("customizemetadata_script", "customizemetadata.py")).strip():
            return "MetaPhlAn customizemetadata.py path is required"
        return True
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {
            "required": {
                "database_path": (
                    "DIRECTORY",
                    {"description": "Directory containing the cached MetaPhlAn BowTie2 database files"},
                ),
                "database_key": (
                    "STRING",
                    {
                        "default": "mpa_vJun23_CHOCOPhlAnSGB_202403",
                        "description": "MetaPhlAn database key/prefix, used with the matching .pkl metadata file",
                    },
                ),
            },
            "optional": {
                "customizemetadata_script": (
                    "FILE",
                    {
                        ...
                        "description": "Path to MetaPhlAn customizemetadata.py used to convert database metadata pickle to JSON",
                    },
                ),
            },
