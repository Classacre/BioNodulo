...
def main() -> None:
    ...
    if not bwrap:
        raise RuntimeError("Python Code requires bubblewrap (bwrap) in the workflow environment")

    base_executable = Path(getattr(sys, "_base_executable", sys.executable)).resolve()
    python_root = Path(sys.base_prefix).resolve()
    if not base_executable.exists() or not python_root.exists():
        raise RuntimeError("Python Code could not resolve the workflow Python runtime")
    ...
    for index, site_path in enumerate(site_paths):
        sandbox_path = f"/site-packages-{index}"
    ...
    command.extend(
        [
            ...
            "--chdir",
            "/work",
            f"/python/bin/{base_executable.name}",
            "/work/runner.py",
        ]
    )
...
class PythonCodeNode(BaseNode):
    ...
    REQUIRED_EXECUTABLES = ["python", "bwrap"]
    REQUIRED_CONDA_PACKAGES = ["python", "bubblewrap", "numpy", "pandas"]
    VERSION = "1.0.0"
    GIT_URL = "https://github.com/containers/bubblewrap.git"
    GIT_COMMIT = "1b80120ef26a28e065e67f89bfef873f13bdd317"
    SANDBOX_VERSION = "bubblewrap 0.11.2"
    PRODUCT_SOURCE_COMMIT = "01eba0d68c3af63cb725a81bed2a47f29499d4bf"
    DOCUMENTATION_URL = "https://github.com/containers/bubblewrap/blob/v0.11.2/README.md"
    ...
    async def run(self, **kwargs: Any) -> tuple[str, Any, str]:
        ...
        launcher_path = out_dir / "sandbox_launcher.py"
        ...
        launcher_python = "python" if getattr(context, "env_prefix", None) else sys.executable
        command = [launcher_python, str(launcher_path), str(out_dir)]
        try:
