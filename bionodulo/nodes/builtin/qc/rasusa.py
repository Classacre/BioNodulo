"""rasusa — qc node(s). One tool per file (extracted from rasusa.py)."""
...
RASUSA_CITATION_TEXT = 'Rasusa: Randomly subsample sequencing reads to a specified coverage; Efficient downsampling of genome alignments with Rasusa.'
...
class RasusaNode(CommandNode):
    ...
    def _append_reads_subsample_args(cls, cmd: list[str], inputs: dict[str, Any]) -> None:
        ...
        if subsample_type == 'coverage':
            cmd.extend(['--genome-size', _format_size(inputs.get('genome_size', ''), inputs.get('genome_size_unit', 'b')), '--coverage', str(inputs.get('coverage', ''))])
    ...
    def _render_aligned_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        cmd.extend(['--output-format', 'bam', str(inputs.get('aligned_input', '')), '|', 'samtools', 'sort', '--no-PG', '-@', '1', '-T', f'{out}/tmp', '-O', 'bam', '-o', f'{out}/subsampled.bam', '-'])
        return cmd
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input_selector': ('STRING', {'default': 'single', 'options': ['single', 'paired', 'paired_collection', 'aligned'], 'description': 'Input mode matching the Galaxy Rasusa wrapper'})}, 'optional': {'reads': ('FASTQ_LIST', {'default': '', 'description': 'Single read file or FASTQ/FASTA pair'}), 'reads1': ('FASTQ', {'default': '', 'description': 'Forward reads for paired mode'}), 'reads2': ('FASTQ', {'default': '', 'description': 'Reverse reads for paired mode'}), 'collection_forward': ('FASTQ', {'default': '', 'description': 'Forward reads from a paired collection', 'advanced': True}), 'collection_reverse': ('FASTQ', {'default': '', 'description': 'Reverse reads from a paired collection', 'advanced': True}), 'aligned_input': ('BAM', {'default': '', 'description': 'Coordinate-sorted BAM alignment for alignment downsampling'}), 'subsample_type': ('STRING', {'default': 'frac_reads', 'options': ['coverage', 'num_bases', 'num_reads', 'frac_reads'], 'description': 'Subsampling target for reads mode'}), 'genome_size': ('FLOAT', {'default': '', 'min': 0, 'description': 'Genome size value for coverage-based read subsampling'}), 'genome_size_unit': ('STRING', {'default': 'b', 'options': ['b', 'k', 'm', 'g', 't'], 'description': 'Genome size unit'}), 'coverage': ('FLOAT', {'default': '', 'min': 0, 'description': 'Desired read coverage or BAM depth'}), 'bases': ('FLOAT', {'default': '', 'min': 0, 'description': 'Target number of bases'}), 'num_bases_unit': ('STRING', {'default': 'b', 'options': ['b', 'k', 'm', 'g', 't'], 'description': 'Target bases unit'}), 'num': ('INT', {'default': '', 'min': 1, 'description': 'Target number of reads'}), 'frac': ('FLOAT', {'default': 0.1, 'min': 0, 'max': 1, 'description': 'Fraction of reads to keep'}), 'seed': ('INT', {'default': '', 'description': 'Random seed for reproducible subsampling'}), 'strict': ('BOOLEAN', {'default': False, 'description': 'Exit if the requested subsample cannot be met', 'advanced': True}), 'verbose': ('BOOLEAN', {'default': False, 'description': 'Enable verbose Rasusa logging', 'advanced': True}), 'output_ext': ('STRING', {'default': 'fastq.gz', 'options': ['fastq.gz', 'fastq', 'fasta.gz', 'fasta'], 'description': 'Read output extension'}), 'compress_type': ('STRING', {'default': '', 'options': ['', 'u', 'g', 'b', 'x', 'z'], 'description': 'Override FASTA/FASTQ output compression', 'advanced': True}), 'compress_level': ('INT', {'default': '', 'min': 1, 'max': 21, 'description': 'Compression level for compressed reads output', 'advanced': True}), 'output_format': ('STRING', {'default': '', 'options': ['', 'fasta', 'fastq', 'sam', 'bam', 'cram'], 'description': 'Override Rasusa reads output format', 'advanced': True}), 'strategy': ('STRING', {'default': 'stream', 'options': ['stream', 'fetch'], 'description': 'Alignment downsampling strategy', 'advanced': True}), 'swap_distance': ('INT', {'default': '', 'min': 0, 'description': 'Stream strategy swap distance', 'advanced': True}), 'step_size': ('INT', {'default': 100, 'min': 1, 'description': 'Fetch strategy step size', 'advanced': True}), 'batch_size': ('INT', {'default': '', 'min': 1000, 'description': 'Fetch strategy batch size', 'advanced': True})}, 'hidden': {'output': ('STRING', {})}}
