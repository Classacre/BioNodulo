"""Shared BlastXML, CAT, and cawlign contracts for focused owners."""
# ruff: noqa: F403,F405
...
class _BlastxmlToGappedGff3Contract(ToolsIUCCommandContract):
    """Convert BLAST XML alignments into gapped GFF3 features."""

    LEGACY_NODE_ID = "blastxml_to_gapped_gff3"
...
class _CatBaseContract(ToolsIUCCommandContract):
...
class _CatClassifyBaseContract(_CatBaseContract):
...
class _CatContigsContract(_CatClassifyBaseContract):
    """Classify metagenomic contigs with CAT taxonomic assignments."""

    LEGACY_NODE_ID = "cat_contigs"
...
class _CatBinsContract(_CatClassifyBaseContract):
    """Classify genome bins with BAT taxonomic assignments."""

    LEGACY_NODE_ID = "cat_bins"
    ...
    RETURN_TYPES = _CatContigsContract.RETURN_TYPES
    RETURN_NAMES = (
        "log",
        "predicted_proteins_faa",
        "predicted_proteins_gff",
    ...
    BASE_OUTPUT_FILES = {
        **{
            key: value
            for key, value in _CatClassifyBaseContract.BASE_OUTPUT_FILES.items()
...
class _CatAddNamesContract(_CatBaseContract):
    """Add taxonomic names to CAT or BAT classification outputs."""

    LEGACY_NODE_ID = "cat_add_names"
...
class _CatSummariseContract(_CatBaseContract):
    """Summarise CAT or BAT taxonomic assignments by official name."""

    LEGACY_NODE_ID = "cat_summarise"
...
class _CawlignContract(ToolsIUCCommandContract):
    """Codon-aware pairwise alignment with cawlign."""

    LEGACY_NODE_ID = "cawlign"
