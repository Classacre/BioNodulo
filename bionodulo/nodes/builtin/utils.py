...
class TextPreviewNode(CommandNode):
    ...
    # Reject obvious binary/image payloads; everything else is treated as text.
    _BINARY_EXTS = {
        ".png", ".jpg", ".jpeg", ".gif", ".bmp", ".webp", ".ico",
        ".bam", ".cram", ".bai", ".gz", ".bgz", ".zip", ".bcf", ".pdf",
    }
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {
            ...
            "optional": {
                ...
                "max_bytes": ("INT", {"default": 262144, "min": 1024, "max": 5242880, "label": "Max bytes", "advanced": True}),
            },
        }
    ...
    async def run(self, **kwargs: Any) -> tuple:
        ...
        try:
            ...
            out_html.write_text(
                f"<!doctype html><meta charset=utf-8><body style='font-family:sans-serif;padding:16px;color:#b91c1c'>"
                f"<strong>Text preview failed:</strong> {escape(str(exc))}</body>",
                encoding="utf-8",
            )
            if context is not None and hasattr(context, "register_preview"):
...
<h1>{escape(src.name)}</h1>{note_html}<pre>{body}</pre>""",
            encoding="utf-8",
        )
