"""samtools — samtools node(s). One tool per file (extracted from samtools.py)."""
...
class SamtoolsSortNode(CommandNode):
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        cmd = ['samtools', 'sort', '-@', str(inputs.get('threads', 4)), '-o', f'{output}/sorted_bam.bam', '-T', f'{output}/tmp']
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'bam': ('BAM', {'description': 'Input BAM file'}), 'threads': ('INT', {'default': 4, 'min': 1, 'max': 64, 'display': 'slider'})}, 'optional': {'memory': ('STRING', {'default': '2G'})}, 'hidden': {'output': ('STRING', {})}}
...
class SamtoolsIndexNode(CommandNode):
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        cmd = ['samtools', 'index', '-@', str(inputs.get('threads', 2))]
        if inputs.get('csi'):
            cmd.append('--csi')
        if output_path:
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'bam': ('BAM', {'description': 'Sorted BAM file to index'}), 'threads': ('INT', {'default': 2, 'min': 1, 'max': 64, 'display': 'slider'})}, 'optional': {'csi': ('BOOLEAN', {'default': False, 'description': 'Create CSI index instead of BAI'}), 'output_path': ('STRING', {'default': '', 'description': 'Output index path', 'advanced': True})}, 'hidden': {'output': ('STRING', {})}}
...
class SamtoolsCollateNode(CommandNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'bam': ('BAM', {'description': 'Input BAM file'}), 'threads': ('INT', {'default': 4, 'min': 1, 'max': 64, 'display': 'slider'})}, 'optional': {'output_name': ('STRING', {'default': '', 'description': 'Optional output filename stem'}), 'temp_prefix': ('STRING', {'default': '', 'description': 'Temporary file prefix', 'advanced': True})}, 'hidden': {'output': ('STRING', {})}}

    @classmethod
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        output = Path(str(inputs.get('output', inputs.get('output_dir', '.'))))
        output_bam = output / f'{cls._output_stem(inputs)}.name_collated.bam'
        cmd = ['samtools', 'collate', '-@', str(inputs.get('threads', 4)), '-o', str(output_bam)]
...
class SamtoolsFixmateNode(CommandNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'bam': ('BAM', {'description': 'Name-collated BAM from samtools collate'}), 'threads': ('INT', {'default': 4, 'min': 1, 'max': 64, 'display': 'slider'})}, 'optional': {'add_markdup_tags': ('BOOLEAN', {'default': True, 'description': 'Add ms tags required by samtools markdup'}), 'remove_secondary_and_unmapped': ('BOOLEAN', {'default': False, 'description': 'Remove secondary and unmapped alignments', 'advanced': True}), 'output_name': ('STRING', {'default': '', 'description': 'Optional output filename stem'})}, 'hidden': {'output': ('STRING', {})}}

    @classmethod
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        output = Path(str(inputs.get('output', inputs.get('output_dir', '.'))))
        output_bam = output / f'{cls._output_stem(inputs)}.fixmate.bam'
        cmd = ['samtools', 'fixmate', '-@', str(inputs.get('threads', 4))]
...
class SamtoolsMarkdupNode(CommandNode):
    ...
    REQUIRED_CONDA_PACKAGES = ['samtools']
    CATEGORY = 'samtools'
    DESCRIPTION = 'Mark or remove duplicate alignments from coordinate-sorted BAM files prepared with samtools fixmate -m'
    SEARCH_ALIASES = ['samtools', 'markdup', 'mark duplicates', 'remove duplicates', 'duplicate marking', 'picard equivalent', 'variant preprocessing']
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'bam': ('BAM', {'description': 'Coordinate-sorted BAM prepared with samtools fixmate -m'}), 'threads': ('INT', {'default': 4, 'min': 1, 'max': 64, 'display': 'slider'})}, 'optional': {'remove_duplicates': ('BOOLEAN', {'default': False, 'description': 'Remove duplicates instead of only marking them'}), 'mark_supplementary': ('BOOLEAN', {'default': False, 'description': 'Mark supplementary duplicate alignments', 'advanced': True}), 'mark_optical_duplicates': ('BOOLEAN', {'default': False, 'description': 'Use optical duplicate distance tagging', 'advanced': True}), 'optical_distance': ('INT', {'default': 100, 'min': 0, 'advanced': True}), 'read_name_regex': ('STRING', {'default': '', 'description': 'Regex for extracting read coordinates', 'advanced': True}), 'clear_existing_duplicate_flags': ('BOOLEAN', {'default': False, 'description': 'Clear existing duplicate flags before marking', 'advanced': True}), 'output_name': ('STRING', {'default': '', 'description': 'Optional output filename stem'})}, 'hidden': {'output': ('STRING', {})}}
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        cmd = ['samtools', 'markdup', '-@', str(inputs.get('threads', 4))]
    ...
    def PLAN_OUTPUTS(cls, inputs: dict[str, Any], output_dir: str | Path) -> list[Path]:
        ...
        node_out.mkdir(parents=True, exist_ok=True)
        output_name = cls._output_stem(inputs)
        return [node_out / f'{output_name}.markdup.bam', node_out / f'{output_name}.duplicate_stats.txt']
...
class SamtoolsFlagstatNode(CommandNode):
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        output = str(inputs.get('output', inputs.get('output_dir', '.')))
        cmd = ['samtools', 'flagstat', '-@', str(inputs.get('threads', 2))]
        if inputs.get('output_format'):
            cmd.extend(['-O', str(inputs['output_format'])])
        cmd.extend([str(inputs.get('bam', '')), '>', f'{output}/stats.stats.txt'])
        return cmd

    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'bam': ('BAM', {'description': 'Input BAM file'}), 'threads': ('INT', {'default': 2, 'min': 1, 'max': 64, 'display': 'slider'})}, 'optional': {'output_format': ('STRING', {'default': '', 'description': 'Output format (json, tsv)', 'advanced': True})}, 'hidden': {'output': ('STRING', {})}}
...
class SamtoolsViewNode(CommandNode):
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        output = str(inputs.get('output', inputs.get('output_dir', '.')))
        cmd = ['samtools', 'view', '-b', '-@', str(inputs.get('threads', 4)), '-o', f'{output}/bam.bam']
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'sam': ('SAM', {'description': 'Input SAM file'}), 'threads': ('INT', {'default': 4, 'min': 1, 'max': 64, 'display': 'slider'})}, 'optional': {'f': ('INT', {'default': None, 'description': 'Require ALL flags'}), 'F': ('INT', {'default': None, 'description': 'Filter out ALL flags'})}, 'hidden': {'output': ('STRING', {})}}
...
class SamtoolsMergeNode(CommandNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'bams': ('BAM', {'description': 'List of BAM files to merge'}), 'threads': ('INT', {'default': 4, 'min': 1, 'max': 64, 'display': 'slider'})}, 'optional': {}, 'hidden': {'output': ('STRING', {})}}
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        if isinstance(bams, str):
            bams = [bams]
        cmd = ['samtools', 'merge', '-@', str(inputs.get('threads', 4)), f"{inputs.get('output', '.')}/merged_bam.bam"] + list(bams)
        return cmd
...
class SamtoolsStatsNode(CommandNode):
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        output = str(inputs.get('output', inputs.get('output_dir', '.')))
        cmd = ['samtools', 'stats', '-@', str(inputs.get('threads', 2))]
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'bam': ('BAM', {'description': 'Input BAM file'}), 'threads': ('INT', {'default': 2, 'min': 1, 'max': 64, 'display': 'slider'})}, 'optional': {'target_regions': ('BED', {'description': 'Optional target regions BED'})}, 'hidden': {'output': ('STRING', {})}}
...
class SamtoolsIdxstatsNode(CommandNode):
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        output = str(inputs.get('output', inputs.get('output_dir', '.')))
        return ['samtools', 'idxstats', '-@', str(addthreads), str(inputs.get('input', inputs.get('bam', ''))), '>', f'{output}/idxstats.tsv']
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('BAM', {'description': 'Indexed BAM or CRAM alignment file'}), 'threads': ('INT', {'default': 1, 'min': 1, 'max': 64, 'display': 'slider'})}, 'optional': {}, 'hidden': {'output': ('STRING', {})}}
...
class SamtoolsDepthNode(CommandNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input_bams': ('BAM_LIST', {'description': 'One or more indexed BAM files'})}, 'optional': {'all': ('STRING', {'default': '', 'options': ['', '-a', '-aa'], 'description': 'Emit zero-depth positions'}), 'region': ('STRING', {'default': '', 'description': 'Manual region such as chr1:100-200'}), 'input_bed': ('BED', {'description': 'BED regions to restrict depth calculation'}), 'minlength': ('INT', {'default': '', 'min': 0, 'description': 'Ignore reads shorter than this length'}), 'maxdepth': ('INT', {'default': '', 'min': 0, 'description': 'Maximum read depth considered'}), 'basequality': ('INT', {'default': '', 'min': 0, 'description': 'Minimum base quality'}), 'mapquality': ('INT', {'default': '', 'min': 0, 'description': 'Minimum mapping quality'}), 'required_flags': ('STRING', {'default': '', 'description': 'Comma-separated SAM flags that must be set', 'advanced': True}), 'skipped_flags': ('STRING', {'default': '', 'description': 'Comma-separated SAM flags to exclude', 'advanced': True}), 'deletions': ('BOOLEAN', {'default': False, 'description': 'Include deletions in depth calculation'}), 'single_read': ('BOOLEAN', {'default': False, 'description': 'Count only one read in overlapping pairs'}), 'header': ('BOOLEAN', {'default': False, 'description': 'Print a file header'})}, 'hidden': {'output': ('STRING', {})}}
...
class SamtoolsFaidxNode(CommandNode):
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        if is_compressed:
            ...
            cmd.extend(['||', '(', 'echo', 'Failed to index compressed reference. Trying decompressed ...', '1>&2', '&&', 'gzip', '-dc', linked_input, '>', f'{output}/input.plain', '&&', 'samtools', 'faidx'])
            if is_fastq:
                cmd.append('--fastq')
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('FASTA', {'description': 'FASTA or FASTQ sequence file'})}, 'optional': {'fastq': ('BOOLEAN', {'default': False, 'description': 'Pass --fastq for FASTQ input'}), 'compressed': ('BOOLEAN', {'default': False, 'description': 'Treat input as gzip/BGZF compressed'})}, 'hidden': {'output': ('STRING', {})}}
...
class SamtoolsCoverageNode(CommandNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('BAM', {'description': 'Indexed BAM file'})}, 'optional': {'input_bams': ('BAM_LIST', {'description': 'Multiple BAM files to pool'}), 'min_read_length': ('INT', {'default': 0, 'min': 0}), 'min_mq': ('INT', {'default': 0, 'min': 0, 'description': 'Minimum mapping quality'}), 'min_bq': ('INT', {'default': 0, 'min': 0, 'description': 'Minimum base quality'}), 'required_flags': ('STRING', {'default': '', 'description': 'Comma-separated SAM flags that must be set', 'advanced': True}), 'skipped_flags': ('STRING', {'default': '', 'description': 'Comma-separated SAM flags to exclude', 'advanced': True}), 'region': ('STRING', {'default': '', 'description': 'Region such as chr1:100-200'}), 'histogram': ('BOOLEAN', {'default': False, 'description': 'Emit histogram data'}), 'n_bins': ('INT', {'default': 100, 'min': 1, 'description': 'Number of histogram bins'})}, 'hidden': {'output': ('STRING', {})}}
...
class SamtoolsBedcovNode(CommandNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input_bed': ('BED', {'description': 'BED intervals to summarize'}), 'input_bams': ('BAM_LIST', {'description': 'One or more indexed BAM files'})}, 'optional': {'mapq': ('INT', {'default': '', 'min': 0, 'description': 'Minimum mapping quality'}), 'countdel': ('BOOLEAN', {'default': False, 'description': 'Exclude deletions and reference skips from coverage totals'}), 'required_flags': ('STRING', {'default': '', 'description': 'Comma-separated SAM flags that must be set', 'advanced': True}), 'skipped_flags': ('STRING', {'default': '', 'description': 'Comma-separated SAM flags to exclude', 'advanced': True}), 'depth_thresh': ('INT', {'default': '', 'min': 0, 'description': 'Add a column counting bases with coverage at or above this threshold'})}, 'hidden': {'output': ('STRING', {})}}
...
class SamtoolsCalmdNode(CommandNode):
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        cmd.extend(['-b', '-@', str(_additional_threads(inputs)), str(inputs.get('input', inputs.get('bam', ''))), str(inputs.get('reference', '')), '>', f'{output}/calmd.bam'])
        return cmd
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('BAM', {'description': 'BAM file to recalculate'}), 'reference': ('FASTA', {'description': 'Reference FASTA used for the alignment'}), 'threads': ('INT', {'default': 1, 'min': 1, 'max': 64, 'display': 'slider'})}, 'optional': {'calculate_baq': ('BOOLEAN', {'default': False, 'description': 'Calculate BAQ scores'}), 'modify_quality': ('BOOLEAN', {'default': False, 'description': 'Use BAQ to cap read base qualities', 'advanced': True}), 'extended_baq': ('BOOLEAN', {'default': False, 'description': 'Use extended BAQ calculation', 'advanced': True}), 'change_identical': ('BOOLEAN', {'default': False, 'description': "Change reference-identical bases to '='", 'advanced': True}), 'adjust_mq': ('INT', {'default': 0, 'min': 0, 'max': 255, 'description': 'Coefficient for capping mapping quality of poorly mapped reads', 'advanced': True})}, 'hidden': {'output': ('STRING', {})}}
...
class SamtoolsAmpliconclipNode(CommandNode):
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        cmd = ['samtools', 'ampliconclip', '--hard-clip' if inputs.get('hard_clip') else '--soft-clip']
        ...
        cmd.extend(['-@', addthreads, str(inputs.get('input_bam', inputs.get('bam', ''))), '|', 'samtools', 'collate', '-@', addthreads, '-O', '-u', '-', '|', 'samtools', 'fixmate', '-@', addthreads, '-u', '-', '-', '|', 'samtools', 'sort', '-@', addthreads, '-m', _sort_memory(inputs), '-T', f'{output}/tmp', '-o', f'{output}/clipped.bam'])
        return cmd
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input_bed': ('BED', {'description': 'BED file defining primer or amplicon intervals'}), 'input_bam': ('BAM', {'description': 'BAM file to clip'}), 'threads': ('INT', {'default': 1, 'min': 1, 'max': 64, 'display': 'slider'})}, 'optional': {'hard_clip': ('BOOLEAN', {'default': False, 'description': 'Hard clip primer bases instead of soft clipping'}), 'strand': ('BOOLEAN', {'default': False, 'description': 'Only clip reads matching BED strand annotation'}), 'both_ends': ('BOOLEAN', {'default': False, 'description': "Clip both read ends instead of the 5' end only"}), 'no_excluded': ('BOOLEAN', {'default': False, 'description': 'Do not write excluded reads to output', 'advanced': True}), 'min_length': ('INT', {'default': '', 'min': 0, 'description': 'Mark reads QCFAIL at this length or shorter'}), 'tolerance': ('INT', {'default': 5, 'min': 0, 'description': 'Primer match tolerance in bases'}), 'write_primer_counts': ('BOOLEAN', {'default': False, 'description': 'Write per-primer clipping counts as bedGraph'}), 'memory_mb': ('INT', {'default': 768, 'min': 1, 'description': 'Memory per sort thread in MB', 'advanced': True})}, 'hidden': {'output': ('STRING', {})}}
...
class SamtoolsFastxNode(CommandNode):
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        if inputs.get('name_sorted'):
            ...
            cmd = ['samtools', 'sort', '-@', addthreads, '-m', _sort_memory(inputs), '-n', input_path, '-T', f'{output}/tmp', '>', input_alias, '&&']
    ...
    def PLAN_OUTPUTS(cls, inputs: dict[str, Any], output_dir: str | Path) -> list[Path]:
        ...
        return [node_out / f'reads.{extension}', node_out / f'read1.{extension}', node_out / f'read2.{extension}', node_out / f'singletons.{extension}', node_out / f'nonspecific.{extension}', node_out / f'index1.{extension}', node_out / f'index2.{extension}']
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('BAM', {'description': 'SAM, BAM, or CRAM alignment file'}), 'threads': ('INT', {'default': 1, 'min': 1, 'max': 64, 'display': 'slider'})}, 'optional': {'name_sorted': ('BOOLEAN', {'default': False, 'description': 'Input is already query-name sorted'}), 'output_format': ('STRING', {'default': 'fasta', 'options': ['fasta', 'fastq'], 'description': 'Extract FASTA or FASTQ'}), 'outputs': ('STRING', {'default': ['other'], 'options': ['other', 'read1', 'read2', 'singletons', 'nonspecific'], 'description': 'Read subsets to split into dedicated files'}), 'default_quality': ('INT', {'default': '', 'min': 0, 'description': 'Default FASTQ quality if none is present'}), 'output_quality': ('BOOLEAN', {'default': False, 'description': 'Use OQ tag quality values when available', 'advanced': True}), 'illumina_casava': ('BOOLEAN', {'default': False, 'description': 'Add Illumina Casava 1.8 header fields', 'advanced': True}), 'copy_tags': ('BOOLEAN', {'default': False, 'description': 'Copy RG, BC, and QT tags to sequence headers'}), 'copy_arbitrary_tags': ('STRING', {'default': '', 'description': 'Comma-separated tags to copy to FASTA headers', 'advanced': True}), 'read_numbering': ('STRING', {'default': '', 'options': ['', '-n', '-N'], 'description': 'Control /1 and /2 read-name suffixes', 'advanced': True}), 'required_flags': ('STRING', {'default': '', 'description': 'Comma-separated SAM flags that must be set', 'advanced': True}), 'skipped_flags': ('STRING', {'default': '', 'description': 'Comma-separated SAM flags to exclude', 'advanced': True}), 'skipped_flags_all': ('STRING', {'default': '', 'description': 'Comma-separated SAM flags that must not all be set', 'advanced': True}), 'write_index_reads': ('BOOLEAN', {'default': False, 'description': 'Write index reads from barcode tags', 'advanced': True}), 'write_i1': ('BOOLEAN', {'default': True, 'description': 'Write first index read output', 'advanced': True}), 'write_i2': ('BOOLEAN', {'default': True, 'description': 'Write second index read output', 'advanced': True}), 'index_format': ('STRING', {'default': '', 'description': 'Index-format string for parsing barcode tags', 'advanced': True}), 'barcode_tag': ('STRING', {'default': '', 'description': 'Barcode tag name, default BC in samtools', 'advanced': True}), 'quality_tag': ('STRING', {'default': '', 'description': 'Barcode quality tag name, default QT in samtools', 'advanced': True}), 'memory_mb': ('INT', {'default': 768, 'min': 1, 'description': 'Memory per sort thread in MB', 'advanced': True})}, 'hidden': {'output': ('STRING', {})}}
...
class SamtoolsMpileupNode(CommandNode):
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        output = str(inputs.get('output', inputs.get('output_dir', '.')))
        cmd = ['samtools', 'mpileup', '-f', str(inputs.get('reference', ''))]
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input_bams': ('BAM_LIST', {'description': 'One or more indexed BAM files'}), 'reference': ('FASTA', {'description': 'Reference FASTA'})}, 'optional': {'required_flags': ('STRING', {'default': '', 'description': 'Comma-separated SAM flags that must be set', 'advanced': True}), 'skipped_flags': ('STRING', {'default': '', 'description': 'Comma-separated SAM flags to exclude', 'advanced': True}), 'region': ('STRING', {'default': '', 'description': 'Region such as chr17:100-150'}), 'positions_bed': ('BED', {'description': 'BED or positions file restricting pileup positions'}), 'exclude_read_groups': ('FILE', {'description': 'Read-group exclusion list', 'advanced': True}), 'ignore_overlaps': ('BOOLEAN', {'default': False, 'description': 'Disable read-pair overlap detection'}), 'count_orphans': ('BOOLEAN', {'default': False, 'description': 'Do not discard anomalous read pairs'}), 'disable_baq': ('BOOLEAN', {'default': False, 'description': 'Disable BAQ computation'}), 'adjust_mq': ('INT', {'default': 0, 'min': 0, 'description': 'Coefficient for downgrading mapping qualities'}), 'max_depth': ('INT', {'default': 8000, 'min': 0, 'description': 'Maximum per-file depth'}), 'redo_baq': ('BOOLEAN', {'default': False, 'description': 'Recalculate BAQ on the fly'}), 'min_mq': ('INT', {'default': 0, 'min': 0, 'description': 'Minimum mapping quality'}), 'min_bq': ('INT', {'default': 13, 'min': 0, 'description': 'Minimum base quality'}), 'illumina13': ('BOOLEAN', {'default': False, 'description': 'Input quality is Illumina 1.3+ encoded', 'advanced': True}), 'output_bp': ('BOOLEAN', {'default': False, 'description': 'Output base positions on reads', 'advanced': True}), 'output_mq': ('BOOLEAN', {'default': False, 'description': 'Output mapping qualities', 'advanced': True}), 'output_qname': ('BOOLEAN', {'default': False, 'description': 'Output read names', 'advanced': True}), 'all_positions': ('STRING', {'default': '', 'options': ['', '-a', '-aa'], 'description': 'Emit zero-depth positions'}), 'output_extra': ('STRING', {'default': '', 'description': 'Comma-separated extra tags to output', 'advanced': True})}, 'hidden': {'output': ('STRING', {})}}
...
class SamtoolsReheaderNode(CommandNode):
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        output = str(inputs.get('output', inputs.get('output_dir', '.')))
        cmd = ['samtools', 'reheader', str(inputs.get('input_header', '')), str(inputs.get('input_file', inputs.get('bam', '')))]
        if inputs.get('no_pg'):
            cmd.append('--no-PG')
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input_header': ('BAM', {'description': 'SAM or BAM source header dataset'}), 'input_file': ('BAM', {'description': 'Target BAM file whose header will be replaced'})}, 'optional': {'no_pg': ('BOOLEAN', {'default': False, 'description': 'Keep the replacement header unmodified by omitting @PG edits'})}, 'hidden': {'output': ('STRING', {})}}
...
class SamtoolsSplitNode(CommandNode):
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        cmd = ['samtools', 'split', '-f', f'{output_dir}/Read_Group_%!.bam', '--output-fmt', 'bam']
        if inputs.get('header'):
            cmd.extend(['-h', str(inputs['header'])])
        cmd.extend(['-u', f'{output}/unaccounted.bam', '-@', str(_additional_threads(inputs)), str(inputs.get('input_bam', inputs.get('bam', '')))])
        return cmd
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input_bam': ('BAM', {'description': 'BAM file to split by read group'}), 'threads': ('INT', {'default': 1, 'min': 1, 'max': 64, 'display': 'slider'})}, 'optional': {'header': ('BAM', {'description': 'Optional SAM/BAM header replacement', 'advanced': True})}, 'hidden': {'output': ('STRING', {})}}
...
class SamtoolsSliceBamNode(CommandNode):
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        cmd = ['samtools', 'view', '-@', addthreads, '-b']
        ...
        cmd.extend(['&&', 'samtools', 'sort', '-O', 'bam', '-T', f'{output}/tmp', '-@', addthreads, '-m', _sort_memory(inputs), '-o', f'{output}/sliced.bam', unsorted_output])
        return cmd
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input_bam': ('BAM', {'description': 'Indexed BAM file to slice'}), 'slice_method': ('STRING', {'default': 'bed', 'options': ['bed', 'chromosomes', 'manual'], 'description': 'Region source'}), 'threads': ('INT', {'default': 1, 'min': 1, 'max': 64, 'display': 'slider'})}, 'optional': {'input_interval': ('BED', {'description': 'BED intervals for slice_method=bed'}), 'refs': ('STRING', {'default': '', 'description': 'Comma-separated contigs for slice_method=chromosomes'}), 'regions': ('STRING', {'default': '', 'description': 'Manual regions such as chrM:1-1000', 'advanced': True}), 'memory_mb': ('INT', {'default': 768, 'min': 1, 'description': 'Memory per sort thread in MB', 'advanced': True})}, 'hidden': {'output': ('STRING', {})}}
...
class SamtoolsPhaseNode(CommandNode):
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        output = str(inputs.get('output', inputs.get('output_dir', '.')))
        cmd = ['samtools', 'phase', '-b', f'{output}/phase_wrapper']
        if inputs.get('ignore_chimeras'):
            cmd.append('-F')
        cmd.extend(['-k', str(inputs.get('block_length', 13)), '-q', str(inputs.get('min_het', 37)), '-Q', str(inputs.get('min_bq', 13)), '-D', str(inputs.get('read_depth', 256))])
        if inputs.get('drop_ambiguous'):
            cmd.append('-A')
        cmd.extend([str(inputs.get('input_bam', inputs.get('bam', ''))), '>', f'{output}/phase_sets.txt'])
        return cmd
    ...
    def PLAN_OUTPUTS(cls, inputs: dict[str, Any], output_dir: str | Path) -> list[Path]:
        node_out = Path(output_dir) / cls.NODE_ID
        node_out.mkdir(parents=True, exist_ok=True)
        return [node_out / 'phase_sets.txt', node_out / 'phase_wrapper.0.bam', node_out / 'phase_wrapper.1.bam', node_out / 'phase_wrapper.chimera.bam']

    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input_bam': ('BAM', {'description': 'BAM file to phase'})}, 'optional': {'block_length': ('INT', {'default': 13, 'min': 1, 'description': 'Maximum length for local phasing'}), 'min_het': ('INT', {'default': 37, 'min': 0, 'description': 'Minimum heterozygote score'}), 'min_bq': ('INT', {'default': 13, 'min': 0, 'description': 'Minimum base quality'}), 'read_depth': ('INT', {'default': 256, 'min': 0, 'description': 'Maximum read depth'}), 'ignore_chimeras': ('BOOLEAN', {'default': False, 'description': 'Do not attempt to fix chimeric reads'}), 'drop_ambiguous': ('BOOLEAN', {'default': False, 'description': 'Drop reads with ambiguous phase'})}, 'hidden': {'output': ('STRING', {})}}
...
class SamtoolsConsensusNode(CommandNode):
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        output = str(inputs.get('output', inputs.get('output_dir', '.')))
        output_format = cls._output_format(inputs)
        cmd = ['samtools', 'consensus', str(inputs.get('input', inputs.get('bam', ''))), '-f', output_format, '-@', str(_additional_threads(inputs)), '--min-MQ', str(inputs.get('min_mq', 0)), '--min-BQ', str(inputs.get('min_bq', 0))]
        required_flags = _flag_sum(inputs.get('required_flags'))
        skipped_flags = _flag_sum(inputs.get('skipped_flags'))
        if required_flags:
            cmd.extend(['--rf', str(required_flags)])
        ...
        if inputs.get('output_all'):
            cmd.append('-a')
        cmd.extend(['--show-del', 'yes' if inputs.get('show_deletions') else 'no', '--show-ins', 'yes' if inputs.get('show_insertions', True) else 'no'])
        if inputs.get('ambig'):
            cmd.append('--ambig')
    ...
    def _append_simple_options(cls, cmd: list[str], inputs: dict[str, Any]) -> None:
        if inputs.get('use_qual'):
            cmd.append('-q')
        cmd.extend(['-c', str(inputs.get('consensus_fraction', 0.75)), '-H', str(inputs.get('heterozygous_fraction', 0.15))])
    ...
    def _append_bayesian_options(cls, cmd: list[str], inputs: dict[str, Any]) -> None:
        ...
        if inputs.get('use_mq', True):
            ...
            cmd.extend(['--NM-halo', str(inputs.get('nm_halo', 50)), '--low-MQ', str(inputs.get('low_mq', 1)), '--high-MQ', str(inputs.get('high_mq', 60)), '--scale-MQ', str(inputs.get('scale_mq', 1.0))])
        cmd.extend(['--P-het', str(inputs.get('p_het', 0.001)), '--P-indel', str(inputs.get('p_indel', 0.0002)), '--het-scale', str(inputs.get('het_scale', 1.0))])
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('BAM', {'description': 'SAM, BAM, or CRAM alignment file'}), 'threads': ('INT', {'default': 1, 'min': 1, 'max': 64, 'display': 'slider'})}, 'optional': {'format': ('STRING', {'default': 'fasta', 'options': ['fasta', 'fastq', 'pileup'], 'description': 'Consensus output format'}), 'min_mq': ('INT', {'default': 0, 'min': 0, 'description': 'Minimum mapping quality'}), 'min_bq': ('INT', {'default': 0, 'min': 0, 'description': 'Minimum base quality'}), 'required_flags': ('STRING', {'default': '', 'description': 'Comma-separated SAM flags that must be set', 'advanced': True}), 'skipped_flags': ('STRING', {'default': '', 'description': 'Comma-separated SAM flags to exclude', 'advanced': True}), 'mode': ('STRING', {'default': 'bayesian', 'options': ['simple', 'bayesian', 'bayesian_116'], 'description': 'Consensus algorithm'}), 'use_qual': ('BOOLEAN', {'default': False, 'description': 'Weight simple-mode base counts by base quality'}), 'consensus_fraction': ('FLOAT', {'default': 0.75, 'min': 0, 'max': 1, 'description': 'Simple-mode minimum consensus fraction'}), 'heterozygous_fraction': ('FLOAT', {'default': 0.15, 'min': 0, 'max': 1, 'description': 'Simple-mode heterozygous fraction'}), 'config': ('STRING', {'default': 'manual', 'options': ['manual', 'hiseq', 'hifi', 'r10.4_sup', 'r10.4_dup', 'ultima'], 'description': 'Bayesian configuration preset'}), 'cutoff': ('INT', {'default': 10, 'min': 0, 'max': 93, 'description': 'Bayesian quality cutoff threshold'}), 'use_mq': ('BOOLEAN', {'default': True, 'description': 'Use mapping qualities for Bayesian consensus', 'advanced': True}), 'adjust_mq': ('BOOLEAN', {'default': True, 'description': 'Adjust mapping quality using nearby mismatches', 'advanced': True}), 'nm_halo': ('INT', {'default': 50, 'min': 1, 'description': 'Local mismatch window for MQ adjustment', 'advanced': True}), 'low_mq': ('INT', {'default': 1, 'min': 0, 'max': 60, 'description': 'Minimum MQ cap'}), 'high_mq': ('INT', {'default': 60, 'min': 0, 'max': 60, 'description': 'Maximum MQ cap'}), 'scale_mq': ('FLOAT', {'default': 1.0, 'min': 0, 'description': 'Mapping-quality scale factor'}), 'p_het': ('FLOAT', {'default': 0.001, 'min': 0, 'max': 1, 'description': 'Prior probability of heterozygosity'}), 'p_indel': ('FLOAT', {'default': 0.0002, 'min': 0, 'max': 1, 'description': 'Prior probability of indels'}), 'het_scale': ('FLOAT', {'default': 1.0, 'min': 0, 'description': 'Heterozygous SNP probability multiplier'}), 'homopoly_fix': ('BOOLEAN', {'default': False, 'description': 'Apply homopolymer quality correction', 'advanced': True}), 'homopoly_score': ('FLOAT', {'default': '', 'min': 0, 'description': 'Homopolymer quality scaling', 'advanced': True}), 'qual_calibration': ('STRING', {'default': 'file', 'options': ['file', ':hiseq', ':hifi', ':r10.4_sup', ':r10.4_dup', ':ultima'], 'description': 'Quality calibration preset', 'advanced': True}), 'qual_calibration_file': ('FILE', {'description': 'Custom quality calibration table', 'advanced': True}), 'min_depth': ('INT', {'default': 1, 'min': 0, 'description': 'Minimum depth required to make a call'}), 'region': ('STRING', {'default': '', 'description': 'Region such as chr1:100-200'}), 'reference': ('FASTA', {'description': 'Optional reference FASTA'}), 'line_len': ('INT', {'default': 70, 'description': 'Maximum FASTA/FASTQ line length'}), 'output_all': ('BOOLEAN', {'default': False, 'description': 'Output all positions, including references with no aligned data'}), 'show_deletions': ('BOOLEAN', {'default': False, 'description': "Show deletions as '*' instead of omitting them"}), 'show_insertions': ('BOOLEAN', {'default': True, 'description': 'Show insertions in the consensus'}), 'ambig': ('BOOLEAN', {'default': False, 'description': 'Enable IUPAC ambiguity codes in the consensus output'}), 'mark_insertions': ('BOOLEAN', {'default': False, 'description': 'Mark insertions with underscores', 'advanced': True})}, 'hidden': {'output': ('STRING', {})}}
...
class SamtoolsBamToCramNode(CommandNode):
    ...
    SEARCH_ALIASES = [GALAXY_ALIAS, 'samtools', 'BAM to CRAM', 'SAM to CRAM', 'CRAM compression', 'reference based compression']
    RETURN_TYPES = ('CRAM',)
    RETURN_NAMES = ('cram',)
    REQUIRED_EXECUTABLES = ['samtools']
    DOCUMENTATION_URL = 'https://www.htslib.org/doc/samtools-view.html'
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        cmd.extend(['-@', str(_additional_threads(inputs)), '-C', '-h', '-o', f'{output}/output.cram', '-T', reference, '-t', reference_index, '--output-fmt-option', 'no_ref', str(inputs.get('input', ''))])
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': (('BAM', 'SAM'), {'description': 'BAM or SAM alignment file'}), 'reference': ('FASTA', {'description': 'Reference FASTA used for CRAM compression'}), 'threads': ('INT', {'default': 1, 'min': 1, 'max': 64, 'display': 'slider'})}, 'optional': {'reference_index': ('FASTA_INDEX', {'description': 'Optional FASTA .fai index; defaults to reference.fa.fai', 'advanced': True}), 'target_region': ('STRING', {'default': 'entire_input_file', 'options': ['entire_input_file', 'region', 'regions_bed_file'], 'description': 'Convert the entire input or restrict to specific genomic regions'}), 'region_string': ('STRING', {'default': '', 'description': 'Region such as chrX or chr:start-end'}), 'regions_bed_file': ('BED', {'description': 'Only include reads overlapping regions in this BED file'})}, 'hidden': {'output': ('STRING', {})}}


class SamtoolsCramToBamNode(CommandNode):
    ...
    SEARCH_ALIASES = [GALAXY_ALIAS, 'samtools', 'CRAM to BAM', 'CRAM decompression', 'alignment conversion', 'reference']
    RETURN_TYPES = ('BAM',)
    RETURN_NAMES = ('bam',)
    REQUIRED_EXECUTABLES = ['samtools']
    DOCUMENTATION_URL = 'https://www.htslib.org/doc/samtools-view.html'
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        if inputs.get('target_region') == 'regions_bed_file':
            _add_if_value(cmd, '-L', inputs.get('regions_bed_file'))
        cmd.extend(['-@', str(_additional_threads(inputs)), '-b', '-T', str(inputs.get('reference', '')), '-o', f'{output}/output.bam', str(inputs.get('input', ''))])
        if inputs.get('target_region') == 'region':
            cmd.append(str(inputs.get('region_string', '')))
        return cmd
    ...
    def VALIDATE_INPUTS(cls, inputs: dict[str, Any]) -> bool | str:
        ...
        if base_validation is not True:
            return base_validation
        if not str(inputs.get('reference', '') or '').strip():
            return 'reference is required for CRAM to BAM conversion'
        target_region = str(inputs.get('target_region', 'entire_input_file'))
        if target_region == 'region' and (not str(inputs.get('region_string', '') or '').strip()):
            return 'region_string is required when target_region is region'
        if target_region == 'regions_bed_file' and (not str(inputs.get('regions_bed_file', '') or '').strip()):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('CRAM', {'description': 'CRAM alignment file'}), 'reference': ('FASTA', {'description': 'Reference FASTA used to decode the CRAM file'}), 'threads': ('INT', {'default': 1, 'min': 1, 'max': 64, 'display': 'slider'})}, 'optional': {'target_region': ('STRING', {'default': 'entire_input_file', 'options': ['entire_input_file', 'region', 'regions_bed_file'], 'description': 'Convert the entire input or restrict to specific genomic regions'}), 'region_string': ('STRING', {'default': '', 'description': 'Region such as chrX or chr:start-end'}), 'regions_bed_file': ('BED', {'description': 'Only include reads overlapping regions in this BED file'})}, 'hidden': {'output': ('STRING', {})}}


class SamtoolsBamToSamNode(CommandNode):
    ...
    SEARCH_ALIASES = [GALAXY_ALIAS, 'samtools', 'BAM to SAM', 'SAM output', 'header only', 'include header']
    RETURN_TYPES = ('SAM',)
    RETURN_NAMES = ('sam',)
    REQUIRED_EXECUTABLES = ['samtools']
    DOCUMENTATION_URL = 'https://www.htslib.org/doc/samtools-view.html'
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        cmd = ['samtools', 'view', '-o', f'{output}/output.sam']
        if header:
            cmd.append(header)
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('BAM', {'description': 'BAM alignment file to convert'})}, 'optional': {'header': ('STRING', {'default': '-h', 'options': ['-h', '-H', ''], 'description': 'Include the SAM header, return the header only, or exclude the header'})}, 'hidden': {'output': ('STRING', {})}}
...
class SamtoolsSamToBamNode(CommandNode):
    ...
    SEARCH_ALIASES = [GALAXY_ALIAS, 'samtools', 'SAM to BAM', 'sorted BAM', 'reference index', 'alignment conversion']
    RETURN_TYPES = ('BAM',)
    RETURN_NAMES = ('bam',)
    REQUIRED_EXECUTABLES = ['samtools']
    DOCUMENTATION_URL = 'https://www.htslib.org/doc/samtools-view.html'
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        reference_index = str(inputs.get('reference_index') or f'{reference}.fai')
        addthreads = str(_additional_threads(inputs))
        return ['samtools', 'view', '-b', '-@', addthreads, '-t', reference_index, str(inputs.get('input', '')), '|', 'samtools', 'sort', '-O', 'bam', '-@', addthreads, '-m', _sort_memory(inputs), '-o', f'{output}/output.bam', '-T', f'{output}/tmp']
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('SAM', {'description': 'SAM alignment file to convert'}), 'reference': ('FASTA', {'description': 'Reference FASTA for resolving SAM target names'}), 'threads': ('INT', {'default': 1, 'min': 1, 'max': 64, 'display': 'slider'})}, 'optional': {'reference_index': ('FASTA_INDEX', {'description': 'Optional FASTA .fai index; defaults to reference.fa.fai', 'advanced': True}), 'memory_mb': ('INT', {'default': 768, 'min': 1, 'description': 'Memory per sort thread in MB', 'advanced': True})}, 'hidden': {'output': ('STRING', {})}}
