...
class SamblasterNode(CommandNode):
    ...
    def render_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        cmd = [
            ...
            threads,
            "-h",
            f"<(sambamba sort -t {threads} -n {stream} -o /dev/stdout)",
            "|",
            "samblaster",
        ]
    ...
    def _sort_sam_tokens(out: str, stem: str, threads: str) -> list[str]:
        return [
            ...
            "-t",
            threads,
            f"<(sambamba view -S -f bam {out}/{stem}.sam)",
        ]
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {
            "required": {
                "input": ("BAM", {"description": "Input SAM or BAM alignment file"}),
            },
            "optional": {
                "output_bam": ("BOOLEAN", {"default": True, "description": "Output BAM file for all input alignments"}),
                "discordantFile": ("BOOLEAN", {"default": False, "description": "Output discordant read pairs"}),
                "splitterFile": ("BOOLEAN", {"default": False, "description": "Output split reads"}),
                "unmappedFile": ("BOOLEAN", {"default": False, "description": "Output unmapped or clipped reads as FASTQ"}),
                "acceptDupMarks": ("BOOLEAN", {"default": False, "description": "Accept duplicate marks already in the input"}),
                "excludeDups": ("BOOLEAN", {"default": False, "description": "Exclude duplicate reads from optional side outputs"}),
                "removeDups": ("BOOLEAN", {"default": False, "description": "Remove duplicate reads from all outputs"}),
                "addMateTags": ("BOOLEAN", {"default": False, "description": "Add MC and MQ tags to paired-end SAM records"}),
                "compatibility_mode": ("BOOLEAN", {"default": False, "description": "Treat 0x100 and 0x800 as chimeric alignments"}),
                "maxSplitCount": ("INT", {"default": 2, "min": 2, "description": "Maximum split alignments for splitter output"}),
                "maxUnmappedBases": ("INT", {"default": 50, "min": 1, "description": "Maximum unaligned bases between split alignments"}),
                "minIndelSize": ("INT", {"default": 50, "min": 1, "description": "Minimum structural variant size for split alignments"}),
                "minNonOverlap": ("INT", {"default": 20, "min": 1, "description": "Minimum non-overlapping bases between split alignments"}),
                "minClipSize": ("INT", {"default": 20, "min": 1, "description": "Minimum clipped bases for unmapped/clipped output"}),
                "threads": ("INT", {"default": 4, "min": 1, "max": 64, "description": "Sambamba sort/view threads"}),
                "input_format": ("STRING", {"default": "bam", "options": ["bam", "sam"], "advanced": True, "description": "Input format hint"}),
            },
