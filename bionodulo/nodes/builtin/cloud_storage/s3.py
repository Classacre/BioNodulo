"""s3 — cloud_storage node(s). One tool per file (extracted from cloud_storage.py)."""
...
S3_STORAGE_CLASSES = ('STANDARD', 'REDUCED_REDUNDANCY', 'STANDARD_IA', 'ONEZONE_IA', 'INTELLIGENT_TIERING', 'GLACIER', 'DEEP_ARCHIVE', 'GLACIER_IR')
...
class _S3BaseNode(BaseNode):
    ...
    def _common_input_types(cls) -> dict[str, Any]:
        return {'profile': ('STRING', {'default': '', 'description': 'Optional AWS CLI profile name'}), 'region': ('STRING', {'default': '', 'description': 'Optional AWS region override'}), 'extra_args': ('STRING', {'default': '', 'advanced': True, 'description': 'Additional aws s3 cp arguments, parsed like a shell command line'})}
...
class S3UploadNode(_S3BaseNode):
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'local_path': ('FILE', {'default': '', 'description': 'Local file to upload'}), 'bucket': ('STRING', {'default': '', 'description': 'S3 bucket name'}), 'key': ('STRING', {'default': '', 'description': 'Destination object key'})}, 'optional': {'storage_class': (list(S3_STORAGE_CLASSES), {'default': 'STANDARD'}), **cls._common_input_types()}, 'hidden': {}}
    ...
    async def run(self, **kwargs: Any) -> dict[str, Any]:
        ...
        metadata = {'operation': 'upload', 'bucket': str(kwargs.get('bucket', '') or '').strip(), 'key': str(kwargs.get('key', '') or '').strip().lstrip('/'), 's3_uri': s3_uri, 'local_path': str(kwargs.get('local_path', '')), 'command': command, 'returncode': result.get('returncode', 0), 'stdout': result.get('stdout', ''), 'stderr': result.get('stderr', '')}
        metadata_path = out_dir / 'upload_metadata.json'
        metadata_path.write_text(json.dumps(metadata, indent=2), encoding='utf-8')
        return {'outputs': {'s3_uri': s3_uri, 'metadata': str(metadata_path)}}
...
class S3DownloadNode(_S3BaseNode):
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'bucket': ('STRING', {'default': '', 'description': 'S3 bucket name'}), 'key': ('STRING', {'default': '', 'description': 'Source object key'}), 'local_path': ('FILE', {'default': '', 'description': 'Local destination path'})}, 'optional': cls._common_input_types(), 'hidden': {}}
    ...
    async def run(self, **kwargs: Any) -> dict[str, Any]:
        ...
        s3_uri = _s3_uri(kwargs.get('bucket', ''), kwargs.get('key', ''))
        metadata = {'operation': 'download', 'bucket': str(kwargs.get('bucket', '') or '').strip(), 'key': str(kwargs.get('key', '') or '').strip().lstrip('/'), 's3_uri': s3_uri, 'local_path': str(local_path), 'command': command, 'returncode': result.get('returncode', 0), 'stdout': result.get('stdout', ''), 'stderr': result.get('stderr', '')}
