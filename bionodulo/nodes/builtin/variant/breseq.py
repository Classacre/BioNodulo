"""breseq — variant node(s). One tool per file (extracted from wrapped_variant_assembly.py)."""
...
class BreseqNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'breseq', 'breseq mutation detection', 'GenomeDiff', 'gdtools ANNOTATE', 'microbial resequencing', 'haploid microbial genomes', 'laboratory evolution']
    ...
    VERSION = '0.35.5'
    SHELL = True
    DETECT_OUTPUTS = {'html': 'report.html', 'gd': 'output.gd', 'zip': 'results.tar.gz', 'log': 'log.txt'}
    ANNOTATE_OUTPUTS = {'html': 'annotated_report.html', 'gd': 'annotated.gd', 'tsv': 'annotated.tsv', 'phylip': 'comparison.phy', 'json': 'annotated.json'}
    ...
    def _detect_command(cls, inputs: dict[str, Any], out: str) -> list[str]:
        ...
        if 'html' in formats:
            cmd.extend(['&&', 'cp', f'{results_dir}/output/index.html', f'{out}/report.html', '&&', 'mkdir', '-p', f'{out}/report_extra_files', '&&', 'cp', '-R', f'{results_dir}/output/.', f'{out}/report_extra_files'])
        if 'zip' in formats:
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'references': ('FILE_LIST', {'multiple': True, 'description': 'One or more FASTA or GenBank reference genomes'})}, 'optional': {'mode': ('STRING', {'default': 'detect', 'options': ['detect', 'annotate', 'compare'], 'description': 'Run breseq variant detection or gdtools annotation/comparison'}), 'fastqs': ('FASTQ_LIST', {'description': 'FASTQ reads for detect mode'}), 'gds': ('TSV_LIST', {'description': 'GenomeDiff files for annotate or compare mode'}), 'formats': ('STRING_LIST', {'description': 'Output formats selected in the Galaxy wrapper'}), 'polymorphism_prediction': ('BOOLEAN', {'default': False, 'description': 'Detect polymorphic variants rather than consensus mutations'}), 'name': ('STRING', {'default': '', 'description': 'Human-readable analysis name'}), 'predict_junctions': ('BOOLEAN', {'default': True, 'description': 'Predict new sequence junctions'}), 'threads': ('INT', {'default': 4, 'min': 1, 'max': 128, 'display': 'slider'})}, 'hidden': {'output': ('STRING', {})}}
