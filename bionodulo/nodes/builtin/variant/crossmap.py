"""crossmap — variant node(s). One tool per file (extracted from wrapped_alignment_taxonomy.py)."""
...
class CrossMapVcfNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'CrossMap', 'crossmap_vcf', 'liftover VCF', 'variant coordinate conversion', 'reference allele liftover', 'VCF assembly conversion', 'chain file']
    ...
    def render_command(cls, inputs: dict[str, Any]) -> str:
        ...
        crossmap_cmd = ['CrossMap', 'vcf', str(inputs.get('input_chain', '')), str(inputs.get('input', '')), genome_fasta]
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input': ('VCF', {'description': 'VCF variants to lift over'}), 'input_fasta': ('FASTA', {'description': 'Target assembly genome FASTA'}), 'input_chain': ('TXT', {'description': 'LiftOver chain file'})}, 'optional': {'index_source_s': ('STRING', {'default': 'history', 'options': cls.SOURCE_OPTIONS, 'description': 'Galaxy source selector for input VCF and target FASTA'}), 'index_source': ('STRING', {'default': 'history', 'options': cls.INDEX_SOURCE_OPTIONS, 'description': 'Galaxy source selector for cached or history chain files'}), 'no_comp_alleles': ('BOOLEAN', {'default': False, 'description': 'Pass --no-comp-alleles to skip reference/alternate allele comparison'})}, 'hidden': {'output': ('STRING', {})}}
