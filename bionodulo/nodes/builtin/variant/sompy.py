"""sompy — variant node(s). One tool per file (extracted from wrapped_alignment_taxonomy.py)."""
...
class HappySompyNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'hap.py', 'som.py', 'happy', 'Haplotype Comparison Tools', 'variant benchmarking', 'VCF comparison', 'truth query comparison']
    ...
    def render_command(cls, inputs: dict[str, Any]) -> str:
        ...
        setup: list[str] = []
        if cls._reference_source(inputs) == 'history':
            setup.extend([_shell_join(['ln', '-sf', str(inputs.get('history_item', '')), reference_path]), _shell_join(['samtools', 'faidx', reference_path])])
        cmd = [cls._program(inputs), str(inputs.get('truth', '')), str(inputs.get('query', '')), '-r', reference_path, '-o', cls._out_prefix(inputs)]
        sed_whitespace_to_tab = shlex.quote('s/\\s\\+/\\t/g')
        results_path = shlex.quote(f'{out}/results.tsv')
        compare_cmd = f'export HGREF={shlex.quote(reference_path)} && {_shell_join(cmd)} | sed {sed_whitespace_to_tab} | tail -n+2 > {results_path}'
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'truth': ('VCF', {'description': 'Ground-truth variant calls'}), 'query': ('VCF', {'description': 'Query variant calls to benchmark'})}, 'optional': {'program_select': ('STRING', {'default': 'som.py', 'options': cls.PROGRAM_OPTIONS, 'description': 'Comparison method: som.py allele matching or hap.py haplotype benchmarking'}), 'reference_source': ('STRING', {'default': 'indexed', 'options': cls.REFERENCE_SOURCE_OPTIONS, 'description': 'Use an indexed reference path or stage a history FASTA'}), 'reference_path': ('FASTA', {'default': '', 'description': 'Indexed reference FASTA path for built-in/reference data mode'}), 'history_item': ('FASTA', {'default': '', 'description': 'History reference FASTA to stage and index before comparison'})}, 'hidden': {'output': ('STRING', {})}}
