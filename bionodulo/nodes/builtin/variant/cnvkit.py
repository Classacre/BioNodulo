"""cnvkit — variant node(s). One tool per file (extracted from cnvkit.py)."""
...
CNVKIT_CITATION_TEXT = 'CNVkit: Genome-Wide Copy Number Detection and Visualization from Targeted DNA Sequencing.'
def _out(inputs: dict[str, Any]) -> str:
    return str(inputs.get('output', inputs.get('output_dir', '.')))
def _safe_label(value: str) -> str:
    return sub('[^\\w\\-.]', '_', value)
...
class CNVkitAccessNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'CNVkit', 'CNVkit Access', 'cnvkit.py access', 'sequence-accessible coordinates', 'copy number variation', 'accessible genome regions', 'masked N regions']
    RETURN_TYPES = ('BED',)
    ...
    def _exclude_items(cls, inputs: dict[str, Any]) -> list[dict[str, str]]:
        ...
        for index, value in enumerate(values, start=1):
            if isinstance(value, dict):
                path = next((str(value[key]) for key in ('path', 'file', 'input', 'location') if value.get(key) is not None and str(value[key]).strip()), '')
                label = next((str(value[key]) for key in ('element_identifier', 'name', 'identifier', 'id') if value.get(key) is not None and str(value[key]).strip()), Path(path).stem or f'exclude_{index}')
            else:
                path = str(value)
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'fa_fname': ('FASTA', {'description': 'Reference genome FASTA file'})}, 'optional': {'min_gap_size': ('INT', {'default': 5000, 'min': 0, 'description': 'Minimum gap size between accessible regions; smaller gaps are joined'}), 'exclude': ('BED', {'default': [], 'multiple': True, 'description': 'Additional BED regions to exclude from accessible coordinates'})}, 'hidden': {'output': ('STRING', {})}}
...
class CNVkitAntitargetNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'CNVkit', 'CNVkit Antitarget', 'cnvkit.py antitarget', 'antitarget regions', 'off-target bins', 'targeted resequencing', 'copy number variation']
    RETURN_TYPES = ('BED',)
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'targets_file': ('BED', {'description': 'Input BED or interval file with target regions'})}, 'optional': {'access': ('BED', {'description': 'Regions of accessible sequence on chromosomes'}), 'avg_size': ('INT', {'default': 150000, 'min': 1, 'description': 'Average size of antitarget bins'}), 'min_size': ('INT', {'default': 25000, 'min': 1, 'description': 'Minimum size of antitarget bins'})}, 'hidden': {'output': ('STRING', {})}}
...
class CNVkitTargetNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'CNVkit', 'CNVkit Target', 'cnvkit.py target', 'baited regions', 'capture targets', 'target BED', 'split target bins', 'copy number variation']
    RETURN_TYPES = ('BED',)
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input_file': ('BED', {'description': 'Capture or target BED file'})}, 'optional': {'annotate': ('FILE', {'description': 'Gene model file used to assign names to target regions'}), 'short_names': ('BOOLEAN', {'default': False, 'description': 'Reduce multi-accession bait labels to short names'}), 'split': ('BOOLEAN', {'default': False, 'description': 'Split large tiled intervals into smaller targets'}), 'avg_size': ('INT', {'default': 266, 'min': 1, 'description': 'Average size of split target bins'})}, 'hidden': {'output': ('STRING', {})}}
