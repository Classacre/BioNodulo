"""calculate — visualization node(s). One tool per file (extracted from wrapped_alignment_taxonomy.py)."""
...
class CalculateContrastThresholdNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'calculate_contrast_threshold', 'Calculate Contrast threshold', 'tag pileup CDT', 'heatmap contrast', 'contrast threshold', 'calcThreshold.txt', 'ChIP-QC']
    RETURN_TYPES = ('TXT',)
    ...
    @classmethod
    def _numeric_at_least(cls, inputs: dict[str, Any], name: str, default: int | float, minimum: int | float, *, integer: bool) -> bool | str:
    ...
    def render_command(cls, inputs: dict[str, Any]) -> str:
        out = _out(inputs)
        cmd = ['python', str(inputs.get('script_path', 'calculate_contrast_threshold.py') or 'calculate_contrast_threshold.py'), '-i', str(inputs.get('input_file', ''))]
        ...
        cmd.extend(['-d', cls._header_value(inputs), '-s', str(inputs.get('start_col', 2)), '-r', str(inputs.get('row_num', 600)), '-l', str(inputs.get('col_num', 300))])
        return f"{_shell_join(['mkdir', '-p', out])} && cd {shlex.quote(out)} && {_shell_join(cmd)} && {_shell_join(['cp', 'calcThreshold.txt', cls._output_path(inputs)])}"
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'input_file': ('TXT', {'description': 'Tag pileup CDT data matrix'})}, 'optional': {'header': ('BOOLEAN', {'default': True, 'description': 'Whether the input file has a header row'}), 'start_col': ('INT', {'default': 2, 'min': 1, 'description': '1-based valid data start column'}), 'col_num': ('INT', {'default': 300, 'min': 1, 'description': 'Heatmap plot width in pixels'}), 'row_num': ('INT', {'default': 600, 'min': 1, 'description': 'Heatmap plot height in pixels'}), 'quantile_type_selector': ('STRING', {'default': 'b_option', 'options': cls.QUANTILE_TYPE_OPTIONS, 'description': 'Calculate thresholds from data or enforce an absolute threshold'}), 'quantile': ('FLOAT', {'default': 95.0, 'min': 0, 'max': 100, 'description': 'Percentile threshold'}), 'min_contrast': ('FLOAT', {'default': 0.0, 'min': 0, 'description': 'Minimum upper limit after quantile calculation'}), 'quantile2': ('FLOAT', {'default': 0.0, 'min': 0, 'description': 'Absolute tag threshold for t_option mode'}), 'script_path': ('FILE', {'default': 'calculate_contrast_threshold.py', 'advanced': True, 'description': 'Path to the Galaxy calculate_contrast_threshold.py helper script'})}, 'hidden': {'output': ('STRING', {})}}
