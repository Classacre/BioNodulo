"""Shared contracts for focused workflow robustness and observability nodes."""
...
PSUTIL_SOURCE_COMMIT = "9eea97dd6f1d16ea33f5144c8925f1ce7a0688e1"


class WorkflowEnhancementContract(BaseNode):
    """Source identity shared by BioNodulo-native workflow control nodes."""
    ...
    DOCUMENTATION_URL = (
        ...
        f"{BIONODULO_SOURCE_COMMIT}/bionodulo/nodes/builtin/workflow_enhancement.py"
    )
    SOURCE_URL = DOCUMENTATION_URL
    UPSTREAM_SOURCE = "bionodulo/nodes/builtin/workflow_enhancement.py"
    ...
    EXIT_SEMANTICS = "Invalid inputs and failed operations raise; successful runs return declared outputs."
...
class TimerNode(WorkflowEnhancementContract):
    """Record a workflow timestamp and pass data through unchanged."""

    LEGACY_NODE_ID = "timer"
    DISPLAY_NAME = "Timer"
    CATEGORY = "workflow"
...
class ResourceMonitorNode(WorkflowEnhancementContract):
    """Check local CPU, memory, and disk availability before continuing."""

    LEGACY_NODE_ID = "resource_monitor"
    ...
    REQUIRES_EXTERNAL_TOOLS = False
    REQUIRED_CONDA_PACKAGES = ["psutil"]
    CONDA_PACKAGE_CONSTRAINTS = {"psutil": PSUTIL_VERSION}
    DEPENDENCY_SOURCE = {
        "name": "psutil",
        "version": PSUTIL_VERSION,
        "git_commit": PSUTIL_SOURCE_COMMIT,
        "source_url": f"https://github.com/giampaolo/psutil/tree/release-{PSUTIL_VERSION}",
    }
    ...
    async def run(self, **kwargs: Any) -> tuple[Any, bool, str]:
        ...
        while not resources_ok and check_interval > 0 and max_wait > 0 and (time.monotonic() - started) < max_wait:
    ...
    def _get_resource_stats(self) -> dict[str, Any]:
        try:
            ...
            raise RuntimeError(
                f"Resource Monitor requires psutil {PSUTIL_VERSION}; resource availability cannot be inferred"
            ) from exc
...
class DataValidatorNode(WorkflowEnhancementContract):
    """Validate file/data quality before downstream workflow steps."""

    LEGACY_NODE_ID = "data_validator"
...
class ProvenanceNode(WorkflowEnhancementContract):
    """Capture reproducibility metadata and pass data through unchanged."""

    LEGACY_NODE_ID = "provenance"
            ...
            "optional": {
                ...
                "timestamp": (
                    "STRING",
                    {
                        "default": "",
                        "advanced": True,
                        "description": "Optional fixed ISO-8601 timestamp for deterministic provenance",
                    },
                ),
            },
            "hidden": {
    ...
    async def run(self, **kwargs: Any) -> tuple[Any, str, str]:
        ...
        timestamp = kwargs.get("timestamp", "") or getattr(context, "provenance_timestamp", "")
        timestamp_iso, timestamp_unix = self._timestamp_values(timestamp)
        system_info = (
            self._system_info(timestamp_iso, timestamp_unix)
            if bool(kwargs.get("include_system_info", True))
            else {}
        )
        ...
        merged.update(
            {
                ...
                "_timestamp": timestamp_iso,
                "_timestamp_unix": timestamp_unix,
            }
        )
        return merged
    ...
    @staticmethod
    def _timestamp_values(value: Any) -> tuple[str, float]:
        text = str(value or "").strip()
        if not text:
            timestamp_unix = time.time()
            return time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime(timestamp_unix)), timestamp_unix
        try:
            parsed = datetime.fromisoformat(text.replace("Z", "+00:00"))
        except ValueError as exc:
            raise ValueError("Input 'timestamp' must be a valid ISO-8601 timestamp") from exc
    ...
    @staticmethod
    def _system_info(timestamp_iso: str, timestamp_unix: float) -> dict[str, Any]:
        return {
...
class CompareResultsNode(WorkflowEnhancementContract):
    """Compare outputs from two workflow branches."""

    LEGACY_NODE_ID = "compare_results"
...
class CheckpointNode(WorkflowEnhancementContract):
    """Persist a workflow value to a checkpoint artifact."""

    LEGACY_NODE_ID = "checkpoint"
...
class MemoizeNode(WorkflowEnhancementContract):
    """Hash inputs and persist a reusable memoization marker."""

    LEGACY_NODE_ID = "memoize"
...
class CacheControlNode(WorkflowEnhancementContract):
    """Store and retrieve a value through an explicit cache marker."""

    LEGACY_NODE_ID = "cache_control"
...
class NotificationNode(WorkflowEnhancementContract):
    """Send or record workflow notifications when this node is reached."""

    LEGACY_NODE_ID = "notification"
...
class RetryNode(WorkflowEnhancementContract):
    """Record a retry policy and pass data through unchanged."""

    LEGACY_NODE_ID = "retry"
    DISPLAY_NAME = "Retry"
    CATEGORY = "workflow"
...
class BatchSubmitterNode(WorkflowEnhancementContract):
    """Create or queue one workflow run for each parameter set."""

    LEGACY_NODE_ID = "batch_submitter"
...
class WorkflowTriggerNode(WorkflowEnhancementContract):
    """Trigger a webhook immediately or record a deferred trigger intent."""

    LEGACY_NODE_ID = "workflow_trigger"
...
class PauseResumeNode(WorkflowEnhancementContract):
    """Record a human review gate and optionally block until a decision."""

    LEGACY_NODE_ID = "pause_resume"
...
class SubWorkflowNode(WorkflowEnhancementContract):
    """Prepare or execute another workflow as a nested routine."""

    LEGACY_NODE_ID = "sub_workflow"
