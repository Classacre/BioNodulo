"""phylot node(s) — phylogeny category (extracted, one tool per file)."""
...
from bionodulo.nodes.builtin.phylogeny._shared import MAX_RETRIES, REQUEST_TIMEOUT_S, RETRY_DELAY_S, _html_to_text, _phylogeny_node_output_dir, _safe_filename, _split_text_list
...
def _validate_phylot_tree_response(text: str) -> None:
    ...
    if re.search('(?is)<\\s*(?:!doctype\\s+html|html|body|head|title|h[1-6])\\b', stripped):
        summary = _html_to_text(stripped)[:500] or 'HTML error response'
        raise RuntimeError(f'PhyloT returned an error page: {summary}')

async def _phylot_request_text(endpoint: str, data: dict[str, str], *, retries: int=MAX_RETRIES, timeout: float=REQUEST_TIMEOUT_S) -> str:
    response = await _phylot_request(endpoint, data=data, retries=retries, timeout=timeout)
    return response.text

async def _phylot_request(endpoint: str, *, data: dict[str, str], retries: int, timeout: float) -> httpx.Response:
    endpoint = endpoint.lstrip('/')
    url = f'{PHYLOT_BASE_URL}/{endpoint}'
    client = APIHttpClient(cache=PHYLOT_API_CACHE, rate_limiter=PHYLOT_RATE_LIMITER)
    try:
        return await client.request('POST', url, data=data, headers={'User-Agent': PHYLOT_USER_AGENT}, timeout=timeout, retries=retries, retry_delay=RETRY_DELAY_S, cache_ttl=None, follow_redirects=True)
...
class PhyloTNode(BaseNode):
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'taxa': ('STRING', {'default': '', 'multiline': True, 'description': 'Taxon names, taxonomy IDs, accessions, or subtree requests separated by newlines or commas'})}, 'optional': {'taxonomy_source': ('STRING', {'default': 'ncbi', 'options': ['ncbi', 'gtdb']}), 'output_format': ('STRING', {'default': 'newick', 'options': list(PHYLOT_OUTPUT_FORMATS)}), 'node_identifiers': ('STRING', {'default': 'name', 'options': list(PHYLOT_NCBI_NODE_IDENTIFIERS)}), 'collapse_internal_nodes': ('BOOLEAN', {'default': False}), 'force_binary_tree': ('BOOLEAN', {'default': False}), 'interrupt_at': ('STRING', {'default': '0', 'options': list(PHYLOT_INTERRUPT_LEVELS)}), 'filter_terms': ('STRING', {'default': '', 'advanced': True}), 'ignore_errors': ('BOOLEAN', {'default': False, 'advanced': True}), 'gtdb_source': ('STRING', {'default': 'bac', 'options': list(PHYLOT_GTDB_SOURCES), 'advanced': True}), 'include_gtdb_branch_support': ('BOOLEAN', {'default': True, 'advanced': True}), 'include_gtdb_genome_ids': ('BOOLEAN', {'default': False, 'advanced': True}), 'gtdb_version': ('STRING', {'default': '232', 'options': list(PHYLOT_GTDB_VERSIONS), 'advanced': True}), 'output_name': ('STRING', {'default': '', 'description': 'Optional output filename stem'})}, 'hidden': {}}
    ...
    async def run(self, **kwargs: Any) -> tuple[str, str]:
        ...
        endpoint, params = self._request_params(taxa=taxa, taxonomy_source=taxonomy_source, output_format=output_format, output_name=output_name, interrupt_at=interrupt_at, kwargs=kwargs)
        ...
        metadata = {'endpoint': endpoint, 'format': output_format, 'taxonomy_source': taxonomy_source, 'taxa_count': len(taxa), 'tree': str(tree_path), 'params': params}
    ...
    def PLAN_OUTPUTS(cls, inputs: dict[str, Any], output_dir: str | Path) -> list[Path]:
        ...
        return [node_out / f'{output_name}{PHYLOT_FORMAT_EXTENSIONS[output_format]}', node_out / 'request_metadata.json']

    def _request_params(self, *, taxa: list[str], taxonomy_source: str, output_format: str, output_name: str, interrupt_at: str, kwargs: dict[str, Any]) -> tuple[str, dict[str, str]]:
        common = {'itol': '0', 'itolProject': '0', 'treeElements': '\n'.join(taxa), 'filter': str(kwargs.get('filter_terms', '') or ''), 'interrupt': interrupt_at, 'format': output_format, 'fileName': output_name, 'noerror': '1' if bool(kwargs.get('ignore_errors', False)) else '0'}
        if taxonomy_source == 'gtdb':
            ...
            params = {'phylotgtd': '1', **common, 'src': gtdb_source, 'boot': '1' if bool(kwargs.get('include_gtdb_branch_support', True)) else '0', 'gid': '1' if bool(kwargs.get('include_gtdb_genome_ids', False)) else '0', 'gtdb_version': gtdb_version}
        ...
        params = {'phylot': '1', **common, 'ids': node_identifiers, 'collapse': '1' if bool(kwargs.get('collapse_internal_nodes', False)) else '0', 'binary': '1' if bool(kwargs.get('force_binary_tree', False)) else '0'}
