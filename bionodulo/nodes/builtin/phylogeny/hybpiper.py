"""hybpiper — phylogeny node(s). One tool per file (extracted from wrapped_taxonomy_humann.py)."""
...
class HybPiperNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'HybPiper', 'targeted sequence capture', 'target loci assembly', 'check targetfile', 'fix targetfile', 'retrieve sequences', 'recovery heatmap', 'paralog warnings']
    RETURN_TYPES = ('FASTA', 'TEXT', 'TSV', 'FILE', 'DIRECTORY', 'DIRECTORY', 'DIRECTORY', 'DIRECTORY', 'DIRECTORY', 'DIRECTORY', 'TEXT')
    RETURN_NAMES = ('fixed_targetfile', 'targetfile_ctl_file', 'targetfile_report', 'hybpiper_archive', 'hybpiper_stats', 'hybpiper_heatmaps', 'dna_sequences', 'aa_sequences', 'intron_sequences', 'supercontig_sequences', 'dummy_output')
    ...
    def _sample_name_error(sample_name: str) -> str | None:
        if not sample_name or sub('[^A-Za-z0-9_-]', '', sample_name) != sample_name:
    ...
    def render_command(cls, inputs: dict[str, Any]) -> str:
        ...
        if job == 'check_and_fix_targetfile':
            commands.extend([_shell_join(['hybpiper', 'check_targetfile', '--targetfile_dna', 'target_file.fasta']), 'mv fix_targetfile*.ctl hybpiper.ctl', _shell_join(['hybpiper', 'fix_targetfile', '--targetfile_dna', 'target_file.fasta', '--allow_gene_removal', 'hybpiper.ctl'])])
            return ' && '.join(commands)
        if job == 'assemble':
            sample_name = str(inputs.get('sample_name', '')).strip() or 'sample'
            commands.append(_shell_join(['hybpiper', 'assemble', '--readfiles', str(inputs.get('paired_forward', '')), str(inputs.get('paired_reverse', '')), '--targetfile_dna', 'target_file.fasta', '--diamond', '--cpu', str(inputs.get('threads', 1)), '--prefix', sample_name]))
            commands.append(_shell_join(['tar', '-cvf', cls._archive_path(inputs), f'--directory={sample_name}', '.']))
            return ' && '.join(commands)
        if job == 'stats':
            ...
            for stats_type in cls._stats_types(inputs):
                commands.append(_shell_join(['hybpiper', 'stats', '--targetfile_dna', 'target_file.fasta', '--stats_filename', f'stats.{stats_type}', '--seq_lengths_filename', f'seq_lengths.{stats_type}', stats_type, 'namelist.txt']))
                if inputs.get('heatmap', False):
                    commands.append(_shell_join(['hybpiper', 'recovery_heatmap', '--heatmap_filename', f'heatmap.{stats_type}', '--heatmap_filetype', 'svg', f'seq_lengths.{stats_type}.tsv']))
            for sequence_type in cls._sequence_types(inputs):
                commands.append(_shell_join(['mkdir', f'fasta.{sequence_type}']))
                commands.append(_shell_join(['hybpiper', 'retrieve_sequences', '--targetfile_dna', 'target_file.fasta', '--sample_names', 'namelist.txt', '--fasta_dir', f'fasta.{sequence_type}', sequence_type]))
            out = _out(inputs)
            ...
            if stats_types:
                ...
                for stats_type in stats_types:
                    commands.append(_shell_join(['cp', f'stats.{stats_type}.tsv', f'{out}/hybpiper_stats/stats.{stats_type}.tsv']))
                    commands.append(_shell_join(['cp', f'seq_lengths.{stats_type}.tsv', f'{out}/hybpiper_stats/seq_lengths.{stats_type}.tsv']))
                if inputs.get('heatmap', False):
                    commands.append(_shell_join(['mkdir', '-p', f'{out}/hybpiper_heatmaps']))
                    for stats_type in stats_types:
                        commands.append(_shell_join(['cp', f'heatmap.{stats_type}.svg', f'{out}/hybpiper_heatmaps/heatmap.{stats_type}.svg']))
            sequence_outputs = {'dna': 'dna_sequences', 'aa': 'aa_sequences', 'intron': 'intron_sequences', 'supercontig': 'supercontig_sequences'}
    ...
    def PLAN_OUTPUTS(cls, inputs: dict[str, Any], output_dir: str | Path) -> list[Path]:
        ...
        sequence_outputs = {'dna': out / 'dna_sequences', 'aa': out / 'aa_sequences', 'intron': out / 'intron_sequences', 'supercontig': out / 'supercontig_sequences'}
        for sequence_type in sequence_types:
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'targetfile_dna': ('FASTA', {'description': 'Target file in FASTA format'})}, 'optional': {'hybpiper_job': ('STRING', {'default': 'assemble', 'options': cls.JOBS, 'description': 'Galaxy HybPiper run type'}), 'paired_forward': ('FASTQ', {'default': '', 'description': 'Forward reads from the Galaxy paired collection'}), 'paired_reverse': ('FASTQ', {'default': '', 'description': 'Reverse reads from the Galaxy paired collection'}), 'sample_name': ('STRING', {'default': '', 'description': 'Sample identifier used as the HybPiper assembly prefix'}), 'threads': ('INT', {'default': 1, 'min': 1, 'description': 'CPU threads for HybPiper assemble'}), 'hybpiper_results': ('FILE', {'default': [], 'multiple': True, 'description': 'Archives from HybPiper assemble runs'}), 'sample_names': ('STRING', {'default': [], 'multiple': True, 'description': 'Galaxy collection element identifiers for archive extraction'}), 'stats_type_select': ('STRING', {'default': ['gene'], 'multiple': True, 'options': cls.STATS_TYPES, 'description': 'Statistics outputs to report'}), 'heatmap': ('BOOLEAN', {'default': False, 'description': 'Produce SVG recovery heatmaps for selected statistics'}), 'sequence_type_select': ('STRING', {'default': ['dna'], 'multiple': True, 'options': cls.SEQUENCE_TYPES, 'description': 'Sequence collections to retrieve'})}, 'hidden': {'output': ('STRING', {})}}
