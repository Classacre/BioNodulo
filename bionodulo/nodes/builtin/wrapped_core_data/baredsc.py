"""baredsc — wrapped_core_data node(s). One tool per file (extracted from wrapped_core_data.py)."""
...
class Baredsc2DNode(Baredsc1DNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'baredSC', 'baredsc_2d', 'baredSC 2d', 'pair of genes', 'two-gene expression distribution', 'single-cell expression distribution', 'Bayesian Approach', 'correlation', 'splity', 'MCMC']
    ...
    def _output_paths(cls, inputs: dict[str, Any], output_dir: str | Path) -> list[Path]:
        out = Path(output_dir) / cls.NODE_ID
        return [out / 'output.npz', out / 'output' / 'baredSC_neff.txt', out / 'QC', out / 'output' / 'baredSC_pdf2d.txt', out / 'output' / 'baredSC_pdf2d_flat.txt', out / f'baredSC.{cls._image_format(inputs)}', out / 'other_outputs', out / 'logevidence.txt']
    ...
    def _append_required_inputs(cls, cmd: list[str], inputs: dict[str, Any]) -> None:
        ...
        cmd.extend(['--geneXColName', str(inputs.get('geneXColName', '') or ''), '--geneYColName', str(inputs.get('geneYColName', '') or '')])

    @classmethod
    def _append_mcmc(cls, cmd: list[str], inputs: dict[str, Any]) -> None:
        scale = str(inputs.get('xscale', 'Seurat') or 'Seurat')
        cmd.extend(['--xmin', str(inputs.get('xmin', 0)), '--xmax', str(inputs.get('xmax', 2.5)), '--nx', str(inputs.get('nx', 100)), '--minScalex', str(inputs.get('minScalex', 0.1)), '--ymin', str(inputs.get('ymin', 0)), '--ymax', str(inputs.get('ymax', 2.5)), '--ny', str(inputs.get('ny', 100)), '--minScaley', str(inputs.get('minScaley', 0.1)), '--scale', scale])
        if scale == 'Seurat':
            cmd.extend(['--targetSum', str(inputs.get('targetSum', 10000))])
        cmd.extend(['--seed', str(inputs.get('seed', 1)), '--nnorm', str(inputs.get('nnorm', 2)), '--nsampMCMC', str(inputs.get('nsampMCMC', 100000))])
    ...
    def _append_advanced(cls, cmd: list[str], inputs: dict[str, Any]) -> None:
        cmd.extend(['--osampx', str(inputs.get('osampx', 10)), '--osampxpdf', str(inputs.get('osampxpdf', 4)), '--osampy', str(inputs.get('osampy', 10)), '--osampypdf', str(inputs.get('osampypdf', 4)), '--coviscale', str(inputs.get('coviscale', 1)), '--nis', str(inputs.get('nis', 1000)), '--scalePrior', str(inputs.get('scalePrior', 0.3))])
    ...
    def render_command(cls, inputs: dict[str, Any]) -> str:
        ...
        commands = [_shell_join(cmd), 'mkdir QC', 'mv baredSC_convergence.* QC', f'mv baredSC_p.{shlex.quote(image_format)} QC', 'mv baredSC_corner.* QC', 'mkdir output', 'mv baredSC_neff.txt output', 'mv baredSC_pdf2d.txt output', 'mv baredSC_pdf2d_flat.txt output', f'mv baredSC.{shlex.quote(image_format)} baredSC']
        splity = str(inputs.get('splity', '') or '')
        commands.extend((_shell_join(['mv', f'baredSC_split{value}.txt', f'baredSC_split{value}_pdf.txt']) for value in splity.split()))
    ...
    def VALIDATE_INPUTS(cls, inputs: dict[str, Any]) -> bool | str:
        ...
        numeric_mins = {'nx': (1, int, 100), 'ny': (1, int, 100), 'nnorm': (1, int, 2), 'nsampMCMC': (1, int, 100000), 'nsampInPlot': (1, int, 100000), 'osampx': (1, int, 10), 'osampxpdf': (1, int, 4), 'osampy': (1, int, 10), 'osampypdf': (1, int, 4)}
    ...
    @classmethod
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'geneXColName': ('STRING', {'description': 'Name of the x-axis gene count column'}), 'geneYColName': ('STRING', {'description': 'Name of the y-axis gene count column'})}, 'optional': {'filetype': ('STRING', {'default': 'tabular', 'options': cls.FILETYPES}), 'input': ('TSV', {'description': 'Input count table with one row per cell'}), 'inputAnnData': ('H5AD', {'description': 'AnnData file containing raw counts'}), 'filter_nb': ('STRING', {'default': '0', 'options': cls.FILTER_COUNTS}), 'metadata1ColName': ('STRING', {'default': ''}), 'metadata1Values': ('STRING', {'default': ''}), 'metadata2ColName': ('STRING', {'default': ''}), 'metadata2Values': ('STRING', {'default': ''}), 'metadata3ColName': ('STRING', {'default': ''}), 'metadata3Values': ('STRING', {'default': ''}), 'xmin': ('FLOAT', {'default': 0}), 'xmax': ('FLOAT', {'default': 2.5}), 'nx': ('INT', {'default': 100, 'min': 1}), 'minScalex': ('FLOAT', {'default': 0.1}), 'ymin': ('FLOAT', {'default': 0}), 'ymax': ('FLOAT', {'default': 2.5}), 'ny': ('INT', {'default': 100, 'min': 1}), 'minScaley': ('FLOAT', {'default': 0.1}), 'xscale': ('STRING', {'default': 'Seurat', 'options': cls.SCALE_OPTIONS}), 'targetSum': ('FLOAT', {'default': 10000}), 'seed': ('INT', {'default': 1}), 'nnorm': ('INT', {'default': 2, 'min': 1}), 'nsampMCMC': ('INT', {'default': 100000, 'min': 1}), 'automatic_restart': ('STRING', {'default': 'yes', 'options': cls.RESTART_OPTIONS}), 'minNeff': ('FLOAT', {'default': 200}), 'image_file_format': ('STRING', {'default': 'png', 'options': cls.IMAGE_FORMATS}), 'title': ('STRING', {'default': ''}), 'removeFirstSamples': ('INT', {'default': -1}), 'nsampInPlot': ('INT', {'default': 100000, 'min': 1}), 'prettyBinsx': ('INT', {'default': -1, 'min': -1}), 'prettyBinsy': ('INT', {'default': -1, 'min': -1}), 'log1pColorScale': ('BOOLEAN', {'default': False}), 'splity': ('STRING', {'default': ''}), 'osampx': ('INT', {'default': 10, 'min': 1, 'advanced': True}), 'osampxpdf': ('INT', {'default': 4, 'min': 1, 'advanced': True}), 'osampy': ('INT', {'default': 10, 'min': 1, 'advanced': True}), 'osampypdf': ('INT', {'default': 4, 'min': 1, 'advanced': True}), 'coviscale': ('FLOAT', {'default': 1, 'advanced': True}), 'nis': ('INT', {'default': 1000, 'advanced': True}), 'scalePrior': ('FLOAT', {'default': 0.3, 'advanced': True}), 'burn_custom': ('STRING', {'default': 'no', 'options': ['no', 'yes'], 'advanced': True}), 'nsampBurnMCMC': ('INT', {'default': -1, 'advanced': True}), 'T0BurnMCMC': ('FLOAT', {'default': 100, 'min': 1, 'advanced': True})}, 'hidden': {'output': ('STRING', {})}}
...
class BaredscCombine1DNode(Baredsc1DNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'baredSC', 'baredsc_combine_1d', 'Combine multiple 1D Models', 'combine 1D', 'model averaging', 'single gene', 'Bayesian Approach', 'MCMC']
    ...
    def _append_mcmc(cls, cmd: list[str], inputs: dict[str, Any]) -> None:
        ...
        if scale == 'Seurat':
            cmd.extend(['--targetSum', str(inputs.get('targetSum', 10000))])
        cmd.extend(['--nx', str(inputs.get('nx', 100)), '--minScale', str(inputs.get('minScalex', 0.1)), '--seed', str(inputs.get('seed', 1))])

    @classmethod
    def _output_paths(cls, inputs: dict[str, Any], output_dir: str | Path) -> list[Path]:
        out = Path(output_dir) / cls.NODE_ID
        return [out / 'output' / 'baredSC_pdf.txt', out / f'baredSC.{cls._image_format(inputs)}', out / 'other_outputs']
    ...
    def render_command(cls, inputs: dict[str, Any]) -> str:
        ...
        commands.extend([_shell_join(cmd), 'mkdir output', 'mv baredSC_pdf.txt output', f'mv baredSC.{shlex.quote(image_format)} baredSC', 'gunzip baredSC_means.txt.gz'])
        return ' && '.join(commands)
...
class BaredscCombine2DNode(Baredsc2DNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'baredSC', 'baredsc_combine_2d', 'Combine multiple 2D Models', 'combine 2D', 'model averaging', 'pair of genes', 'Bayesian Approach', 'correlation', 'MCMC']
    ...
    def _output_paths(cls, inputs: dict[str, Any], output_dir: str | Path) -> list[Path]:
        out = Path(output_dir) / cls.NODE_ID
        return [out / 'output' / 'baredSC_pdf2d.txt', out / 'output' / 'baredSC_pdf2d_flat.txt', out / f'baredSC.{cls._image_format(inputs)}', out / 'other_outputs']
    ...
    def render_command(cls, inputs: dict[str, Any]) -> str:
        ...
        cls._append_advanced(cmd, inputs)
        cmd.extend(['--figure', f'baredSC.{image_format}'])
        commands.extend([_shell_join(cmd), 'mkdir output', 'mv baredSC_pdf2d.txt output', 'mv baredSC_pdf2d_flat.txt output', f'mv baredSC.{shlex.quote(image_format)} baredSC'])
        splity = str(inputs.get('splity', '') or '')
        commands.extend((_shell_join(['mv', f'baredSC_split{value}.txt', f'baredSC_split{value}_pdf.txt']) for value in splity.split()))
        return ' && '.join(commands)
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        ...
        optional['getPVal'] = ('BOOLEAN', {'default': False, 'advanced': True, 'description': 'Use fewer samples to estimate the p-value'})
