"""featurecounts — rna_seq node(s). One tool per file (extracted from wrapped_annotation_sequence.py)."""
...
class FeatureCountsNode(CommandNode):
    ...
    SEARCH_ALIASES = [BIONODULO_BUILTIN_ALIAS, 'featureCounts', 'featurecounts', 'featureCounts gene counts', 'subread', 'gene counts', 'RNA-seq read counting', 'GTF annotation']
    ...
    DOCUMENTATION_URL = f'{DOI_URL}{FEATURECOUNTS_CITATION_DOI}'
    ...
    def _counts_command(cls, inputs: dict[str, Any]) -> list[str]:
        ...
        cmd.extend(['-o', 'output', '-T', '${GALAXY_SLOTS:-2}', '-s', str(inputs.get('strand_specificity', inputs.get('strandness', '0'))), '-Q', str(inputs.get('mapping_quality', 0))])
        for flag in (str(inputs.get('splitonly', '')), cls._flag_value(inputs, 'primary', '--primary'), cls._flag_value(inputs, 'ignore_dup', '--ignoreDup')):
            if flag:
        ...
        if junction:
            ...
            if genome:
                cmd.extend(['-G', genome])
        for flag in (cls._flag_value(inputs, 'long_reads', '-L'), cls._flag_value(inputs, 'by_read_group', '--byReadGroup'), cls._flag_value(inputs, 'largest_overlap', '--largestOverlap')):
            if flag:
                cmd.append(flag)
        cmd.extend(['--minOverlap', str(inputs.get('min_overlap', 1)), '--fracOverlap', str(inputs.get('frac_overlap', 0)), '--fracOverlapFeature', str(inputs.get('frac_overlap_feature', 0))])
        ...
        if paired_end_status != 'single_end' or inputs.get('count_read_pairs') is not False:
            if paired_end_status == 'single_end' and inputs.get('count_read_pairs') is not True:
                ...
                if paired_end_status == 'PE_fragments' and inputs.get('check_distance'):
                    cmd.extend(['-P', '-d', str(inputs.get('minimum_fragment_length', 50)), '-D', str(inputs.get('maximum_fragment_length', 600))])
    ...
    def render_command(cls, inputs: dict[str, Any]) -> str:
        ...
        commands = ["export FC_PATH=$(command -v featureCounts | sed 's@/bin/featureCounts$@@')", _shell_join(cls._counts_command(inputs)).replace("'${GALAXY_SLOTS:-2}'", '${GALAXY_SLOTS:-2}').replace("'${FC_PATH}/", '${FC_PATH}/').replace(".txt'", '.txt'), f"grep -v '^#' output | {sed_sample} > body.txt"]
        format_value = str(inputs.get('format', 'tabdel_short'))
        counts_path = f'{out}/counts.tsv'
        if format_value == 'tabdel_medium':
            commands.extend(['cut -f 1,7 body.txt > expression_matrix.txt', 'cut -f 6 body.txt > gene_lengths.txt', 'paste expression_matrix.txt gene_lengths.txt > expression_matrix.txt.bak', _shell_join(['mv', '-f', 'expression_matrix.txt.bak', counts_path])])
        ...
        if inputs.get('R'):
            commands.append(f"""samtools sort --no-PG -o {shlex.quote(f'{out}/annotated.bam')} -@ ${{GALAXY_SLOTS:-2}} -T "${{TMPDIR:-.}}" *.featureCounts.bam""")
    ...
    def INPUT_TYPES(cls) -> dict[str, dict[str, Any]]:
        return {'required': {'alignment': ('BAM', {'description': 'SAM or BAM alignment file for read counting'})}, 'optional': {'anno_select': ('STRING', {'default': 'history', 'options': cls.ANNO_SELECT_OPTIONS, 'description': 'Use built-in, cached, or history gene annotations'}), 'reference_gene_sets': ('GFF_GTF', {'default': '', 'description': 'History GFF/GTF annotation file'}), 'reference_gene_sets_cached': ('GFF_GTF', {'default': '', 'description': 'Cached GFF/GTF annotation path'}), 'bgenome': ('STRING', {'default': 'hg38', 'options': cls.BUILTIN_GENOME_OPTIONS, 'description': 'Built-in featureCounts annotation genome'}), 'format': ('STRING', {'default': 'tabdel_short', 'options': cls.FORMAT_OPTIONS, 'description': 'Counts table format'}), 'strand_specificity': ('STRING', {'default': '0', 'options': cls.STRAND_OPTIONS}), 'include_feature_length_file': ('BOOLEAN', {'default': False}), 'gff_feature_type': ('STRING', {'default': 'exon', 'description': 'GFF feature type filter'}), 'gff_feature_attribute': ('STRING', {'default': 'gene_id', 'description': 'GFF attribute for grouping'}), 'summarization_level': ('BOOLEAN', {'default': False, 'description': 'Count at feature level'}), 'paired_end_status': ('STRING', {'default': 'single_end', 'options': cls.PAIRED_END_OPTIONS, 'advanced': True}), 'only_both_ends': ('BOOLEAN', {'default': False, 'advanced': True}), 'exclude_chimerics': ('BOOLEAN', {'default': True, 'advanced': True}), 'check_distance': ('BOOLEAN', {'default': False, 'advanced': True}), 'minimum_fragment_length': ('INT', {'default': 50, 'min': 0, 'advanced': True}), 'maximum_fragment_length': ('INT', {'default': 600, 'min': 1, 'advanced': True}), 'mapping_quality': ('INT', {'default': 0, 'min': 0}), 'splitonly': ('STRING', {'default': '', 'options': cls.SPLITONLY_OPTIONS, 'advanced': True}), 'primary': ('BOOLEAN', {'default': False, 'advanced': True}), 'ignore_dup': ('BOOLEAN', {'default': False, 'advanced': True}), 'multifeat': ('STRING', {'default': '', 'options': cls.MULTIFEAT_OPTIONS, 'advanced': True}), 'fraction': ('BOOLEAN', {'default': False, 'advanced': True}), 'count_exon_exon_junction_reads': ('STRING', {'default': '', 'options': cls.JUNCTION_OPTIONS, 'advanced': True}), 'genome': ('FASTA', {'default': '', 'advanced': True}), 'long_reads': ('BOOLEAN', {'default': False, 'advanced': True}), 'by_read_group': ('BOOLEAN', {'default': False, 'advanced': True}), 'largest_overlap': ('BOOLEAN', {'default': False, 'advanced': True}), 'min_overlap': ('INT', {'default': 1, 'advanced': True}), 'frac_overlap': ('FLOAT', {'default': 0, 'min': 0, 'max': 1, 'advanced': True}), 'frac_overlap_feature': ('FLOAT', {'default': 0, 'min': 0, 'max': 1, 'advanced': True}), 'read_reduction': ('STRING', {'default': '', 'options': cls.READ_REDUCTION_OPTIONS, 'advanced': True}), 'R': ('BOOLEAN', {'default': False, 'advanced': True}), 'read_extension_5p': ('INT', {'default': 0, 'min': 0, 'advanced': True}), 'read_extension_3p': ('INT', {'default': 0, 'min': 0, 'advanced': True}), 'gtf': ('GFF_GTF', {'default': '', 'description': 'Compatibility alias for history annotation'}), 'bam': ('BAM', {'default': '', 'description': 'Compatibility alias for alignment'}), 'threads': ('INT', {'default': 2, 'min': 1, 'advanced': True}), 'count_read_pairs': ('BOOLEAN', {'default': False, 'advanced': True}), 'feature_type': ('STRING', {'default': '', 'advanced': True}), 'attribute': ('STRING', {'default': '', 'advanced': True}), 'strandness': ('STRING', {'default': '0', 'options': cls.STRAND_OPTIONS, 'advanced': True})}, 'hidden': {'output': ('STRING', {})}}
