"""Shared chewBBACA 3.3.10+galaxy1 contracts for focused owners."""
# ruff: noqa: F403,F405
from __future__ import annotations

from bionodulo.nodes.builtin._wrapped_tool_utils import *
from bionodulo.nodes.builtin.wrapped_assembly_typing_family.contracts import (
    CHEWBBACA_GALAXY1_GIT_COMMIT,
    ToolsIUCCommandContract,
)


class ChewBBACAContractNode(ToolsIUCCommandContract):
    GIT_COMMIT = CHEWBBACA_GALAXY1_GIT_COMMIT
    GALAXY_WRAPPER_GIT_COMMIT = CHEWBBACA_GALAXY1_GIT_COMMIT
    SOURCE_URL = (
        "https://github.com/galaxyproject/tools-iuc/tree/"
        f"{CHEWBBACA_GALAXY1_GIT_COMMIT}/tools/chewbbaca"
    )
    GALAXY_WRAPPER_SOURCE_URL = SOURCE_URL
    GALAXY_WRAPPER_VERSION = "3.3.10+galaxy1"
    PACKAGE_CONSTRAINT = "chewbbaca==3.3.10; blast==2.15.0; zip==3.0; fasttree==2.1.11"


class _ChewBBACAAlleleCallContract(ChewBBACAContractNode):
    """Determine allelic profiles for genome assemblies with chewBBACA."""

    LEGACY_NODE_ID = "chewbbaca_allelecall"
...
class _ChewBBACAAlleleCallEvaluatorContract(ChewBBACAContractNode):
    """Build chewBBACA allele calling evaluation reports."""

    LEGACY_NODE_ID = "chewbbaca_allelecallevaluator"
...
class _ChewBBACACreateSchemaContract(ChewBBACAContractNode):
    """Create chewBBACA gene-by-gene schemas from genome assemblies."""

    LEGACY_NODE_ID = "chewbbaca_createschema"
...
class _ChewBBACADownloadSchemaContract(ChewBBACAContractNode):
    """Download chewBBACA schemas from Chewie-NS."""

    LEGACY_NODE_ID = "chewbbaca_downloadschema"
...
class _ChewBBACAExtractCgMLSTContract(ChewBBACAContractNode):
    """Determine core-genome loci from chewBBACA allelic profiles."""

    LEGACY_NODE_ID = "chewbbaca_extractcgmlst"
...
class _ChewBBACAJoinProfilesContract(ChewBBACAContractNode):
    """Join chewBBACA allele calling profiles from multiple runs."""

    LEGACY_NODE_ID = "chewbbaca_joinprofiles"
...
class _ChewBBACANSStatsContract(ChewBBACAContractNode):
    """Retrieve Chewie-NS species and schema statistics with chewBBACA."""

    LEGACY_NODE_ID = "chewbbaca_nsstats"
        ...
        return {
            ...
            "optional": {
                "species_id": (
                    ...
                    {
                        "default": "",
                        "options": list(_ChewBBACADownloadSchemaContract.SPECIES_OPTIONS),
                        "option_labels": _ChewBBACADownloadSchemaContract.SPECIES_OPTIONS,
                    },
                ),
    ...
    def VALIDATE_INPUTS(cls, inputs: dict[str, Any]) -> bool | str:
        ...
        if species_id and species_id not in _ChewBBACADownloadSchemaContract.SPECIES_OPTIONS:
            return f"species_id must be one of: {', '.join(_ChewBBACADownloadSchemaContract.SPECIES_OPTIONS)}"
...
class _ChewBBACAPrepExternalSchemaContract(ChewBBACAContractNode):
    """Adapt external schemas for chewBBACA."""

    LEGACY_NODE_ID = "chewbbaca_prepexternalschema"
    ...
    def VALIDATE_INPUTS(cls, inputs: dict[str, Any]) -> bool | str:
        if not str(inputs.get("input_schema", "")).strip():
            return "input_schema is required"
        result = _ChewBBACACreateSchemaContract._validate_float_min(inputs, "blast_score_ratio", 0.6, 0, 1)
        if result is not True:
            return result
        result = _ChewBBACACreateSchemaContract._validate_int_min(inputs, "minimum_length", 0, 0)
        if result is not True:
            return result
        result = _ChewBBACACreateSchemaContract._validate_int_min(inputs, "translation_table", 11, 0)
        if result is not True:
            return result
        result = _ChewBBACACreateSchemaContract._validate_float_min(inputs, "size_threshold", 0.2, 0)
        if result is not True:
            return result
        return True
